...
## GitHub Copilot Coding Agent (Issue/PR Automation)

When working as the **GitHub Copilot coding agent** (assigned to issues via the
`copilot-solve` label), follow this streamlined workflow:

### What You Can Deliver
- Java source code changes (`src/main/java/neqsim/`)
- JUnit 5 tests (`src/test/java/neqsim/`)
- CSV data files (`src/main/resources/`)
- Documentation (markdown, JavaDoc)
- Build configuration changes

### Coding Agent Workflow

1. **Read the issue** — understand the task, acceptance criteria, and linked files
2. **Search for context** — find related classes, existing patterns, and tests
3. **Verify API signatures** — read actual class files before using any NeqSim API
4. **Implement** — write code following the patterns and constraints below
5. **Write/update tests** — every code change needs a test
6. **Update TASK_LOG.md** — add an entry at `docs/development/TASK_LOG.md`
7. **Open PR** — with clear description of what was changed and why

### Coding Agent Constraints
- **Java 8 only** — see the critical section below
- **Verify before using** — never assume method signatures; read the source first
- **Complete JavaDoc** — all classes and methods need `@param`, `@return`, `@throws`
- **Run tests** — include the Maven command to verify: `./mvnw test -Dtest=YourTest`
- **Small, focused PRs** — one logical change per PR

### Iteration on Feedback
When reviewers comment on your PR or the original issue:
- Re-read all comments (issue + PR) to understand the full feedback
- Address every comment — either fix the code or explain why not
- Push updates to the same branch
- Reply to review comments confirming what was changed

### Engineering Task Workflow (label: `copilot-engineering`)

When assigned to an issue with the `copilot-engineering` label, you have
terminal access and can run full simulations. Follow this extended workflow:

1. **Set up the environment** (run in terminal):
   ```bash
...
2. **Create the task folder**:
   ```bash
   python devtools/new_task.py "TASK TITLE FROM ISSUE" --type B --author "Copilot"
   ```

3. **Fill scope** — write `step1_scope_and_research/task_spec.md` with:
   - Problem statement from the issue
   - Applicable standards
   - Acceptance criteria

4. **Create simulation notebook** in `step2_analysis/` using NeqSim:
   - Use the dual-boot setup cell (devtools local build preferred, pip fallback):
     ```python
     try:
         ...
         NEQSIM_MODE = "pip"
     ```
   - Create fluid, process equipment, run simulation
   - Generate matplotlib figures, save to `figures/`
   - Save `results.json` in the task root

5. **Run the notebook**:
   ```bash
   jupyter nbconvert --execute --to notebook step2_analysis/*.ipynb
   ```

6. **Write Java tests** if new classes were added

7. **Copy reusable outputs to the repo** (notebooks, tests, docs):
   ```bash
   cp step2_analysis/*.ipynb examples/notebooks/
   ```

8. **Update TASK_LOG.md** and open PR with the reusable files

**Important:** The `task_solve/` folder is gitignored — only copy reusable
deliverables (notebooks, tests, docs) into tracked paths before committing.
