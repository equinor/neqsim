...
artifact. It complements `book-creation` (which handles structural authoring)
and `journal-formatting` (which handles paper layout).
...
* `journal-formatting` — for journal articles instead of books, the same
