...
| Showing only the leading constraint | Near-active constraints and weak evidence remain hidden from debottleneck decisions | Use `ProcessModelSimulationEvaluator.rankCapacityConstraints(model)` for the immutable utilization-ranked list, preserve `getEvidenceApplicability()` beside each item, and keep confidence/applicability out of the utilization order and feasibility calculation |
