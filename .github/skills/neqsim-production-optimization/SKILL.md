...
| Joining plant-wide bottlenecks by unit name alone | Duplicate names in separate process areas resolve to the wrong train or area | For `ProcessModel.getUtilizationSnapshotJson()`, preserve the bottleneck `area` and use `qualifiedName` (`area::unit`) as the stable join key |
