Warning: truncated output (original token count: 81769)
Total output lines: 5934
...
## 2026-08-05 — Conservative species tracing through OnePhasePipeLine

- `OnePhasePipeLine.setConservativeCompositionalTracking(true)` now selects the validated,
  positive-flow, isothermal conservative species path (solver type `1`) for both initialization
  and transient advancement.
- The process-equipment API delegates convergence policy and exposes immutable
  `OnePhaseSpeciesConservationReport` and `OnePhaseSpeciesConservationHistory` diagnostics,
  physical-cell mass-fraction profiles, and time-aligned outlet mass-fraction histories directly.
- `setCompositionalTracking(true)` remains source-compatible and continues to select the legacy
  staged type `20` path; it does not carry the conservative validation guarantee.
- `devtools/neqsim_dev_setup.neqsim_classes(...)` now exposes `ns.OnePhasePipeLine` for direct
  Python/JPype use.
...
- `GasTurbine` / `GasTurbineVendorPerform…31769 tokens truncated…` phase param, sign error, zero guards, velocity/enthalpy phase fixes
