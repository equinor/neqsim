Warning: truncated output (original token count: 90415)
Total output lines: 6475
...
## 2026-08-22 — Shared contactor internals capacity and retrofit rating

`DistillationColumnMechanicalDesign` now performs an integrated hydraulic rating for the column
family, including `PackedColumn`, `AbsorptionColumn`, `StrippingColumn`, and ordinary
`DistillationColumn` equipment.

- `ContactorCapacityResult` reports Fs, packing or tray flood utilization, packing wetting,
  optional outlet-demister K-factor utilization, total pressure-drop utilization, overall
  utilization, the controlling bottleneck, and screening-level gas headroom.
- `configurePackedInternals(...)`, `configureOutletDemister(...)`, and
  `setColumnDiameterOverride(...)` configure greenfield sizing or a fixed-shell brownfield rating.
- `comparePackedInternals(...)` compares baseline and candidate packing/demister internals at the
  same vessel diameter and process conditions. `ContactorCapacityComparison` reports the estimated
  gas-capacity increase.
- `PackingHydraulicsCalculator`, `ColumnInternalsDesigner`, and `PackedColumn` accept an explicit
  relative hydraulic-capacity factor. The default is 1.0; values above one must come from
  applicable vendor data and are not inferred from a packing trade name.
- `calcDesign()` registers normalized mechanical-design constraints named
  `column internals flooding`, `outlet demister`, and `contactor pressure drop` for utilization and
  bottleneck reporting.

The headroom estimate assumes unchanged properties and liquid rate. Re-run the process at candidate
rates and use supplier rating software for final internals guarantees.
...
from the immediately preceding converged thermodynamic state. This preserves CPA associ…50415 tokens truncated…ed kinetics; batch, fed-batch, continuous modes. Extends `Fermenter`.                   |
