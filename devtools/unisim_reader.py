...
class UniSimToNeqSim:
    ...
    def to_python(self, include_subflowsheets: bool = True) -> str:
        ...
        _a('SystemSrkEos = jneqsim.thermo.system.SystemSrkEos')
        _a('SystemPrEos  = jneqsim.thermo.system.SystemPrEos')
        _a('ProcessSystem = jneqsim.process.processmodel.ProcessSystem')
        _a('Stream = jneqsim.process.equipment.stream.Stream')
        _a('Separator = jneqsim.process.equipment.separator.Separator')
        _a('ThreePhaseSeparator = jneqsim.process.equipment.separator.ThreePhaseSeparator')
        _a('Mixer = jneqsim.process.equipment.mixer.Mixer')
        _a('Splitter = jneqsim.process.equipment.splitter.Splitter')
        _a('Compressor = jneqsim.process.equipment.compressor.Compressor')
        _a('ThrottlingValve = jneqsim.process.equipment.valve.ThrottlingValve')
        _a('Cooler = jneqsim.process.equipment.heatexchanger.Cooler')
        _a('Heater = jneqsim.process.equipment.heatexchanger.Heater')
        _a('HeatExchanger = jneqsim.process.equipment.heatexchanger.HeatExchanger')
        _a('Pump = jneqsim.process.equipment.pump.Pump')
        _a('Recycle = jneqsim.process.equipment.util.Recycle')
        _a('AdiabaticPipe = jneqsim.process.equipment.pipeline.AdiabaticPipe')
        _a('')
        ...
        _a('# Fluid definition')
        _a('# ============================================================')
        _a(f'fluid = {eos_class}({fluid["temperature"]}, {fluid["pressure"]})')
        ...
        # --- build process topology (reuse the same logic as to_json) ---
        ...
        # Track variable names for wiring
        var_names: dict = {}  # unit/stream name → python var name
        used_vars: set = set()

        def _unique_var(name: str) -> str:
            base = _var(name)
            v = base
            i = 2
        ...
        # Outlet-port variable refs: "SepName.gasOut" → python expression
        def _outlet_ref(ref: str) -> str:
            """Return a Python expression that evaluates to the outlet stream."""
            if '.' not in ref:
                return var_names.get(ref, _var(ref))
            parent, port = ref.rsplit('.', 1)
            pvar = var_names.get(parent, _var(parent))
            port_methods = {
        ...
        for feed_name in sorted(external_feeds):
            ...
            v = _unique_var(feed_name)
            _a(f'{v} = Stream("{feed_name}", fluid.clone())')
            if sd:
                if sd.mass_flow_kgh is not None and sd.mass_flow_kgh > 0:
                    _a(f'{v}.setFlowRate({sd.mass_flow_kgh}, "kg/hr")')
                if sd.temperature_C is not None:
                    _a(f'{v}.setTemperature({sd.temperature_C}, "C")')
                if sd.pressure_bara is not None:
                    _a(f'{v}.setPressure({sd.pressure_bara}, "bara")')
            _a(f'process.add({v})')
            _a('')
        ...
        for op in sorted_ops:
            neqsim_type = UniSimReader.OPERATION_TYPE_MAP.get(op.type_name)
            if neqsim_type is None:
                _a(f'# SKIPPED: unsupported type "{op.type_name}": "{op.name}"')
                continue
            if neqsim_type in ('Adjuster', 'Set', 'Spreadsheet', 'SubFlowsheet',
                               'PIDController', 'SurgeController', 'Reactor',
                               'Absorber'):
                _a(f'# SKIPPED ({neqsim_type}): "{op.name}"')
                continue

            v = _unique_var(op.name)

            # Resolve inlet references
            inlet_refs = []
            if op.feeds:
                for f in op.feeds:
                    inlet_refs.append(self._resolve_inlet_ref(f, stream_producer))

            # --- generate constructor + wiring ---
            if neqsim_type == 'Mixer':
                _a(f'{v} = Mixer("{op.name}")')
                for ref in inlet_refs:
                    _a(f'{v}.addStream({_outlet_ref(ref)})')
            elif neqsim_type == 'HeatExchanger' and len(inlet_refs) >= 2:
                _a(f'{v} = HeatExchanger("{op.name}")')
                _a(f'{v}.setFeedStream(0, {_outlet_ref(inlet_refs[0])})')
                _a(f'{v}.setFeedStream(1, {_outlet_ref(inlet_refs[1])})')
            elif neqsim_type == 'Splitter':
                ref_expr = _outlet_ref(inlet_refs[0]) if inlet_refs else 'None'
                n_splits = len(op.products) if op.products else 2
                _a(f'{v} = Splitter("{op.name}", {ref_expr}, {n_splits})')
            elif neqsim_type == 'DistillationColumn':
                n_trays = op.properties.get('numberOfTrays', 5)
                ref_expr = _outlet_ref(inlet_refs[0]) if inlet_refs else 'None'
                _a(f'# DistillationColumn: {op.name}')
                _a(f'from neqsim.process.equipment.distillation import DistillationColumn as DC')
                _a(f'{v} = DC("{op.name}", {n_trays}, True, True)')
                if inlet_refs:
                    _a(f'{v}.addFeedStream({ref_expr})')
            elif neqsim_type == 'Recycle':
                ref_expr = _outlet_ref(inlet_refs[0]) if inlet_refs else 'None'
                _a(f'{v} = Recycle("{op.name}")')
                if inlet_refs:
                    _a(f'{v}.setInletStream({ref_expr})')
            else:
                # Standard single-inlet equipment
                ref_expr = _outlet_ref(inlet_refs[0]) if inlet_refs else 'None'
                _a(f'{v} = {neqsim_type}("{op.name}", {ref_expr})')
