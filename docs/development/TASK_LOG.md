...
**Validation:** On the 19.81 m flowline + 14.94 m riser benchmark the steady riser hold-up goes from a constant 0.90 clamp to a physical 0.74-0.65 profile, and the riser-base pressure swing goes from 7.5-10.4 kPa to 81-97 kPa against a measured 98 kPa. The mesh spread between 16 and 24 sections collapses from a factor of 5 to 16%, so the amplitude is now a model result rather than a mesh artefact, and the outlet slug tracker registers a slug for the first time. `SevereSluggingExperimentalBenchmarkTest` 7/7. `neqsim.process.equipment.pipeline.**` plus `FlowRegime*Test` and `PipeBeggsAndBrills*Test` = 287 tests, 0 failures, 7 pre-existing skips - terrain-slugging, separated-friction, phase-degeneracy and lean-gas hold-up assertions all unchanged. `spotless:apply` clean. Still open: the cycle period is ~2x short of the measured 38 s, and `isTransientOutletBackflowClamped()` is still true, so liquid fallback at the riser top is clamped at the transmissive outlet - that is a boundary-condition fix, not a closure fix.
