...
**Type:** E (Feature)
**Keywords:** production allocation, uncertainty propagation, first-order propagation, variance propagation, Sigma_v = J Sigma_b J^T, independent metering, AllocationUncertaintyEstimator, SourceAllocator, neqsim.process.allocation, closed form, cached factorisation
**Solution:** `src/main/java/neqsim/process/allocation/AllocationUncertaintyEstimator.java` (new, ~540 lines) + `src/test/java/neqsim/process/allocation/AllocationUncertaintyEstimatorTest.java` (4 JUnit cases); `SourceAllocator.getSources()` and `getCustodyOutlets()` added (unmodifiable views). Doc: new "Uncertainty propagation" section in `docs/process/production-allocation.md`. CHANGELOG_AGENT_NOTES.md entry. Paper §2.8 in `neqsim-paperlab/papers/production_allocation_2026/paper.md` (local-only — gitignored).
**Notes:** Closed-form first-order Gaussian propagation that reuses the cached `(I − A_k)⁻¹` factorisation already built by `SourceAllocator.allocate()`. For independent per-source metering with variance `σ²_{s,k}`, the per-source per-custody flow variance is `Var(alloc(s,j,k)) = f_w(j,k)² · (J_k)_{w,e_s}² · σ²_{s,k}` — one extra multi-RHS solve per component, no Monte-Carlo. **GOTCHAS:** (1) The estimator MUST reuse the allocator's registered names — initial helpers auto-detected and silently re-named "Well-A" → "WellAStream", causing `Unknown custody: ExportGas`. Fixed by exposing `getSources()`/`getCustodyOutlets()` on `SourceAllocator` and consulting them before auto-detect. (2) Spotless apply silently dropped a local-variable declaration during a multi-line edit; always read the file back and re-run tests after Spotless. (3) PowerShell breaks `-DspotlessFiles=.*(A|B).*` even when quoted because `|` is a pipe — run `./mvnw spotless:apply` unrestricted instead. (4) Correlated metering and frozen-split-factor uncertainty are listed as future work in the paper §2.8 and in the doc's Scope subsection. PR #2325 updated (commit 3f8485ff1c). 11/11 tests pass.
...
**Solution:** New package `src/main/java/neqsim/process/allocation/` — `SourceAllocator` (facade), `RecoveryFactorExtractor` (freezes per-unit per-component split factors from a converged base case), `AllocationNetwork` (builds routing matrices + identifies source/custody terminal streams), `LinearAllocationSolver` (EJML SimpleMatrix multi-RHS solve of `(I-A_k)v=b` with iterative Neumann fallback + negative clipping), `ProductionAllocationResult` (queries + JSON + mass-closure renormalisation), plus `UnitSplit`, `AllocationSource`, `CustodyOutlet`, `ProductType`, `package-info`. Test: `src/test/java/neqsim/process/allocation/SourceAllocatorTest.java` (7 tests pass). Doc: `docs/process/production-allocation.md`.
**Notes:** Generic for any oil/gas field topology. Each non-Stream unit with outlets is a network node; Stream objects are edges. Split factor f_u(s,k) = outlet-s component-k molar flow / total inlet component-k molar flow, frozen from the converged base case, so allocation is a linear superposition of source injections — exact for the linearised model and robust to recycles/reflux (loop gain <1 guarantees convergence). Custody outlets MUST be tagged at the producing equipment's actual outlet stream (e.g. `separator.getGasOutStream()`), not a downstream wrapper `Stream` (object identity via IdentityHashMap). Auto-detection (`autoDetectSources`/`autoDetectCustodyOutlets` with phase-beta product inference) returns canonical streams. Component names must match the NeqSim database (`nC10`, not `n-decane`). Mass closure on by default. Java 8, full JavaDoc, Spotless applied, 7/7 tests pass.
