...
| [production-allocation.md](production-allocation) | **Production allocation** — back-allocate metered production to wells/sources via a linear recovery-factor proxy network (handles recycle/reflux); multi-method comparison (component-ratio / all-in / stand-alone) with auto-recommendation |
