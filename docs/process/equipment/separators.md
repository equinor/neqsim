...
| (4) | **Pi-number with EQN scrubber testing database** *(private / proprietary)* | Same geometry / internals inputs as (3), plus access to the internal Equinor (EQN) scrubber testing database. Distributed as a closed-source plug-in — not part of the public NeqSim build. | Primary-separation efficiency from an empirical equation of the form Efficiency = f(π), where π is a dimensionless group built from operating conditions and vessel geometry. Returns carry-over kg/h tied to the test envelope. | Equinor-internal scrubber benchmarking and operating-envelope decisions where the EQN test database is the reference. |
...
- Working **inside Equinor** and want carry-over benchmarked against the
  ...
  inputs as (3), but the primary-separation efficiency comes from an
  empirical equation Efficiency = f(π) built from a dimensionless group of
  operating conditions and vessel geometry. The plug-in is not part of
