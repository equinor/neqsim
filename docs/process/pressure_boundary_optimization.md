...
// Export tab-separated capacity diagnostics
...
`LiftCurveTable.toDiagnosticTable()` writes tab-separated capacity diagnostics. It does **not**
...
// Get the tab-separated capacity diagnostics
String diagnosticTable = table.toDiagnosticTable();
```

Illustrative tab-separated output (synthetic values, not quick-start results):

```text
# Process capacity screening; diagnostic text only
# Table: Example capacity screening
inlet pressure [bara]	outlet pressure [bara]	maximum flow [kg/hr]	power [kW]	bottleneck	feasible
40.0	30.0	450.5	0.0	feed	true
40.0	35.0	NaN	NaN		false
50.0	30.0	520.3	0.0	feed	true
50.0	35.0	NaN	NaN		false
60.0	30.0	580.0	0.0	feed	true
60.0	35.0	NaN	NaN		false
```
...
When generating capacity tables, infeasible flow rates are retained as `Double.NaN`
internally, `NaN` with `feasible=false` in diagnostic text, and `null` in JSON.
These diagnostics are not reservoir deck records.
