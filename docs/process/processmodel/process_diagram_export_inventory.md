...
| Native DEXPI 2.0 Process | `Dexpi20ProcessModelWriter.write(...)`, `writeAndAssess(...)`, `writeAndAssessTopology(...)`; opt-in `Dexpi20CanonicalOperatingProcessWriter.writeAndAssessTopology(...)` | Direct `ProcessSystem` traversal for compatibility APIs; canonical material projection for the assessed topology path; canonical successful-run operating nodes for the opt-in writer | Native DEXPI 2.0 Process XML with steps, material ports, streams, quantities, conformance report, and optional structured topology evidence | Preserve the native Process profile and existing sequential serialization; keep canonical operating-value consumption opt-in until wider equivalence evidence supports migration |
...
| Units and provenance | Optional stream labels/tables; canonical topology plus opt-in current-case stream values in K, bara absolute, and kg/s with review-required simulation provenance | Missing document model | Selected physical quantities with explicit units; the opt-in canonical writer consumes successful `CALCULATED` value nodes, converts kg/s to kg/h and K to degree Celsius, retains bara as absolute bar, and omits unsuccessful-run values | Simulation/design metadata and governed package artifacts |
...
| `Dexpi20ProcessModelWriterTest`, `Dexpi20CanonicalOperatingProcessWriterTest`, `Dexpi20SemanticValidatorTest` | Native DEXPI 2.0 Process structure, direct and canonical operating quantities, unsuccessful-run suppression, semantic rules, simple/parallel material-topology equivalence, explicit ports, and scoped loss/conformance evidence |
...
and the DEXPI Process comparison gate drives the assessed material projection. The opt-in canonical
operating writer consumes calculated canonical value nodes while preserving the established assessed
material topology and explicit unsuccessful-run loss behavior. Compatibility APIs still traverse
their established source model and retain their sequential XML identities. The next reviewed
extension can widen exchange to multi-area `ProcessModel`; hierarchy and energy/signal mappings
must not be silently flattened or lost.
