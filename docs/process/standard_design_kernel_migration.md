...
`StandardDesignKernelVerificationSuite.evaluateRegression()` executes the eight registered exact
...
expansibility identities, plus DNV-RP-C203 S-N/Miner damage identities using an explicitly named
project-controlled demonstration curve. API 526, API 610, API 617,
and API 12J are evaluated as explicit historical edition labels; the suite does not reclassify them
as current. M-506, ISO 5167-2, and DNV-RP-C203 support their catalogued current editions but remain
screening-only.
