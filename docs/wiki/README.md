# NeqSim Wiki Index

Welcome to the NeqSim Wiki - your comprehensive guide to thermodynamic and process simulation.

## 📚 Documentation by Topic

### 🚀 Getting Started
- [Getting Started Guide](getting_started.md) - Installation and first steps
- [Usage Examples](usage_examples.md) - Basic usage patterns
- [FAQ](faq.md) - Frequently asked questions

### 🔬 Thermodynamics
- [Thermodynamics Guide](thermodynamics_guide.md) - Comprehensive thermodynamics overview
- [Fluid Characterization](fluid_characterization.md) - Characterizing petroleum fluids
- [TBP Fraction Models](tbp_fraction_models.md) - True boiling point fractions
- [Viscosity Models](viscosity_models.md) - Viscosity calculation methods
- [Steam Tables (IF97)](steam_tables_if97.md) - Water/steam properties

### ⚙️ Process Simulation
- [Process Simulation Guide](process_simulation.md) - Building process flowsheets
- [Advanced Process Simulation](advanced_process_simulation.md) - Advanced techniques
- [Logical Unit Operations](logical_unit_operations.md) - Logic and control
- [Process Control](process_control.md) - Control systems
- [Transient Simulation](process_transient_simulation_guide.md) - Dynamic simulation

### 🔧 Equipment Models
- **Separation**: [Distillation Column](distillation_column.md) | [Membrane Separation](membrane_separation.md)
- **Rotating**: [Pump Usage](pump_usage_guide.md)
- **Heat Transfer**: [Air Cooler](air_cooler.md) | [Water Cooler](water_cooler.md) | [Steam Heater](steam_heater.md) | [Mechanical Design](heat_exchanger_mechanical_design.md)
- **Flow Control**: [Flow Meters](flow_meter_models.md) | [Venturi](venturi_calculation.md)
- **Special**: [Gibbs Reactor](gibbs_reactor.md) | [Battery Storage](battery_storage.md) | [Solar Panel](solar_panel.md)

### 🔄 Pipeline & Flow
- [Pipeline Index](pipeline_index.md) - Pipeline documentation hub
- [Flow Equations](pipeline_flow_equations.md) - Fundamental equations
- [Pressure Drop](pipeline_pressure_drop.md) - Pressure loss calculations
- [Beggs & Brill](beggs_and_brill_correlation.md) - Two-phase correlation
- [Friction Factors](friction_factor_models.md) - Friction factor models
- [Heat Transfer](pipeline_heat_transfer.md) - Pipeline heat loss
- [Two-Fluid Model](two_fluid_model.md) - Two-fluid approach
- [Transient Pipeline](pipeline_transient_simulation.md) - Dynamic flow
- [Water Hammer](water_hammer_implementation.md) - Pressure surge

### 🛡️ Safety Systems
- [PSV Dynamic Sizing](psv_dynamic_sizing_example.md) - Relief valve sizing
- [PSD Valve Trip](psd_valve_hihi_trip.md) - Shutdown valve logic
- [ESD/Fire Alarm](esd_fire_alarm_system.md) - Emergency systems
- [Choke Protection](choke_collapse_psd_protection.md) - Choke failure protection

### 📊 PVT & Reservoir
- [PVT Workflows](pvt_simulation_workflows.md) - PVT simulation methods
- [Property Flash](property_flash_workflows.md) - Flash calculations
- [Black Oil Playbook](black_oil_flash_playbook.md) - Black oil models
- [Gas Quality](gas_quality_standards_from_tests.md) - Gas specifications

### 🔍 Advanced Topics
- [Bottleneck Analysis](bottleneck_analysis.md) - System optimization
- [Field Development](field_development_planning.md) - Field planning
- [Humid Air](humid_air_math.md) - Psychrometric calculations

### 👨‍💻 Development
- [Test Overview](test-overview.md) - Testing guide
- [Flash Tests](flash_equations_and_tests.md) - Equation testing
- [Colab Notebooks](java_simulation_from_colab_notebooks.md) - Running in Colab

---

## 📖 Complete Reference Manual

For the complete structured documentation, see:
- **[Reference Manual Index](../REFERENCE_MANUAL_INDEX.md)** - Master table of contents

## 🔗 Quick Links

- [GitHub Repository](https://github.com/equinor/neqsim)
- [Maven Central](https://search.maven.org/artifact/com.equinor.neqsim/neqsim)
- [API Javadoc](https://equinor.github.io/neqsim/apidocs/)
- [Python Package (neqsim-python)](https://github.com/equinor/neqsim-python)

---

## 📁 All Wiki Files

<details>
<summary>Click to expand complete file list</summary>

| File | Description |
|------|-------------|
| activity_coefficients.md | Activity coefficient models |
| advanced_process_simulation.md | Advanced simulation techniques |
| air_cooler.md | Air cooler equipment |
| battery_storage.md | Battery storage systems |
| beggs_and_brill_correlation.md | Beggs & Brill correlation |
| black_oil_flash_playbook.md | Black oil flash guide |
| bottleneck_analysis.md | System bottleneck analysis |
| choke_collapse_psd_protection.md | Choke protection systems |
| compressor_calculations.md | Compressor calculations |
| distillation_column.md | Distillation columns |
| erosion_velocity.md | Erosion velocity limits |
| esd_fire_alarm_system.md | ESD/fire alarm systems |
| faq.md | Frequently asked questions |
| field_development_planning.md | Field development |
| flash_equations_and_tests.md | Flash equation testing |
| flow_meter_models.md | Flow metering models |
| fluid_characterization.md | Fluid characterization |
| friction_factor_models.md | Friction factor models |
| fugacity_calculations.md | Fugacity calculations |
| gas_quality_standards_from_tests.md | Gas quality standards |
| getting_started.md | Getting started guide |
| gibbs_reactor.md | Gibbs reactor model |
| heat_exchanger_calculations.md | Heat exchanger calcs |
| heat_exchanger_mechanical_design.md | HX mechanical design |
| humid_air_math.md | Humid air calculations |
| java_simulation_from_colab_notebooks.md | Colab integration |
| logical_unit_operations.md | Logical operations |
| membrane_separation.md | Membrane separation |
| multiphase_transient_model.md | Multiphase transient |
| pipe_wall_heat_transfer.md | Pipe heat transfer |
| pipeline_design.md | Pipeline design |
| pipeline_flow_equations.md | Flow equations |
| pipeline_heat_transfer.md | Pipeline heat transfer |
| pipeline_index.md | Pipeline doc index |
| pipeline_model_recommendations.md | Model recommendations |
| pipeline_pressure_drop.md | Pressure drop |
| pipeline_transient_simulation.md | Transient pipeline |
| process_control.md | Process control |
| process_simulation.md | Process simulation |
| process_transient_simulation_guide.md | Transient simulation |
| property_flash_workflows.md | Property flash |
| psd_valve_hihi_trip.md | PSD valve trip |
| psv_dynamic_sizing_example.md | PSV dynamic sizing |
| pump_usage_guide.md | Pump usage guide |
| pvt_simulation_workflows.md | PVT workflows |
| separator_calculations.md | Separator calculations |
| slug_flow.md | Slug flow modeling |
| solar_panel.md | Solar panel model |
| steam_heater.md | Steam heater model |
| steam_tables_if97.md | IF97 steam tables |
| tbp_fraction_models.md | TBP fraction models |
| test-overview.md | Testing overview |
| thermodynamics_guide.md | Thermodynamics guide |
| two_fluid_model.md | Two-fluid model |
| usage_examples.md | Usage examples |
| venturi_calculation.md | Venturi calculations |
| viscosity_models.md | Viscosity models |
| water_cooler.md | Water cooler model |
| water_hammer_implementation.md | Water hammer |

</details>

---

*NeqSim Wiki - Maintained by the NeqSim Community*
