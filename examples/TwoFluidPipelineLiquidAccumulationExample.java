 ...
 * pipeline, showing how liquid accumulates as a function of feed flow rate using the
 * {@link TwoFluidPipe} model which solves separate momentum equations for each phase.
 * </p>
 *
 * <h2>Key Features Demonstrated:</h2>
 * <ul>
 * <li>Three-phase flow (gas + oil/condensate + water)</li>
 * <li>Transient liquid accumulation dynamics with two-fluid model</li>
 ...
 * <li>Comparison of TwoFluidPipe vs TransientPipe (drift-flux) models</li>
 * </ul>
 *
 * <h2>TwoFluidPipe Model Advantages:</h2>
 * <ul>
 * <li>Separate momentum equations for gas and liquid phases</li>
 * <li>Better prediction of countercurrent flow</li>
 * <li>More accurate slug flow dynamics</li>
 * <li>Improved terrain-induced liquid accumulation modeling</li>
 * </ul>
 *
 * @author NeqSim Team
 * @version 1.0
 * @see TwoFluidPipe
 */
public class TwoFluidPipelineLiquidAccumulationExample {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("  Using TwoFluidPipe Model (Separate Phase Momentum)");
  ...
  public static void runFlowRateSensitivityStudy() {
    ...
    double outletPressure = 100.0; // bara (delivery pressure - realistic pressure drop)
    ...
    System.out.println("  Model: TwoFluidPipe (separate phase momentum equations)");
    System.out.println();
    ...
    for (double flowRate : flowRates) {
      ...
      if (flowRate == flowRates[0]) {
        ...
        for (int p = 0; p < inlet.getFluid().getNumberOfPhases(); p++) {
          System.out.println("  Phase " + p + ": " + inlet.getFluid().getPhase(p).getPhaseTypeName()
              + " vol=" + String.format("%.4f", inlet.getFluid().getPhase(p).getVolume("m3")));
        }
        // Calculate water cut
        if (inlet.getFluid().hasPhaseType("oil") && inlet.getFluid().hasPhaseType("aqueous")) {
          double volOil = inlet.getFluid().getPhase("oil").getVolume("m3");
          double volWater = inlet.getFluid().getPhase("aqueous").getVolume("m3");
          double waterCut = volWater / (volOil + volWater);
          System.out.println("  Inlet water cut (vol): " + String.format("%.4f", waterCut) + " ("
              + String.format("%.1f", waterCut * 100) + "%)");
        }
        System.out.println();
      }

      // Create TwoFluidPipe (uses separate momentum equations for each phase)
      ...
      // Note: Outlet pressure is calculated from pressure drop, not constrained
      // Setting both inlet P/flow AND outlet P over-constrains the problem
  ...
  private static void runDetailedTransientSimulation(double pipeLength, double pipeDiameter,
    ...
    double totalSimTime = 14400.0; // 4 hours simulation
    double startFlowRate = 50.0;
    double endFlowRate = 150.0;
    double reportInterval = 1200.0; // Report every 20 minutes
  ...
  private static void printHoldupProfileSummary(TwoFluidPipe pipe, double[] elevationProfile) {
    ...
    System.out.println("  Model: TwoFluidPipe");
    System.out.println("=============================================================");
    System.out.println();
    ...
    System.out.println();
    System.out.println("=============================================================");
    System.out.println("  MODEL NOTES");
    System.out.println("=============================================================");
    System.out.println("TwoFluidPipe uses the two-fluid model with:");
    System.out.println("  - Separate mass conservation for gas and liquid");
    System.out.println("  - Separate momentum equations for each phase");
    System.out.println("  - Interfacial friction and drag correlations");
    System.out.println("  - Better prediction of countercurrent flow situations");
    System.out.println();
    System.out.println("For comparison, TransientPipe uses drift-flux model with:");
    System.out.println("  - Mixture momentum equation");
    System.out.println("  - Slip relation (drift velocity correlation)");
    System.out.println("  - Faster computation but less accurate for complex flows");
  }
