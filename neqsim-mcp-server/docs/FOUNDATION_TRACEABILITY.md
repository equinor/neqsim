...
Inventory version `1.14` advances `inspectApi` from source-qualified to transport-qualified while deliberately leaving its current `CONFIRMED_GAP` status and the 20/9/42 accounting unchanged. `ApiKnowledgeRunner` restricts reflection to `neqsim.*`, explicit common NeqSim classes, and canonical `EquipmentFactory` aliases; `ApiKnowledgeRunnerTest` verifies representative alias/class resolution, member filtering/source pointers, and fail-closed rejection of `java.lang.Runtime`; and the existing `NeqSimTools` facade preserves normal access enforcement and response standardization. The focused `test_inspect_api_protocol.py` now executes both the successful `ProcessModel` inspection and the fail-closed non-NeqSim case against the packaged STDIO server. The read-only `MCP protocol qualification` workflow builds the exact branch artifact and runs those checks on pull requests and `master`. The candidate therefore reports `promotionReady=true`; only the primary packaged protocol's frozen classification accounting still prevents promotion. `docs/evidence/API_INSPECTION_CONTRACT.md` records the detailed boundary.
...
Promote `inspectApi` only when its `CONFIRMED_GAP` coverage record and the primary packaged protocol's frozen trust accounting move together from 20/9/42 to 20/10/41 on one exact validated head. Then audit the remaining trust gaps against current source and promote only where concrete tests, public benchmark evidence, authoritative data, or a clearly non-numerical contract supports the classification. Do not manufacture accuracy bounds for discovery/catalog tools and do not reconstruct a second MCP-side conservation model when canonical NeqSim does not expose independent evidence.
