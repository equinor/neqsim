...
| Trust coverage records | 71 = 20 explicit benchmark + 26 bounded contract-tested software contracts + 25 confirmed gaps | `getCapabilities.phase0EvidenceInventory` | `BenchmarkTrust`, `McpImplementationInventory`, MCP contract tests |
| Contract-promotion candidates | 0 | `getCapabilities.phase0EvidenceInventory` | No candidate is queued in inventory 1.27; any future promotion must move machine-readable coverage and primary protocol accounting atomically |
...
classification. It does **not** mean the MCP surface is scientifically validated: 25 records remain
`CONFIRMED_GAP`, twenty-six are `CONTRACT_TESTED`, `scientificValidationComplete=false`, and the
...
now reconciles inventory 1.27 with 20/26/25 coverage accounting while retaining
`inspectApi=CONTRACT_TESTED`. The primary `test_mcp_server.py` independently includes `inspectApi`
among its twenty-six bounded software contracts and requires 25 confirmed gaps. The read-only
...
`manageValidationProfile` in its twenty-six bounded software contracts and requires 25 confirmed
gaps.
...
promotion moved coverage from 20/10/41 to 20/11/40. Current inventory 1.27 retains that contract
...
No promotion candidate remains queued in inventory 1.27. A future transition must again move the
...
every published tool. Twenty-six bounded discovery, catalog, lookup, progress, trust-retrieval,
...
automation advisory and simulation-variable mutation contracts are contract-tested, leaving 25 confirmed trust gaps and no queued promotion candidate.
