...
| Trust coverage records | 71 = 20 explicit benchmark + 25 bounded contract-tested software contracts + 26 confirmed gaps | `getCapabilities.phase0EvidenceInventory` | `BenchmarkTrust`, `McpImplementationInventory`, MCP contract tests |
| Contract-promotion candidates | 0 | `getCapabilities.phase0EvidenceInventory` | No candidate is queued in inventory 1.26; any future promotion must move machine-readable coverage and primary protocol accounting atomically |
...
classification. It does **not** mean the MCP surface is scientifically validated: 26 records remain
`CONFIRMED_GAP`, twenty-five are `CONTRACT_TESTED`, `scientificValidationComplete=false`, and the
...
now reconciles inventory 1.26 with 20/25/26 coverage accounting while retaining
`inspectApi=CONTRACT_TESTED`. The primary `test_mcp_server.py` independently includes `inspectApi`
among its twenty-five bounded software contracts and requires 26 confirmed gaps. The read-only
...
`manageValidationProfile` in its twenty-five bounded software contracts and requires 26 confirmed
gaps.
...
promotion moved coverage from 20/10/41 to 20/11/40. Current inventory 1.26 retains that contract
...
No promotion candidate remains queued in inventory 1.26. A future transition must again move the
...
every published tool. Twenty-five bounded discovery, catalog, lookup, progress, trust-retrieval,
...
automation advisory contracts are contract-tested, leaving 26 confirmed trust gaps and no queued promotion candidate.
