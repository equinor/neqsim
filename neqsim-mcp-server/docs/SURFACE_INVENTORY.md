...
| Trust coverage records | 71 = 20 explicit benchmark + 29 bounded contract-tested software contracts + 22 confirmed gaps | `getCapabilities.phase0EvidenceInventory` | `BenchmarkTrust`, `McpImplementationInventory`, MCP contract tests |
| Contract-promotion candidates | 0 | `getCapabilities.phase0EvidenceInventory` | No candidate is queued in inventory 1.29; any future promotion must move machine-readable coverage and primary protocol accounting atomically |
...
classification. It does **not** mean the MCP surface is scientifically validated: 22 records remain
`CONFIRMED_GAP`, twenty-nine are `CONTRACT_TESTED`, `scientificValidationComplete=false`, and the
...
now reconciles inventory 1.29 with 20/29/22 coverage accounting while retaining
`inspectApi=CONTRACT_TESTED`. The primary `test_mcp_server.py` independently includes `inspectApi`
among its twenty-nine bounded software contracts and requires 22 confirmed gaps. The read-only
...
`manageValidationProfile` in its twenty-nine bounded software contracts and requires 22 confirmed
gaps.
...
promotion moved coverage from 20/10/41 to 20/11/40. Current inventory 1.29 retains that contract
...
### Qualified process-local plugin contract

The current source adds focused qualification for `runPlugin` while deliberately retaining its
`CONFIRMED_GAP` inventory status. Java evidence covers process-local registration, metadata
discovery, invocation, same-name replacement, cleanup, absent-plugin diagnostics, and exception
normalization. The packaged-STDIO harness covers empty-catalog and default-list discovery,
absent-plugin failure, unknown actions, malformed input, standard envelopes, and real transport.

This evidence does not establish plugin provenance, installation, signing, sandboxing, resource or
tenant isolation, persistence, external IAM, transport security, scientific validity, plant or
control authority, certification, or accountable engineering approval. A registered plugin runs
application code in the MCP server process. The detailed evidence is in
`docs/evidence/PLUGIN_EXECUTION_CONTRACT.md`; any `CONTRACT_TESTED` classification requires a
separate atomic promotion.
...
No promotion candidate remains queued in inventory 1.29. A future transition must again move the
...
every published tool. Twenty-nine bounded discovery, catalog, lookup, progress, trust-retrieval,
governance, validation-profile, API-inspection, model-registry, session-lifecycle, security-management,
persisted-state, adjustable-parameter discovery, pre-flight validation, result-validation, reporting handoff, and
automation advisory, simulation-variable mutation, paired snapshot, and visualization-generation contracts are contract-tested, leaving 22 confirmed trust gaps and no queued promotion candidate.
