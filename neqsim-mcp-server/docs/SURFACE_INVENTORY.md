...
| Trust coverage records | 71 = 20 explicit benchmark + 28 bounded contract-tested software contracts + 23 confirmed gaps | `getCapabilities.phase0EvidenceInventory` | `BenchmarkTrust`, `McpImplementationInventory`, MCP contract tests |
| Contract-promotion candidates | 0 | `getCapabilities.phase0EvidenceInventory` | No candidate is queued in inventory 1.28; any future promotion must move machine-readable coverage and primary protocol accounting atomically |
...
classification. It does **not** mean the MCP surface is scientifically validated: 23 records remain
`CONFIRMED_GAP`, twenty-eight are `CONTRACT_TESTED`, `scientificValidationComplete=false`, and the
...
now reconciles inventory 1.28 with 20/28/23 coverage accounting while retaining
`inspectApi=CONTRACT_TESTED`. The primary `test_mcp_server.py` independently includes `inspectApi`
among its twenty-eight bounded software contracts and requires 23 confirmed gaps. The read-only
...
`manageValidationProfile` in its twenty-eight bounded software contracts and requires 23 confirmed
gaps.
...
promotion moved coverage from 20/10/41 to 20/11/40. Current inventory 1.28 retains that contract
...
No promotion candidate remains queued in inventory 1.28. A future transition must again move the
...
every published tool. Twenty-eight bounded discovery, catalog, lookup, progress, trust-retrieval,
...
automation advisory, simulation-variable mutation, and paired snapshot contracts are contract-tested, leaving 23 confirmed trust gaps and no queued promotion candidate.
