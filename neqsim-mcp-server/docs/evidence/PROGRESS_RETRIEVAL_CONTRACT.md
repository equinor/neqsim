...
- Phase 0 evidence inventory version `1.11` records `getProgress` as the sole evidence-qualified promotion candidate, with the exact source, regression, protocol, and evidence-note paths above.
...
`getProgress` remains a `CONFIRMED_GAP` in the current Phase 0 trust-coverage accounting. The merged baseline is `20 EXPLICIT_TRUST + 8 CONTRACT_TESTED + 43 CONFIRMED_GAP`; this candidate increment deliberately leaves those counts unchanged. A future promotion to `CONTRACT_TESTED` must update the machine-readable classification and exact packaged protocol expectation together on one validated head.
