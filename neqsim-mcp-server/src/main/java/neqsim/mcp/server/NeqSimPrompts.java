...
public class NeqSimPrompts {
  ...
  public PromptMessage designGasProcessing(@PromptArg(name = "feedGasDescription",
    ...
    String text = "## Gas Processing Design Workflow\n\n" + "**Feed:** " + feedGasDescription + "\n"
        ...
        + "formation risk at operating conditions.\n\n" + "### Step 4: Discover the Process JSON Contract\n"
        + "Call `getCapabilities`, then `getSchema` for `run_process` input. Retrieve "
        + "`getExample(category='process', name='mixer-splitter-recycle')` when the design "
        + "uses mixers, splitters, recycle loops, multiple inlets, or named ports. Use a single "
        + "ProcessSystem for one flowsheet or top-level `areas` plus `interAreaLinks` for a "
        + "multi-area ProcessModel. Do not reject topology that the schema and example support.\n\n"
        + "### Step 5: Build and Validate the Process Simulation\n"
        + "Construct the JSON flowsheet including:\n"
        ...
        + "Use `inlets` for mixers, `unit.port` references for separator/splitter outlets, "
        + "and forward references for recycle loops. Call `validateInput` before `runProcess`. "
        + "Repair diagnostics against the schema/example and rerun. Accept the model only after "
        + "checking convergence, unresolved-inlet warnings, and mass/energy balance closure.\n\n"
