...
def test_protocol():
    ...
    expected_resource_uris = sorted([
        "neqsim://components", "neqsim://data-tables", "neqsim://example-catalog",
        "neqsim://models", "neqsim://schema-catalog", "neqsim://setup-templates",
        "neqsim://standards",
    ])
    check("exact static resource inventory", resource_uris == expected_resource_uris, f"got {resource_uris}")
    ...
    expected_template_uris = sorted([
        "neqsim://api/{className}", "neqsim://components/{name}",
        "neqsim://examples/{category}/{name}", "neqsim://materials/{type}",
        "neqsim://schemas/{tool}/{type}", "neqsim://setup-templates/{id}",
        "neqsim://standards/{code}",
    ])
    check("exact resource-template inventory", template_uris == expected_template_uris, f"got {template_uris}")
    ...
    expected_prompt_names = sorted([
        "biorefinery_analysis", "co2_ccs_chain", "design_gas_processing",
        "dynamic_simulation", "field_development_screening", "flow_assurance_screening",
        "pipeline_sizing", "pvt_study", "teg_dehydration_design",
    ])
    check("exact guided-prompt inventory", prompt_names == expected_prompt_names, f"got {prompt_names}")
...
def test_examples_and_schemas():
    ...
    check("schema catalog is JSON", "_protocolError" not in schema_catalog, schema_catalog.get("_protocolError", ""))
    check("schema catalog has 71 tools", len(schema_catalog) == 71, f"got {len(schema_catalog)}")
    schema_uri_errors = []
    ...
    for tool_name, schema_refs in sorted(schema_catalog.items()):
        ...
        for schema_type in ("input", "output"):
            ...
            if schema_uri != expected_uri:
                schema_uri_errors.append(f"{tool_name}/{schema_type}: {schema_uri!r} != {expected_uri!r}")
                continue
    ...
    check("all 142 catalog schema URIs are canonical", not schema_uri_errors, "; ".join(schema_uri_errors))
    check("all 142 catalog schemas resolve as JSON objects", not schema_resource_errors, f"invalid: {schema_resource_errors}")
    example_catalog = read_json_resource("neqsim://example-catalog")
    check("example catalog is JSON", "_protocolError" not in example_catalog, example_catalog.get("_protocolError", ""))
    check("example catalog has 24 categories", len(example_catalog) == 24, f"got {len(example_catalog)}")
    example_count = sum(len(examples) for examples in example_catalog.values() if isinstance(examples, dict))
    check("example catalog has 114 entries", example_count == 114, f"got {example_count}")
    tool_examples = example_catalog.get("tool", {})
    check("schema tools exactly match canonical tool examples", set(tool_examples) == set(schema_catalog),
          f"missing={sorted(set(schema_catalog) - set(tool_examples))}, extra={sorted(set(tool_examples) - set(schema_catalog))}")
    ...
    check("all 114 catalog examples resolve as JSON objects", not example_resource_errors, f"invalid: {example_resource_errors}")
...
def test_tp_flash_pr_two_phase():
    ...
    r = run_flash({"nitrogen": 0.01, "CO2": 0.01, "methane": 0.68, "ethane": 0.10, "n-heptane": 0.20}, 24.85, 10.0, eos="PR")
    check("status=success", r.get("status") == "success", r.get("message", ""))
    flash = r.get("flash", {})
...
def test_tp_flash_two_phase_ch4_c3():
    ...
    if gas_d and liq_d:
        check("gas density < liquid density", gas_d < liq_d, f"gas={gas_d}, liq={liq_d}")


def test_tp_flash_rich_gas():
    print("\n=== TP Flash: Rich Gas Near Cricondenbar (0C, 100 bara) ===")
    r = run_flash({"nitrogen": 3.43, "CO2": 0.34, "methane": 62.51, "ethane": 15.65, "propane": 13.22, "i-butane": 1.61, "n-butane": 2.48, "i-pentane": 0.35, "n-pentane": 0.29, "n-hexane": 0.12}, 0.0, 100.0)
    check("status=success", r.get("status") == "success", r.get("message", ""))
    nphases = r.get("flash", {}).get("numberOfPhases", 0)
...
def test_tp_flash_rich_gas_single_phase():
    ...
    r = run_flash({"nitrogen": 3.43, "CO2": 0.34, "methane": 62.51, "ethane": 15.65, "propane": 13.22, "i-butane": 1.61, "n-butane": 2.48, "i-pentane": 0.35, "n-pentane": 0.29, "n-hexane": 0.12}, 30.0, 100.0)
    check("status=success", r.get("status") == "success", r.get("message", ""))
    nphases = r.get("flash", {}).get("numberOfPhases", 0)
...
def test_tp_flash_low_temperature():
    ...
    r = run_flash({"methane": 0.80, "ethane": 0.10, "propane": 0.05, "n-butane": 0.05}, -40.0, 50.0)
    check("status=success", r.get("status") == "success", r.get("message", ""))
    nphases = r.get("flash", {}).get("numberOfPhases", 0)
...
def test_tp_flash_with_co2_h2s():
    ...
    r = run_flash({"methane": 0.75, "CO2": 0.10, "H2S": 0.05, "ethane": 0.05, "propane": 0.05}, 40.0, 80.0)
    check("status=success", r.get("status") == "success", r.get("message", ""))
...
def test_tp_flash_with_water_cpa():
    ...
    r = run_flash({"methane": 0.85, "ethane": 0.10, "water": 0.05}, 25.0, 50.0, eos="CPA")
    check("status=success", r.get("status") == "success", r.get("message", ""))
    nphases = r.get("flash", {}).get("numberOfPhases", 0)
...
def test_dew_point_temperature():
    ...
    r = run_flash({"methane": 0.85, "ethane": 0.10, "propane": 0.05}, 25.0, 50.0, flash_type="dewPointT")
    check("status=success", r.get("status") == "success", r.get("message", ""))
...
def test_bubble_point_pressure():
    ...
    r = run_flash({"methane": 0.50, "ethane": 0.30, "propane": 0.20}, -50.0, 30.0, flash_type="bubblePointP")
    check("status=success", r.get("status") == "success", r.get("message", ""))
...
def test_dew_point_pressure():
    ...
    r = run_flash({"methane": 0.85, "ethane": 0.10, "propane": 0.05}, 0.0, 50.0, flash_type="dewPointP")
    check("status=success", r.get("status") == "success", r.get("message", ""))


def test_bubble_point_temperature():
    print("\n=== Bubble Point Temperature (30 bara) ===")
    r = run_flash({"methane": 0.30, "propane": 0.40, "n-butane": 0.30}, -20.0, 30.0, flash_type="bubblePointT")
    check("status=success", r.get("status") == "success", r.get("message", ""))


def test_process_separator():
    print("\n=== Process: HP Separator ===")
    r = run_process({"fluid":{"model":"SRK","temperature":298.15,"pressure":50.0,"mixingRule":"classic","components":{"methane":0.85,"ethane":0.10,"propane":0.05}},"process":[{"type":"Stream","name":"feed","properties":{"flowRate":[10000.0,"kg/hr"]}},{"type":"Separator","name":"HP Sep","inlet":"feed"}]})
    check("status=success", r.get("status") == "success", r.get("message", ""))
...
def test_process_compressor():
    ...
    r = run_process({"fluid":{"model":"SRK","temperature":298.15,"pressure":20.0,"components":{"methane":0.90,"ethane":0.10}},"process":[{"type":"Stream","name":"feed","properties":{"flowRate":[5000.0,"kg/hr"]}},{"type":"Compressor","name":"Comp","inlet":"feed","properties":{"outletPressure":[60.0,"bara"]}}]})
    check("status=success", r.get("status") == "success", r.get("message", ""))


def test_process_cooler():
    print("\n=== Process: Cooler (80C -> 30C) ===")
    r = run_process({"fluid":{"model":"SRK","temperature":353.15,"pressure":50.0,"components":{"methane":0.90,"ethane":0.10}},"process":[{"type":"Stream","name":"feed","properties":{"flowRate":[5000.0,"kg/hr"]}},{"type":"Cooler","name":"cooler1","inlet":"feed","properties":{"outletTemperature":[303.15,"K"]}}]})
    check("status=success", r.get("status") == "success", r.get("message", ""))


def test_process_heater():
    print("\n=== Process: Heater (25C -> 80C) ===")
    r = run_process({"fluid":{"model":"SRK","temperature":298.15,"pressure":50.0,"components":{"methane":0.90,"ethane":0.10}},"process":[{"type":"Stream","name":"feed","properties":{"flowRate":[5000.0,"kg/hr"]}},{"type":"Heater","name":"heater1","inlet":"feed","properties":{"outletTemperature":[353.15,"K"]}}]})
    check("status=success", r.get("status") == "success", r.get("message", ""))


def test_process_valve():
    print("\n=== Process: Throttling Valve (50 -> 20 bara) ===")
    r = run_process({"fluid":{"model":"SRK","temperature":298.15,"pressure":50.0,"components":{"methane":0.90,"ethane":0.10}},"process":[{"type":"Stream","name":"feed","properties":{"flowRate":[10000.0,"kg/hr"]}},{"type":"valve","name":"choke","inlet":"feed","properties":{"outletPressure":20.0}}]})
    check("status=success", r.get("status") == "success", r.get("message", ""))


def test_process_separator_and_compressor():
    print("\n=== Process: Separator + Compressor Train ===")
    r = run_process({"fluid":{"model":"SRK","temperature":298.15,"pressure":50.0,"mixingRule":"classic","components":{"methane":0.85,"ethane":0.10,"propane":0.05}},"process":[{"type":"Stream","name":"feed","properties":{"flowRate":[50000.0,"kg/hr"]}},{"type":"Separator","name":"HP Sep","inlet":"feed"},{"type":"Compressor","name":"Comp","inlet":"HP Sep.gasOut","properties":{"outletPressure":[80.0,"bara"]}}]})
    check("status=success", r.get("status") == "success", r.get("message", ""))
...
def test_process_compression_cooling_train():
    ...
    if "fluid" in r:
        ...
        check("compression+cooling success", result.get("status") == "success", result.get("message", ""))


def test_process_two_phase_separator():
    print("\n=== Process: Two-Phase Separator ===")
    r = run_process({"fluid":{"model":"SRK","temperature":253.15,"pressure":10.0,"mixingRule":"classic","components":{"methane":0.50,"propane":0.50}},"process":[{"type":"Stream","name":"feed","properties":{"flowRate":[20000.0,"kg/hr"]}},{"type":"Separator","name":"sep","inlet":"feed"}]})
    check("status=success", r.get("status") == "success", r.get("message", ""))


def test_process_valve_then_separator():
    print("\n=== Process: Valve + Separator (JT Effect) ===")
    r = run_process({"fluid":{"model":"SRK","temperature":298.15,"pressure":80.0,"components":{"methane":0.80,"ethane":0.10,"propane":0.05,"n-butane":0.05}},"process":[{"type":"Stream","name":"feed","properties":{"flowRate":[30000.0,"kg/hr"]}},{"type":"valve","name":"JT","inlet":"feed","properties":{"outletPressure":20.0}},{"type":"Separator","name":"LTS","inlet":"JT"}]})
    check("status=success", r.get("status") == "success", r.get("message", ""))


def test_process_multi_fluid():
    print("\n=== Process: Multiple Named Fluids ===")
    r = run_process({"fluids":{"gas":{"model":"SRK","temperature":298.15,"pressure":50.0,"components":{"methane":0.9,"ethane":0.1}},"oil":{"model":"PR","temperature":350.0,"pressure":100.0,"components":{"methane":0.3,"nC10":0.7}}},"process":[{"type":"Stream","name":"gasFeed","fluidRef":"gas","properties":{"flowRate":[10000.0,"kg/hr"]}},{"type":"Stream","name":"oilFeed","fluidRef":"oil","properties":{"flowRate":[50000.0,"kg/hr"]}}]})
    check("status=success", r.get("status") == "success", r.get("message", ""))


def test_process_pr_model():
    print("\n=== Process: PR EOS ===")
    r = run_process({"fluid":{"model":"PR","temperature":298.15,"pressure":50.0,"components":{"methane":0.85,"ethane":0.15}},"process":[{"type":"Stream","name":"feed","properties":{"flowRate":[10000.0,"kg/hr"]}},{"type":"Separator","name":"sep","inlet":"feed"}]})
    check("PR process success", r.get("status") == "success", r.get("message", ""))


def test_validation_valid_flash():
    print("\n=== Validation: Valid Flash ===")
    r = call_tool("validateInput", {"inputJson":json.dumps({"model":"SRK","temperature":{"value":25.0,"unit":"C"},"pressure":{"value":50.0,"unit":"bara"},"flashType":"TP","components":{"methane":0.85,"ethane":0.10,"propane":0.05},"mixingRule":"classic"})})
...
def test_validation_unknown_component():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"components":{"metane":1.0}})})
...
def test_validation_unknown_model():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"model":"NONEXISTENT","components":{"methane":1.0}})})
    check("valid=false", r.get("valid") is False)
...
def test_validation_negative_fraction():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"components":{"methane":-0.5}})})
    check("valid=false", r.get("valid") is False)
    codes = [i.get("code") for i in r.get("issues", [])]
...
def test_validation_composition_sum_warning():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"components":{"methane":0.5,"ethane":0.3}})})
...
def test_validation_missing_enthalpy_ph():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"flashType":"PH","components":{"methane":1.0}})})
    check("valid=false", r.get("valid") is False)
    codes = [i.get("code") for i in r.get("issues", [])]
...
def test_validation_unknown_flash_type():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"flashType":"XYZ","components":{"methane":1.0}})})
    check("valid=false", r.get("valid") is False)
    codes = [i.get("code") for i in r.get("issues", [])]
...
def test_validation_extreme_temperature():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"temperature":50000.0,"components":{"methane":1.0}})})
    check("valid=true (warning)", r.get("valid") is True)
    codes = [i.get("code") for i in r.get("issues", [])]
...
def test_validation_process_missing_type():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"fluid":{"components":{"methane":1.0}},"process":[{"name":"feed"}]})})
    check("valid=false", r.get("valid") is False)
    codes = [i.get("code") for i in r.get("issues", [])]
...
def test_validation_process_duplicate_names():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"fluid":{"components":{"methane":1.0}},"process":[{"type":"Stream","name":"feed"},{"type":"Stream","name":"feed"}]})})
    codes = [i.get("code") for i in r.get("issues", [])]
...
def test_validation_multiple_errors():
    ...
    r = call_tool("validateInput", {"inputJson":json.dumps({"model":"FAKEOS","flashType":"PH","components":{"fakey":1.0}})})
    check("valid=false", r.get("valid") is False)
    issues = r.get("issues", [])
...
def test_run_all_flash_examples():
    ...
    for name in ["tp-simple-gas","tp-two-phase","dew-point-t","bubble-point-p","cpa-with-water"]:
        ...
        result = call_tool("runFlash", {"components":json.dumps(comps),"temperature":temp_val,"temperatureUnit":temp_unit,"pressure":pres_val,"pressureUnit":pres_unit,"eos":model,"flashType":ft})
        check(f"flash example '{name}' runs", result.get("status") == "success", result.get("message", ""))
...
def test_run_all_process_examples():
    ...
    for name in ["simple-separation","compression-with-cooling"]:
        ...
        check(f"process example '{name}' runs", result.get("status") == "success", result.get("message", ""))


def test_batch_temperature_sweep():
    print("\n=== Batch: Temperature Sweep ===")
    r = call_tool("runBatch", {"components":json.dumps({"methane":0.85,"ethane":0.10,"propane":0.05}),"eos":"SRK","flashType":"TP","cases":json.dumps([{"temperature":{"value":-20.0,"unit":"C"},"pressure":{"value":50.0,"unit":"bara"}},{"temperature":{"value":25.0,"unit":"C"},"pressure":{"value":50.0,"unit":"bara"}},{"temperature":{"value":80.0,"unit":"C"},"pressure":{"value":50.0,"unit":"bara"}}])})
...
def test_batch_pressure_sweep():
    ...
    r = call_tool("runBatch", {"components":json.dumps({"methane":0.70,"ethane":0.15,"propane":0.10,"n-butane":0.05}),"eos":"PR","flashType":"TP","cases":json.dumps([{"temperature":{"value":25.0,"unit":"C"},"pressure":{"value":10.0,"unit":"bara"}},{"temperature":{"value":25.0,"unit":"C"},"pressure":{"value":30.0,"unit":"bara"}},{"temperature":{"value":25.0,"unit":"C"},"pressure":{"value":60.0,"unit":"bara"}},{"temperature":{"value":25.0,"unit":"C"},"pressure":{"value":100.0,"unit":"bara"}}])})
...
def test_batch_example_from_catalog():
    ...
    r = call_tool("runBatch", {"components":json.dumps(example.get("components",{})),"eos":example.get("model","SRK"),"flashType":example.get("flashType","TP"),"cases":json.dumps(example.get("cases",[]))})
...
def test_property_table_temperature_sweep():
    ...
    r = call_tool("getPropertyTable", {"components":json.dumps({"methane":0.85,"ethane":0.10,"propane":0.05}),"sweep":"temperature","sweepFrom":-20.0,"sweepFromUnit":"C","sweepTo":60.0,"sweepToUnit":"C","fixedValue":50.0,"fixedUnit":"bara","points":9,"eos":"SRK"})
...
def test_property_table_pressure_sweep():
    ...
    r = call_tool("getPropertyTable", {"components":json.dumps({"methane":0.90,"ethane":0.07,"propane":0.03}),"sweep":"pressure","sweepFrom":10.0,"sweepFromUnit":"bara","sweepTo":100.0,"sweepToUnit":"bara","fixedValue":25.0,"fixedUnit":"C","points":10,"eos":"SRK"})
...
def test_property_table_example_from_catalog():
    ...
    r = call_tool("getPropertyTable", {"components":json.dumps(example.get("components",{})),"sweep":example.get("sweep","temperature"),"sweepFrom":example["sweepFrom"]["value"],"sweepFromUnit":example["sweepFrom"]["unit"],"sweepTo":example["sweepTo"]["value"],"sweepToUnit":example["sweepTo"]["unit"],"fixedValue":example["fixedPressure"]["value"],"fixedUnit":example["fixedPressure"]["unit"],"points":example.get("points",13),"eos":example.get("model","SRK")})
...
def test_phase_envelope_natural_gas():
    ...
    r = call_tool("getPhaseEnvelope", {"components":json.dumps({"methane":0.80,"ethane":0.10,"propane":0.05,"n-butane":0.03,"n-pentane":0.02}),"eos":"SRK"})
...
def test_phase_envelope_example_from_catalog():
    ...
    r = call_tool("getPhaseEnvelope", {"components":json.dumps(example.get("components",{})),"eos":example.get("model","SRK")})
...
def test_capabilities():
    ...
    check("capability descriptors cover every published tool", coverage.get("complete") is True, str(coverage))
    check("capability coverage reports 71 published tools", coverage.get("publishedToolCount") == 71, str(coverage))
    check("capability coverage reports 71 described tools", coverage.get("describedToolCount") == 71, str(coverage))
    check("capability coverage has no missing or undeclared descriptors", not coverage.get("missingDescriptors") and not coverage.get("undeclaredDescriptors"), str(coverage))
    implementation = r.get("implementationInventory", {})
    bindings = implementation.get("toolImplementationBindings", {})
    check("implementation inventory is complete", implementation.get("complete") is True, str(implementation))
    check("implementation inventory binds 71 tools", implementation.get("toolBindingCount") == 71 and len(bindings) == 71, str(implementation))
    check("implementation inventory resolves 60 classes", implementation.get("implementationClassCount") == 60, str(implementation))
    check("implementation inventory exposes 205 factory equipment types", implementation.get("equipmentTypeCount") == 205, str(implementation))
    report_paths = implementation.get("reportPaths", [])
    check("implementation inventory exposes two report paths", implementation.get("reportPathCount") == 2 and [path.get("tool") for path in report_paths] == ["generateReport","bridgeTaskWorkflow"], str(report_paths))
    check("canonical process and report implementations are explicit", bindings.get("runProcess") == "neqsim.mcp.runners.ProcessRunner" and bindings.get("generateReport") == "neqsim.mcp.runners.ReportRunner" and bindings.get("bridgeTaskWorkflow") == "neqsim.mcp.runners.TaskWorkflowBridge", str(bindings))
    evidence = r.get("phase0EvidenceInventory", {})
    tests = evidence.get("tests", {})
    guides = evidence.get("guides", {})
    limitations = evidence.get("knownLimitations", {})
    check("evidence inventory freezes 67 Java test classes", tests.get("javaTestClassCount") == 67, str(tests))
    check("evidence inventory freezes 94 protocol scenarios", tests.get("protocolScenarioCount") == 94, str(tests))
    check("evidence inventory lists eight MCP guides", guides.get("guideCount") == 8 and len(guides.get("entries",[])) == 8, str(guides))
    baseline_contract = evidence.get("acceptanceBaselineContract", {})
    check("acceptance baseline contract is bounded and non-qualifying", baseline_contract.get("fixtureCount") == 4 and baseline_contract.get("repeatRunCount") == 2 and baseline_contract.get("executionMode") == "ON_DEMAND_TEST_HARNESS" and baseline_contract.get("performanceQualification") is False and baseline_contract.get("scientificValidationComplete") is False, str(baseline_contract))
    ...
    guide_paths = [repo_root / entry.get("path", "") for entry in guides.get("entries", [])]
    check("Java test source count matches the evidence inventory", len(java_test_files) == tests.get("javaTestClassCount"), f"found {len(java_test_files)}")
    check("protocol scenario count matches the evidence inventory", len(protocol_scenarios) == tests.get("protocolScenarioCount"), f"found {len(protocol_scenarios)}")
    check("every inventoried MCP guide resolves on the exact source tree", all(path.is_file() for path in guide_paths), str(guide_paths))
    trust_report = call_tool("getBenchmarkTrust", {"action":"getAll"})
    explicit_trust = trust_report.get("tools", {})
    limitation_count = sum(len(tool.get("knownLimitations",[])) for tool in explicit_trust.values())
    validation_case_count = sum(len(tool.get("validationCases",[])) for tool in explicit_trust.values())
    verified_case_count = sum(1 for tool in explicit_trust.values() for case in tool.get("validationCases",[]) if "verifiedBy" in case)
    check("limitation inventory reconciles the runtime trust report", set(explicit_trust) == set(limitations.get("explicitTrustTools",[])) and limitation_count == limitations.get("knownLimitationCount") and validation_case_count == limitations.get("validationCaseCount") and verified_case_count == limitations.get("verifiedValidationCaseCount"), str(limitations))
    contract_tools = {"getCapabilities","getSchema","getExample","getBenchmarkTrust","checkToolAccess","manageIndustrialProfile","searchComponents","queryDataCatalog","getProgress","inspectApi","manageValidationProfile"}
    coverage_records = limitations.get("coverageRecords", {})
    check("eleven non-numerical contracts have bounded evidence", limitations.get("contractTestedToolCount") == 11 and limitations.get("confirmedGapToolCount") == 40 and set(limitations.get("contractTestedTools",[])) == contract_tools and all(coverage_records.get(tool,{}).get("coverageStatus") == "CONTRACT_TESTED" for tool in contract_tools), str(limitations))
    contract_sources = [source for tool in contract_tools for source in coverage_records.get(tool,{}).get("contractEvidenceSources",[])]
    check("contract evidence paths resolve on the exact source tree", all((repo_root/source).is_file() for source in contract_sources) and all(len(coverage_records.get(tool,{}).get("contractEvidenceSources",[])) == coverage_records.get(tool,{}).get("contractEvidenceCount") for tool in contract_tools), str(contract_sources))
    check("uncovered tool-specific trust remains an explicit Phase 0 gap", limitations.get("publishedToolCount") == 71 and limitations.get("explicitTrustToolCount") == 20 and limitations.get("genericTrustToolCount") == 51 and limitations.get("confirmedGapToolCount") == 40 and limitations.get("unsupportedConditionCount") == 0 and limitations.get("complete") is False and evidence.get("complete") is False, str(limitations))


def test_run_capability_search_and_invoke():
    print("\n=== Generic Runtime Capability ===")
    search = call_tool("runCapability", {"capabilityJson":json.dumps({"action":"search","query":"sulfur vapour pressure","limit":25})})
    ...
    invoke = call_tool("runCapability", {"capabilityJson":json.dumps({"action":"invoke","className":"neqsim.thermo.util.sulfur.SulfurThermodynamics","methodName":"calculateVapourPressureBar","parameterTypes":["double"],"arguments":[717.76]})})
    check("capability invoke status=success", invoke.get("status") == "success", invoke.get("message", ""))
    check("capability invoke result", abs(invoke.get("result",0.0)-1.01325) < 1.0e-10, f"got {invoke.get('result')}")


def test_pvt_saturation_pressure():
    print("\n=== PVT: Saturation Pressure ===")
    r = call_tool("runPVT", {"components":json.dumps({"methane":0.6,"ethane":0.1,"propane":0.1,"n-heptane":0.2}),"model":"SRK","temperature_C":80.0,"pressure_bara":200.0,"experiment":"saturationPressure"})
...
def test_flow_assurance_hydrate():
    ...
    r = call_tool("runFlowAssurance", {"components":json.dumps({"methane":0.85,"ethane":0.08,"propane":0.04,"water":0.03}),"model":"CPA","temperature_C":4.0,"pressure_bara":100.0,"analysis":"hydrateRiskMap"})
    check("hydrate status=success", r.get("status") == "success", r.get("message", ""))


def test_calculate_standard_iso6976():
    print("\n=== Standards: ISO 6976 ===")
    r = call_tool("calculateStandard", {"components":json.dumps({"methane":0.90,"ethane":0.06,"propane":0.03,"nitrogen":0.01}),"model":"SRK","temperature_C":15.0,"pressure_bara":1.01325,"standard":"ISO6976"})
...
def test_pipeline_single_phase_gas():
    ...
    r = call_tool("runPipeline", {"components":json.dumps({"methane":0.95,"ethane":0.05}),"model":"SRK","temperature_C":25.0,"pressure_bara":100.0,"flowRate":json.dumps({"value":5000.0,"unit":"kg/hr"}),"pipe":json.dumps({"diameter_m":0.2,"length_m":10000.0,"elevation_m":0.0,"roughness_m":0.00005,"numberOfIncrements":10})})
...
def test_materials_review_stid_register():
    ...
    payload = {"projectName":"Synthetic MCP materials review","designLifeYears":25,"materialsRegister":[{"tag":"DEMO-LINE-001","equipmentType":"Pipeline","existingMaterial":"Carbon Steel API 5L X65","sourceReferences":["synthetic STID line-list row 1"],"service":{"temperature_C":85.0,"pressure_bara":95.0,"co2_mole_fraction":0.04,"h2s_mole_fraction":0.0008,"free_water":True,"chloride_mg_per_l":55000.0,"pH":5.2,"flow_velocity_m_per_s":7.5}}]}
...
def test_reservoir_simple_tank():
    ...
    r = call_tool("runReservoir", {"components":json.dumps({"methane":0.85,"ethane":0.10,"propane":0.05}),"model":"SRK","reservoirTemperature_C":100.0,"reservoirPressure_bara":200.0,"gasVolume_Sm3":1e9,"oilVolume_Sm3":0.0,"waterVolume_Sm3":0.0,"producers":json.dumps([{"name":"well1","flowRate":{"value":100000.0,"unit":"Sm3/day"}}])})
...
def test_field_economics_production_profile():
    ...
    r = call_tool("runFieldEconomics", {"mode":"productionProfile","declineType":"EXPONENTIAL","initialRate_bblPerDay":10000.0,"annualDeclineRate":0.15,"startYear":2025,"totalYears":20})
...
def test_dynamic_separator():
    ...
    process_json = {"fluid":{"components":{"methane":0.7,"propane":0.2,"nC10":0.1},"model":"SRK","temperature_C":25.0,"pressure_bara":50.0},"process":{"equipment":[{"type":"stream","name":"feed","flowRate":{"value":1000.0,"unit":"kg/hr"}},{"type":"separator","name":"sep","inlet":"feed"}]}}
    r = call_tool("runDynamic", {"processJson":json.dumps(process_json),"duration_seconds":10.0,"timeStep_seconds":1.0})
...
def test_bioprocess_anaerobic_digester():
    ...
    r = call_tool("runBioprocess", {"reactorType":"anaerobicDigester","substrateType":"FOOD_WASTE","feedRate_kgPerHr":100.0,"totalSolidsFraction":0.15,"temperature_C":37.0})
    check("bioprocess status=success", r.get("status") == "success", r.get("message", ""))


def test_size_separator():
    print("\n=== Equipment Sizing: Separator ===")
    r = call_tool("sizeEquipment", {"equipmentType":"separator","model":"SRK","temperature_C":25.0,"pressure_bara":50.0,"components":json.dumps({"methane":0.7,"propane":0.2,"nC10":0.1}),"flowRate":json.dumps({"value":10000.0,"unit":"kg/hr"}),"orientation":"horizontal","liquidRetentionTime_min":5.0})
    check("size sep status=success", r.get("status") == "success", r.get("message", ""))


def test_size_compressor():
    print("\n=== Equipment Sizing: Compressor ===")
    r = call_tool("sizeEquipment", {"equipmentType":"compressor","model":"SRK","temperature_C":25.0,"pressure_bara":20.0,"components":json.dumps({"methane":0.95,"ethane":0.05}),"flowRate":json.dumps({"value":5000.0,"unit":"kg/hr"}),"outletPressure_bara":80.0,"polytropicEfficiency":0.80})
    check("size comp status=success", r.get("status") == "success", r.get("message", ""))


def test_design_utilities():
    print("\n=== Utility Design ===")
    r = call_tool("designUtilities", {"utilityType":"boiler","name":"HP Boiler","boilerEfficiency":0.85,"duties":json.dumps([{"name":"Reboiler","dutyKW":5000.0}])})
    check("boiler status=success", r.get("status") == "success", r.get("message", ""))
    r = call_tool("designUtilities", {"utilityType":"refrigeration","name":"Propane Chiller","evaporatorTempC":-35.0,"condenserTempC":35.0,"duties":json.dumps([{"name":"Gas Chilling","dutyKW":3000.0}])})
    check("refrigeration status=success", r.get("status") == "success", r.get("message", ""))
    r = call_tool("designUtilities", {"utilityType":"nitrogen","name":"N2 Generation","nitrogenDemandNm3h":500.0,"purityPercent":99.5,"generationMethod":"MEMBRANE"})
    check("nitrogen status=success", r.get("status") == "success", r.get("message", ""))
    r = call_tool("designUtilities", {"utilityType":"steamNetwork","name":"Steam System","levels":json.dumps([{"name":"HP","pressureBara":42.0,"saturationTempC":253.0},{"name":"LP","pressureBara":4.5,"saturationTempC":148.0}]),"demands":json.dumps([{"level":"HP","demandKgh":8000.0},{"level":"LP","demandKgh":5000.0}]),"localGeneration":json.dumps([{"level":"LP","generationKgh":2000.0}])})
    check("steamNetwork status=success", r.get("status") == "success", r.get("message", ""))


def test_compare_processes():
    print("\n=== Process Comparison ===")
    case = {"fluid":{"components":{"methane":0.9,"ethane":0.1},"model":"SRK","temperature_C":25.0,"pressure_bara":50.0},"process":{"equipment":[{"type":"stream","name":"feed","flowRate":{"value":1000.0,"unit":"kg/hr"}},{"type":"separator","name":"sep","inlet":"feed"}]}}
    r = call_tool("compareProcesses", {"cases":json.dumps([{"name":"Case-A",**case},{"name":"Case-B",**case}])})
...
def test_validate_results():
    ...
    r = call_tool("validateResults", {"resultsJson":json.dumps(flash_r),"context":"general"})
    check("validateResults status=success", r.get("status") == "success", r.get("message", ""))


def test_cross_validate_models():
    print("\n=== Cross-Validate Models ===")
    base = {"fluid":{"components":{"methane":0.9,"ethane":0.1},"model":"SRK","temperature_C":25.0,"pressure_bara":50.0},"process":{"equipment":[{"type":"stream","name":"feed","flowRate":{"value":1000.0,"unit":"kg/hr"}},{"type":"separator","name":"sep","inlet":"feed"}]}}
    r = call_tool("crossValidateModels", {"baseProcess":json.dumps(base),"models":json.dumps(["SRK","PR"]),"compareVariables":json.dumps([{"address":"sep.gasOutStream.temperature","unit":"C"}])})
...
def test_parametric_study():
    print("\n=== Parametric Study ===")
    base = {"fluid":{"components":{"methane":0.9,"ethane":0.1},"model":"SRK","temperature_C":25.0,"pressure_bara":50.0},"process":{"equipment":[{"type":"stream","name":"feed","flowRate":{"value":1000.0,"unit":"kg/hr"}},{"type":"separator","name":"sep","inlet":"feed"}]}}
    r = call_tool("runParametricStudy", {"baseProcess":json.dumps(base),"sweeps":json.dumps([{"address":"feed.pressure","unit":"bara","from":30.0,"to":80.0,"steps":3}]),"outputs":json.dumps([{"address":"sep.gasOutStream.temperature","unit":"C"}]),"mode":"one_at_a_time"})
    check("parametric status=success", r.get("status") == "success", r.get("message", ""))


def test_session_lifecycle():
    print("\n=== Session: Lifecycle ===")
    r = call_tool("manageSession", {"action":"create","fluid":json.dumps({"methane":0.9,"ethane":0.1})})
    ...
    if session_id:
        r3 = call_tool("manageSession", {"action":"close","sessionId":session_id})
        check("session close status=success", r3.get("status") == "success", r3.get("message", ""))


def test_solve_task():
    print("\n=== Solve Task ===")
    r = call_tool("solveTask", {"task":"Calculate the density of methane at 25C and 50 bar","fluid":json.dumps({"methane":1.0})})
    check("solveTask status=success", r.get("status") == "success", r.get("message", ""))


def test_compose_workflow():
    print("\n=== Compose Workflow ===")
    r = call_tool("composeWorkflow", {"workflow":"test-workflow","fluid":json.dumps({"methane":0.9,"ethane":0.1}),"steps":json.dumps([{"runner":"flash","name":"step1","input":{"temperature_C":25.0,"pressure_bara":50.0}}])})
...
def test_generate_report():
    print("\n=== Generate Report ===")
    flash_r = run_flash({"methane":0.9,"ethane":0.1},25.0,50.0)
    r = call_tool("generateReport", {"reportType":"process_summary","title":"Test Report","data":json.dumps(flash_r)})
...
def test_bridge_task_workflow_schema():
    ...
    check("bridge schema has schema", "schema" in r or "resultsJsonSchema" in str(r), str(r)[:200])
...
def test_bridge_task_workflow_convert():
    ...
    r = call_tool("bridgeTaskWorkflow", {"action":"toResultsJson","toolOutput":json.dumps(flash_r),"toolName":"runFlash","taskTitle":"E2E Test Flash"})
...
def test_visualization_bar_chart():
    ...
    r = call_tool("generateVisualization", {"type":"barChart","labels":json.dumps(["A","B","C"]),"values":json.dumps([10.0,20.0,30.0])})
...
def test_industrial_profile_active():
    ...
    check("profile mode is DESKTOP_ENGINEER", r.get("activeMode") == "DESKTOP_ENGINEER", f"got {r.get('activeMode')}")


def test_industrial_profile_classify():
    print("\n=== Industrial Profile: Classify Tool ===")
    r = call_tool("manageIndustrialProfile", {"action":"classifyTool","toolName":"runFlash"})
    check("classify status=success", r.get("status") == "success", r.get("message", ""))
    check("runFlash is TRUSTED_CORE", r.get("category") == "TRUSTED_CORE", f"got category={r.get('category')}")
...
def test_benchmark_trust_all():
    ...
    check("trust has tools", "tools" in r, f"keys: {list(r.keys())}")
    tools = r.get("tools", {})
...
def test_benchmark_trust_single():
    ...
    r = call_tool("getBenchmarkTrust", {"action":"getTool","toolName":"runFlash"})
    check("trust single status=success", r.get("status") == "success", r.get("message", ""))
    check("trust has trust object", "trust" in r)
    trust = r.get("trust", {})
    check("flash trust maturity=VALIDATED", trust.get("maturityLevel") == "VALIDATED", f"got {trust.get('maturityLevel')}")
