...
require(source, 'inventory.addProperty("inventoryVersion", "1.39")', SOURCE_PATH)
require(
    source,
    "All 71 tools have coverage records; 39 are CONTRACT_TESTED and 12 remain "
    "CONFIRMED_GAP.",
    SOURCE_PATH,
)
...
require(
    surface,
    "scientifically validated: 12 records remain\n"
    "`CONFIRMED_GAP`, thirty-nine are `CONTRACT_TESTED`",
    SURFACE_PATH,
)
...
contract_line = next(
    (
        ...
        if line.startswith("- Thirty-nine bounded software contracts")
    ),
    None,
)
...
if len(contract_tools) != 39 or "runSIL" not in contract_tools or "compareProcesses" not in contract_tools:
    raise AssertionError(
        f"{FOUNDATION_PATH}: expected 39 named contracts including runSIL and compareProcesses, "
        f"found {len(contract_tools)}"
    )
require(foundation, "- 12 tools remain `CONFIRMED_GAP`", FOUNDATION_PATH)

api_reference = API_PATH.read_text(encoding="utf-8")
require(
    api_reference,
    "all 71 tools\nhave coverage records, but only 20 have tool-specific trust pages; "
    "39 generic-fallback tools have\nbounded `CONTRACT_TESTED` evidence and "
    "12 remain `CONFIRMED_GAP`",
    API_PATH,
)
...
print("Phase 0 documentation accounting is consistent: 1.39 / 20 + 39 + 12")
