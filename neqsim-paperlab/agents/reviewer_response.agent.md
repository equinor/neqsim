...
- `diff_r<N>.md` — Highlighted changes (text summary)
- `diff_r<N-1>_vs_paper.html` — Visual HTML diff (generated by tool)

## Tool Integration

### Visual Diff Report (MANDATORY after each revision)

After revising the manuscript, generate a color-coded HTML diff:

```bash
python paperflow.py diff papers/<paper_slug>/
```

This compares the revision baseline (`revision_N/paper_r<N-1>_baseline.md`) with
the current `paper.md` and produces an HTML report showing line-level changes,
section additions/removals, and word count delta. Attach the report summary to
the response letter to help reviewers see exactly what changed.

### Prose Quality Check (after revision edits)

```bash
python paperflow.py check-prose papers/<paper_slug>/
```

Verify that revisions haven't degraded writing quality (e.g., rushed additions
introducing passive voice or long sentences).

### Citation Discovery (if reviewers request additional references)

```bash
python paperflow.py suggest-refs papers/<paper_slug>/
```

Use when reviewers say "should cite X" or "missing key references" — the tool
finds highly-cited papers matching your topic that aren't in `refs.bib`.
