# Electrochemistry and Electrolyzer Stack Models
...
1. Explain how to simulate PEM, alkaline, SOEC, and AEM electrolyzers with I-V and Faradaic efficiency.
2. Translate the topic into a reproducible NeqSim Python workflow.
3. Identify the dominant assumptions and sanity checks for green and pink H2 production from water and power.
...
Simulate PEM, alkaline, SOEC, and AEM electrolyzers with I-V and Faradaic efficiency. The practical setting is green and pink H2 production from water and power. In a desktop simulator this
...
4. Convert the output to engineering KPIs: stack power, specific energy, pressure, temperature, and H2 production.
...
Specific NeqSim/Python surfaces and engineering references emphasized here: **Electrolyzer, ElectrolyzerTechnology, ElectrolyzerIVCharacteristic**.
...
|---|---:|---|---|---|---|---|
| Green hydrogen | 1 | Alkaline water electrolysis | Run ElectrolyzerTechnology.ALKALINE with current-density and voltage assumptions from the technology selector. | Current density, operating pressure, Faradaic efficiency, and stack temperature. | Cell voltage, kWh/kg H2, hydrogen rate, cooling or heat-rejection index. | \cite{doeElectrolysis,buttler2018water,irena2022greenhydrogen} |
| Green hydrogen | 2 | PEM electrolysis | Run ElectrolyzerTechnology.PEM with pressure operation, I-V curve, and compression-offset accounting. | Current density, pressure, membrane resistance, and Faradaic efficiency. | Specific energy, stack power, pressure credit, CAPEX index, durability attention index. | \cite{carmo2013pem,doeElectrolysis,iso22734} |
| Green hydrogen | 3 | Anion exchange membrane electrolysis | Run ElectrolyzerTechnology.AEM as an emerging low-precious-metal case with a membrane-resistance sensitivity. | AEM resistance factor, pressure, current density, and technology maturity factor. | Specific energy, CAPEX index, durability risk index, water-management attention. | \cite{doeElectrolysis,irena2022greenhydrogen,roger2017catalysts} |
| Green hydrogen | 4 | Solid oxide electrolysis cells | Run ElectrolyzerTechnology.SOEC with high-temperature efficiency and heat-credit assumptions. | Stack temperature, heat-credit fraction, current density, and steam-feed condition. | Electrical kWh/kg H2, thermal-duty credit, overall energy index, high-temperature materials attention. | \cite{doeElectrolysis,buttler2018water,irena2022greenhydrogen} |
| Green hydrogen | 5 | Electrocatalysts for HER and OER | Represent HER/OER catalyst improvements as reduced activation overpotential in the I-V characteristic. | HER overpotential, OER overpotential, Tafel slope proxy, and noble-metal loading index. | Cell-voltage reduction, kWh/kg H2 reduction, catalyst-cost index, stability attention index. | \cite{roger2017catalysts,buttler2018water,doeElectrolysis} |
| Green hydrogen | 6 | Membrane and ionomer development | Model membrane changes as area-specific resistance, crossover, and degradation factors in a stack screening table. | Membrane resistance, gas-crossover index, PFAS-free material flag, and lifetime multiplier. | Ohmic-loss voltage, Faradaic-efficiency loss, durability index, product-purity attention. | \cite{carmo2013pem,iso22734,doeElectrolysis} |
...
| Technology | Cell voltage (V) | Specific energy (kWh/kg H2) | Relative CAPEX index |
|---|---|---|---|
| PEM | 1.8 | 52.0 | 1.0 |
| Alkaline | 1.85 | 56.0 | 0.75 |
| SOEC | 1.3 | 38.0 | 1.7 |
| AEM | 1.85 | 60.0 | 1.15 |

![Figure 7.2: Expected electrolyzer technology output from stack I-V and cost-hook cells.](figures/notebook_electrolyzer_technology_output.png)
...
chapter notebook should be longer and more explicit. For green and pink H2 production from water and power, build the
notebook around the technology base case, a current-density sweep, and a stack-cost screening case. Keep the cells small enough that a reviewer can run
...
| KPI cell | Extract stack power, specific energy, pressure, temperature, and H2 production. | A table with units and engineering labels. |
...
For Electrochemistry and Electrolyzer Stack Models, a strong final cell should contain a compact executive table. It
...
include stack power, specific energy, pressure, temperature, and H2 production, plus a note about the thermodynamic model, the process
...
![Figure 7.1: Evidence-backed calculation and study basis for Electrochemistry and Electrolyzer Stack Models.](figures/fig_ch07_iv_curves.png)
...
**Discussion.** The figure turns green and pink H2 production from water and power into a data-backed study with traceable assumptions. The plotted values are generated from the chapter's calculation table, while the basis panel records the independent study or standard used to interpret stack power, specific energy, pressure, temperature, and H2 production. The physical mechanism behind the figure is
...
remain in the model. For green and pink H2 production from water and power, a simple equilibrium or equipment block is
...
case for green and pink H2 production from water and power is converged, the next study can sweep feed composition,
...
For green and pink H2 production from water and power, a useful design-review plot has three properties. First, the axes
...
- What decision would change if stack power, specific energy, pressure, temperature, and H2 production moved by 10 percent?
...
- Set up a NeqSim Python notebook that addresses green and pink H2 production from water and power with an explicit
  fluid, equipment block, run, and validation step.
- Identify the KPIs (stack power, specific energy, pressure, temperature, and H2 production) and report them with units and a screening band.
...
This chapter positioned green and pink H2 production from water and power inside a reproducible NeqSim workflow. The main
