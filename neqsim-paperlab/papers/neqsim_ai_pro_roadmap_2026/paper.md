# NeqSim as Computational Backbone for AI-Driven Integrated Production Optimization: Capabilities, Gaps, and a Development Roadmap

<!-- Target Journal: Computers & Chemical Engineering -->
<!-- Paper Type: application / perspective -->
<!-- Generated: 2026-03-31 -->
<!-- Status: FINAL -->

## Highlights

- NeqSim open-source process simulator capabilities mapped to AI-integration requirements
- Surrogate equipment interface enables hybrid physics-AI process simulation
- Gymnasium-compatible wrapper design for RL-based production optimization
- Equipment utilization framework provides structured bottleneck metrics for RL rewards
- MCP server enables direct LLM-to-simulation coupling for agentic workflows
...
### 1.1 The computational challenge in integrated production optimization
...
### 1.2 The role of open-source process simulation
...
1. **Ground-truth generator.** The simulator provides the rigorous thermodynamic and process calculations against which surrogate models are trained and validated.
2. **RL environment.** The simulator serves as the environment in which RL agents observe states, take actions, and receive rewards.
3. **Hybrid simulation host.** In hybrid architectures, some equipment uses full physics while other equipment uses trained surrogates --- the simulator must accommodate both seamlessly.
4. **Digital twin component.** The simulator provides the process domain calculations in a modular digital twin that couples with reservoir and flow domain models.
5. **Agentic interface.** The simulator exposes its capabilities to LLM agents through structured protocols for natural-language-driven simulation.
...
## 2. Requirements for AI-Driven Integrated Production Optimization
...
### 2.1 Surrogate training data generation (R1)
...
- **R1.1 Throughput.** Generate $10^5$--$10^6$ simulation evaluations efficiently. At 100 ms per evaluation, $10^5$ evaluations require approximately 2.8 hours --- feasible for overnight batch runs.
- **R1.2 Parametric variation.** Systematically vary inlet conditions (pressure, temperature, flow rate, composition), equipment parameters (setpoints, capacities), and operating modes across the expected operating envelope.
- **R1.3 Output completeness.** Extract not only stream conditions but also equipment-level metrics: power consumption, duty, pressure drops, efficiency, capacity utilization, and phase fractions.
- **R1.4 Physical consistency.** Every data point must satisfy mass and energy conservation to within numerical tolerance, ensuring that surrogates trained on this data inherit physical consistency.
- **R1.5 Failure mode coverage.** Include operating points near equipment limits (compressor surge, separator flooding, valve choking) where nonlinear behavior is most pronounced and surrogates are most likely to fail.

### 2.2 Reinforcement learning environment (R2)

For RL-based optimization, the simulator must function as a Gymnasium-compatible environment \cite{Brockman2016, Towers2023}:

- **R2.1 State observation.** At each timestep, return a structured observation vector containing all measurable quantities: stream pressures, temperatures, flow rates, compositions, equipment operating points, and utilization metrics.
- **R2.2 Action application.** Accept an action vector that modifies controllable variables: valve positions, compressor speeds, separator pressure setpoints, routing decisions, and injection rates.
- **R2.3 Reward computation.** Compute a scalar reward from production rate, energy consumption, equipment health metrics, and safety constraint satisfaction.
- **R2.4 Episode management.** Support `reset()` to reinitialize the process to a reference state, `step(action)` to advance one timestep, and `render()` for visualization.
- **R2.5 Constraint encoding.** Explicitly represent hard constraints (maximum pressures, minimum flows, trip limits) so the RL agent can learn to respect safety boundaries.

### 2.3 Hybrid simulation (R3)

In hybrid architectures, some equipment units use rigorous physics while others use trained neural network surrogates:

- **R3.1 Surrogate equipment interface.** Each equipment unit must be replaceable by a surrogate that accepts the same inputs and produces the same outputs, with no changes to the flowsheet wiring.
- **R3.2 Conservation enforcement.** The coupling between physics-based and surrogate units must preserve mass and energy balance at every connection.
- **R3.3 Selective fidelity.** The simulator must support per-unit fidelity selection: some units at full physics, others at surrogate level, and the ability to switch dynamically.
- **R3.4 Model interchange format.** Surrogate models trained externally (e.g., in PyTorch or TensorFlow) must be importable in a standardized format (e.g., ONNX \cite{ONNX2019}).

### 2.4 Digital twin coupling (R4)

The process simulator is one domain in a multi-domain digital twin:

- **R4.1 Interface variable contract.** Formally defined interface variables (pressure, temperature, flow rate, composition) at domain boundaries with specified units and physical constraints.
- **R4.2 Iterative coupling.** Support repeated calls from an external orchestrator that passes interface boundary conditions and receives the simulator's response, converging the coupled system iteratively.
- **R4.3 State serialization.** Save and restore complete simulation state for checkpoint/restart in long-running digital twin operations.
- **R4.4 Programmatic API.** All operations accessible through a well-documented programmatic API (not just command-line or GUI).
- **R4.5 Structured result reporting.** Return results as structured data (JSON) with error codes, warnings, and remediation hints for automated error handling.

### 2.5 Agentic AI access (R5)

For LLM-mediated simulation access:

- **R5.1 Tool discovery.** Expose simulation capabilities as discoverable tools with structured input/output schemas.
- **R5.2 Natural language bridge.** Accept high-level descriptions that can be translated to simulation configurations through the tool interface.
- **R5.3 Example-driven learning.** Provide embedded examples and templates that LLMs can reference when constructing simulation requests.
- **R5.4 Validation before execution.** Validate simulation inputs before execution, returning actionable error messages when inputs are invalid.
- **R5.5 Standardized protocol.** Use an established protocol (e.g., MCP) that multiple LLM clients support natively.

---

## 3. Current Capabilities of NeqSim
...
**Cubic equations of state.** Soave-Redlich-Kwong (SRK) \cite{Soave1972} and Peng-Robinson (PR) \cite{PengRobinson1976} with multiple alpha function variants (Mathias-Copeman, Twu-Coon), volume translation (Peneloux), and advanced mixing rules (Huron-Vidal, PSRK, UMR-PRU). These are the workhorses for natural gas and hydrocarbon systems and execute in microseconds per evaluation.

**Association equations of state.** Cubic-Plus-Association (CPA) \cite{Kontogeorgis1996} for systems with hydrogen bonding (water, methanol, MEG, glycols), including electrolyte extensions for brine systems. Essential for accurate water content prediction in gas processing.

**Reference equations of state.** GERG-2008 \cite{Kunz2012} for custody-transfer-quality natural gas calculations, Span-Wagner for pure CO$_2$, IAPWS-IF97 for steam, and specialized EOS for hydrogen (Leachman) and LNG (Vega).
...
### 3.3 Equipment utilization and bottleneck analysis
...
- Per-equipment capacity constraints with design, maximum, and minimum values
- Real-time utilization calculation as percentage of rated capacity
- Bottleneck detection (utilization > threshold)
- Debottlenecking recommendations

**CapacityConstraint class.** Models individual constraints with:
- `getUtilization()` returning a ratio (0.0 to 1.0+)
- `isExceeded()` testing if capacity is exceeded
- `isNearLimit()` testing against configurable warning thresholds
- `getHeadroom()` returning remaining available capacity

**BottleneckAnalyzer class.** Operates on a complete process system to:
- Identify and rank all bottlenecks by utilization
- Return the primary (most constrained) bottleneck
- Filter active bottlenecks above a configurable threshold
- Provide structured `BottleneckResult` objects with equipment name, utilization percentage, constraint type, and debottlenecking recommendation

**ProcessSystem-level integration.** The process system provides:
- `getCapacityUtilizationSummary()` returning a plant-wide utilization map
- `getBottleneck()` returning the primary system bottleneck
...
### 3.4 Process system orchestration and JSON API

**ProcessSystem class.** The central flowsheet orchestrator provides:
- Sequential execution of equipment in insertion order
- Recycle convergence and adjuster coordination
- Named equipment lookup: `getUnit(name)`
- Unified element query: `getAllElements()` returning equipment, controllers, and measurement devices
- Typed connection metadata: `connect(source, target, type, label)`
- Deep cloning: `copy()` for parallel evaluations
...
- The constructed `ProcessSystem` (for further programmatic access)
- Structured error details with machine-readable codes and remediation hints
- Tolerant mode where wiring failures become warnings rather than errors

**Assessment against R4.4, R4.5:** The programmatic API and JSON interface satisfy the digital twin coupling requirements. The structured error reporting with remediation hints is particularly valuable for automated error handling in AI middleware.

### 3.5 MCP server for agentic AI access
...
- **`ProcessSystem.runTransient(dt, UUID)`:** Advances the process by one timestep of duration `dt`.
- **DynamicProcessHelper:** Auto-instruments a steady-state process with PID controllers and measurement devices (pressure, temperature, level, flow transmitters), configures default tuning, and wires controllers to valves.
- **Named controllers:** Multiple PID controllers per equipment via `addController(tag, controller)`.
- **TwoFluidPipe:** Transient two-fluid pipe model with 7 conservation equations, AUSM+ flux splitting, and slug tracking.
...
### 3.7 Multiphase flow and pipeline network

NeqSim provides three levels of pipeline modeling:

- **`PipeBeggsAndBrills`:** Steady-state multiphase pressure drop using the Beggs and Brill correlation \cite{BeggsBrill1973} with formation temperature gradient support.
- **`TwoFluidPipe`:** Transient two-fluid model with 7 conservation equations, multiple boundary condition types, terrain following, and slug tracking.
- **`LoopedPipeNetwork`:** Looped network solver using the Hardy Cross method \cite{Cross1936} with source/sink/junction nodes, loop detection, and iterative convergence.

**Assessment against R1, R4:** These capabilities cover the wellbore-to-process transport domain within the integrated model, providing the multiphase flow calculations needed at the interface between reservoir/well and process domains.

### 3.8 Capability coverage summary
...
## 4. Gap Analysis and Development Roadmap
...
### 4.1 Development Area 1: Surrogate Equipment Interface (Priority: Critical)
...
**Model interchange.** We recommend ONNX \cite{ONNX2019} as the model interchange format, with a lightweight Java ONNX Runtime binding for inference. This allows surrogates trained in PyTorch, TensorFlow, JAX, or any ONNX-exporting framework to be loaded and executed within the Java simulation.
...
### 4.2 Development Area 2: RL Environment Wrapper (Priority: Critical)
...
### 4.3 Development Area 3: Reservoir-Process Coupling Interface (Priority: High)
...
1. Reservoir simulator computes well inflow at current wellhead pressure
2. Orchestrator passes wellhead conditions to process simulator
3. Process simulator computes facility response (production rates, backpressure)
4. Orchestrator feeds back the effective backpressure to reservoir simulator
5. Repeat until convergence (pressure balance at wellhead)
...
### 4.4 Development Area 4: Automatic Differentiation Support (Priority: Medium)
...
### 4.5 Development Area 5: High-Throughput Batch Execution (Priority: Medium)
...
### 4.6 Development roadmap summary
...
![Development roadmap](figures/fig6_gap_roadmap.png)
*Figure 6.* AI-PRO development roadmap showing the timeline for the five development areas and their alignment with project work packages and deliverables. Milestone markers indicate key deliverable dates.
...
1. **Flash calculations** at three complexity levels:
   - Lean gas (5 components: CH$_4$, C$_2$H$_6$, C$_3$H$_8$, nC$_4$, nC$_5$), SRK EOS
   - Natural gas (9 components: as above + iC$_4$, iC$_5$, N$_2$, CO$_2$), SRK EOS
   - Oil-gas-water (13 components: as above + nC$_6$, nC$_7$, nC$_8$, water), SRK EOS with multiphase check

2. **Process equipment configurations:**
   - Single separator (feed → 2-phase separator)
   - Two-stage separation (HP separator → LP separator)
   - Three-stage compression train (compressor → cooler → compressor → cooler → compressor)
   - Full process (2-stage separation + compression + cooling)
...
### 5.2 Flash calculation performance

Figure 9 shows flash execution time as a function of system complexity (number of components).

![Flash scaling](figures/fig9_flash_scaling.png)
*Figure 9.* Flash calculation execution time vs. number of components. Error bars show $\pm 1\sigma$. The quadratic fit suggests $O(n_c^2)$ scaling, consistent with mixing rule evaluation dominating the computation.
...
### 5.3 Process equipment performance

Figure 3 presents execution times across the tested equipment configurations.

![Execution times](figures/fig3_execution_times.png)
*Figure 3.* Execution time across representative process configurations. Bars show mean execution time on logarithmic scale; error bars show $\pm 1\sigma$. Color coding distinguishes flash calculations (blue), individual equipment (green), full process (orange), and RL steps (purple).

Key results:

- **Single separator:** $6.5 \pm 17.6$ ms (first-evaluation warm-up; steady-state: $\sim$0.6 ms)
- **Two-stage separation:** $13.3 \pm 11.5$ ms
- **Compression train:** $4.9 \pm 3.3$ ms
- **Full process:** $28.5 \pm 18.1$ ms
...
- **Mean step time:** $1.50 \pm 0.18$ ms
- **Throughput:** 669 steps/second

Figure 5 shows the step time distribution and the implications for RL training episode throughput.

![RL step distribution](figures/fig5_rl_step_distribution.png)
*Figure 5.* Left: Distribution of RL step execution times across 20 consecutive steps. The tight distribution ($\sigma = 0.18$ ms) indicates predictable per-step cost. Right: Episode duration and episode throughput as a function of episode length, demonstrating feasibility for standard DRL training regimes.

**Training feasibility analysis.** Modern deep RL algorithms (PPO \cite{Schulman2017}, SAC) typically require $10^5$--$10^7$ environment steps for training. At 669 steps/second:
...
### 5.5 Training data generation throughput
...
## 6. Proposed Software Architecture for AI Integration
...
![Surrogate architecture](figures/fig7_surrogate_architecture.png)
*Figure 7.* Hybrid physics-AI simulation architecture. Within a `ProcessSystem.run()` execution, individual equipment units can be either physics-based (green) or surrogate-eligible (yellow). The `SurrogateModelRegistry` (purple) manages trained models with physics validation and automatic fallback. The `TrainingDataCollector` captures physics-based evaluations for continuous surrogate improvement.
...
1. **Retrieves** the appropriate model for the equipment type and operating domain
2. **Validates** the input is within the model's training domain
3. **Evaluates** the neural network in forward mode
4. **Checks** the output against physical constraints (mass balance, entropy direction, phase consistency)
5. **Falls back** to the physics-based calculation if any validation check fails
...
### 6.4 Requirements-capability gap analysis

Figure 10 provides a visual comparison of current capabilities and required enhancements for each AI-PRO requirement.

![Gap matrix](figures/fig10_gap_matrix.png)
*Figure 10.* Requirements vs. current capabilities gap analysis. Green bars show current capability level (0=missing, 3=full). Red bars show the magnitude of enhancement required. The largest gaps are in PINN integration, conservation enforcement, and reservoir coupling --- areas where new development is needed.

---

## 7. Discussion

### 7.1 The case for open-source process simulation in AI research
...
1. **Reproducibility.** Every result can be exactly reproduced by any researcher, eliminating the "black box" criticism that plagues AI models trained on proprietary simulator data.
2. **Extensibility.** New equipment types, thermodynamic models, and AI interfaces can be added by the research community without vendor approval cycles.
3. **Inspection.** Researchers can examine exactly how thermodynamic properties are computed, enabling physically meaningful interpretation of surrogate behavior.
4. **Coupling.** Direct programmatic access allows tight integration with AI frameworks, RL libraries, and data pipelines without API limitations.
...
### 7.2 The surrogate fidelity challenge
...
- The separator produces a gas outlet stream with rigorously computed composition, phase properties, and enthalpy
- The surrogate compressor predicts outlet conditions based on learned input-output mapping
- Unless the compressor surrogate exactly preserves enthalpy balance, the energy balance of the downstream cooler (if physics-based) will receive inconsistent energy input
...
### 7.3 RL environment design considerations
...
1. **Modify** the simulation source code to understand how thermodynamic calculations work
2. **Build** surrogate models using real simulation data (not toy problems)
3. **Train** RL agents on physics-based environments
4. **Deploy** trained models back into the simulation loop
5. **Compare** surrogate vs. physics results to understand approximation trade-offs
...
**Even Solbraa:** Conceptualization, Methodology, Software, Investigation, Data Curation, Writing --- Original Draft, Writing --- Review & Editing, Visualization.
**Ashkan Jahanbani Ghahfarokhi:** Conceptualization, Methodology, Writing --- Review & Editing, Supervision, Funding Acquisition.
**Thomas Alan Adams II:** Methodology, Writing --- Review & Editing, Supervision.
...
\cite{Michelsen2007}
\cite{Soave1972}
\cite{PengRobinson1976}
\cite{Kontogeorgis1996}
\cite{Kunz2012}
\cite{BeggsBrill1973}
\cite{Cross1936}
\cite{ONNX2019}
...
\cite{Grossmann2021}
\cite{BendiksenMalnesBrateauMoe1991}
\cite{Sahinidis2004}
\cite{Bell2022}
