...
def build_book_typst_preamble(cfg, profile=None):
  ...
  #align(center)[
    ...
    # ── Copyright page (optional, controlled by settings.include_copyright) ──
    if book_builder.include_copyright(cfg):
        copyright_page_typst = rf'''// ── Copyright Page ──
...
def _preprocess_chapter(text, ch_num, figures_dir=None, key_to_num=None,
                        chapter_dir=None, submission_dir=None, cfg=None):
...
def render_book_pdf(book_dir, chapter_filter=None):
    ...
    for ch_num, ch, part_title in book_builder.iter_chapters(cfg):
        ...
        text = _preprocess_chapter(
            ...
            chapter_dir=ch_dir,
            submission_dir=submission_dir, cfg=cfg)
