 ...
 */
public class IOC {
    private static final Logger LOGGER = LogManager.getLogger(IOC.class.getName());
    private static final String[] phaseName = {"gas", "oil", "aqueous"};;

    public CalculationResult doCalculation(Double Sp1, Double Sp2, ) throws NeqSimException {
        Double[][] fluidProperties = new Double[Sp1.size()][70]; // 70 cols
        String[] calculationError = new String[Sp1.size()];
        ...
        for (int t = 0; t < Sp1.size(); t++) {
            int k = 0;
            try {
                Double Sp1 = Sp1.get(t);
                Double Sp2 = Sp2.get(t);
                ...
                if (numberOfMole!=1) {
                    calculationError[t] = "Number of moles is " + fluid.getNumberOfMoles() + " and not 1. Check input fragments.";
                    LOGGER.info("Number of moles is " + fluid.getNumberOfMoles() + " and not 1. Check input fragments.", t);
                    continue;
                }
                ...
                //fluidProperties[t][k++] = fluid.getEnthalpy()/fluid.getNumberOfMoles(); // Mix Enthalpy [J/mol]
                fluidProperties[t][k++] = fluid.getEnthalpy("J/mol");
                //fluidProperties[t][k++] = fluid.getEntropy()/fluid.getNumberOfMoles(); // Mix Entropy [J/molK]
...
//                fluidProperties[t][k++] = fluid.getCp()/fluid.getCv();// Mix Gamma (Cp/Cv)
                ...
                //fluidProperties[t][0] = fluid.getInterfacialTension("gas","oil"); // Surface Tension(N/m) between gas and oil phase** NOT USED
                //fluidProperties[t][0] = fluid.getInterfacialTension("gas","aqueous"); // Surface Tension(N/m) between gas and aqueous phase** NOT USED
                ...
                for (int j = 0; j < 3; j++) {
                    if (fluid.hasPhaseType(phaseName[j])) {
                        ...
                        fluidProperties[t][k++] = 1.0 / fluid.getPhase(phaseNumber).getDensity("mol/m3"); // Phase Molar Volume [m3/mol]
                        fluidProperties[t][k++] = fluid.getCorrectedVolumeFraction(phaseNumber) * 100;// Phase Volume Percent
                      ...
                      //  fluidProperties[t][k++] = fluid.getPhase(phaseNumber).getCp() / fluid.getPhase(phaseNumber).getCv(); // Phase Kappa (Cp/Cv)
                        fluidProperties[t][k++] = fluid.getPhase(phaseNumber).getGamma(); // Phase Gamma (Cp/Cv)
                ...
                fluidProperties[t][k++] = fluid.getInterfacialTension("gas", "oil"); // Interfacial Tension Gas/Oil [N/m]
                } else {
                    fluidProperties[t][k++] = Double.NaN;
                }
                if (fluid.hasPhaseType("gas") && fluid.hasPhaseType("aqueous")) {
                    fluidProperties[t][k++] = fluid.getInterfacialTension("gas", "aqueous"); // Interfacial Tension Gas/Aqueous [N/m]
                } else {
                    fluidProperties[t][k++] = Double.NaN;
                }
                if (fluid.hasPhaseType("oil") && fluid.hasPhaseType("aqueous")) {
                   fluidProperties[t][k++] = fluid.getInterfacialTension("oil", "aqueous"); // Interfacial Tension Oil/Aqueous [N/m]
                } else  {
                    fluidProperties[t][k++] = Double.NaN;
                }
            } catch (Exception ex) {
