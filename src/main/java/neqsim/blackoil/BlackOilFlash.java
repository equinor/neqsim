...
/**
 * Core Black-Oil flash calculator. Temperature dependence is assumed
 * to be captured by the PVT table (single Tref) unless you extend it.
 */
public final class BlackOilFlash {
  ...
  public BlackOilFlash(BlackOilPVTTable pvt,
  ...
  public BlackOilFlashResult flash(double P, double T,
                                   double Otot_std, double Gtot_std, double W_std) {
    ...
    if (Gtot_std <= Rs * Otot_std) {
      ...
      if (Gf_std < 0) Gf_std = 0.0;
      Ostd_liq = Otot_std - Rv * Gf_std;
      if (Ostd_liq < 0) Ostd_liq = 0.0;
    }
    ...
    r.Rs = Rs; r.Rv = Rv; r.Bo = Bo; r.Bg = Bg; r.Bw = Bw;
    return r;
  }
}
