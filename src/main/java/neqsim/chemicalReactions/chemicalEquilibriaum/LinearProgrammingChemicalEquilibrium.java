...
public class LinearProgrammingChemicalEquilibrium extends Object implements neqsim.thermo.ThermodynamicConstantsInterface, java.io.Serializable {
    ...
    public double[][] calcA() {
        ...
        A_matrix.print(10, 10);
        if (A_matrix.rank() < (elements.length)) {
        Amatrix = Am;
        return Am;
        } else {
        Amatrix = A;
    ...
    Matrix solved;
    Matrix atemp = new Matrix(Amatrix).getMatrix(0,Amatrix.length-1,0,Amatrix[0].length-1).copy();
    Matrix mutemp = new Matrix(chemRefPot,1).times(1.0/(R*system.getPhase(phase).getTemperature())).copy();
    Matrix lagrangeTemp = atemp.transpose().solve(mutemp.transpose()).copy();
    //bmatrix and Ans Added by Neeraj
    //bmatrix = new Matrix(bVector,1);
    //int rank = atemp.rank();
    //System.out.println("Rank of A "+rank);
    //Ans = atemp.solve(bmatrix.transpose());
    //System.out.println("Ans");
    //Ans.print(10,8);
    //Print statements added by Neeraj
    ...
    //Neeraj -- Source of error
    //lagrange have very high +ve values. So, their exp becomes infinity
    zTemp.set(i,0, Math.exp(lagrangeTemp.get(i,0)));
    }
    
    System.out.println("zMatTemp: ");
    //zTemp.print(10,5);
    ...
    fVal[i] += betaTemp.get(i,j)*Math.exp(-chemRefPot[j]/(R*system.getPhase(phase).getTemperature()))*phiTemp;
    }
    ...
    dfVal[i][j] += betaTemp.get(i,k)*alphaTemp.get(j,k)*Math.exp(-chemRefPot[k]/(R*system.getPhase(phase).getTemperature()))*phiTemp;
    }
    }
    }
    ...
    double sum=0;
    for(int k=0;k<atemp.getColumnDimension();k++){
    xEts[k] = Math.exp(-chemRefPot[k]/(R*system.getPhase(0).getTemperature()));
    //System.out.println("x check1: " + xEts[k]);
    for(int i=0;i<atemp.getRowDimension();i++) {
    xEts[k] = xEts[k]*Math.pow(zTemp.get(i,0),atemp.get(i,k));
    }
    sum += xEts[k];
    //System.out.println("x check2: " + xEts[k]);
    }
    //System.out.println("sum: " + sum);
    
    double moles=0;
    for(int k=0;k<atemp.getColumnDimension();k++){
    moles += xEts[k]*atemp.get(0,k);
    }
    //Print added by Neeraj
    //System.out.println("mole tot " + moles);
    ...
    double totm=0.0;
    for(int k=0;k<atemp.getColumnDimension();k++){
    nEts[k] = xEts[k]*moles;//system.getPhases()[1].getNumberOfMolesInPhase();
    totm += nEts[k];
    //System.out.println("N check: " + "  comp " + components[k].getComponentName() +  "  "  + nEts[k]);
    }
...
//Method added by Neeraj
 /*public double[] generateInitialEstimates(SystemInterface system,  double[] bVector, double inertMoles, int phase){
    ...
    Matrix mutemp = new Matrix(chemRefPot,1).times(1.0/(R*system.getPhase(phase).getTemperature())).copy();
    Matrix ntemp;
    atemp = new Matrix(7,7);
    btemp = new Matrix(1,7);
    //for (i=0;i<4;i++)
    for (i=0;i<5;i++)
    {
    for (j=0;j<7;j++)
    atemp.set(i,j,Amatrix[i][j]);
    btemp.set(0,i,bVector[i]);
    ...
    //btemp.print(5,5);
    //mutemp.print(5,5);
    ntemp = atemp.solve(btemp.transpose());
    ntemp.print(5,5);
    for (i=0;i<7;i++)
    n[i] = ntemp.get(i,0);
    int rank = atemp.rank();
