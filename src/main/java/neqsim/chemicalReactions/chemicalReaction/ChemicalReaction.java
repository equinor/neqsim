 ...
 * @version
 */
public class ChemicalReaction extends Object implements neqsim.thermo.ThermodynamicConstantsInterface, java.io.Serializable{
 ...
 */
    ...
    public double getRateFactor(PhaseInterface phase){
        //return rateFactor * Math.exp(-activationEnergy/R*(1.0/phase.getTemperature() - 1.0/refT));
    ...
    public double calcKgamma(neqsim.thermo.system.SystemInterface system, int phaseNumb){
        ...
        for(int i=0;i<names.length;i++){
            ...
            if(system.getPhase(phaseNumb).getComponent(names[i]).calcActivity()){
                kgamma *= Math.pow(system.getPhase(phaseNumb).getActivityCoefficient(system.getPhase(phaseNumb).getComponent(names[i]).getComponentNumber(),system.getPhase(phaseNumb).getComponent("water").getComponentNumber()),stocCoefs[i]);
            }
        }
        return  kgamma;
    }
    ...
    public void initMoleNumbers(PhaseInterface phase, ComponentInterface[] components, double[][] Amatrix, double[] chemRefPot){
        ...
        for(int i=0;i<names.length;i++){
            for(int j=0;j<components.length;j++){
                //   System.out.println("names: " +   names[i] + " " + system.getPhases()[0].getComponents()[j].getName());
    ...
    public void checkK(SystemInterface system){
        ...
        //   for(int i=0;i<names.length;i++){
        //           //  cK -= Math.log(moles[i]/system.getPhases()[0].getNumberOfMolesInPhase())*stocCoefs[i];
        //   }
    ...
    /** Getter for property reactionHeat.
     * Van't HOffs equation dh = d lnK/dT * R * T^2
