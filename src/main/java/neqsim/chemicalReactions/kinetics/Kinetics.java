                ...
                if (phase.getComponents()[comp].getName().equals(reaction.getNames()[j])) {
                    for (int k = 0; k < reaction.getNames().length; k++) {
                        if (reaction.getStocCoefs()[k] * reaction.getStocCoefs()[j] > 0 && !(k == j)
                                && !(phase.getComponent(reaction.getNames()[k]).getName()
                                        .equals("water"))) {
                            ...
                            phiInfinite = Math
                                    .sqrt(phase.getPhysicalProperties()
                                            .getEffectiveDiffusionCoefficient(comp)
                                            / phase.getPhysicalProperties()
                                                    .getEffectiveDiffusionCoefficient(phase
                                                            .getComponent(reaction.getNames()[k])
                                                            .getComponentNumber()))
                                    + Math.sqrt(
                                            phase.getPhysicalProperties()
                                                    .getEffectiveDiffusionCoefficient(phase
