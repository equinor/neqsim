...
public class FluidBoundarySolver implements FluidBoundarySolverInterface{
    ...
    public void initProfiles(){
        ...
        for(int i=0;i<boundary.getNumberOfNodes()-1;i++){
            ...
            //if(reactive)  reacRates = boundary.getNode(i).getBulkSystem().getChemicalReactionOperations().calcReacRates(1);
            
            for(int j=0;j<boundary.getNode(0).getBulkSystem().getPhases()[0].getNumberOfComponents();j++){
                double xbulk = boundary.getFluidBoundary().getBulkSystem().getPhases()[1].getComponents()[j].getx();
                double xinterphase = boundary.getFluidBoundary().getInterphaseSystem().getPhases()[1].getComponents()[j].getx();
                double dx =  xinterphase - xbulk;
                double last = boundary.getNode(i).getBulkSystem().getPhases()[1].getComponents()[j].getx();
                if(reactive){
                    boundary.getNode(i+1).getBulkSystem().getPhases()[1].getComponents()[j].setx(last-dx-reacRates.get(j,0)/boundary.getNode(i).getBulkSystem().getPhases()[1].getPhysicalProperties().getEffectiveDiffusionCoefficient(j)*Math.pow(boundary.getNodeLength(),2.0));
                }
                else{
                    boundary.getNode(i+1).getBulkSystem().getPhases()[1].getComponents()[j].setx(xinterphase - dx*((double)(i+1)/boundary.getNumberOfNodes()));
                }
                System.out.println("x comp " + reactive + "  "+ boundary.getNode(i).getBulkSystem().getPhases()[1].getComponents()[j].getx());
            }
        }
    }
    ...
    public void initMatrix(){
        for(int j=0;j<boundary.getNode(0).getBulkSystem().getPhases()[0].getNumberOfComponents();j++){
            ...
            for(int i=0;i<boundary.getNumberOfNodes();i++){
                solMatrix[j].set(i,0, boundary.getNode(i).getBulkSystem().getPhases()[1].getComponents()[j].getx());
            }
        }
    }
    ...
    public void initComposition(int iter){
        for(int j = 0;j<boundary.getNumberOfNodes();j++){
            for(int p=0;p<boundary.getNode(0).getBulkSystem().getPhases()[0].getNumberOfComponents();p++){
                boundary.getNode(j).getBulkSystem().getPhases()[1].getComponents()[p].setx(solMatrix[p].get(j,0));
            }
    ...
    public void setComponentConservationMatrix(int componentNumber){
        for(int i=0;i<boundary.getNumberOfNodes();i++){
            boundary.getNode(i).getBulkSystem().getPhases()[1].initPhysicalProperties();
            boundary.getNode(i).getBulkSystem().getPhases()[1].getPhysicalProperties().calcEffectiveDiffusionCoefficients();
            ///if(reactive)  boundary.getNode(i).getBulkSystem().getChemicalReactionOperations().calcReacRates(1);
        }
        ...
        for(int i=1;i<boundary.getNumberOfNodes()-1;i++){
            double Dw =  (boundary.getNode(i-1).getBulkSystem().getPhases()[1].getPhysicalProperties().getEffectiveDiffusionCoefficient(componentNumber)+ boundary.getNode(i).getBulkSystem().getPhases()[1].getPhysicalProperties().getEffectiveDiffusionCoefficient(componentNumber))/2.0;
            double De =  (boundary.getNode(i).getBulkSystem().getPhases()[1].getPhysicalProperties().getEffectiveDiffusionCoefficient(componentNumber)+ boundary.getNode(i+1).getBulkSystem().getPhases()[1].getPhysicalProperties().getEffectiveDiffusionCoefficient(componentNumber))/2.0;
    ...
    public double getMolarFlux(int componentNumber){
        ...
        return temp*boundary.getNode(0).getBulkSystem().getPhases()[1].getPhysicalProperties().getEffectiveDiffusionCoefficient(componentNumber)*
        (boundary.getNode(0).getBulkSystem().getPhases()[1].getComponents()[componentNumber].getx()-boundary.getNode(1).getBulkSystem().getPhases()[1].getComponents()[componentNumber].getx()) / boundary.getNodeLength();
    }
    ...
    public void solve(){
        ...
        double diff=0;
        xNew = new double[boundary.getNode(0).getBulkSystem().getPhases()[0].getNumberOfComponents()][boundary.getNumberOfNodes()];
