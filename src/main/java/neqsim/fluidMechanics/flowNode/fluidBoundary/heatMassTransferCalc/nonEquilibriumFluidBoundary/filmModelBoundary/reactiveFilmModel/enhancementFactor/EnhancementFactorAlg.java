...
public class EnhancementFactorAlg extends EnhancementFactor{
    ...
    public void calcEnhancementVec(int phase){
        ...
        for(int j=0;j<fluidBoundary.getBulkSystem().getPhases()[phase].getNumberOfComponents();j++){
            if(fluidBoundary.getBulkSystem().getPhases()[phase].getComponent(j).getName().equals("CO2") && phase==1){
                enhancementVec[j] = fluidBoundary.getBulkSystem().getChemicalReactionOperations().solveKinetics(phase, fluidBoundary.getInterphaseSystem().getPhase(phase), j);
                //System.out.println("enh " + enhancementVec[j]);
                hatta = Math.sqrt(enhancementVec[j]*fluidBoundary.getBulkSystem().getPhases()[phase].getPhysicalProperties().getEffectiveDiffusionCoefficient(j))/fluidBoundary.getEffectiveMassTransferCoefficient(phase,j);
                hattaNumber[j] = hatta;
    ...
    //        double[][] reacMatrix = fluidBoundary.getBulkSystem().getChemicalReactionOperations().getReactionList().getReacMatrix();
    //        double[][] stocMatrix = fluidBoundary.getBulkSystem().getChemicalReactionOperations().getReactionList().getStocMatrix();
    ...
    //            for(int j=0;j<fluidBoundary.getBulkSystem().getPhases()[phase].getNumberOfComponents();j++){
    //                if(i==j || (stocMatrix[i][i]*stocMatrix[i][j])<=0) enhancementFactor[phase].set(i,j, 1.0);
    //                else{
    //                    double pseudoReacRate = reacMatrix[i][j]*fluidBoundary.getBulkSystem().getPhases()[phase].getComponents()[j].getx()*fluidBoundary.getBulkSystem().getPhases()[phase].getDensity()/fluidBoundary.getBulkSystem().getPhases()[phase].getMolarMass()*Math.abs(stocMatrix[i][i]);
    ...
    //                    double phiVal = 1.0 + (fluidBoundary.getBulkSystem().getPhases()[phase].getPhysicalProperties().getEffectiveDiffusionCoefficient(j)/fluidBoundary.getBulkSystem().getPhases()[phase].getPhysicalProperties().getEffectiveDiffusionCoefficient(i))*
    ...
    //
    //                    System.out.println("components " + fluidBoundary.getBulkSystem().getPhases()[phase].getComponents()[i].getComponentName() + "  " + fluidBoundary.getBulkSystem().getPhases()[phase].getComponents()[j].getComponentName());
    ...
    //                        double val = hattaNumber[phase].get(i,j)/fun.apply(hattaNumber[phase].get(i,j));
    //                        enhancementFactor[1].set(i,j, (1.0 + (phiVal-1.0)*(1.0-Math.exp(-(hattaNumber[phase].get(i,j)-1.0)/(phiVal-1.0)))));
    //                    }
    //                    else{
    //                        enhancementFactor[1].set(i,j, (1.0 + (phiVal-1.0)*(1.0-Math.exp(-1.0/(phiVal-1.0)))*Math.exp(1.0-2.0/hattaNumber[phase].get(i,j))));
    //                    }
    //                    if(enhancementFactor[1].get(i,j)>1000) enhancementFactor[1].set(i,j,1000.0);
    //                    System.out.println("enhance : " + enhancementFactor[1].get(i,j) + "  phase " + phase);
    //                }
    //            }
    //        }
    //    }
