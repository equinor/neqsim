 ...
 * @version
 */
public class InterphaseStratifiedFlow extends InterphaseTwoPhasePipeFlow implements neqsim.thermo.ThermodynamicConstantsInterface{
    ...
    public double calcInterphaseMassTransferCoefficient(int phase, double schmidtNumber,  FlowNodeInterface node){
        ...
        if(phase==1){
            if(Math.abs(node.getReynoldsNumber(phase))< 300){
                redMassTrans = 1.099e-2 * Math.pow(node.getReynoldsNumber(phase),0.3955) * Math.pow(schmidtNumber, 0.5);
            }
            else if(Math.abs(node.getReynoldsNumber(phase))< 1600){
                redMassTrans = 2.995e-2 * Math.pow(node.getReynoldsNumber(phase),0.2134) * Math.pow(schmidtNumber, 0.5) ;
            }
            else{
...
//            System.out.println("redmass" + redMassTrans + "  redmass " + redMassTrans/Math.sqrt(schmidtNumber) +"  rey " + node.getReynoldsNumber(phase)/schmidtNumber);
        ...
        if(phase==0){
            if(Math.abs(node.getReynoldsNumber(phase))< 2300){
                ...
                //  System.out.println("hyd diam " + node.getHydraulicDiameter(phase) +"  phase " + phase);
                //   System.out.println("kin visk " +  node.getBulkSystem().getPhases()[phase].getPhysicalProperties().getKinematicViscosity() +"  phase " + phase);
                // System.out.println("mas " +3.66 / node.getHydraulicDiameter(phase) / schmidtNumber * node.getBulkSystem().getPhases()[phase].getPhysicalProperties().getKinematicViscosity() +"  phase " + phase);
                ...
                double temp = node.getVelocity(phase);
                // System.out.println("mass " + 1e-5* 0.5 * this.calcWallFrictionFactor(phase, node) * Math.pow(schmidtNumber, -2.0/3.0) * temp);
                massTrans = 0.5 * this.calcWallFrictionFactor(phase, node) * Math.pow(schmidtNumber, -2.0/3.0) * temp;
            }
        }
        // System.out.println("mass "+  massTrans + "  phase " + phase + "  rey " + node.getReynoldsNumber(phase) + " COMP " + );
        return massTrans;
    }
//    
//      public double calcInterphaseMassTransferCoefficient(int phase, double schmidtNumber,  FlowNodeInterface node){
...
//        }
//        if(phase==0){
//            //System.out.println("diff " + node.getBulkSystem().getPhases()[phase].getPhysicalProperties().getKinematicViscosity() / schmidtNumber);
//            
//            //massTrans = 3.66 / node.getHydraulicDiameter(phase) / schmidtNumber * node.getBulkSystem().getPhases()[phase].getPhysicalProperties().getKinematicViscosity();
//            massTrans=0.010;
//        }
