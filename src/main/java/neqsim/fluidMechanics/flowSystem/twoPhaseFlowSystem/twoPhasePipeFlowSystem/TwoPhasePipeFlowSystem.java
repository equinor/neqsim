...
public class TwoPhasePipeFlowSystem extends neqsim.fluidMechanics.flowSystem.twoPhaseFlowSystem.TwoPhaseFlowSystem {
    ...
    public void createSystem(){
        ...
        if(initFlowPattern.equals("stratified")) {
            flowNode[0] = new neqsim.fluidMechanics.flowNode.twoPhaseNode.twoPhasePipeFlowNode.StratifiedFlowNode(thermoSystem, equipmentGeometry[0]);
        } else if(initFlowPattern.equals("annular")) {
            flowNode[0] = new neqsim.fluidMechanics.flowNode.twoPhaseNode.twoPhasePipeFlowNode.AnnularFlow(thermoSystem, equipmentGeometry[0]);
        } else {
            flowNode[0] = new neqsim.fluidMechanics.flowNode.twoPhaseNode.twoPhasePipeFlowNode.StratifiedFlowNode(thermoSystem, equipmentGeometry[0]);
        }
    ...
    public void solveSteadyState(int solverType){
        double[] times = {0.0};
        display = new neqsim.fluidMechanics.util.fluidMechanicsVisualization.flowSystemVisualization.twoPhaseFlowVisualization.twoPhasePipeFlowVisualization.TwoPhasePipeFlowVisualization(this.getTotalNumberOfNodes(),1);
        ...
        flowSolver = new neqsim.fluidMechanics.flowSolver.twoPhaseFlowSolver.twoPhasePipeFlowSolver.TwoPhaseFixedStaggeredGridSolver(this, getSystemLength(), this.getTotalNumberOfNodes(), false);
    ...
    public static void main(String[] args){
        ...
        neqsim.fluidMechanics.geometryDefinitions.GeometryDefinitionInterface [] pipeGemometry = new neqsim.fluidMechanics.geometryDefinitions.pipe.PipeData[6];    // Deffinerer geometrien for røret
