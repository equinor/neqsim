 ...
 * particles (bubbles or droplets) in a continuous phase. Uses Ranz-Marshall correlation for particle mass transfer: Sh
 * = 2 + 0.6 * Re^0.5 * Sc^0.33
 * </p>
 *
 * <p>
 * Optionally supports the Abramzon-Sirignano (1989) extended film model for evaporating droplets, which accounts for
 * Stefan flow (blowing) at the droplet surface: Sh* = 2 + (Sh_0 - 2) / F(B_M), where F(B_M) = (1+B_M)^0.7 * ln(1+B_M) /
 * B_M and B_M is the Spalding mass transfer number.
 * </p>
 *
 * @author esol
 * @version $Id: $Id
...
public class InterphaseDropletFlow extends InterphaseTwoPhasePipeFlow
  ...
  /**
   * Flag to enable the Abramzon-Sirignano extended film model for evaporating droplets. When true, the Ranz-Marshall
   * Sherwood number is corrected for Stefan flow (blowing) using the Spalding mass transfer number. Default is false
   * (standard Ranz-Marshall).
   */
  private boolean useAbramzonSirignano = false;

  /**
   * The Spalding mass transfer number B_M = (Y_s - Y_inf) / (1 - Y_s), where Y_s is the vapor mass fraction at the
   * droplet surface and Y_inf is the far-field vapor mass fraction. Set externally based on current evaporation
   * conditions.
   */
   ...
   *
   * <p>
   * When B_M approaches zero (no blowing), F(B_M) approaches 1.0, recovering the standard Ranz-Marshall correlation.
   * For large B_M (vigorous evaporation), F(B_M) &gt; 1, which effectively thickens the film and reduces the corrected
   * Sherwood number.
   * </p>
   *
  ...
  /**
   * Gets the characteristic particle diameter from the flow node. For DropletFlowNode, returns the average droplet
   * diameter. For BubbleFlowNode, returns the average bubble diameter. Falls back to a default of 100 microns if the
   ...
   *
   * <p>
   * For droplet/bubble flow, uses Ranz-Marshall correlation for mass transfer from/to spherical particles: Sh = 2 + 0.6
   * * Re^0.5 * Sc^0.33
   * </p>
   *
   * <p>
   * This correlation is valid for single spheres and is widely used for dispersed phase mass transfer in two-phase
   * flows.
   * </p>
   */
  @Override
  ...
  public double calcInterphaseHeatTransferCoefficient(int phaseNum, double prandtlNumber, FlowNodeInterface node) {
    ...
    double conductivity = node.getBulkSystem().getPhase(phaseNum).getPhysicalProperties().getConductivity();
   ...
   *
   * <p>
   * Calculates the interphase mass transfer coefficient for droplet/mist and bubble flow using Ranz-Marshall
   * correlation. The characteristic length scale is the particle (droplet or bubble) diameter, not the pipe hydraulic
   * diameter.
   * </p>
   *
   * <p>
   * The continuous phase uses Ranz-Marshall Sh with particle Reynolds number based on the relative velocity. The
   * dispersed phase uses Kronig-Brink Sh = 17.66 for internally circulating spheres.
   * </p>
   *
   * <p>
   * For droplet flow: gas (phase 0) is continuous, liquid (phase 1) is dispersed. For bubble flow: liquid (phase 1) is
   * continuous, gas (phase 0) is dispersed.
   * </p>
   *
   * <p>
  ...
  public double calcInterphaseMassTransferCoefficient(int phaseNum, double schmidtNumber, FlowNodeInterface node) {
    ...
    double kinVisc = node.getBulkSystem().getPhase(phaseNum).getPhysicalProperties().getKinematicViscosity();
    if (kinVisc < 1e-15) {
      kinVisc = 1e-6;
    }
