...
public class TwoPhaseFlowNodeVisualization extends FlowNodeVisualization {
  ...
  public void setData(FlowNodeInterface node) {
    ...
    for (int i = 0; i < node.getBulkSystem().getPhases()[0].getNumberOfComponents(); i++) {
      ...
      interfaceComposition[0][i] = node.getFluidBoundary().getInterphaseSystem().getPhases()[0].getComponent(i).getx();
      interfaceComposition[1][i] = node.getFluidBoundary().getInterphaseSystem().getPhases()[1].getComponent(i).getx();
