...
public class TwoPhasePipeFlowVisualization extends
   ...
   * neqsim.dataPresentation.filehandling.createNetCDF.netCDF2D.NetCdf2D(); file.setOutputFileName(name);
   * file.setXvalues(xPlace, "length", "meter"); file.setYvalues(pressurePoint[0][0], "gas pressure", "sec");
   * file.setYvalues(pressurePoint[1][0], "liquid pressure", "sec"); file.setYvalues(velocityPoint[0][0],
   * "gas velocity", "sec"); file.setYvalues(velocityPoint[1][0], "liquid velocity", "sec");
   * file.setYvalues(reynoldsNumber[0][0], "gas reynolds number", "sec"); file.setYvalues(reynoldsNumber[1][0],
   * "liquid reynolds number", "sec"); file.setYvalues(temperaturePoint[0][0], "gas temperature", "sec");
   * file.setYvalues(temperaturePoint[1][0], "liquid temperature", "sec"); file.setYvalues(phaseFraction[0][0],
   * "void fraction", "sec"); file.setYvalues(phaseFraction[1][0], "holdup", "sec");
   ...
   *
   * for (int p = 0; p < flowNodes[0][0].getNumberOfComponents(); p++) { String comp = "component molefraction " + p;
   * file.setYvalues(bulkComposition[0][p][0], ("gas " + comp + p), "sec"); file.setYvalues(bulkComposition[1][p][0],
   * ("liquid " + comp + p), "sec"); file.setYvalues(interfaceComposition[0][p][0], ("gas (interphase) " + comp),
   * "sec"); file.setYvalues(interfaceComposition[1][p][0], ("liquid (interphase) " + comp), "sec");
   * file.setYvalues(molarFlux[0][p][0], ("gas molar flux " + comp + p), "sec"); file.setYvalues(molarFlux[1][p][0],
   * ("liquid molar flux " + comp + p), "sec"); file.setYvalues(totalVolumetricMassTransferRate[0][p][0],
   * ("total gas Volumetric Mass Transfer " + comp + p), "sec");
   * file.setYvalues(totalVolumetricMassTransferRate[1][p][0], ("total liq Volumetric Mass Transfer " + comp + p),
   * "sec"); file.setYvalues(effectiveMassTransferCoefficient[0][p][0], ("eff. masstransfer coef gas " + comp + p),
   * "sec"); file.setYvalues(effectiveMassTransferCoefficient[1][p][0], ("eff. masstransfer coef liq " + comp + p),
   * "sec"); file.setYvalues(schmidtNumber[0][p][0], ("gas scmidtnumber " + comp + p), "-");
   * file.setYvalues(schmidtNumber[1][p][0], ("liquid scmidtnumber " + comp + p), "-"); } //
   * file.setYvalues(temperaturePoint[0][0], "time","sec"); file.createFile(); }
   */
}
