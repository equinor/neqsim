 ...
 *
 * <p>
 * Provides ready-to-use example JSON strings for flash calculations, process simulations, component queries, and
 * validation. These examples serve a dual purpose:
 * </p>
 * <ul>
 * <li><strong>MCP Resources</strong> — served via {@code neqsim://examples/{category}/{name}} URIs so that language
 * models can read them and learn the input format</li>
 * <li><strong>Test fixtures</strong> — used by unit tests to verify runners work correctly</li>
 * </ul>
 *
 * <p>
 * All methods are static and return JSON strings that can be passed directly to the corresponding runner's
 * {@code run(String)} method.
 * </p>
 *
 * @author Even Solbraa
 * @version 1.0
...
public final class ExampleCatalog {
  ...
  public static String rootCauseCompressorHighVibration() {
    String processJson = processCompressionWithCooling().replace("\\", "\\\\").replace("\"", "\\\"").replace("\n",
        "\\n");
  ...
  /**
   * Returns a separator liquid-carryover root-cause analysis example with rich tagreader historian data showing the
   * complete integration between process simulation, STID design data, and historian time-series from plant data
   * systems (e.g. OSIsoft PI / Aspen IP.21).
   *
   * @return JSON string for RootCauseRunner.run()
   */
  public static String rootCauseSeparatorLiquidCarryover() {
    String processJson = processSimpleSeparation().replace("\\", "\\\\").replace("\"", "\\\"").replace("\n", "\\n");
    return "{\n" + "  \"equipmentName\": \"HP Sep\",\n" + "  \"symptom\": \"LIQUID_CARRYOVER\",\n"
        + "  \"processJson\": \"" + processJson + "\",\n" + "  \"simulationEnabled\": true,\n"
        + "  \"historianCsv\": \"time,demisterDp,liquidLevel,gasOutFlow," + "feedFlow,inletTemperature\\n"
        ...
        + "    \"sourceReference\": \"STID datasheet DS-V-2001 rev.C " + "and tagreader trend 01.06.2025-01.06.2025\"\n"
        + "  }\n" + "}";
  }

  /**
   * Returns a heat exchanger fouling root-cause analysis example integrating process simulation, tagreader historian
   * trend, design limits from STID datasheet, and equipment design data from vendor technical documents.
   *
   * @return JSON string for RootCauseRunner.run()
   */
  public static String rootCauseHeatExchangerFouling() {
    return "{\n" + "  \"equipmentName\": \"Intercooler\",\n" + "  \"symptom\": \"FOULING\",\n" + "  \"processJson\": \""
        ...
        + "    \"sourceReference\": \"STID datasheet DS-E-3501 rev.B " + "and vendor technical document VD-E-3501\"\n"
        + "  }\n" + "}";
  }
  ...
  /**
   * Returns a process safety system review example based on normalized technical documentation and instrument evidence.
   *
   ...
   */
  private static String commonClause10Item(String functionId, String functionType, String equipmentTag,
      boolean logicEvidence) {
  ...
  public static String economicsNorwegianNCS() {
    return "{\n" + "  \"mode\": \"cashflow\",\n" + "  \"country\": \"NO\",\n"
        ...
        + "    \"oil\": {\"2027\": 15000000, \"2028\": 13500000, " + "\"2029\": 12150000, \"2030\": 10935000},\n"
  ...
  public static List<String> getCategories() {
    return Collections.unmodifiableList(Arrays.asList("flash", "process", "validation", "batch", "property-table",
        "phase-envelope", "pvt", "flow-assurance", "standards", "pipeline", "water-hammer", "root-cause", "reservoir",
        "economics", "materials-review", "bioprocess", "open-drain-review", "process-safety-review", "session",
        "visualization", "equipment-sizing", "comparison", "safety", "tool"));
  }
  ...
  /**
   * Returns a generic, contract-level example for MCP tools that do not yet have a dedicated runnable example.
   *
  ...
  private static String genericToolExample(String toolName) {
    ...
    example.put("notes",
        Arrays.asList("This is a contract-level starter for " + toolName,
  ...
  public static String getCatalogJson() {
    ...
    validationExamples.put("error-flash", "Flash input with known errors (unknown model, misspelled component)");
    catalog.put("validation", validationExamples);

    // Batch examples
    Map<String, String> batchExamples = new LinkedHashMap<String, String>();
    batchExamples.put("temperature-sweep", "Batch of 3 TP flashes at different temperatures (SRK, 3-component gas)");
    batchExamples.put("pressure-sweep", "Batch of 4 TP flashes at different pressures (PR, 4-component gas)");
    ...
    pipeExamples.put("multiphase-flow", "Beggs & Brill multiphase pipeline flow for 50 km gas line");
    ...
    rcaExamples.put("hx-fouling", "Heat exchanger fouling RCA with IP.21 historian, vendor data, and UA degradation");
    ...
    resExamples.put("gas-depletion", "Simple gas reservoir depletion (1 BCM) with a single producer over 20 years");
    ...
    sessionExamples.put("add-equipment", "Add a separator to an existing session (requires sessionId)");
