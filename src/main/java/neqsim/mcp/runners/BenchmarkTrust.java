 ...
 *
 * <p>
 * Provides per-tool validation status, known accuracy ranges, reference validation cases, known limitations, and
 * unsupported regions. This is the "trust page" that industrial users require before relying on a tool for engineering
 * decisions.
 * </p>
 *
 * <p>
...
public final class BenchmarkTrust {
  ...
  private static JsonObject buildPVTTrust() {
    JsonObject trust = new JsonObject();
    trust.addProperty("maturityLevel", "VALIDATED");
    trust.addProperty("description", "PVT laboratory simulations validated against experimental data.");
  ...
  private static JsonObject buildMaterialsReviewTrust() {
    ...
    JsonArray limitations = new JsonArray();
    limitations.add("Screening-level; final materials selection requires discipline engineer approval");
  ...
  private static JsonObject buildBioprocessTrust() {
    JsonObject trust = new JsonObject();
    trust.addProperty("maturityLevel", "EXPERIMENTAL");
    trust.addProperty("description", "Bioprocess reactor simulations using published kinetic models.");
  ...
  private static JsonObject buildPhaseEnvelopeTrust() {
    JsonObject trust = new JsonObject();
    trust.addProperty("maturityLevel", "VALIDATED");
    trust.addProperty("description", "PT phase envelope calculation. Validated for hydrocarbon mixtures.");
  ...
  private static JsonObject buildPropertyTableTrust() {
    JsonObject trust = new JsonObject();
    trust.addProperty("maturityLevel", "VALIDATED");
    trust.addProperty("description", "Property sweep across T or P range. Same accuracy as runFlash at each point.");

    JsonArray limitations = new JsonArray();
    limitations.add("Phase transitions may cause discontinuities in property values");
    limitations.add("Transport properties (viscosity) have larger uncertainty than " + "thermodynamic properties");
    trust.add("knownLimitations", limitations);
