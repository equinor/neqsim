 ...
 *
 * <p>
 * Returns a structured manifest of everything NeqSim can calculate, organized by domain. This is the entry point for
 * external agents that need to understand what NeqSim offers before making tool calls. The manifest is static and
 * cached — it does not require any simulation to run.
 * </p>
 *
 * @author Even Solbraa
 * @version 1.0
...
public class CapabilitiesRunner {
  ...
  private static String buildCapabilities() {
    ...
    root.addProperty("description", "Java toolkit for thermodynamic calculations, process simulation, "
	+ "mechanical design, and engineering standards compliance");

    // --- Thermodynamic Models ---
    JsonObject thermo = new JsonObject();
    thermo.add("equationsOfState",
	toJsonArray(Arrays.asList("SRK — Soave-Redlich-Kwong (general purpose, hydrocarbons + inorganics)",
	...
	toJsonArray(Arrays.asList("TP — Temperature-Pressure (most common)", "PH — Pressure-Enthalpy",
	    "PS — Pressure-Entropy", "TV — Temperature-Volume", "dewPointT — Dew point temperature at given pressure",
	...
	toJsonArray(Arrays.asList("Hydrate prediction (sI, sII, sH structures)", "Wax appearance temperature (WAT)",
	    "Asphaltene onset pressure", "Electrolyte systems (produced water, brine, scale prediction)",
    ...
    process.add("equipmentTypes", toJsonArray(Arrays.asList("Stream — Feed and intermediate streams",
	...
	"Expander — Turboexpander with efficiency", "Heater — Generic heat addition", "Cooler — Generic heat removal",
	"HeatExchanger — Shell-and-tube with UA-based rating", "ThrottlingValve — Isenthalpic expansion (JT valve)",
	"Mixer — Multi-stream mixing", "Splitter — Stream splitting by fraction",
	...
	"AdiabaticPipe — Adiabatic pipeline flow", "PipeBeggsAndBrills — Beggs & Brill multiphase flow correlation",
	"Recycle — Convergence of recycle loops", "Adjuster — Set-point adjustment (design spec)",
	"GasTurbine — Gas turbine with fuel consumption", "SteamTurbine — Steam power generation",
	"HRSG — Heat recovery steam generator", "SubseaWell — Subsea well with casing design")));
    process.add("processFeatures",
	toJsonArray(Arrays.asList("Steady-state and dynamic (transient) simulation",
	    "Automatic stream connection and naming", "ProcessModel for multi-area plant composition",
    ...
    modes.add("available", toJsonArray(Arrays.asList("runFlash — Single flash calculation (9 flash types x 6 EOS)",
	"runBatch — Multiple flash calculations in one call (sensitivity studies)",
	"runProcess — Full process simulation from JSON", "getPropertyTable — Sweep T or P and get property table",
	...
	"searchComponents — Component database search", "validateInput — Pre-flight input validation",
	...
	"Use runFlash or getPropertyTable for single-query answers. " + "No process flowsheet required.");
    modes.addProperty("fullSimulation",
	...
	toJsonArray(Arrays.asList("design_gas_processing — Step-by-step gas processing facility design",
	...
	toJsonArray(Arrays.asList("Pre-flight input validation (component names, T/P ranges, EOS compatibility)",
	...
	neqsim.mcp.model.ApiEnvelope.validationStatus(true, "discovery", "Capabilities manifest generated"),
	neqsim.mcp.model.ApiEnvelope.qualityGate("passed", "Capabilities manifest generated", true));
  ...
  private static JsonObject buildToolCapabilities() {
	...
	Arrays.asList("model", "temperature", "pressure", "flashType", "mixingRule", "enthalpy", "entropy", "volume"),
	...
	Arrays.asList("fluid and process", "or areas"), Arrays.asList("fluids", "connections", "autoRun", "name"),
	eosModels(), processUnits(), "process-system",
	...
	"Pre-flight validation of flash or process JSON", Arrays.asList("inputJson"), Collections.<String>emptyList(),
	eosModels(), validationUnits(), "validation",
	...
	"Search thermodynamic component names", Arrays.asList("query"), Collections.<String>emptyList(),
	Collections.<String>emptyList(), Collections.<String>emptyList(), "data-catalog",
	...
	"thermodynamic-flash", Arrays.asList("Inspect per-case failures before using aggregate results",
	    "Large batches can be computationally expensive"));
    addToolCapability(tools, "getPropertyTable", "get_property_table", "PropertyTableRunner", "thermodynamics",
	"Property sweep over temperature or pressure", Arrays.asList("components", "sweep"),
	Arrays.asList("model", "mixingRule", "points", "properties"), eosModels(), thermoUnits(), "property-sweep",
    ...
    addToolCapability(tools, "getPhaseEnvelope", "get_phase_envelope", "PhaseEnvelopeRunner", "thermodynamics",
	"PT phase envelope calculation", Arrays.asList("components"), Arrays.asList("model", "mixingRule"), eosModels(),
	thermoUnits(), "phase-envelope", Arrays.asList("Near-critical regions may have reduced curve quality",
	    "Three-phase and hydrate boundaries require specialist workflows"));
    addToolCapability(tools, "getCapabilities", "get_capabilities", "CapabilitiesRunner", "discovery",
	"Discover schema-backed tools, setup templates, examples, and contracts", Collections.<String>emptyList(),
	Arrays.asList("includeExamples", "includeSetupTemplates"), Collections.<String>emptyList(),
	Collections.<String>emptyList(), "capability-discovery",
	Arrays.asList("Capability metadata describes the MCP integration surface, not every Java API"));
    addToolCapability(tools, "runPVT", "run_pvt", "PVTRunner", "pvt", "PVT laboratory experiment simulation",
	Arrays.asList("experiment", "components"),
    ...
    addToolCapability(tools, "runFlowAssurance", "run_flow_assurance", "FlowAssuranceRunner", "flow-assurance",
	"Hydrate, wax, corrosion, scale, erosion, and cooldown screening", Arrays.asList("components", "analysis"),
	Arrays.asList("model", "temperature_C", "pressure_bara", "analysisConfig"), eosModels(), processUnits(),
	"flow-assurance",
	Arrays.asList("Screening calculations need specialist review before chemical dosing decisions"));
    addToolCapability(tools, "calculateStandard", "calculate_standard", "StandardsRunner", "standards",
	"Gas and oil quality calculations per industry standards", Arrays.asList("standard", "components"),
	...
	processUnits(), "reservoir-forecast",
	Arrays.asList("Tank models are screening-level and do not replace reservoir simulation"));
    addToolCapability(tools, "runFieldEconomics", "run_field_economics", "FieldDevelopmentRunner", "economics",
	"Field-development economics and decline curves", Arrays.asList("mode"),
	Arrays.asList("country", "capex", "opex", "production", "prices"), Collections.<String>emptyList(),
	Arrays.asList("USD", "MUSD", "Sm3", "bbl", "year"), "field-economics",
	Arrays.asList("Cost and price assumptions dominate uncertainty"));
	...
	Arrays.asList("duration_seconds", "timeStep_seconds", "tuning"), eosModels(), processUnits(),
	"dynamic-simulation", Arrays.asList("Default controller tuning is screening-level",
	...
	Arrays.asList("feedRate_kgPerHr", "temperature_C", "volume_m3", "biomass"), Collections.<String>emptyList(),
	Arrays.asList("kg/hr", "C", "m3", "%"), "bioprocess",
	Arrays.asList("Bioprocess yields are screening correlations unless calibrated"));
    addToolCapability(tools, "sizeEquipment", "size_equipment", "EquipmentSizingRunner", "equipment-design",
	"Quick separator and compressor sizing", Arrays.asList("equipmentType"),
	Arrays.asList("flowRate", "orientation", "outletPressure_bara", "polytropicEfficiency"), eosModels(),
	processUnits(), "equipment-sizing",
	Arrays.asList("Sizing is preliminary and should be followed by mechanical design"));
    addToolCapability(tools, "compareProcesses", "compare_processes", "ProcessComparisonRunner", "process",
	"Compare process cases side by side", Arrays.asList("cases"), Arrays.asList("outputs", "tolerances"),
	eosModels(), processUnits(), "process-system",
	...
	Arrays.asList("sessionId", "fluid", "processJson", "equipment", "address", "value", "unit"), eosModels(),
	processUnits(), "session-management", Arrays.asList("Session state is transient unless explicitly persisted"));
    addToolCapability(tools, "generateVisualization", "visualize", "VisualizationRunner", "visualization",
	"Generate SVG, Mermaid, or table visualizations", Arrays.asList("type"),
	Arrays.asList("data", "processJson", "components", "labels", "values"), Collections.<String>emptyList(),
	Collections.<String>emptyList(), "visualization-reporting",
	Arrays.asList("Visualizations summarize data and should not be used as calculation sources"));
	...
	Arrays.asList("nodes", "failureModes", "barrierRegister", "runSimulations"), eosModels(), processUnits(),
	"safety-analysis", Arrays.asList("HAZOP output requires facilitator and discipline review"));
    addToolCapability(tools, "runBarrierRegister", "run_barrier_register", "BarrierRegisterRunner", "safety",
	"Validate and transform safety barrier registers", Arrays.asList("register"),
	...
	"SafetySystemPerformanceRunner", "safety", "Analyze active and passive safety-system performance",
	Arrays.asList("register"), Arrays.asList("demands", "measurementDevices", "logicSifs", "quantitativeSifs"),
  ...
  private static void addAdditionalMcpToolCapabilities(JsonObject tools) {
    ...
    addGenericToolCapability(tools, "getSimulationVariable", "get_simulation_variable", "automation",
	"Read a process variable by automation address", "model-lifecycle");
    addGenericToolCapability(tools, "setSimulationVariable", "set_simulation_variable", "automation",
	"Set an input variable and rerun a process", "model-lifecycle");
    ...
    addGenericToolCapability(tools, "compareSimulationStates", "compare_simulation_states", "lifecycle",
	"Compare two process state snapshots", "model-lifecycle");
    ...
    addGenericToolCapability(tools, "getAutomationLearningReport", "get_automation_learning_report", "automation",
	"Summarize automation corrections and operation history", "model-lifecycle");
	...
	"Plan engineering workflows, evaluate evidence trust, rank studies, and gate readiness", "flowsheet-builder");
	...
	"Review process materials, degradation mechanisms, and remaining life", "safety-governance");
    ...
    addGenericToolCapability(tools, "runNorsokS001Clause10Review", "run_norsok_s001_clause10_review", "safety",
    ...
    addGenericToolCapability(tools, "runRootCauseAnalysis", "run_root_cause_analysis", "diagnostics",
	"Rank equipment root-cause hypotheses from evidence and simulation", "benchmark-registry");
    ...
    addGenericToolCapability(tools, "runPlugin", "run_plugin", "platform", "Run or list registered MCP runner plugins",
    ...
    addGenericToolCapability(tools, "composeMultiServerWorkflow", "compose_multi_server_workflow", "workflow",
	"Compose workflows across multiple MCP servers", "flowsheet-builder");
    ...
    addGenericToolCapability(tools, "manageValidationProfile", "manage_validation_profile", "governance",
    ...
    addGenericToolCapability(tools, "runSIL", "run_sil", "safety", "Verify SIF PFDavg and SIL claims",
	"safety-governance");
    ...
    addGenericToolCapability(tools, "manageIndustrialProfile", "manage_industrial_profile", "governance",
	"Configure tool access profiles for industrial deployment", "safety-governance");
    ...
    addGenericToolCapability(tools, "getAdjustableParameters", "get_adjustable_parameters", "automation",
  ...
  private static void addGenericToolCapability(JsonObject tools, String mcpToolName, String schemaToolName,
    ...
    addToolCapability(tools, mcpToolName, schemaToolName, "neqsim.mcp.server.NeqSimTools", workflowCategory, purpose,
	Collections.<String>emptyList(), Arrays.asList("action", "inputJson", "processJson", "arguments", "options"),
	eosModels(), processUnits(), setupTemplateId,
  ...
  private static void addToolCapability(JsonObject tools, String mcpToolName, String schemaToolName, String runnerClass,
      ...
      List<String> supportedModels, List<String> supportedUnits, String setupTemplateId, List<String> limitations) {
    ...
    descriptor.add("standardResponseFields", toJsonArray(
	Arrays.asList("apiVersion", "status", "tool", "data", "provenance", "validation", "qualityGate", "warnings")));
  ...
  private static JsonObject schemaRefs(String schemaToolName) {
    ...
    schemas.addProperty("inputSchemaAvailable", SchemaCatalog.getSchema(schemaToolName, "input") != null);
    schemas.addProperty("outputSchemaAvailable", SchemaCatalog.getSchema(schemaToolName, "output") != null);
    return schemas;
  }
  ...
  private static JsonObject validationCoverage(String schemaToolName) {
    ...
    coverage.addProperty("exampleAvailable", ExampleCatalog.getExample("tool", schemaToolName) != null);
  ...
  private static JsonObject buildSetupTemplates() {
	...
	Arrays.asList("Call getCapabilities first", "Use toolCapabilities[*].schemas to fetch schemas",
	...
	Arrays.asList("Choose EOS and mixing rule", "Provide components as mole fractions",
	    "Specify temperature/pressure with units", "Call validateInput for reusable JSON payloads"),
	...
	Arrays.asList("Choose sweep variable", "Set sweepFrom/sweepTo with units",
	    "Set fixedPressure or fixedTemperature", "Request only needed properties for compact output"),
	ExampleCatalog.propertyTableTemperatureSweep());
    addSetupTemplate(templates, "phase-envelope", "PT phase-envelope setup", Arrays.asList("getPhaseEnvelope"),
	...
	Arrays.asList("Define a default fluid", "Add ordered process units",
	    "Reference upstream outlets by name or name.port", "Use [value, unit] for unit-aware equipment properties"),
	ExampleCatalog.processSimpleSeparation());
    addSetupTemplate(templates, "process-model", "Multi-area ProcessModel setup", Arrays.asList("runProcess"),
	Arrays.asList("Create top-level areas object", "Each area contains a standard fluid/process JSON",
	    "Use shared stream names carefully", "Inspect returned area reports"),
	...
	Arrays.asList("searchComponents"), Arrays.asList("Search partial component names",
	    "Use returned names verbatim in components maps", "Fetch component resources for properties"),
	ExampleCatalog.componentSearchMethane());
    addSetupTemplate(templates, "pvt-experiment", "PVT laboratory experiment setup", Arrays.asList("runPVT"),
	Arrays.asList("Define reservoir fluid composition", "Choose experiment type",
	    "Add experimentConfig arrays/stages as needed"),
	ExampleCatalog.pvtCME());
    addSetupTemplate(templates, "flow-assurance", "Flow assurance screening setup", Arrays.asList("runFlowAssurance"),
	Arrays.asList("Select analysis type", "Use CPA when water/hydrates dominate",
	    "Include operating envelope in analysisConfig"),
	ExampleCatalog.flowAssuranceHydrate());
    addSetupTemplate(templates, "standards-calculation", "Gas and oil quality standard setup",
	Arrays.asList("calculateStandard"), Arrays.asList("Select standard code",
	    "Set reference temperature and pressure", "Add standardConfig for method-specific values"),
	ExampleCatalog.standardISO6976());
    addSetupTemplate(templates, "pipeline-flow", "Pipeline hydraulic screening setup", Arrays.asList("runPipeline"),
	Arrays.asList("Define fluid and inlet conditions", "Specify pipe diameter, length, elevation, roughness",
	    "Review flow-regime limitations"),
	ExampleCatalog.pipelineMultiphase());
    addSetupTemplate(templates, "reservoir-forecast", "Reservoir material-balance setup", Arrays.asList("runReservoir"),
	Arrays.asList("Define in-place volumes", "Add producers/injectors", "Set simulation years and timestep"),
	ExampleCatalog.reservoirDepletion());
    addSetupTemplate(templates, "field-economics", "Field economics setup", Arrays.asList("runFieldEconomics"),
	Arrays.asList("Choose cashflow or productionProfile mode",
	    "Declare price, CAPEX, OPEX, production, and discount assumptions", "Report currency and fiscal regime"),
	ExampleCatalog.economicsNorwegianNCS());
    addSetupTemplate(templates, "dynamic-simulation", "Dynamic simulation setup", Arrays.asList("runDynamic"),
    ...
    addSetupTemplate(templates, "bioprocess", "Bioprocess reactor setup", Arrays.asList("runBioprocess"),
	Arrays.asList("Choose reactorType", "Provide feed or biomass properties", "Check energy and yield outputs"),
	ExampleCatalog.bioprocessAnaerobicDigestion());
    addSetupTemplate(templates, "equipment-sizing", "Quick equipment sizing setup", Arrays.asList("sizeEquipment"),
	Arrays.asList("Pick separator or compressor", "Provide fluid and flow rate",
	    "Treat results as preliminary sizing"),
	ExampleCatalog.sizingSeparator());
    addSetupTemplate(templates, "session-management", "Stateful session setup", Arrays.asList("manageSession"),
	Arrays.asList("Create a session from fluid or processJson", "Add equipment incrementally",
	    "Close or persist sessions when finished"),
	ExampleCatalog.sessionCreate());
    addSetupTemplate(templates, "visualization-reporting", "Visualization setup",
	Arrays.asList("generateVisualization"), Arrays.asList("Choose visualization type",
	    "Pass result data or processJson", "Use generated media as reporting artifacts"),
    ...
    addSetupTemplate(templates, "flowsheet-builder", "Convert process descriptions into validated process JSON",
	Arrays.asList("getCapabilities", "getSchema", "getExample", "validateInput", "runProcess", "composeWorkflow",
	    "bridgeTaskWorkflow"),
    ...
    addSetupTemplate(templates, "benchmark-registry", "Attach benchmark trust and validation evidence to tool results",
	...
	Arrays.asList("Use value/unit objects or [value, unit] arrays instead of bare numbers",
	    "Pick units from unitSystem.dimensions", "Validate dimensional intent before execution"),
	"{\"temperature\":{\"value\":25.0,\"unit\":\"C\"},\"pressure\":{\"value\":80.0,\"unit\":\"bara\"}}");
    addSetupTemplate(templates, "optimization-uncertainty", "Run sensitivity, uncertainty, and model-risk workflows",
	Arrays.asList("runParametricStudy", "runBatch", "crossValidateModels", "compareProcesses", "runFieldEconomics"),
    ...
    addSetupTemplate(templates, "model-lifecycle", "Create, inspect, save, restore, and compare process model states",
	Arrays.asList("manageSession", "saveSimulationState", "compareSimulationStates", "manageState",
	...
	Arrays.asList("runRelief", "runLOPA", "runSIL", "runRiskMatrix", "runFlareNetwork", "manageIndustrialProfile",
	    "checkToolAccess", "manageValidationProfile"),
	Arrays.asList("Check tool access before execution", "Select jurisdiction and standard basis",
	    "Require evidence references for safety findings", "Keep engineeringReviewRequired true",
  ...
  private static JsonObject buildProcessJsonContract() {
    ...
    commonProperties.add("Compressor",
	toJsonArray(Arrays.asList("outletPressure", "isentropicEfficiency", "polytropicEfficiency")));
    commonProperties.add("Pump", toJsonArray(Arrays.asList("outletPressure", "isentropicEfficiency")));
    ...
    commonProperties.add("ThrottlingValve", toJsonArray(Arrays.asList("outletPressure", "percentValveOpening")));
    commonProperties.add("Separator", toJsonArray(Arrays.asList("entrainment", "mechanicalDesign")));
    ...
    commonProperties.add("HeatExchanger", toJsonArray(Arrays.asList("UAvalue", "guessOutTemperature")));
    commonProperties.add("DistillationColumn",
	toJsonArray(Arrays.asList("numberOfTrays", "hasReboiler", "hasCondenser")));
    commonProperties.add("Adjuster", toJsonArray(Arrays.asList("adjustedEquipment", "adjustedVariable",
	"targetEquipment", "targetVariable", "targetValue", "targetUnit", "stepSize")));
    commonProperties.add("PipeBeggsAndBrills",
	toJsonArray(Arrays.asList("diameter", "length", "elevation", "roughness", "numberOfIncrements")));
    contract.add("commonPropertiesByEquipment", commonProperties);

    JsonObject propertyEncoding = new JsonObject();
    propertyEncoding.addProperty("bareNumber", "{\"outletPressure\": 80.0} uses the Java setter default unit");
    ...
    units.add("pressure", toJsonArray(Arrays.asList("bara", "barg", "Pa", "kPa", "MPa", "psi", "psia", "atm")));
    units.add("flowRate", toJsonArray(Arrays.asList("kg/hr", "kg/s", "mol/sec", "Sm3/day", "MSm3/day", "m3/hr")));
  ...
  /**
   * Builds a graph-style capability map for agents that reason over domains, tools, contracts, and workflows.
   *
  ...
  private static JsonObject buildCapabilityGraph() {
    ...
    List<String> domains = Arrays.asList("thermodynamics", "process", "automation", "pvt", "flow-assurance", "pipeline",
	"reservoir", "economics", "dynamic", "bioprocess", "safety", "governance", "validation", "benchmark",
	"lifecycle", "workflow", "data", "reporting", "visualization", "uncertainty", "platform", "discovery");
    ...
    addGraphNode(nodes, "contract:processJsonContract", "contract", "processJsonContract", "process");
  ...
  private static JsonObject buildEquipmentPropertyOntology() {
    ...
    addEquipmentOntology(equipment, "ThreePhaseSeparator", Arrays.asList("gasOut", "oilOut", "waterOut"),
	Arrays.asList("retentionTime"), true,
    ...
    addEquipmentOntology(equipment, "DistillationColumn", Arrays.asList("gasOut", "liquidOut", "trayStreams"),
    ...
    ontology.add("propertyDimensions", toJsonArray(Arrays.asList("temperature", "pressure", "flowRate", "energy",
	"power", "length", "area", "volume", "time", "efficiency", "composition", "cost")));
    ontology.add("portRules",
	toJsonArray(Arrays.asList("Use name.port for equipment with multiple outlets", "Default outlet is 'outlet'",
  ...
  private static JsonObject buildBenchmarkRegistry() {
	...
	toJsonArray(Arrays.asList("runFlash", "runProcess", "runPVT", "runFlowAssurance", "calculateStandard",
	    "runPipeline", "runReservoir", "runFieldEconomics", "runBatch", "runDynamic", "runBioprocess",
	    "crossValidateModels", "runParametricStudy", "getPhaseEnvelope", "getPropertyTable", "sizeEquipment")));
    registry.add("referenceTypes", toJsonArray(Arrays.asList("NIST property data", "ISO/GPA/AGA standard examples",
	"published VLE and hydrate references", "mass and energy balance closure", "process convergence fixtures")));
    registry.add("requiredTrustFields", toJsonArray(Arrays.asList("maturityLevel", "description", "validationCases",
	"accuracyBounds", "knownLimitations", "referenceData")));
    registry.add("agentPolicy",
	toJsonArray(Arrays.asList("Fetch trust metadata before using results in engineering reports",
  ...
  private static JsonObject buildUnitSystem() {
    ...
    dimensions.add("pressure", toJsonArray(Arrays.asList("bara", "barg", "Pa", "kPa", "MPa", "psi", "psia", "atm")));
    dimensions.add("flowRate", toJsonArray(Arrays.asList("kg/hr", "kg/s", "mol/sec", "Sm3/day", "MSm3/day", "m3/hr")));
    ...
    units.add("encodings", toJsonArray(Arrays.asList("{\"value\": 25.0, \"unit\": \"C\"}",
	"[25.0, \"C\"] for process equipment property setters", "bare number only when schema default is clear")));
    units.add("validationRules",
	toJsonArray(Arrays.asList("Prefer explicit units in all agent-generated payloads",
	    "Validate physical bounds before run", "Do not mix absolute and gauge pressure without declaring the unit",
  ...
  private static JsonObject buildAutomaticFlowsheetBuilder() {
    ...
    builder.add("tools", toJsonArray(Arrays.asList("getCapabilities", "getSchema", "getExample", "validateInput",
	"runProcess", "compareProcesses", "generateReport")));
    builder.add("stages",
	toJsonArray(Arrays.asList("extract fluid composition and conditions", "map equipment names and stream topology",
	    "choose EOS and unit system", "compose processJsonContract payload", "validateInput", "runProcess",
  ...
  private static JsonObject buildOptimizationUncertaintyWorkflows() {
    ...
    workflows.add("standardTools", toJsonArray(Arrays.asList("runParametricStudy", "runBatch", "crossValidateModels",
	"compareProcesses", "runFieldEconomics")));
    workflows.add("patterns",
	toJsonArray(Arrays.asList("one-factor-at-a-time sensitivity", "multi-case batch sweep",
	    "EOS model risk comparison", "scenario comparison", "Monte Carlo or P10/P50/P90 post-processing",
	    "tornado ranking for report outputs")));
    workflows.add("requiredOutputs", toJsonArray(
	Arrays.asList("baseCase", "caseResults", "uncertainParameters", "outputMetrics", "ranking", "warnings")));
    workflows.add("agentPolicy",
	toJsonArray(Arrays.asList("Use full NeqSim simulations inside uncertainty loops when practical",
  ...
  private static JsonObject buildSafetyGatePolicy() {
    ...
    policy.add("safetyTools", toJsonArray(Arrays.asList("runRelief", "runLOPA", "runSIL", "runRiskMatrix",
    ...
    policy.add("blockingConditions", toJsonArray(Arrays.asList("missing source documents", "unsupported standard basis",
	"failed validation", "qualityGate verdict failed", "safety-critical result without evidenceRefs")));
    policy.add("outputPolicy", toJsonArray(Arrays.asList("Clearly separate screening calculation from approved design",
	"Carry evidenceRefs forward", "Include assumptions, limitations, and benchmark maturity in reports")));
    return policy;
  }
   ...
   */
  private static void addGraphNode(JsonArray nodes, String id, String type, String label, String group) {
  ...
  private static String categoryForSchemaTool(String toolName) {
    if (toolName.indexOf("flash") >= 0 || toolName.indexOf("property") >= 0 || toolName.indexOf("phase") >= 0
	|| toolName.indexOf("batch") >= 0) {
    ...
    if (toolName.indexOf("safety") >= 0 || toolName.indexOf("hazop") >= 0 || toolName.indexOf("barrier") >= 0
	|| toolName.indexOf("relief") >= 0 || toolName.indexOf("lopa") >= 0 || toolName.indexOf("sil") >= 0
    ...
    if (toolName.indexOf("schema") >= 0 || toolName.indexOf("capabilities") >= 0 || toolName.indexOf("example") >= 0) {
      return "discovery";
    }
  ...
  private static List<String> getSetupTemplateIds() {
    return Arrays.asList("capability-discovery", "thermodynamic-flash", "property-sweep", "phase-envelope",
	"process-system", "process-model", "validation", "data-catalog", "pvt-experiment", "flow-assurance",
	"standards-calculation", "pipeline-flow", "reservoir-forecast", "field-economics", "dynamic-simulation",
	"bioprocess", "equipment-sizing", "session-management", "visualization-reporting", "safety-analysis",
	"flowsheet-builder", "benchmark-registry", "unit-safe-schema", "optimization-uncertainty", "model-lifecycle",
	"safety-governance");
  }
  ...
  private static List<String> thermoUnits() {
    return Arrays.asList("K", "C", "F", "bara", "barg", "Pa", "kPa", "MPa", "psi", "J/mol", "J/kg", "J/molK", "m3/mol");
  }
  ...
  private static List<String> processUnits() {
    return Arrays.asList("K", "C", "bara", "barg", "kg/hr", "kg/s", "mol/sec", "Sm3/day", "m", "m2", "m3", "kJ/hr",
	"kW", "seconds");
  }
