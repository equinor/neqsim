 ...
 *
 * <p>
 * Takes a process JSON (same format as {@link ProcessRunner}), instruments it with controllers and measurement devices
 * using {@link DynamicProcessHelper}, then runs a transient simulation for a specified duration. Returns time-series
 * data from all transmitters and controllers.
 * </p>
 *
 * @author Even Solbraa
 * @version 1.0
...
public class DynamicRunner {
  ...
  public static String run(String json) {
    ...
    try {
      ...
      ProcessSystem process = ProcessSystem.fromJsonAndRun(normalizedProcessJson).getProcessSystem();

      // --- Instrument with controllers ---
      double timeStep = input.has("timeStep_seconds") ? input.get("timeStep_seconds").getAsDouble() : 1.0;
      double duration = input.has("duration_seconds") ? input.get("duration_seconds").getAsDouble() : 600.0;
