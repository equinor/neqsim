...
/**
 * Engineering validation layer that checks simulation results against industry design rules, physical constraints, and
 * best-practice ranges.
 *
 * <p>
 * Validation is applied post-simulation to flag issues like compressor efficiency out of range, separator residence
 * time too low, approach temperatures below minimum, hydrate risk, etc. Each check has a severity (ERROR, WARNING,
 * INFO) and a remediation hint.
 * </p>
 *
 * @author Even Solbraa
 * @version 1.0
...
public final class EngineeringValidator {
  ...
  private static Double findNumericValue(JsonObject obj, String fieldName) {
    if (obj.has(fieldName) && obj.get(fieldName).isJsonPrimitive() && obj.getAsJsonPrimitive(fieldName).isNumber()) {
  ...
  private static String buildReport(List<ValidationFinding> findings, String context) {
    ...
    if (errors > 0) {
      ...
      verdict = "PASS_WITH_WARNINGS — " + warnings + " warning(s). Review before finalizing design.";
    } else {
