 ...
 *
 * <p>
 * Accepts a JSON string describing a fluid and flash specification, performs the flash calculation, and returns a JSON
 * string with the results including per-phase properties, compositions, and conditions. Uses the same standard response
 * envelope as the process simulation pipeline.
 * </p>
 *
...
public class FlashRunner {
  ...
  private static final List<String> SUPPORTED_FLASH_TYPES = Collections
      .unmodifiableList(Arrays.asList("TP", "PH", "PS", "TV", "dewPointT", "dewPointP", "bubblePointT", "bubblePointP",
	  "hydrateTP", "dewPointTemperature", "dewPointPressure", "bubblePointTemperature", "bubblePointPressure"));
  ...
  public static String run(String json) {
    if (json == null || json.trim().isEmpty()) {
      return errorJson("INPUT_ERROR", "JSON input is null or empty", "Provide a valid JSON flash specification");
    }
    ...
    if (!SUPPORTED_MODELS.contains(model)) {
      return errorJson("UNKNOWN_MODEL", "Unknown thermodynamic model: " + model, "Use one of: " + SUPPORTED_MODELS);
    }
	...
	return errorJson("UNKNOWN_COMPONENT",
	    "Unknown component: '" + name + "'" + (suggestion != null ? ". Did you mean '" + suggestion + "'?" : ""),
	    "Use ComponentQuery.search() to find valid component names");
      }
      components.put(name, entry.getValue().getAsDouble());
    }
    if (components.isEmpty()) {
      return errorJson("MISSING_COMPONENTS", "Components map is empty", "Provide at least one component");
    }
	...
	return errorJson("UNKNOWN_FLASH_TYPE", "Unknown flash type: " + flashType,
      ...
      return errorJson("FLASH_ERROR", "Flash calculation failed (" + flashType + "): " + e.getMessage(),
	...
	if (overall.has("temperature") && overall.get("temperature").isJsonObject()) {
	  ...
	  if (temperature.has("value")) {
	    try {
	      ...
	      String temperatureUnit = temperature.has("unit") ? temperature.get("unit").getAsString() : "K";
  ...
  private static boolean containsAssociatingComponent(Map<String, Double> components) {
    for (String componentName : components.keySet()) {
      ...
      if (normalized.contains("water") || normalized.contains("h2o") || normalized.contains("methanol")
	  || normalized.contains("ethanol") || normalized.contains("meg") || normalized.contains("deg")
	  || normalized.contains("teg") || normalized.contains("glycol")) {
	return true;
      }
    }
    return false;
   ...
   *
   * <p>
   * This is the typed counterpart to {@link #run(String)}. It accepts a {@link FlashRequest} POJO and returns a typed
   * {@link ApiEnvelope} with a {@link FlashResult} payload. For direct Java consumers this avoids JSON parsing overhead
  ...
  public static ApiEnvelope<FlashResult> runTyped(FlashRequest request) {
    if (request == null) {
      return typedError("INPUT_ERROR", "FlashRequest is null", "Provide a valid FlashRequest object");
    }
    ...
    if (!SUPPORTED_MODELS.contains(model)) {
      return typedError("UNKNOWN_MODEL", "Unknown thermodynamic model: " + model, "Use one of: " + SUPPORTED_MODELS);
    }
	...
	break;
      case "PS":
	if (entropySpec == null) {
	  return typedError("MISSING_SPEC", "PS flash requires entropy specification", "Set entropy via setEntropy()");
	}
	ops.PSflash(entropySpec.getValue(), entropySpec.getUnit());
	break;
      case "TV":
	if (volumeSpec == null) {
	  return typedError("MISSING_SPEC", "TV flash requires volume specification", "Set volume via setVolume()");
	}
	...
	return typedError("UNKNOWN_FLASH_TYPE", "Unknown flash type: " + flashType,
      ...
      return typedError("FLASH_ERROR", "Flash calculation failed (" + flashType + "): " + e.getMessage(),
	  "Check that inputs are physically reasonable.");
    }
	...
	phaseNames.add(fluid.getPhase(i).getPhaseTypeName());
      ...
      FlashResult result = new FlashResult(model, flashType, fluid.getNumberOfPhases(), phaseNames, fluidResponse);
   ...
   */
  private static ApiEnvelope<FlashResult> typedError(String code, String message, String remediation) {
