 ...
 *
 * <p>
 * Supports simple reservoir material balance (tank model) with producer and injector wells. Provides production
 * forecasting by running transient steps to simulate depletion over time.
 * </p>
 *
 * @author Even Solbraa
 * @version 1.0
...
public class ReservoirRunner {
  ...
  public static String run(String json) {
    if (json == null || json.trim().isEmpty()) {
      return errorJson("INPUT_ERROR", "JSON input is null or empty", "Provide a valid JSON reservoir specification");
    }
	...
	return errorJson("MISSING_COMPONENTS", "No 'components' specified",
      ...
      // --- Reservoir volumes ---
      double gasVolume = input.has("gasVolume_Sm3") ? input.get("gasVolume_Sm3").getAsDouble() : 1e9;
      double oilVolume = input.has("oilVolume_Sm3") ? input.get("oilVolume_Sm3").getAsDouble() : 0;
      double waterVolume = input.has("waterVolume_Sm3") ? input.get("waterVolume_Sm3").getAsDouble() : 0;
