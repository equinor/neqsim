 ...
 *
 * <p>
 * Supports gas quality standards (ISO 6976, AGA, GPA, EN) and oil quality standards (ASTM, BSW). Each standard returns
 * its results (heating value, Wobbe index, density, distillation curve, etc.) in the standard JSON envelope format.
 * </p>
 *
 * @author Even Solbraa
 * @version 1.0
...
public class StandardsRunner {
  ...
  public static String run(String json) {
    ...
    if (!SUPPORTED_STANDARDS.contains(standard)) {
      return errorJson("UNKNOWN_STANDARD", "Unknown standard: " + standard, "Use one of: " + SUPPORTED_STANDARDS);
    }
  ...
  private static JsonObject runAGA3(SystemInterface fluid, JsonObject input) {
    neqsim.standards.gasquality.Standard_AGA3 aga = new neqsim.standards.gasquality.Standard_AGA3(fluid);
  ...
  private static JsonObject runASTM_D86(SystemInterface fluid, JsonObject input) {
    neqsim.standards.oilquality.Standard_ASTM_D86 d86 = new neqsim.standards.oilquality.Standard_ASTM_D86(fluid);
    d86.calculate();
    JsonObject data = new JsonObject();
   ...
   */
  private static neqsim.standards.StandardInterface createGasStandard(String name, SystemInterface fluid) {
    switch (name) {
    case "ISO6976":
   ...
   * @return the standard instance, or null if not found
   */
  private static neqsim.standards.StandardInterface createOilStandard(String name, SystemInterface fluid) {
    switch (name) {
    case "ASTM_D86":
