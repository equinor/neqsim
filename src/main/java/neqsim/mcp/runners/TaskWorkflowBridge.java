 ...
 *
 * <p>
 * The task_solve workflow (docs/development/TASK_SOLVING_GUIDE.md) expects a results.json file with specific keys:
 * key_results, validation, approach, conclusions, figure_captions, figure_discussion, equations, tables, references,
 * uncertainty, risk_evaluation.
 * </p>
 *
 * <p>
 * This class takes raw MCP tool outputs (flash results, process results, PVT data, etc.) and transforms them into the
 * task_solve results.json schema so that the report generator (step3_report/generate_report.py) can produce
...
public final class TaskWorkflowBridge {
  ...
  private static String convertToResultsJson(JsonObject input) {
    ...
    String sourceRunner = input.has("sourceRunner") ? input.get("sourceRunner").getAsString() : "unknown";
  ...
  private static JsonObject extractValidation(JsonObject output) {
    ...
    if (output.has("validation")) {
      ...
      val.addProperty("acceptance_criteria_met", "success".equals(output.get("status").getAsString()));
    }
