 ...
 *
 *
 *        Near a fluid-fluid interface, we consider the variation of densities
 *        of chemical species, \f$\boldsymbol{n}(z)\f$, as function of position,
 *        \f$z\f$, where element \f$k\f$ of the vector \f$\boldsymbol{n}(z)\f$
 *        is the number density of chemical species \f$k\f$ in the mixture. The
 *        surface tension is a functional of these densities as explained below.
 *
 *        The density profiles near the interface are such that the total
 *        Helmholz energy of the system is minimized. The effect of spatial
 *        density variations on the Helmholz energy is approximated by a second
 *        order Taylor series expansion around the homogeneous fluid of the same
 *        composition.
 *
 *        \f{equation}{ H=\int_{-\infty}^{\infty} h \,
 *        dz=\int_{-\infty}^{\infty} \left( h_0 + \frac{1}{2}\boldsymbol{n_z}^T
 *        \boldsymbol{C} \boldsymbol{n_z} \right) dz \f}
 *
 *        where \f$z\f$ is the spatial coordinate normal to the interface,
 *        \f$h_0\f$ is the Helmholz energy of the homogeneous fluid,
 *        \f$\boldsymbol{n_z}=\frac{d\boldsymbol{n}}{dz}\f$ is the local density
 *        gradient and the matrix of influence parameters \f$\boldsymbol{C}\f$
 *        is the hessian of \f$h\f$ with respect to \f$\boldsymbol{n_z}\f$. The
 *        influence parameters, \f$\boldsymbol{C}\f$, are normally assumed to be
 *        very weak functions of composition, often taken as constant through
 *        the interface.
 *
 ...
 *        be at equilibrium). Having solved equation for the density profiles,
 *        \f$\boldsymbol{n}\left(z\right)\f$, the surface tension can be
 *        calculated from: \f{equation}{ \sigma = \int_{-\infty}^{\infty}
 *        \boldsymbol{n_z}^T \boldsymbol{C} \boldsymbol{n_z} \, dz \f}
 *
 *        The class uses two different methods based on the value of
 *        this.useFullGT. If this variable is set to 0, the surface tension is
 *        calculated by solving set of differential algebraic equations using
 *        the component density of the highest boiling component in the mixture
 ...
 *
 *        For the general case, where the monotonicity can not be guaranteed or
 *        where \f$\beta_{ij}\f$ is nonzero for \f$i\neq j\f$ the full set of
 *        equations for the gradient theory must solved in the domain of the
...
public class GTSurfaceTension extends SurfaceTension {
     ...
     *
     *        The pertinent equations for calculating the interfacial tension
     *        according to Gradient Theory can be represented by the system of
     *        equations below, providing that the reference component number density
     *        varies monotonically through the interface, and that the joint
     *        influence parameter \f$c_{ij}\f$ can be described by a quadratic
     *        mixing rule, without a binary interaction parameter. The
     *        transformation of the integration variable from spacial position in
     *        the interface to the number density of a reference component
     *        simplifies the differential algebraic problem to an initial value
     *        problem with finite bounds. It is usually not known in advance whether
     *        the chosen reference component number density varies monotonically
     *        over the interface, and caution should be used in interpreting the
     *        results.
     *
     *        \f{gather}{ \sigma =
     ...
     *        \mathrm{d}\rho_\text{ref}\\ 0 =
     *        \sqrt{c_i}\left(\mu_\text{ref}^\text{eq}
     *        -\mu_\text{ref}(\boldsymbol{\rho})\right) -\sqrt{c_\text{ref}}
     *        \left(\mu_i^\text{eq}-\mu_i(\boldsymbol{\rho})\right)\\ c_i =
     *        \left(A_it_i + B_i\right)a_ib_i^{\frac{2}{3}}\\ t_i = 1-T/T_{C,i}\\
     *        A_i = \frac{1}{\xi_1 + \xi_2\omega_i}\\ B_i = \frac{1}{\xi_3 +
     *        \xi_4\omega_i}\\ c_{ij} = \sqrt{c_ic_j} \f}
     *
     ...
     *        simplifications, solving this problem induces a higher numerical cost
     *        and complexity.
     *
     ...
     *        equilibrium relations are solved as a set of algebraic equations for
    ...
    public static double solveWithRefcomp(SystemInterface system, int interface1, int interface2, int refcompIndex) {
        ...
        /*
         * Setup system of ODE-equations. Abscissa variable number density of the
         * reference component is scaled to the range [0,1]. The ordinate variable
         * surface tension is scaled scaled according to the variable yscale.
         */
        ...
        /*
         * Tolerances for the integrator. Note that you can adjust the odesystem.reltol
         * and odesystem.normtol in order to control the internal Newton-Rhapson solver
         * if the odesystem. Tolerances for the odesystem.abstol and .reltol should be
         * less than the integrator tolerances for stability. Default Newton-Rhapson
         * values are odesystem.normtol = 1e-10; odesystem.reltol = 1e-8;
         * 
         */
     ...
     *        \mathbf{n}_{i+1}}{\Delta z^2} &=& - \mathbf{F}_i\\ \mathbf{n}_i &:=&
     *        \mathbf{n}_i + \beta \Delta \mathbf{n}_i \f} where
     *        \f$\mathbf{J}=\frac{\partial \Delta \mu}{\partial \mathbf{n}_i}\f$ is
     *        a Jacobian matrix and \f$\mathbf{F}_i=\mathbf{F}\left( \mathbf{n}_i
