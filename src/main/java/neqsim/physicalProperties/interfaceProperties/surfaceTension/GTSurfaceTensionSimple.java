...
public class GTSurfaceTensionSimple extends SurfaceTension {
    ...
    public void calcInfluenceParameters() {
        ...
        for (int i = 0; i < localSystem.getPhase(0).getNumberOfComponents(); i++) {
            influenceParam[i] = localSystem.getPhase(0).getComponent(i).getSurfaceTenisionInfluenceParameter(localSystem.getPhase(0).getTemperature());
        }
    }

    /**
     * Calculates the surfacetension using the Gradient Theory for mixtures
     * Units: N/m
     */
    public double calcSurfaceTension(int interface1, int interface2) {
        ...
        int referenceComponentNumber = getComponentWithHighestBoilingpoint();//2;//localSystem.getPhase(0).getNumberOfComponents() - 1;
        ...
        double[][][] dmudn = new double[ite_step][localSystem.getPhase(0).getNumberOfComponents()][localSystem.getPhase(0).getNumberOfComponents()];
        dmudn2 = new double[ite_step][localSystem.getPhase(0).getNumberOfComponents()][localSystem.getPhase(0).getNumberOfComponents()];
        double[][] mu_inter = new double[ite_step][localSystem.getPhase(0).getNumberOfComponents()];
        double[] mu_times_den = new double[ite_step];
        double[][] fmatrix = new double[localSystem.getPhase(0).getNumberOfComponents() - 1][localSystem.getPhase(0).getNumberOfComponents() - 1];
        ...
        for (int i = 0; i < localSystem.getPhase(0).getNumberOfComponents(); i++) {
            ...
            den_interface[0][i] = 1e5 * system.getPhase(interface1).getComponent(i).getx() / system.getPhase(interface1).getMolarVolume();
            localSystem.addComponent(localSystem.getPhase(0).getComponent(i).getName(), -system.getPhase(0).getComponent(i).getNumberOfmoles());
            localSystem.addComponent(localSystem.getPhase(0).getComponent(i).getName(), system.getPhase(interface1).getComponent(i).getx() / system.getPhase(interface1).getMolarVolume());
        }
        
        
        del_den_interface[referenceComponentNumber] = (1e5 * system.getPhase(interface2).getComponent(referenceComponentNumber).getx() / system.getPhase(interface2).getMolarVolume() - 1e5 * system.getPhase(interface1).getComponent(referenceComponentNumber).getx() / system.getPhase(interface1).getMolarVolume()) / (ite_step * 1.0);
        /*
         * System.out.println("del den ref " +
         * system.getPhase(interface1).getComponent(0).getx() /
         * system.getPhase(interface1).getMolarVolume());
         * System.out.println("del den ref2 " +
         * system.getPhase(interface2).getComponent(0).getx() /
         * system.getPhase(interface2).getMolarVolume());
         *
         * System.out.println("del den ref " +
         * system.getPhase(interface1).getComponent(1).getx() /
         * system.getPhase(interface1).getMolarVolume());
         * System.out.println("del den ref2 " +
        ...
        for (int j = 1; j < ite_step; j++) {
            for (int i = 0; i < localSystem.getPhase(0).getNumberOfComponents(); i++) {
                ...
                if (java.lang.Double.isNaN(mu_inter[j][i])) {
                    double chemicalPotential = localSystem.getPhase(0).getComponent(i).getChemicalPotential(localSystem.getPhase(0));
                }
            ...
            for (int i = 0; i < localSystem.getPhase(0).getNumberOfComponents(); i++) {
                ...
                bmatrix[ii] = Math.sqrt(influenceParam[referenceComponentNumber]) * dmudn[j][i][referenceComponentNumber] - Math.sqrt(influenceParam[i]) * dmudn[j][referenceComponentNumber][referenceComponentNumber];
                kk = 0;
                for (int k = 0; k < localSystem.getPhase(0).getNumberOfComponents(); k++) {
                    if (k == referenceComponentNumber) {
                        continue;
                    }
                    fmatrix[ii][kk] = Math.sqrt(influenceParam[i]) * dmudn[j][referenceComponentNumber][k] - Math.sqrt(influenceParam[referenceComponentNumber]) * dmudn[j][i][k];
                    kk++;
                }
                ii++;
            }
            ...
            while (err > 1e-15 && iterations < 1200) {
                ...
                for (int i = 0; i < localSystem.getPhase(0).getNumberOfComponents(); i++) {
                    ...
                    localSystem.addComponent(localSystem.getPhase(0).getComponent(i).getName(), (del_den_interface[i] - del_den_interface_old[i]) / 1.0e5);
                ...
                for (int i = 0; i < localSystem.getPhase(0).getNumberOfComponents(); i++) {
                    ...
                    bmatrix[ii] = -Math.sqrt(influenceParam[i]) * (mu_equi[referenceComponentNumber] - mu_inter[j][referenceComponentNumber]) + Math.sqrt(influenceParam[referenceComponentNumber]) * (mu_equi[i] - mu_inter[j][i]);
                    kk = 0;
                    for (int k = 0; k < localSystem.getPhase(0).getNumberOfComponents(); k++) {
                        if (k == referenceComponentNumber) {
                            continue;
                ...
                for (int i = 0; i < localSystem.getPhase(0).getNumberOfComponents(); i++) {
                    if (i != referenceComponentNumber) {
                        ...
                        del_den_interface[i] += 1e5 * ans2.getEntry(pp, 0);// * (iterations) / (10.0 + iterations);
                        pp++;
                    }
                }
            }
            ...
            for (int i = 0; i < localSystem.getPhase(0).getNumberOfComponents(); i++) {
                ...
                for (int k = 0; k < localSystem.getPhase(0).getNumberOfComponents(); k++) {
                    if ((localSystem.getPhase(0).getComponent(i).getName().equals("water") || localSystem.getPhase(0).getComponent(k).getName().equals("water")) && i != k) {
                        interact = 0.0;
                    } else {
                        interact = 0.0;
                    }
            ...
            z_step[j] = z_step[j - 1] + Math.sqrt(kappa / (2.0 * mu_times_den[j])) * del_den_interface[referenceComponentNumber];
            if(Double.isNaN(z_step[j])) {
                break;
            }
            surdenstemp += Math.sqrt(2.0 * kappa * mu_times_den[j]) * del_den_interface[referenceComponentNumber];// * thermo.ThermodynamicConstantsInterface.avagadroNumber;
            
        }



      //  System.out.println("del den ref " + localSystem.getPhase(interface1).getComponent(0).getx() / localSystem.getPhase(interface1).getMolarVolume() );

        for (int j = 0; j < ite_step; j++) {
         //   System.out.println("z " + z_step[j] + " density " + j + " " + den_interface[j][0] + " mu_times_den[j] " + mu_times_den[j]+ " pressure_interface[j] " + pressure_interface[j] + " "+ pressure_interface[0]);// + " " + den_interface[j][1] + " " + den_interface[j][0] / den_interface[j][1]);
        }
    ...
    public double getInfluenceParameter(double interfaceTension, int componentNumber) {
        ...
        do {
            iter++;
            oldCalcVal = calcVal;
            //System.out.println("surface tenison " + calcSurfaceTension(0, 1) + " error " + calcVal + " influenceParam " + influenceParam[componentNumber]);
