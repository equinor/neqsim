...
/**
 * <p>PFCTConductivityMethodMod86 class.</p>
 *
 * @author esol
...
public class PFCTConductivityMethodMod86 extends Conductivity {
    ...
    double[] GVcoef = { -2.147621e5, 2.190461e5, -8.618097e4, 1.496099e4, -4.730660e2, -2.331178e2, 3.778439e1,
            -2.320481, 5.311764e-2 };
    double condRefA = -0.25276292, condRefB = 0.33432859, condRefC = 1.12, condRefF = 168.0, condRefE = 1.0,
            condRefG = 0.0;
    ...
    /**
     * <p>Constructor for PFCTConductivityMethodMod86.</p>
     *
	...
	public double calcConductivity() {
        ...
        /*
         * double tempMMix1 = 0.0; double tempMMix2 = 0.0; double tempMMix3 = 0.0;
         * double tempMMix4 = 0.0; for (int i = 0; i <
         * phase.getPhase().getNumberOfComponents(); i++) { for (int j = 0; j <
         * phase.getPhase().getNumberOfComponents(); j++) { tempMMix1 =
         * phase.getPhase().getComponent(i).getx() *
         * phase.getPhase().getComponent(j).getx() * Math.sqrt(1.0 /
         * phase.getPhase().getComponent(i).getMolarMass() + 1.0 /
         * phase.getPhase().getComponent(j).getMolarMass()) *
         * Math.pow(phase.getPhase().getComponent(i).getTC() /
         * phase.getPhase().getComponent(j).getTC(), 1.0 / 4.0); tempMMix2 =
         * Math.pow(Math.pow(phase.getPhase().getComponent(i).getTC() /
         * phase.getPhase().getComponent(i).getPC(), 1.0 / 3.0) +
         * Math.pow(phase.getPhase().getComponent(j).getTC() /
         * phase.getPhase().getComponent(j).getPC(), 1.0 / 3.0), 2.0); tempMMix3 +=
         * tempMMix1 / tempMMix2; } } tempMMix4 = 1.0 / 16.0 * Math.pow(tempMMix3, -2.0)
         * * Math.pow(TCmix, -1.0 / 3.0) * Math.pow(PCmix, 4.0 / 3.0);
         */
        ...
        double condIntRef = (1.18653 * nstarRef * (CpID - 2.5 * neqsim.thermo.ThermodynamicConstantsInterface.R)
    ...
    /**
     * <p>getRefComponentConductivity.</p>
     *
     * @param temp a double
     * @param pres a double
     * @return a double
    ...
    public double calcMixLPViscosity() {
        ...
        // double PCmix = 0.0, TCmix = 0.0, Mmix = 0.0;
        /*
         * for (int i = 0; i < phase.getPhase().getNumberOfComponents(); i++) { for (int
         * j = 0; j < phase.getPhase().getNumberOfComponents(); j++) { double tempVar =
         * phase.getPhase().getComponent(i).getx() *
         * phase.getPhase().getComponent(j).getx() *
         * Math.pow(Math.pow(phase.getPhase().getComponent(i).getTC() /
         * phase.getPhase().getComponent(i).getPC(), 1.0 / 3.0) +
         * Math.pow(phase.getPhase().getComponent(j).getTC() /
         * phase.getPhase().getComponent(j).getPC(), 1.0 / 3.0), 3.0); tempTC1 +=
         * tempVar * Math.sqrt(phase.getPhase().getComponent(i).getTC() *
         * phase.getPhase().getComponent(j).getTC()); tempTC2 += tempVar; tempPC1 +=
         * tempVar * Math.sqrt(phase.getPhase().getComponent(i).getTC() *
         ...
         * 
         * PCmix = 8.0 * tempPC1 / (tempPC2 * tempPC2); TCmix = tempTC1 / tempTC2; Mmix
         * = (Mmtemp + 1.304e-4 * (Math.pow(Mwtemp / Mmtemp, 2.303) - Math.pow(Mmtemp,
         * 2.303))) * 1e3; //phase.getPhase().getMolarMass();
         */
