...
public class PFCTConductivityMethodMod86 extends Conductivity {
    ...
    double condRefA = -0.25276292, condRefB = 0.33432859, condRefC = 1.12, condRefF = 168.0,
            condRefE = 1.0, condRefG = 0.0;
    ...
    public double calcConductivity() {
        ...
        Mmix = (Mmtemp + 1.304e-4 * (Math.pow(Mwtemp / Mmtemp, 2.303) - Math.pow(Mmtemp, 2.303)))
                * 1e3; // phase.getPhase().getMolarMass();
        /*
         * double tempMMix1 = 0.0; double tempMMix2 = 0.0; double tempMMix3 = 0.0; double tempMMix4
         * = 0.0; for (int i = 0; i < phase.getPhase().getNumberOfComponents(); i++) { for (int j =
         * 0; j < phase.getPhase().getNumberOfComponents(); j++) { tempMMix1 =
         ...
         * phase.getPhase().getComponent(j).getPC(), 1.0 / 3.0), 2.0); tempMMix3 += tempMMix1 /
         * tempMMix2; } } tempMMix4 = 1.0 / 16.0 * Math.pow(tempMMix3, -2.0) * Math.pow(TCmix, -1.0
        ...
        for (int i = 0; i < phase.getPhase().getNumberOfComponents(); i++) {
            for (int j = 0; j < phase.getPhase().getNumberOfComponents(); j++) {
                alfaMix += phase.getPhase().getComponent(i).getx()
                        * phase.getPhase().getComponent(j).getx()
        ...
        double condIntMix = (1.18653 * nstarMix
                * (CpIDmix - 2.5 * neqsim.thermo.ThermodynamicConstantsInterface.R) * FfuncMix)
                / (Mmix / 1.0e3);// *1e3;
        ...
        conductivity = Math.pow(TCmix / Tc0, -1.0 / 6.0) * Math.pow(PCmix / Pc0, 2.0 / 3.0)
                * Math.pow(Mmix / M0, -0.5) * alfaMix / alfa0 * (refConductivity - condIntRef)
                + condIntMix;
        return conductivity;
    }
    ...
    public double getRefComponentConductivity(double temp, double pres) {
        ...
        double viscRef1 =
                (condRefA + condRefB * Math.pow(condRefC - Math.log(temp / condRefF), 2.0))
                        * molDens;

        double temp1 =
                Math.pow(molDens, 0.1) * (condRefJ[1] + condRefJ[2] / Math.pow(temp, 3.0 / 2.0));
        ...
        double HTAN = (Math.exp(dTfreeze) - Math.exp(-dTfreeze))
                / (Math.exp(dTfreeze) + Math.exp(-dTfreeze));
        ...
        double temp4 =
                Math.pow(molDens, 0.1) * (condRefK[1] + condRefK[2] / Math.pow(temp, 3.0 / 2.0));
    ...
    public double calcMixLPViscosity() {
        ...
        double P0 = 1.01325 * referenceSystem.getPhase(0).getComponent(0).getPC() / PCmix * alfaMix
                / alfa0;
