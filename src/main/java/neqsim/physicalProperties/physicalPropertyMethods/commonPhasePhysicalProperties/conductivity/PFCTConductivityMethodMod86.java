...
public class PFCTConductivityMethodMod86 extends Conductivity {
    ...
    double condRefA = -0.25276292, condRefB = 0.33432859, condRefC = 1.12, condRefF = 168.0, condRefE = 1.0, condRefG = 0.0;
    double condRefJ[] = {-7.04036339907, 12.319512908, -8.8525979933e2, 72.835897919, 0.74421462902, -2.9706914540, 2.2209758501e3};
...
/*
        double tempMMix1 = 0.0;
        double tempMMix2 = 0.0;
        double tempMMix3 = 0.0;
        double tempMMix4 = 0.0;
        for (int i = 0; i < phase.getPhase().getNumberOfComponents(); i++) {
            for (int j = 0; j < phase.getPhase().getNumberOfComponents(); j++) {
                tempMMix1 = phase.getPhase().getComponent(i).getx() * phase.getPhase().getComponent(j).getx() * Math.sqrt(1.0 / phase.getPhase().getComponent(i).getMolarMass() + 1.0 / phase.getPhase().getComponent(j).getMolarMass()) * Math.pow(phase.getPhase().getComponent(i).getTC() / phase.getPhase().getComponent(j).getTC(), 1.0 / 4.0);
                tempMMix2 = Math.pow(Math.pow(phase.getPhase().getComponent(i).getTC() / phase.getPhase().getComponent(i).getPC(), 1.0 / 3.0) + Math.pow(phase.getPhase().getComponent(j).getTC() / phase.getPhase().getComponent(j).getPC(), 1.0 / 3.0), 2.0);
                tempMMix3 += tempMMix1 / tempMMix2;
            }
        }
        tempMMix4 = 1.0 / 16.0 * Math.pow(tempMMix3, -2.0) * Math.pow(TCmix, -1.0 / 3.0) * Math.pow(PCmix, 4.0 / 3.0);
         */
        ...
        for (int i = 0; i < phase.getPhase().getNumberOfComponents(); i++) {
            alphai[i] = 1.0 + 6.004e-4 * Math.pow(redDens, 2.043) * Math.pow(phase.getPhase().getComponent(i).getMolarMass() * 1e3, 1.086);
        }
        alfaMix = 0.0;
        alfa0 = 1.0 + 6.004e-4 * Math.pow(redDens, 2.043) * Math.pow(referenceSystem.getMolarMass() * 1e3, 1.086);
        for (int i = 0; i < phase.getPhase().getNumberOfComponents(); i++) {
            for (int j = 0; j < phase.getPhase().getNumberOfComponents(); j++) {
                alfaMix += phase.getPhase().getComponent(i).getx() * phase.getPhase().getComponent(j).getx() * Math.sqrt(alphai[i] * alphai[j]);
            }
        }
        if(alfaMix<1e-10) return 0.0; //this can happen befor init is doen on system
        // alfaMix= alfaMix*2;
        ...
        double condIntRef = (1.18653 * nstarRef * (CpID - 2.5 * neqsim.thermo.ThermodynamicConstantsInterface.R) * Ffunc) / (referenceSystem.getMolarMass());//*1e3;
        ...
        double condIntMix = (1.18653 * nstarMix * (CpIDmix - 2.5 * neqsim.thermo.ThermodynamicConstantsInterface.R) * FfuncMix) / (Mmix / 1.0e3);//*1e3;

        double refConductivity = getRefComponentConductivity(T0, P0);

        conductivity = Math.pow(TCmix / Tc0, -1.0 / 6.0) * Math.pow(PCmix / Pc0, 2.0 / 3.0) * Math.pow(Mmix / M0, -0.5) * alfaMix / alfa0 * (refConductivity - condIntRef) + condIntMix;
        return conductivity;
    }
    ...
    public double getRefComponentConductivity(double temp, double pres) {
        ...
        molDens = referenceSystem.getLowestGibbsEnergyPhase().getDensity() * 1e-3; // density in gr/cm^3
        ...
        double temp2 = redMolDens * Math.pow(molDens, 0.5) * (condRefJ[4] + condRefJ[5] / temp + condRefJ[6] / Math.pow(temp, 2.0));
        ...
        double temp5 = redMolDens * Math.pow(molDens, 0.5) * (condRefK[4] + condRefK[5] / temp + condRefK[6] / Math.pow(temp, 2.0));
    ...
    public double getRefComponentViscosity(double temp, double pres) {
        ...
        double[] GVcoef = {-2.090975e5, 2.647269e5, -1.472818e5, 4.716740e4, -9.491872e3, 1.219979e3, -9.627993e1, 4.274152, -8.141531e-2};
        double visRefE = 1.0;
        double viscRefJ[] = {-1.035060586e1, 1.7571599671e1, -3.0193918656e3, 1.8873011594e2, 4.2903609488e-2, 1.4529023444e2, 6.1276818706e3};
    ...
    public double calcMixLPViscosity() {
        ...
        double T0 = phase.getPhase().getTemperature() * referenceSystem.getPhase(0).getComponent(0).getTC() / TCmix * alfaMix / alfa0;
        double P0 = 1.01325 * referenceSystem.getPhase(0).getComponent(0).getPC() / PCmix * alfaMix / alfa0;
