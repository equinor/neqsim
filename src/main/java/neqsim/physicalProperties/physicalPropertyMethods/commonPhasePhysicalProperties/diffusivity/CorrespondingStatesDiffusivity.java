...
public class CorrespondingStatesDiffusivity extends Diffusivity{
    ...
    public CorrespondingStatesDiffusivity(neqsim.physicalProperties.physicalPropertySystem.PhysicalPropertiesInterface phase) {
        super(phase);
    ...
    public double calcBinaryDiffusionCoefficient(int i, int j, int method){
        ...
        if(phase.getPhase().getPhaseType()==0){
            binaryDiffusionCoeffisients[i][j] = 1.0e-4*9.89e-8 * Math.pow(phase.getViscosity()*1e3, -0.907) *
            Math.pow(1.0/phase.getPhase().getComponents()[i].getNormalLiquidDensity()*phase.getPhase().getComponents()[i].getMolarMass()*1000,-0.45)*Math.pow(1.0/phase.getPhase().getComponents()[j].getNormalLiquidDensity()*phase.getPhase().getComponents()[j].getMolarMass()*1000,0.265)*phase.getPhase().getTemperature();
