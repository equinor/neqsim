...
abstract class Viscosity extends neqsim.physicalProperties.physicalPropertyMethods.commonPhasePhysicalProperties.CommonPhysicalPropertyMethod implements  neqsim.physicalProperties.physicalPropertyMethods.methodInterface.ViscosityInterface{
    ...
    public void calcPureComponentViscosity() {
        ...
        for (int i = 0; i < phase.getPhase().getNumberOfComponents(); i++) {
            ...
            if (phase.getPhase().getTemperature() > phase.getPhase().getComponents()[i].getTC()) {
                ...
                pureComponentViscosity[i] = phase.getPhase().getComponents()[i].getLiquidViscosityParameter(0) * Math.pow(phase.getPhase().getTemperature(), phase.getPhase().getComponents()[i].getLiquidViscosityParameter(1));
    ...
    public double getViscosityPressureCorrection(int i) {
        ...
        double deltaPr = (phase.getPhase().getPressure() - 0.0) / phase.getPhase().getComponent(i).getPC();
        ...
        return (1.0 + D * Math.pow(deltaPr / 2.118, A)) / (1.0 + C * phase.getPhase().getComponent(i).getAcentricFactor() * deltaPr);
    }
