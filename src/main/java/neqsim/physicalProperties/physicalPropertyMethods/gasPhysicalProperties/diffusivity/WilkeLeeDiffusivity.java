...
public class WilkeLeeDiffusivity extends Diffusivity{
    ...
    public double calcBinaryDiffusionCoefficient(int i, int j, int method){
        ...
        binaryDiffusionCoeffisients[i][j] = (3.03 - (0.98/Math.sqrt(binaryMolecularMass[i][j]))*1.0e-3*Math.pow(gasPhase.getPhase().getTemperature(),1.5)) /
        (gasPhase.getPhase().getPressure()*Math.sqrt(binaryMolecularMass[i][j])*Math.pow(binaryMolecularDiameter[i][j],2.0)*binaryLennardJonesOmega[i][j]);
