...
public class Conductivity extends neqsim.physicalProperties.physicalPropertyMethods.liquidPhysicalProperties.LiquidPhysicalPropertyMethod implements neqsim.physicalProperties.physicalPropertyMethods.methodInterface.ConductivityInterface {
    ...
    public double calcConductivity() {
        ...
        for (int i = 0; i < liquidPhase.getPhase().getNumberOfComponents(); i++) {
            ...
            for (int j = 0; j < liquidPhase.getPhase().getNumberOfComponents(); j++) {
                tempVar2 = Math.pow(1.0 + Math.sqrt(pureComponentConductivity[i] / pureComponentConductivity[j]) * Math.pow(liquidPhase.getPhase().getComponents()[j].getMolarMass() / liquidPhase.getPhase().getComponents()[i].getMolarMass(), 0.25), 2.0) / Math.pow(8.0 * (1.0 + liquidPhase.getPhase().getComponents()[i].getMolarMass() / liquidPhase.getPhase().getComponents()[j].getMolarMass()), 0.5);
            ...
            // conductivity =  conductivity + liquidPhase.getPhase().getComponents()[i].getx() * pureComponentConductivity[i];///tempVar;
        }
    ...
    public void calcPureComponentConductivity() {
        for (int i = 0; i < liquidPhase.getPhase().getNumberOfComponents(); i++) {
            ...
            pureComponentConductivity[i] = liquidPhase.getPhase().getComponents()[i].getLiquidConductivityParameter(0) + liquidPhase.getPhase().getComponents()[i].getLiquidConductivityParameter(1) * liquidPhase.getPhase().getTemperature() + liquidPhase.getPhase().getComponents()[i].getLiquidConductivityParameter(2) * Math.pow(liquidPhase.getPhase().getTemperature(), 2.0);
