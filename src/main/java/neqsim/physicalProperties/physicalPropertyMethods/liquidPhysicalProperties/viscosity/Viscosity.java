...
public class Viscosity extends neqsim.physicalProperties.physicalPropertyMethods.liquidPhysicalProperties.LiquidPhysicalPropertyMethod implements neqsim.physicalProperties.physicalPropertyMethods.methodInterface.ViscosityInterface {
        ...
        for (int i = 0; i < liquidPhase.getPhase().getNumberOfComponents(); i++) {
            ...
            for (int j = 0; j < liquidPhase.getPhase().getNumberOfComponents(); j++) {
                ...
                if (i != j) {
                    ...
                    //System.out.println("gij " + liquidPhase.getMixingRule().getViscosityGij(i, j));
                }

                //if(i!=j) tempVar2 += liquidPhase.getPhase().getComponents()[i].getx()*liquidPhase.getPhase().getComponents()[j].getx()*liquidPhase.getMixingRule().getViscosityGij(i,j);
            }
        }
    ...
    public double getViscosityPressureCorrection(int i) {
        ...
        return (1.0 + D * Math.pow(deltaPr / 2.118, A)) / (1.0 + C * liquidPhase.getPhase().getComponent(i).getAcentricFactor() * deltaPr);
    }
}
