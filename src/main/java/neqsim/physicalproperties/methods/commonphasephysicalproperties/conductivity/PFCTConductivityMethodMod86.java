...
public class PFCTConductivityMethodMod86 extends Conductivity {
  ...
  /**
   * Thread-local reference system to prevent race conditions when multiple threads calculate conductivity
   * simultaneously. Each thread gets its own instance.
   */
  private static final ThreadLocal<SystemInterface> threadLocalReferenceSystem = new ThreadLocal<SystemInterface>() {
    @Override
    protected SystemInterface initialValue() {
      SystemInterface sys = new SystemSrkEos(273.0, ThermodynamicConstantsInterface.referencePressure);
  ...
  /**
   * Dilute-gas thermal conductivity coefficients for methane reference fluid. Updated from Friend, Ely, Ingham (1989)
   ...
   * for the overprediction of the Hanley dilute-gas correlation. Calibrated against NIST REFPROP methane thermal
   * conductivity at 400-1000 K.
   */
  private static final double HIGH_T_THRESHOLD = 400.0;

  /**
   * High-temperature correction slope [1/K]. Linear damping: correction = 1 + slope * (T - threshold). At 500 K:
   * correction = 0.91 (reduces 17% error to ~7%). At 600 K: correction = 0.82 (reduces 26% error to ~4%).
   */
  ...
  public double calcConductivity() {
    ...
    /*
     * double tempMMix1 = 0.0; double tempMMix2 = 0.0; double tempMMix3 = 0.0; double tempMMix4 = 0.0; for (int i = 0; i
     * < phase.getPhase().getNumberOfComponents(); i++) { for (int j = 0; j < phase.getPhase().getNumberOfComponents();
     * j++) { tempMMix1 = phase.getPhase().getComponent(i).getx() * phase.getPhase().getComponent(j).getx() *
     * Math.sqrt(1.0 / phase.getPhase().getComponent(i).getMolarMass() + 1.0 /
     * phase.getPhase().getComponent(j).getMolarMass()) * Math.pow(phase.getPhase().getComponent(i).getTC() /
     * phase.getPhase().getComponent(j).getTC(), 1.0 / 4.0); tempMMix2 =
     * Math.pow(Math.pow(phase.getPhase().getComponent(i).getTC() / phase.getPhase().getComponent(i).getPC(), 1.0 / 3.0)
     * + Math.pow(phase.getPhase().getComponent(j).getTC() / phase.getPhase().getComponent(j).getPC(), 1.0 / 3.0), 2.0);
     * tempMMix3 += tempMMix1 / tempMMix2; } } tempMMix4 = 1.0 / 16.0 * Math.pow(tempMMix3, -2.0) * Math.pow(TCmix, -1.0
     * / 3.0) * Math.pow(PCmix, 4.0 / 3.0);
     */
    if (Double.isNaN(PCmix) || Double.isNaN(TCmix)) {
      PCmix = 1.0;
      TCmix = 273.15;
    }
    double TOref = phase.getPhase().getTemperature() * referenceSystem.getPhase(0).getComponent(0).getTC() / TCmix;
    ...
    alfaMix = 0.0;
    alfa0 = 1.0 + 6.004e-4 * Math.pow(redDens, 2.043) * Math.pow(referenceSystem.getMolarMass() * 1e3, 1.086);
    for (int i = 0; i < phase.getPhase().getNumberOfComponents(); i++) {
      for (int j = 0; j < phase.getPhase().getNumberOfComponents(); j++) {
    ...
    double nstarRef = getRefComponentViscosity(T0, ThermodynamicConstantsInterface.referencePressure);
	...
	/ (referenceSystem.getMolarMass()); // *1e3;
    ...
    double condIntMix = (1.18653 * nstarMix * (CpIDmix - 2.5 * neqsim.thermo.ThermodynamicConstantsInterface.R)
	* FfuncMix) / (Mmix / 1.0e3); // *1e3;

    double refConductivity = getRefComponentConductivity(T0, P0);

    conductivity = Math.pow(TCmix / Tc0, -1.0 / 6.0) * Math.pow(PCmix / Pc0, 2.0 / 3.0) * Math.pow(Mmix / M0, -0.5)
	* alfaMix / alfa0 * (refConductivity - condIntRef) + condIntMix;
    return conductivity;
  }
  ...
  public double getRefComponentConductivity(double temp, double pres) {
    ...
    double viscRef1 = (condRefA + condRefB * Math.pow(condRefC - Math.log(temp / condRefF), 2.0)) * molDens;
  ...
  public double calcMixLPViscosity() {
     ...
     * phase.getPhase().getNumberOfComponents(); j++) { double tempVar = phase.getPhase().getComponent(i).getx() *
     * phase.getPhase().getComponent(j).getx() * Math.pow(Math.pow(phase.getPhase().getComponent(i).getTC() /
     * phase.getPhase().getComponent(i).getPC(), 1.0 / 3.0) + Math.pow(phase.getPhase().getComponent(j).getTC() /
     * phase.getPhase().getComponent(j).getPC(), 1.0 / 3.0), 3.0); tempTC1 += tempVar *
     * Math.sqrt(phase.getPhase().getComponent(i).getTC() * phase.getPhase().getComponent(j).getTC()); tempTC2 +=
     * tempVar; tempPC1 += tempVar * Math.sqrt(phase.getPhase().getComponent(i).getTC() *
     * phase.getPhase().getComponent(j).getTC()); tempPC2 += tempVar; } Mwtemp +=
     * phase.getPhase().getComponent(i).getx() * Math.pow(phase.getPhase().getComponent(i).getMolarMass(), 2.0); Mmtemp
     * += phase.getPhase().getComponent(i).getx() * phase.getPhase().getComponent(i).getMolarMass(); }
     *
     * PCmix = 8.0 * tempPC1 / (tempPC2 * tempPC2); TCmix = tempTC1 / tempTC2; Mmix = (Mmtemp + 1.304e-4 *
     * (Math.pow(Mwtemp / Mmtemp, 2.303) - Math.pow(Mmtemp, 2.303))) * 1e3; //phase.getPhase().getMolarMass();
     */
    double redDens = 101325 / ThermodynamicConstantsInterface.R / phase.getPhase().getTemperature() / 1.0e3 / 10.15;
    double alfa0 = 0.0;
    double alfaMix = 0.0;
    double[] alphai = new double[phase.getPhase().getNumberOfComponents()];
    for (int i = 0; i < phase.getPhase().getNumberOfComponents(); i++) {
	...
	* alfaMix / alfa0;
    double P0 = ThermodynamicConstantsInterface.referencePressure * referenceSystem.getPhase(0).getComponent(0).getPC()
	/ PCmix * alfaMix / alfa0;
