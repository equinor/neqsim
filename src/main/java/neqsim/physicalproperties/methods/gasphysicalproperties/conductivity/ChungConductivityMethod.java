 ...
 * model is active on the phase. This avoids the unit-mismatch bug where the PFCT viscosity model returns values in Pa·s
 * while the original code assumed Chung μP units.
 * </p>
 *
 * <p>
 * References:
 ...
 * <li>Chung, T.H., Ajlan, M., Lee, L.L., Starling, K.E. (1988). Ind. Eng. Chem. Res. 27, 671-679.</li>
 * <li>Poling, B.E., Prausnitz, J.M., O'Connell, J.P. (2001). The Properties of Gases and Liquids, 5th ed, Chap.
 * 10.</li>
 * </ul>
 *
 * @author Even Solbraa
...
public class ChungConductivityMethod extends Conductivity {
  ...
  private double calcChungDiluteGasViscosity(int i) {
    ...
    double mu_r = 131.3 * gasPhase.getPhase().getComponent(i).getDebyeDipoleMoment() / Math.sqrt(Vc * Tc);
    ...
    double omegaV = NF_A * Math.pow(Tstar, -NF_B) + NF_C * Math.exp(-NF_D * Tstar) + NF_E * Math.exp(-NF_F * Tstar);
   ...
   *
   * <p>
   * where η₀ is the Chung dilute-gas viscosity computed internally (not from the phase viscosity model) and Ψ is the
