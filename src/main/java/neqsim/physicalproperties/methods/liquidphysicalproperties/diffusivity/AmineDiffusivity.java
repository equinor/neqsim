...
public class AmineDiffusivity extends SiddiqiLucasMethod {
  ...
  public void calcEffectiveDiffusionCoefficients() {
    ...
    try {
      double molConsMDEA = liquidPhase.getPhase().getComponent("MDEA").getx() * liquidPhase.getPhase().getDensity()
          / liquidPhase.getPhase().getMolarMass();
      molConsMDEA += liquidPhase.getPhase().getComponent("MDEA+").getx() * liquidPhase.getPhase().getDensity()
          / liquidPhase.getPhase().getMolarMass();
      effectiveDiffusionCoefficient[liquidPhase.getPhase().getComponent("CO2").getComponentNumber()] = n2oaminediff
          * co2waterdiff / n2owaterdiff;
      effectiveDiffusionCoefficient[liquidPhase.getPhase().getComponent("MDEA").getComponentNumber()] = 0.0207
          * Math.exp(-2360.7 / liquidPhase.getPhase().getTemperature() - 24.727e-5 * molConsMDEA) * 1e-4;
    } catch (Exception ex) {
  ...
  public double[][] calcDiffusionCoefficients(int binaryDiffusionCoefficientMethod, int multicomponentDiffusionMethod) {
    ...
    for (int i = 0; i < liquidPhase.getPhase().getNumberOfComponents(); i++) {
      for (int j = 0; j < liquidPhase.getPhase().getNumberOfComponents(); j++) {
        if (liquidPhase.getPhase().getComponent(i).getComponentName().equals("MDEA")) {
          binaryDiffusionCoefficients[i][j] = effectiveDiffusionCoefficient[liquidPhase.getPhase().getComponent("MDEA")
              .getComponentNumber()];
        } else {
          binaryDiffusionCoefficients[i][j] = effectiveDiffusionCoefficient[liquidPhase.getPhase().getComponent("CO2")
              .getComponentNumber()];
        }
      }
    }
