...
/**
 * Container of {@link WriteValidator} implementations that ship with NeqSim. Each inner class targets a specific
 * equipment class and performs a small set of inexpensive, physically meaningful checks against the proposed write.
 *
 * <p>
 * These validators are deliberately conservative: they reject only writes that are unambiguously unphysical (negative
 * pressures, efficiencies outside {@code [0,1]}, valve openings outside {@code [0,100]}) and emit
 * {@link WriteValidationResult.Severity#WARNING WARNING}-level results for values that are merely unusual (for example
 * outlet pressure below the current inlet pressure on a compressor). Hard hardware-specific limits (surge, choke, MAOP)
 * should be added by users via project-specific validators registered against the same {@link WriteValidatorRegistry}.
 * </p>
 *
 * @author Even Solbraa
 * @version 1.0
...
public final class DefaultWriteValidators {
  ...
  /**
   * Reads the current pressure of a stream in bara, returning {@code Double.NaN} when the stream or its thermo system
   * is not initialised.
   *
  ...
  /**
   * Reads the current temperature of a stream in kelvin, returning {@code Double.NaN} when the stream or its thermo
  ...
  /**
   * Converts a pressure value to bara from one of the units accepted by NeqSim's stream API. Returns {@code value}
  ...
  /**
   * Converts a temperature value to kelvin from one of the units accepted by NeqSim's stream API. Returns {@code value}
   * unchanged when {@code unit} is null/empty/unrecognised.
   *
  ...
  /**
   * Validator for {@link Compressor}. Checks that outlet pressure is positive and at least equal to the current inlet
   * pressure, that efficiencies fall in {@code (0,1]}, and that the speed is non-negative.
   */
  ...
  /**
   * Validator for {@link Pump}. Checks that outlet pressure is positive and at least equal to the current inlet
   * pressure and that efficiency falls in {@code (0,1]}.
   */
  ...
  /**
   * Validator for {@link ThrottlingValve}. Checks that outlet pressure is positive and not higher than inlet pressure,
   * that Cv is non-negative, and that {@code percentValveOpening} falls in {@code [0,100]}.
   */
  ...
  /**
   * Validator for {@link Heater}. Checks that the new outlet temperature is not below the inlet temperature.
   */
  ...
  /**
   * Validator for {@link Cooler}. Checks that the new outlet temperature is not above the inlet temperature.
   */
  ...
  /**
   * Validator for {@link Separator}. Currently a placeholder that always passes; provided so downstream projects can
   * register additional separator-specific checks against {@code Separator.class}.
   */
