...
/**
 * Strategy interface for validating writes to a specific equipment class before they are applied to the simulation.
 * Concrete implementations check that a proposed value is physically meaningful and consistent with the equipment's
 * current state (for example: outlet pressure greater than inlet pressure for a compressor, efficiency in {@code [0,1]}
 * for a pump, or opening percentage in {@code [0,100]} for a control valve).
 *
 * <p>
 ...
 * {@link ProcessAutomation#setValuesTransactional(java.util.Map, String)} before any write is applied. A
 * {@link WriteValidationResult} with {@code valid == false} causes the entire transactional batch to be aborted and
 * rolled back.
 * </p>
 *
 * <p>
 * Implementations must be stateless and thread-safe; they may be invoked concurrently by multiple agents sharing the
 * same registry.
 * </p>
 *
 * @author Even Solbraa
...
public interface WriteValidator {
  ...
  /**
   * Returns the equipment class this validator applies to. The registry uses {@link Class#isAssignableFrom(Class)} for
   * dispatch, so a validator registered for a base class will also be invoked for its subclasses unless a more specific
