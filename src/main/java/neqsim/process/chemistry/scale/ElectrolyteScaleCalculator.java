...
/**
 * Activity-coefficient-corrected scale prediction calculator that bridges electrolyte thermodynamics into the
 * saturation index calculation.
 *
 * <p>
 * Computes ionic strength rigorously from the brine composition, {@code I = 0.5 * sum(c_i * z_i^2)} in mol/kgw, then
 * applies the Davies extension of Debye-Hueckel theory to obtain activity coefficients for each ion:
 *
 * <pre>
 ...
 *
 * <p>
 * Temperature-corrected solubility products (Ksp(T)) follow Plummer-Busenberg (1982) for calcite, Templeton (1960)
 * extrapolations for barite/celestite, and Marshall-Slusher (1966) for gypsum.
 *
 * <p>
 * Outputs replace, but are also benchmarked against, the empirical Oddo-Tomson approach used by
 * {@link neqsim.pvtsimulation.flowassurance.ScalePredictionCalculator}. The bridge therefore gives the user a rigorous
 * EOS-consistent number while preserving the legacy code path.
 *
 * <p>
 * Standards informational: NACE TM0374 (CaSO4 stability), NORSOK M-001 (mineral scale), Plummer &amp; Busenberg (1982)
 * "The solubilities of calcite, aragonite and vaterite in CO2-H2O".
 *
 * @author ESOL
 * @version 1.0
 */
public class ElectrolyteScaleCalculator implements Serializable {
   ...
   * @return this for chaining
   */
  public ElectrolyteScaleCalculator setCations(double ca, double ba, double sr, double mg, double na, double k,
      double fe) {
    this.calciumMgL = ca;
    this.bariumMgL = ba;
  ...
  /**
   * Computes ionic strength, Davies-equation activity coefficients, and activity-corrected saturation indices for
   * CaCO3, BaSO4, CaSO4, SrSO4.
   *
   * @return this for chaining
   */
  public ElectrolyteScaleCalculator calculate() {
    ...
    caco3SaturationIndex = saturationIndex(g2 * Math.max(mCa, 1e-15), g2 * Math.max(mCO3Eff + mCO3, 1e-15), pKspCaCO3);
    baso4SaturationIndex = saturationIndex(g2 * Math.max(mBa, 1e-15), g2 * Math.max(mSO4, 1e-15), pKspBaSO4);
