 ...
 *
 * <p>
 * The generator maintains a built-in library of symptom-to-hypothesis mappings for common equipment types (compressor,
 * pump, separator, heat exchanger, valve). Users may register custom hypotheses via
 * {@link #register(String, Symptom, Hypothesis)}.
 * </p>
 *
 * <p>
 * Prior probabilities are sourced from {@link ReliabilityDataSource} when available, falling back to a uniform
 * distribution over candidate hypotheses.
 * </p>
 *
...
public class HypothesisGenerator implements Serializable {
  ...
  /**
   * Registry: equipmentType + symptom to list of hypothesis builders. Key format: "equipmentType:SYMPTOM".
   */
  ...
  public void register(String equipmentType, Symptom symptom, Hypothesis hypothesis) {
    ...
    List<Hypothesis.Builder> builders = registry.containsKey(key) ? registry.get(key)
        : new ArrayList<Hypothesis.Builder>();
    Hypothesis.Builder builder = Hypothesis.builder().name(hypothesis.getName())
        .description(hypothesis.getDescription()).category(hypothesis.getCategory())
        .failureMode(hypothesis.getFailureMode()).priorProbability(hypothesis.getPriorProbability());
    for (Hypothesis.ExpectedSignal signal : hypothesis.getExpectedSignals()) {
      builder.addExpectedSignal(signal.getParameterPattern(), signal.getExpectedBehavior(), signal.getWeight(),
          signal.getRationale());
    }
   ...
   *
   * <p>
   * The method looks up hypotheses from the registry based on the equipment class name. If no match is found for the
   * specific equipment type, it falls back to a generic set. Prior probabilities are adjusted using OREDA failure-mode
   * data when available.
   * </p>
   *
  ...
  String classifyEquipment(ProcessEquipmentInterface equipment) {
    if (equipment instanceof Compressor) {
      ...
      return "Separator";
    } else if (equipment instanceof HeatExchanger || equipment instanceof Cooler || equipment instanceof Heater) {
      ...
      for (Hypothesis h : hypotheses) {
        if (h.getFailureMode() != null && !h.getFailureMode().isEmpty()) {
          ...
          if (prob != null && prob > 0) {
            ...
            logger.debug("OREDA match for hypothesis '{}': failure mode '{}' = {}", h.getName(), h.getFailureMode(),
                prob);
          }
        }
      }
   ...
   *
   * <p>
   * Uses a three-tier matching strategy: (1) substring containment, (2) token overlap with at least 50% match, (3)
   * Levenshtein edit distance within 3 characters. Returns the first confident match found.
   * </p>
   *
  ...
  private Double findFuzzyModeProbability(Map<String, Double> modeProbabilities, String failureMode) {
    ...
    for (Map.Entry<String, Double> entry : modeProbabilities.entrySet()) {
      ...
      if (normalizedMode.contains(normalizedFailureMode) || normalizedFailureMode.contains(normalizedMode)) {
  ...
  private void registerCompressorHypotheses() {
    ...
    putBuilders("Compressor", Symptom.HIGH_VIBRATION,
        Arrays.asList(
            Hypothesis.builder().name("Bearing degradation").description("Bearing wear causing increased vibration")
                .category(Hypothesis.Category.MECHANICAL).failureMode("Bearing failure").priorProbability(0.30)
                .addExpectedSignal("vibration|bearingvibration|velocity", ExpectedBehavior.INCREASE, 3.0,
                    "bearing defects normally lift vibration before trip")
            ...
            Hypothesis.builder().name("Rotor imbalance")
                ...
                .category(Hypothesis.Category.MECHANICAL).failureMode("Rotor imbalance").priorProbability(0.25)
            ...
            Hypothesis.builder().name("Liquid ingestion")
                ...
                .category(Hypothesis.Category.PROCESS).failureMode("Liquid ingestion").priorProbability(0.20)
                ...
                .addExpectedSignal("scrubberlevel|separatorlevel|level", ExpectedBehavior.HIGH_LIMIT, 3.0,
                    "upstream high level is the strongest liquid-ingestion precursor")
            ...
            Hypothesis.builder().name("Misalignment").description("Shaft misalignment between driver and compressor")
                .category(Hypothesis.Category.MECHANICAL).failureMode("Misalignment").priorProbability(0.15)
                ...
                .addExpectedSignal("bearingtemperature|couplingtemperature", ExpectedBehavior.INCREASE, 1.5,
                    "misalignment adds bearing and coupling heat")
    ...
    putBuilders("Compressor", Symptom.HIGH_TEMPERATURE,
        Arrays.asList(
            Hypothesis.builder().name("Seal degradation")
                ...
                .category(Hypothesis.Category.MECHANICAL).failureMode("Seal failure").priorProbability(0.30)
                .addExpectedSignal("sealflow|sealgas|differentialpressure", ExpectedBehavior.INCREASE, 2.5,
                    "seal leakage changes seal-gas flow or differential pressure")
                .addExpectedSignal("dischargetemperature|outlettemperature|temperature", ExpectedBehavior.INCREASE, 2.0,
            ...
            Hypothesis.builder().name("Fouled intercooler").description("Reduced cooling due to fouled intercooler")
            ...
            Hypothesis.builder().name("Operating above design pressure ratio")
                .description("Compression ratio exceeds design, causing excessive discharge temperature")
                .category(Hypothesis.Category.PROCESS).priorProbability(0.20)
                .addExpectedSignal("dischargepressure|outletpressure|pressureratio", ExpectedBehavior.INCREASE, 3.0,
                    "higher pressure ratio directly raises discharge temperature")
                .addExpectedSignal("dischargetemperature|outlettemperature|temperature", ExpectedBehavior.INCREASE, 2.0,
                    "compression temperature rises with pressure ratio")
                .addAction("Check suction/discharge pressure").addAction("Verify operating point on performance map"),
            Hypothesis.builder().name("Changed gas composition")
                ...
                .addExpectedSignal("molecularweight|heavies|composition", ExpectedBehavior.INCREASE, 2.0,
                    "heavier gas shifts compressor operating point")
                .addExpectedSignal("dischargetemperature|outlettemperature|temperature", ExpectedBehavior.INCREASE, 1.5,
    ...
    putBuilders("Compressor", Symptom.LOW_EFFICIENCY, Arrays.asList(
        Hypothesis.builder().name("Internal seal leakage")
            .description("Internal recirculation reduces effective throughput").category(Hypothesis.Category.MECHANICAL)
            .failureMode("Seal failure").priorProbability(0.30)
            ...
            .addExpectedSignal("sealflow|sealgas|differentialpressure", ExpectedBehavior.INCREASE, 2.0,
                "seal leakage should be visible in seal-system signals")
            .addAction("Monitor seal gas differential pressure"),
        Hypothesis.builder().name("Fouled impeller").description("Deposits on impeller reduce aerodynamic performance")
            .category(Hypothesis.Category.PROCESS).failureMode("Fouling").priorProbability(0.25)
            .addExpectedSignal("efficiency|polytropicefficiency", ExpectedBehavior.DECREASE, 3.0,
    ...
    putBuilders("Compressor", Symptom.SURGE_EVENT,
        Arrays.asList(
            Hypothesis.builder().name("Low suction flow")
                ...
                .addExpectedSignal("antisurgevalve|recyclevalve|valveopening", ExpectedBehavior.INCREASE, 2.0,
                    "anti-surge valve should respond by opening")
                ...
                .addAction("Check anti-surge valve position").addAction("Verify upstream equipment operation"),
            Hypothesis.builder().name("Anti-surge valve malfunction")
                .description("Anti-surge control valve fails to open or respond").category(Hypothesis.Category.CONTROL)
                .failureMode("Control valve failure").priorProbability(0.30).addAction("Test anti-surge valve stroke")
                .addExpectedSignal("antisurgevalve|recyclevalve|valveopening", ExpectedBehavior.LOW_LIMIT, 3.0,
                    "failed anti-surge valve does not open when required")
                .addExpectedSignal("suctionflow|flow", ExpectedBehavior.DECREASE, 1.5, "low flow remains uncorrected")
            ...
            Hypothesis.builder().name("Discharge pressure spike")
                ...
                .category(Hypothesis.Category.PROCESS).priorProbability(0.20)
                .addExpectedSignal("dischargepressure|outletpressure", ExpectedBehavior.INCREASE, 3.0,
                    "downstream restriction raises discharge pressure")
    ...
    putBuilders("Compressor", Symptom.TRIP,
        Arrays.asList(
            Hypothesis.builder().name("High vibration shutdown").description("Vibration exceeded trip setpoint")
                .category(Hypothesis.Category.MECHANICAL).failureMode("Bearing failure").priorProbability(0.25)
            ...
            Hypothesis.builder().name("High discharge temperature").description("Temperature exceeded trip setpoint")
                .category(Hypothesis.Category.PROCESS).priorProbability(0.20)
                .addExpectedSignal("dischargetemperature|outlettemperature|temperature", ExpectedBehavior.HIGH_LIMIT,
                    3.0, "temperature trip should exceed high-high threshold")
                .addAction("Check temperature trend"),
            Hypothesis.builder().name("Surge trip").description("Multiple surge events triggered protective shutdown")
                .category(Hypothesis.Category.PROCESS).priorProbability(0.20)
                .addExpectedSignal("surgecounter|pressurepulsation|vibration", ExpectedBehavior.STEP_CHANGE, 3.0,
                    "surge trip should have oscillatory precursor signals")
                .addAction("Review anti-surge log"),
            Hypothesis.builder().name("Lube oil system failure").description("Loss of lubricating oil pressure or flow")
                .category(Hypothesis.Category.MECHANICAL).failureMode("Lube oil failure").priorProbability(0.15)
  ...
  private void registerPumpHypotheses() {
    putBuilders("Pump", Symptom.HIGH_VIBRATION, Arrays.asList(
        Hypothesis.builder().name("Cavitation").description("Insufficient NPSH causing vapor formation in impeller eye")
        ...
        Hypothesis.builder().name("Bearing wear").description("Worn bearings causing increased vibration")
            .category(Hypothesis.Category.MECHANICAL).failureMode("Bearing failure").priorProbability(0.25)
        ...
        Hypothesis.builder().name("Impeller imbalance").description("Damaged or eroded impeller")
            .category(Hypothesis.Category.MECHANICAL).failureMode("Rotor imbalance").priorProbability(0.20)
    ...
    putBuilders("Pump", Symptom.LOW_EFFICIENCY,
        Arrays.asList(
            Hypothesis.builder().name("Impeller wear").description("Wear ring clearance increased, reducing efficiency")
    ...
    putBuilders("Pump", Symptom.TRIP, Arrays.asList(
        Hypothesis.builder().name("Low suction pressure").description("NPSH too low, protective trip activated")
        ...
        Hypothesis.builder().name("Motor overload").description("Current exceeded motor rating")
            .category(Hypothesis.Category.MECHANICAL).priorProbability(0.25).addExpectedSignal("current|power",
                ExpectedBehavior.HIGH_LIMIT, 3.0, "motor overload must show high current or power")
  ...
  private void registerSeparatorHypotheses() {
    putBuilders("Separator", Symptom.LIQUID_CARRYOVER, Arrays.asList(
        Hypothesis.builder().name("High liquid level").description("Liquid level above normal, approaching demister")
        ...
        Hypothesis.builder().name("Demister fouling").description("Fouled demister pad reducing separation efficiency")
            .category(Hypothesis.Category.PROCESS).failureMode("Fouling").priorProbability(0.25)
        ...
        Hypothesis.builder().name("Feed rate exceedance")
            .description("Actual gas velocity exceeds Souders-Brown design limit").category(Hypothesis.Category.PROCESS)
        ...
        Hypothesis.builder().name("Inlet device damage")
            ...
            .category(Hypothesis.Category.MECHANICAL).failureMode("Internal damage").priorProbability(0.15)
    ...
    putBuilders("Separator", Symptom.PRESSURE_DEVIATION, Arrays.asList(
        Hypothesis.builder().name("Outlet restriction").description("Downstream valve or equipment partially blocked")
            .category(Hypothesis.Category.PROCESS).priorProbability(0.35)
            .addExpectedSignal("pressure|vesselpressure", ExpectedBehavior.INCREASE, 2.5,
                "outlet restriction raises vessel pressure")
            .addExpectedSignal("flow|outletflow", ExpectedBehavior.DECREASE, 2.0, "restriction reduces outlet flow")
            .addAction("Check downstream valve positions"),
        Hypothesis.builder().name("Pressure control valve malfunction").description("PCV not maintaining setpoint")
            .category(Hypothesis.Category.CONTROL).failureMode("Control valve failure").priorProbability(0.30)
    ...
    putBuilders("Separator", Symptom.FOULING,
        Arrays.asList(
            Hypothesis.builder().name("Wax deposition").description("Wax precipitation in vessel or internals")
  ...
  private void registerHeatExchangerHypotheses() {
    putBuilders("HeatExchanger", Symptom.HIGH_TEMPERATURE, Arrays.asList(
        Hypothesis.builder().name("Fouled tubes")
            ...
            .addExpectedSignal("approachtemperature|outlettemperature|temperature", ExpectedBehavior.INCREASE, 3.0,
                "fouling raises approach or outlet temperature")
            .addExpectedSignal("ua|heattransfercoefficient", ExpectedBehavior.DECREASE, 3.0, "fouling reduces UA")
        ...
        Hypothesis.builder().name("Loss of cooling medium").description("Cooling water or air flow reduced or lost")
            .category(Hypothesis.Category.EXTERNAL).failureMode("Utility failure").priorProbability(0.30)
            .addExpectedSignal("coolingwaterflow|cwflow|utilityflow", ExpectedBehavior.LOW_LIMIT, 3.0,
                "utility loss should show reduced cooling-medium flow")
        ...
        Hypothesis.builder().name("Tube leak").description("Internal tube leak causing bypass of heat transfer surface")
            .category(Hypothesis.Category.MECHANICAL).failureMode("Tube leak").priorProbability(0.15)
            ...
            .addExpectedSignal("contamination|conductivity|hydrocarbon", ExpectedBehavior.INCREASE, 2.5,
                "cross-contamination confirms tube leakage")
            .addAction("Check for cross-contamination between fluids")));

    putBuilders("HeatExchanger", Symptom.LOW_EFFICIENCY, Arrays.asList(
        Hypothesis.builder().name("Fouled heat transfer surfaces").description("Declining UA due to fouling buildup")
        ...
        Hypothesis.builder().name("Baffle damage").description("Damaged baffles causing shell-side bypass")
            .category(Hypothesis.Category.MECHANICAL).failureMode("Internal damage").priorProbability(0.20)
    ...
    putBuilders("HeatExchanger", Symptom.PRESSURE_DEVIATION,
        Arrays.asList(
            Hypothesis.builder().name("Tube-side plugging")
                .description("Partial blockage of tubes increasing pressure drop").category(Hypothesis.Category.PROCESS)
                .failureMode("Fouling").priorProbability(0.35)
            ...
            Hypothesis.builder().name("Tube rupture")
                ...
                .category(Hypothesis.Category.MECHANICAL).failureMode("Tube leak").priorProbability(0.25)
                .addExpectedSignal("pressure|differentialpressure", ExpectedBehavior.STEP_CHANGE, 3.0,
                    "tube rupture causes sudden pressure disturbance")
  ...
  private void registerValveHypotheses() {
    putBuilders("Valve", Symptom.FLOW_DEVIATION,
        Arrays.asList(
            Hypothesis.builder().name("Valve trim erosion").description("Eroded trim changing Cv characteristic")
                .category(Hypothesis.Category.MECHANICAL).failureMode("Erosion").priorProbability(0.30)
                .addExpectedSignal("flow|cv", ExpectedBehavior.INCREASE, 2.5, "trim erosion increases effective Cv")
            ...
            Hypothesis.builder().name("Positioner calibration drift").description("Valve positioner out of calibration")
                .category(Hypothesis.Category.CONTROL).failureMode("Instrument failure").priorProbability(0.30)
                .addExpectedSignal("position|valveopening", ExpectedBehavior.ANY_CHANGE, 3.0,
                    "positioner drift shows command-position mismatch")
                .addExpectedSignal("flow", ExpectedBehavior.ANY_CHANGE, 2.0, "flow deviates from requested opening")
                .addAction("Recalibrate positioner"),
            Hypothesis.builder().name("Actuator sticking").description("Actuator friction preventing smooth movement")
                .category(Hypothesis.Category.MECHANICAL).failureMode("Actuator failure").priorProbability(0.20)
    ...
    putBuilders("Valve", Symptom.ABNORMAL_NOISE, Arrays.asList(
        Hypothesis.builder().name("Cavitation").description("Liquid flashing across valve causing cavitation noise")
        ...
        Hypothesis.builder().name("Flashing").description("Two-phase flow through valve generating noise")
            .category(Hypothesis.Category.PROCESS).priorProbability(0.30)
            .addExpectedSignal("vaporfraction|twophase|temperature", ExpectedBehavior.ANY_CHANGE, 2.0,
                "flashing produces two-phase flow after the valve")
            .addExpectedSignal("noise", ExpectedBehavior.INCREASE, 1.5, "flashing generates elevated acoustic noise")
  ...
  private void registerGenericHypotheses() {
    for (Symptom symptom : Symptom.values()) {
      ...
      builders.add(Hypothesis.builder().name("Upstream process change")
          ...
          .category(Hypothesis.Category.PROCESS).priorProbability(0.20)
          .addExpectedSignal("upstream|feed|flow|pressure|temperature", ExpectedBehavior.ANY_CHANGE, 2.0,
              "upstream changes should be visible in feed conditions")
          .addAction("Review upstream equipment operation"));
      builders.add(Hypothesis.builder().name("Instrument error").description("Faulty sensor giving false reading")
          .category(Hypothesis.Category.CONTROL).failureMode("Instrument failure").priorProbability(0.15)
      ...
      builders.add(
          Hypothesis.builder().name("External utility disturbance").description("Loss or degradation of utility supply")
