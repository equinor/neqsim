 ...
 *
 * <p>
 * This class stores compressor curves in a normalized (reduced) form that can be scaled to match any compressor's
 * operating conditions using fan laws. The curves are normalized relative to a reference speed, so:
 * </p>
 * <ul>
...
public class CompressorCurveTemplate implements Serializable {
   ...
   * </ul>
   */
  public static final CompressorCurveTemplate CENTRIFUGAL_STANDARD = createCentrifugalStandardTemplate();
   ...
   * </p>
   */
  public static final CompressorCurveTemplate CENTRIFUGAL_HIGH_FLOW = createCentrifugalHighFlowTemplate();
   ...
   * </p>
   */
  public static final CompressorCurveTemplate CENTRIFUGAL_HIGH_HEAD = createCentrifugalHighHeadTemplate();
   ...
   *
   * <p>
   * Designed for natural gas transmission with high capacity, moderate pressure ratio, and wide operating range.
   * Typical for long-distance gas pipelines.
   * </p>
   * <ul>
   ...
   *
   * <p>
   * Designed for offshore gas export with high discharge pressure and stable operation. Optimized for efficiency at
   * design point.
   * </p>
   * <ul>
   ...
   *
   * <p>
   * Designed for gas injection/reinjection with very high discharge pressure. Used for enhanced oil recovery and
   ...
   *
   * <p>
   * Designed for artificial lift applications with variable operating conditions. Must handle fluctuating inlet
   * conditions and some liquid carryover.
   * </p>
   * <ul>
   ...
   *
   * <p>
   * Designed for process cooling and LNG applications with refrigerants. Optimized for wide operating range and varying
   * loads.
   * </p>
   * <ul>
   ...
   *
   * <p>
   * Designed for moderate pressure boosting in process plants. General-purpose design with good balance of capacity and
   * head.
   * </p>
   * <ul>
   ...
   *
   * <p>
   * Simple single impeller design for low pressure ratio applications. Common for utilities, blowers, and simple
   * compression duty.
   * </p>
   * <ul>
   ...
   *
   * <p>
   * Multiple stages in series within a barrel casing for high pressure applications. Standard design for oil and gas
   * industry.
   * </p>
   * <ul>
   ...
   *
   * <p>
   * Multiple pinions with impellers driven by a bull gear. Flexible staging for air separation and process
   * applications.
   * </p>
   * <ul>
   ...
   *
   * <p>
   * Cantilever design with impeller mounted on extended shaft. Simple maintenance access, common for smaller
   * applications.
   * </p>
   * <ul>
   ...
   *
   * <p>
   * The scaling uses fan laws to generate curves that pass through the specified design point (designFlow, designHead)
  ...
  public CompressorChartInterface scaleToDesignPoint(double designSpeed, double designFlow, double designHead,
      int numberOfSpeeds) {
    return scaleToDesignPoint(designSpeed, designFlow, designHead, numberOfSpeeds, "interpolate and extrapolate");
  }
   ...
   */
  public CompressorChartInterface scaleToDesignPoint(double designSpeed, double designFlow, double designHead,
      int numberOfSpeeds, String chartType) {
    ...
    for (int i = 0; i < numberOfSpeeds; i++) {
      ...
      for (int j = 0; j < numPoints; j++) {
        ...
        scaledHeads[i][j] = reducedHeads[templateIndex][j] * headScaleFactor * speedRatio * speedRatio;
      }
    }
   ...
   * </p>
   *
   * @param chartType The chart type: "simple", "fan law", "interpolate", or "interpolate and extrapolate"
  ...
  private static CompressorCurveTemplate createCentrifugalStandardTemplate() {
    ...
    double[][] flows = {{4512.7, 5120.8, 5760.9, 6401, 6868.27}, {4862.47, 5486.57, 6172.39, 6858.21, 7550.89},
        {5237.84, 5852.34, 6583.88, 7315.43, 8046.97, 8266.43}, {5642.94, 6218.11, 6995.38, 7772.64, 8549.9, 9000.72},
        {6221.77, 6583.88, 7406.87, 8229.85, 9052.84, 9768.84},
        {6888.85, 6949.65, 7818.36, 8687.07, 9555.77, 10424.5, 10546.1},
        {7109.83, 7948.87, 8832.08, 9715.29, 10598.5, 10801.6}, {7598.9, 8229.85, 9144.28, 10058.7, 10973.1, 11338.9},
        {8334.1, 8641.35, 9601.5, 10561.6, 11521.8, 11963.5}};

    double[][] heads = {{61.885, 59.639, 56.433, 52.481, 49.132}, {71.416, 69.079, 65.589, 61.216, 55.858},
        {81.621, 79.311, 75.545, 70.727, 64.867, 62.879}, {92.493, 90.312, 86.3, 81.079, 74.658, 70.216},
        {103.512, 102.073, 97.83, 92.254, 85.292, 77.638}, {114.891, 114.632, 110.169, 104.221, 96.727, 87.002, 85.262},
        {118.595, 114.252, 108.203, 100.55, 90.532, 87.54}, {126.747, 123.376, 117.113, 109.056, 98.369, 92.632},
    ...
    return new CompressorCurveTemplate("CENTRIFUGAL_STANDARD", refSpeed, speeds, flows, heads, efficiencies);
  }
  ...
  private static CompressorCurveTemplate createCentrifugalHighFlowTemplate() {
    ...
    double[][] heads = {{50, 48, 44, 38, 32}, {65, 63, 57, 49, 41}, {82, 79, 72, 62, 52}, {100, 96, 88, 76, 64}};

    double[][] efficiencies = repeatEfficiency(4, new double[] {77, 78, 77.5, 75, 72});

    return new CompressorCurveTemplate("CENTRIFUGAL_HIGH_FLOW", refSpeed, speeds, flows, heads, efficiencies);
  }
  ...
  private static CompressorCurveTemplate createCentrifugalHighHeadTemplate() {
    ...
    double[][] flows = {{3500, 4200, 5000, 5500, 5800}, {4000, 4800, 5700, 6300, 6600}, {4500, 5400, 6400, 7100, 7500},
        {5000, 6000, 7100, 7900, 8300}};
    ...
    return new CompressorCurveTemplate("CENTRIFUGAL_HIGH_HEAD", refSpeed, speeds, flows, heads, efficiencies);
  }
  ...
  private static CompressorCurveTemplate createPipelineTemplate() {
    ...
    double[][] flows = {{22000, 28000, 35000, 42000, 48000, 52000}, {24750, 31500, 39375, 47250, 54000, 58500},
    ...
    double[][] heads = {{42, 41, 39, 36, 32, 28}, {53, 52, 49, 46, 41, 35}, {65, 64, 61, 56, 50, 44},
        {79, 77, 73, 68, 60, 53}};
  ...
  private static CompressorCurveTemplate createExportTemplate() {
    ...
    double[][] flows = {{4500, 5500, 6500, 7500, 8200}, {5200, 6350, 7500, 8650, 9500}, {5700, 6970, 8230, 9500, 10400},
        {6250, 7625, 9000, 10400, 11400}};
  ...
  private static CompressorCurveTemplate createInjectionTemplate() {
    ...
    double[][] flows = {{2500, 3000, 3600, 4100, 4400}, {2800, 3375, 4050, 4612, 4950}, {3125, 3750, 4500, 5125, 5500},
        {3438, 4125, 4950, 5638, 6050}};

    // Very high head (multiple stages with intercooling)
    double[][] heads = {{130, 125, 115, 100, 85}, {165, 158, 146, 127, 108}, {203, 195, 180, 156, 133},
        {245, 236, 218, 189, 161}};
  ...
  private static CompressorCurveTemplate createGasLiftTemplate() {
    ...
    double[][] heads = {{48, 47, 45, 41, 35, 28}, {65, 64, 61, 56, 48, 38}, {80, 78, 75, 68, 58, 47},
        {96, 94, 90, 82, 70, 56}};
  ...
  private static CompressorCurveTemplate createRefrigerationTemplate() {
    ...
    double[][] flows = {{12000, 15000, 19000, 23000, 26000, 28000}, {13350, 16650, 21100, 25550, 28900, 31100},
        {14650, 18300, 23200, 28050, 31750, 34200}, {16000, 20000, 25300, 30600, 34600, 37300}};

    // Lower head per stage (lighter gases)
    double[][] heads = {{28, 27, 25, 22, 19, 16}, {35, 33, 31, 27, 23, 20}, {42, 40, 37, 33, 28, 24},
        {50, 48, 45, 40, 34, 29}};

    // Good efficiency across operating range
    double[][] efficiencies = repeatEfficiency(4, new double[] {75, 77, 78, 77, 75, 72});

    return new CompressorCurveTemplate("REFRIGERATION", refSpeed, speeds, flows, heads, efficiencies);
  }
  ...
  private static CompressorCurveTemplate createBoosterTemplate() {
    ...
    // Medium capacity
    double[][] flows = {{4000, 5000, 6200, 7400, 8300}, {4570, 5710, 7080, 8460, 9490}, {5140, 6430, 7970, 9510, 10670},
        {5710, 7140, 8850, 10570, 11860}};

    // Moderate head
    double[][] heads = {{55, 53, 49, 43, 36}, {72, 69, 64, 56, 47}, {91, 87, 81, 71, 60}, {112, 108, 100, 88, 74}};
  ...
  private static CompressorCurveTemplate createSingleStageTemplate() {
    ...
    double[][] heads = {{25, 24, 22, 19, 15, 11}, {33, 32, 29, 25, 20, 15}, {42, 40, 36, 31, 25, 19},
        {52, 49, 45, 39, 31, 23}};

    // Moderate efficiency
    double[][] efficiencies = repeatEfficiency(4, new double[] {72, 74, 75, 73, 70, 65});

    return new CompressorCurveTemplate("SINGLE_STAGE", refSpeed, speeds, flows, heads, efficiencies);
  }
  ...
  private static CompressorCurveTemplate createMultistageInlineTemplate() {
    ...
    double[][] flows = {{4200, 5200, 6400, 7500, 8300}, {4760, 5890, 7250, 8500, 9410}, {5320, 6590, 8110, 9500, 10520},
        {5880, 7290, 8960, 10500, 11630}};
    ...
    return new CompressorCurveTemplate("MULTISTAGE_INLINE", refSpeed, speeds, flows, heads, efficiencies);
  }
  ...
  private static CompressorCurveTemplate createIntegrallyGearedTemplate() {
    ...
    double[][] heads = {{35, 34, 31, 27, 22}, {46, 44, 40, 35, 29}, {58, 56, 51, 44, 37}, {72, 69, 63, 55, 46}};

    // High efficiency through optimized design
    double[][] efficiencies = repeatEfficiency(4, new double[] {79, 81, 82, 80, 77});

    return new CompressorCurveTemplate("INTEGRALLY_GEARED", refSpeed, speeds, flows, heads, efficiencies);
  }
  ...
  private static CompressorCurveTemplate createOverhungTemplate() {
    ...
    double[][] flows = {{1800, 2300, 2900, 3500, 4000}, {2060, 2630, 3310, 4000, 4570}, {2310, 2960, 3730, 4500, 5140},
        {2570, 3290, 4140, 5000, 5710}};

    // Low-medium head
    double[][] heads = {{32, 31, 28, 24, 19}, {42, 40, 37, 31, 25}, {53, 51, 46, 40, 32}, {65, 63, 57, 49, 39}};
   ...
   *
   * <p>
   * Supports various naming conventions - spaces, hyphens, and underscores are interchangeable. The search is
   * case-insensitive.
   * </p>
   *
  ...
  public static CompressorCurveTemplate getTemplate(String templateName) {
    ...
    if (upperName.contains("PIPELINE")) {
      ...
      return INJECTION;
    } else if (upperName.contains("GAS_LIFT") || upperName.contains("GASLIFT") || upperName.contains("LIFT")) {
      return GAS_LIFT;
    } else if (upperName.contains("REFRIGERAT") || upperName.contains("LNG") || upperName.contains("COOLING")) {
    ...
    // Compressor type templates
    if (upperName.contains("SINGLE_STAGE") || upperName.contains("SINGLESTAGE") || upperName.equals("SINGLE")) {
      return SINGLE_STAGE;
    } else if (upperName.contains("MULTISTAGE") || upperName.contains("MULTI_STAGE") || upperName.contains("BARREL")
        || upperName.contains("INLINE")) {
      return MULTISTAGE_INLINE;
    } else if (upperName.contains("INTEGRALLY") || upperName.contains("GEARED") || upperName.contains("IGC")) {
  ...
  public static String[] getTemplatesByCategory(String category) {
    ...
    if (cat.equals("basic") || cat.equals("centrifugal")) {
      return new String[] {"CENTRIFUGAL_STANDARD", "CENTRIFUGAL_HIGH_FLOW", "CENTRIFUGAL_HIGH_HEAD"};
    } else if (cat.equals("application") || cat.equals("app")) {
      return new String[] {"PIPELINE", "EXPORT", "INJECTION", "GAS_LIFT", "REFRIGERATION", "BOOSTER"};
   ...
   *
   * <p>
   * This method returns a chart with the exact curve data stored in the template, without any scaling. Useful for
   * recreating the original reference curves.
   * </p>
   *
   ...
   *
   * <p>
   * This scales all curves proportionally so that the reference speed maps to the target speed. Uses fan laws: flow
   * scales with speed, head scales with speed².
   * </p>
   *
  ...
  public CompressorChartInterface scaleToSpeed(double targetReferenceSpeed, String chartType) {
    ...
    for (int i = 0; i < numSpeeds; i++) {
      ...
      for (int j = 0; j < numPoints; j++) {
        ...
        scaledHeads[i][j] = reducedHeads[i][j] * speedRatio * speedRatio * speedScaleFactor * speedScaleFactor;
      }
    }
