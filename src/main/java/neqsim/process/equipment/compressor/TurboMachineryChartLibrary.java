 ...
 *
 * <p>
 * Commercial process simulators ship validated OEM curve libraries; NeqSim has historically relied
 * on user-digitised maps. This class provides a small, versioned, in-repository library of named
 * reference maps so that a user can obtain a physically reasonable, dimensionally-correct map by
 * name without digitising one. The shipped maps are generic, vendor-neutral reference
 * characteristics (representative centrifugal-compressor and radial-inflow-expander shapes), not
 * proprietary OEM data. They are intended as starting points and benchmarks; for fiscal or
 * guarantee work the user must still supply the certified OEM curve.
 * </p>
 *
 * <p>
 * Maps are produced as fully-initialised chart objects: compressor maps as
 * {@link CompressorChartKhader2015} (with a generated surge curve) and expander maps as
 * {@link ExpanderChartKhader}. The Khader 2015 normalisation makes every map composition-aware, so
 * a single reference map can be reused across fluids.
 * </p>
 *
 * @author NeqSim
 * @version 1.0
...
public class TurboMachineryChartLibrary implements Serializable {
   ...
   * @param referenceFluid the fluid used to normalise the map (may be {@code null} to skip
   ...
   */
  private CompressorChartKhader2015 buildGenericCentrifugal(SystemInterface fluid,
      double impellerOuterDiameter) {
    ...
    double[][] head = new double[][] {{85000.0, 80000.0, 73000.0, 64000.0, 52000.0},
        {115000.0, 108000.0, 99000.0, 87000.0, 71000.0},
        {150000.0, 141000.0, 129000.0, 113000.0, 92000.0}};
    double[][] polyEff = new double[][] {{0.72, 0.79, 0.82, 0.80, 0.74},
        {0.73, 0.80, 0.83, 0.81, 0.75}, {0.72, 0.79, 0.82, 0.80, 0.74}};
  ...
  private ExpanderChartKhader buildGenericCryoExpander(SystemInterface referenceFluid) {
    ...
    double[][] uc = new double[][] {{0.45, 0.55, 0.65, 0.72, 0.80, 0.90},
        {0.45, 0.55, 0.65, 0.72, 0.80, 0.90}, {0.45, 0.55, 0.65, 0.72, 0.80, 0.90}};
    double[][] eta = new double[][] {{0.64, 0.73, 0.80, 0.82, 0.79, 0.70},
        {0.67, 0.77, 0.84, 0.86, 0.83, 0.74}, {0.69, 0.79, 0.85, 0.87, 0.84, 0.75}};
