...
public class NaphtaliSandholmSolver {
  ...
  public boolean solve(UUID id) {
    ...
    try {
      ...
      double norm = LinearAlgebraOps.vectorNorm(residual);
      ...
      if (mbErrorBP < 0.005 && energyErrorBP >= 0.01) {
        ...
        norm = LinearAlgebraOps.vectorNorm(residual);
      ...
      for (int iter = 1; iter <= maxIterations; iter++) {
        ...
        residual = computeResidual();
        double newNorm = LinearAlgebraOps.vectorNorm(residual);
   ...
   * {@code feed_{j,i} + liq_{j+1,i} + vap_{j-1,i} -
   * liq_{j,i} - vap_{j,i}} (the same equation that {@link #computeResidual()} packs into the F
   * vector), takes the absolute value, and divides by the total feed flow. Returning the worst
   * value across all (j, i) pairs exposes leakage on individual species, which the scalar
   * {@link #computeMassBalanceError()} (overall column closure) and the L2 norm {@code ||F||} can
   * both mask.
   * </p>
   *
  ...
  private double lineSearch(double[] dx, double currentNorm) {
    ...
    for (int bt = 0; bt < maxBacktrack; bt++) {
      ...
      double trialNorm = LinearAlgebraOps.vectorNorm(Ftrial);
