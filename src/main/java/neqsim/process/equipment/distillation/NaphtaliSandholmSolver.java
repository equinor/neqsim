 ...
 *
 * <p>
 * Solves the full MESH (Material balance, Equilibrium, Summation, Heat balance)
 * system of equations
 * simultaneously using Newton-Raphson iteration with block-tridiagonal Jacobian
 * structure.
 * </p>
 *
 * <p>
 ...
 * (tray-by-tray)
 * methods diverge. The variable set per tray j is: liquid component flows
 * l_{i,j} for each
 * component i, temperature T_j, and vapor flow rate V_j. This gives (C+2)
 * variables per tray for
 * C components, and N*(C+2) total variables for N trays.
 * </p>
 *
 * <p>
 * Reference: Naphtali, L.M. and Sandholm, D.P. (1971). "Multicomponent
 * Separation Calculations by
...
public class NaphtaliSandholmSolver {
  ...
  /**
   * Per-stage Murphree efficiency cached at solver init time. trayEta[j] is
   * forced to 1.0 for the reboiler (j=0) and the condenser (j=N-1 when present)
   * so those stages remain rigorous equilibrium. Other stages use the value
   * registered on the parent column via setMurphreeEfficiency(stage, value),
   * defaulting to the global value when no per-stage override is set.
   *
   * <p>
   * The efficiency is embedded into the simultaneous MESH solution through
   * the Edmister proxy K_eff[j][i] = K[j][i]^trayEta[j]. At trayEta = 1.0 the
   * stage is rigorous equilibrium; at trayEta -> 0 the K-values approach 1.0
   * and the tray becomes passive (vapor passes through ~ unchanged), matching
   * the behaviour of a heavily de-rated tray in UniSim / HYSYS.
   */
  ...
  /**
   * Feed liquid component flows F_L_{i,j} in mol/s. Indexed [j][i]. Zero for
   * non-feed trays.
   */
  private double[][] feedLiq;

  /**
   * Feed vapor component flows F_V_{i,j} in mol/s. Indexed [j][i]. Zero for
   * non-feed trays.
   */
  private double[][] feedVap;

  /**
   * Feed liquid enthalpy h_FL_j in J/mol, indexed [0..N-1]. Zero for non-feed
   * trays.
   */
  private double[] feedHL;

  /**
   * Feed vapor enthalpy h_FV_j in J/mol, indexed [0..N-1]. Zero for non-feed
   * trays.
   */
  ...
  /**
   * Fixed temperature specification for each tray in K. NaN means the temperature
   * is a free
  ...
  /**
   * Per-tray seed temperature [K] used by the initializer when the tray is NOT
   * fixed-T (i.e. T[j] is a Newton variable). NaN means "no seed, use Wilson
   * estimate". This lets a user pass a reboiler T value as a hint when they
   * specify boilup ratio instead of T as the actual MESH constraint.
   */
  ...
  /**
   * Enable Boston-Sullivan inside-out refinement of the seed when at least one
   * tray temperature is pinned (e.g. T-spec reboiler). The smooth Wilson BP
   * seed misses sharp T-pinches; B-S inner loop (per-component tridiag MB +
   * bubble-T) captures them and gives SR / Newton a sharper starting point.
   * Auto-detected: only fires when {@code fixedTemperature[j]} is set on at
   * least one tray. Default true.
   */
  ...
  /**
   * When true, the column uses overall mass balance closure L[0] = totalFeed -
   * V[N-1] instead of V[0] = boilupRatio*L[0]. Enabled automatically when the
   * user pins reboiler temperature but does not explicitly set a boilup ratio
   * (i.e. boilupRatio is still the NeqSim default of 0.1). With pinned T, V[0]
   * must be free to be determined by physics, not constrained to an arbitrary
   * default ratio.
   */
  ...
  /**
   * Original feed thermo systems cloned before column init() corrupts them.
   * Map of tray index to list of SystemInterface clones. Null if not provided.
   */
  private Map<Integer, List<SystemInterface>> originalFeedSystems;

  /**
   * Original feed molar flow rates in mol/hr, saved before column init().
   * Map of tray index to list of flow rates. Null if not provided.
   */
  ...
  /**
   * Construct a solver with pre-saved original feed thermo systems and flow
   * rates.
   *
   * <p>
   * The column's init() method may modify feed streams in-place (shared object
   * references with trays). This constructor accepts deep clones of the original
   * feed thermo systems and their molar flow rates, taken before init() runs,
   * so the solver always uses the correct feed T, P, composition, and total
   * moles.
   * </p>
   *
   * @param column                the distillation column to solve
   * @param originalFeedSystems   map of tray index to list of original feed
   *                              SystemInterface clones
   * @param originalFeedFlowRates map of tray index to list of feed molar flow
   *                              rates in mol/hr
   */
  public NaphtaliSandholmSolver(DistillationColumn column,
  ...
  public boolean solve(UUID id) {
    ...
    try {
      ...
      logger.info("NS after BP+EOS: massBalErr={}% energyErr={}%",
          String.format("%.4f", mbErrorBP * 100),
      ...
      if (mbErrorBP < 0.005 && energyErrorBP >= 0.01) {
        ...
        if (useOverallMBClosure && mbAfterSR < 0.05 && energyAfterSR < 0.05) {
          System.out.println("[NS] useOverallMBClosure path: accepting SR result "
              + "(mb=" + String.format("%.4f%%", mbAfterSR * 100)
              + ", energy=" + String.format("%.2f%%", energyAfterSR * 100)
      ...
      for (int iter = 1; iter <= maxIterations; iter++) {
        ...
        if (trScale < 1.0 && (iter <= 3 || iter % 10 == 0)) {
          System.out.println("[Newton] iter " + iter + " trust-region scale="
              + String.format("%.3e", trScale));
        }
        ...
        if (Double.isNaN(newNorm) || Double.isInfinite(newNorm)) {
          ...
          norm = vectorNorm(residual);
          failedSteps++;
          logger.warn("NS: NaN/Inf — reverting to best (||F||={})",
              String.format("%.6e", norm));
          if (failedSteps > 5) {
            applyResultsToColumn(id, iter, bestNorm, startTime);
            return false;
        ...
        if (iter % 10 == 0) {
          if (bestNorm > 0.95 * prevBestNorm) {
            ...
            if (stagnationCount >= 2) {
              ...
              double eErr = computeMaxRelativeEnergyError();
              logger.info(
                  "NS: stagnation detected (bestNorm={}, massBalErr={}%, energyErr={}%)",
                  String.format("%.6e", bestNorm), String.format("%.4f", mbErr * 100),
                  String.format("%.2f", eErr * 100));
              if (mbErr < 0.005) {
  ...
  private void initialize() {
    ...
    for (Map.Entry<Integer, List<StreamInterface>> entry : feedMap.entrySet()) {
      ...
      List<SystemInterface> origSystems = (originalFeedSystems != null)
          ? originalFeedSystems.get(trayIdx)
          : null;
      ...
      for (int fi = 0; fi < feeds.size(); fi++) {
        ...
        System.out.println("[NS-FEED] tray " + trayIdx
            + ": feedMoles=" + String.format("%.2f", feedMoles)
            + " beta=" + String.format("%.4f", beta)
            + " nPhases=" + feedSys.getNumberOfPhases()
            + " phase0Type=" + feedSys.getPhase(0).getType()
            + " singlePhaseIsVapor=" + singlePhaseIsVapor
            + " T=" + String.format("%.1fC", feedSys.getTemperature() - 273.15)
            + " P=" + String.format("%.2fbar", feedSys.getPressure())
            + " totalH=" + String.format("%.0f", feedSys.getEnthalpy()));
    ...
    if (hasReboiler) {
      ...
      if (Math.abs(boilupRatio - 0.1) > 1e-9 && !Double.isNaN(fixedTemperature[0])) {
        logger.info(
            "NS: B={} (non-default) — treating reboiler T={} K as seed, not constraint",
    ...
    useOverallMBClosure = hasReboiler
        && !Double.isNaN(fixedTemperature[0])
        && Math.abs(boilupRatio - 0.1) < 1e-9;
    if (useOverallMBClosure) {
      logger.info(
          "NS: T-spec at reboiler with default boilup — using overall MB closure "
    ...
    if (logger.isDebugEnabled()) {
      for (int j = 0; j < N; j++) {
        logger.debug("  Tray {}: T={}K P={}bara L={} V={}{}", j,
            String.format("%.2f", T[j]), String.format("%.2f", P[j] / 1e5),
            String.format("%.4f", L[j]), String.format("%.4f", V[j]),
  ...
  /**
   * Initialize tray temperatures, flows, and compositions using Wilson K-values
   * and constant molar
   * overflow estimates.
   */
  private void initializeTrayState() {
    ...
    System.out.println("[NS-INIT] bubbleT@bot=" + String.format("%.2f", bubbleTatBot - 273.15)
        + "C dewT@top=" + String.format("%.2f", dewTatTop - 273.15)
        + "C feedT=" + String.format("%.2f", feedTemp - 273.15)
        + "C => botTemp=" + String.format("%.2f", botTemp - 273.15)
        + "C topTemp=" + String.format("%.2f", topTemp - 273.15) + "C");
  ...
  /**
   * Evaluate thermodynamic properties (K-values, enthalpies) at current state for
   * all trays.
   *
   * <p>
   * V[j] is NOT recomputed here — it is a free variable of the solver. The vapor
   * component flows
   * v_{i,j} = K_{i,j} * x_{i,j} * V_j are computed from the current V[j] and
   * K-values.
   * </p>
   */
   ...
   * boilup specification). Bypasses the BP-Wilson V-cascade — which is unstable
   ...
   *
   * <p>
   * Mass conservation is satisfied at the initial guess to within numerical
   * tolerance; Newton then refines energy balance and equilibrium.
   * </p>
   */
  private void seedSolutionForDirectNewton() {
    ...
    logger.info(
        ...
        String.format("%.4f", totalFeed),
        String.format("%.4f", overheadTotal), String.format("%.4f", bottomsTotal),
        String.format("%.4f", boilup),
        String.format("%.4f", V[0]), String.format("%.4f", V[N - 1]),
        String.format("%.4f", L[0]), String.format("%.4f", L[N - 1]),
        String.format("%.4e", maxCompMBerr));
  }
   ...
   *
   * <p>
   * Decomposes the MESH equations into sequential subproblems:
   * 1. Material balance: tridiagonal Thomas algorithm for each component
   * 2. Summation: bubble point calculation for temperature on each tray
   * 3. Energy: energy balance for vapor flow on each tray
   * </p>
   *
   * @return true if converged
   */
  private boolean solveBubblePointMethod() {
      ...
      if (iter > 10 || maxDeltaT < 5.0) {
        for (int j = 0; j < N; j++) {
          ...
          for (int i = 0; i < C; i++) {
            double xi = liq[j][i] / Math.max(L[j], 1e-20);
            double Ki = (Pc[i] / Pbar)
                * Math.exp(5.37 * (1.0 + omega[i]) * (1.0 - Tc[i] / T[j]));
            sumKx += Ki * xi;
          }
    ...
    for (int eosIter = 0; eosIter < maxEosIter; eosIter++) {
      ...
      if (eosIter % 5 == 0 || eosIter < 5) {
        ...
        logger.info(
            ...
            eosIter,
            String.format("%.2f", maxDeltaT), String.format("%.2f", maxDeltaV),
            String.format("%.4f", mbErr * 100), String.format("%.2f", eErr * 100),
            String.format("%.6f", maxSumKxErr),
            String.format("%.1f", T[0] - 273.15),
            String.format("%.1f", T[N - 1] - 273.15));
      }
      ...
      if (maxDeltaT < 0.1 && maxDeltaV < 0.5 && mbErr < 0.005 && maxSumKxErr < 0.01) {
        logger.info("BP-EOS converged at iter {}: massBalErr={}% sumKxErr={}",
            eosIter, String.format("%.4f", mbErr * 100),
            String.format("%.6f", maxSumKxErr));
        break;
      }
    ...
    // within 1e-14 relative error, but temperatures may differ from UniSim due to
   ...
   *
   * <p>
   * After the BP method converges on material balance/equilibrium, tray
   * temperatures may be off because the BP method only solves MESH equations
   * without the E (energy balance). This method computes the enthalpy of all
   * streams entering each tray, mixes them, then PH-flashes to find the
   * energy-consistent temperature. The resulting K-values and flows are fed
   * back to the tridiagonal solver, and the process iterates until the
   * temperature profile stabilizes.
   * </p>
   */
  private void phaseThreePHflashCorrection() {
    System.out.println("Phase 3: Newton-E T correction. T[0]="
        + String.format("%.2f", T[0] - 273.15) + " T[N-1]="
    ...
    for (int iter = 0; iter < maxIter; iter++) {
      ...
      for (int j = 0; j < N; j++) {
        ...
        if (iter == 0) {
          System.out.println("  Tray " + j + ": T=" + String.format("%.1f", T[j] - 273.15)
              + " E=" + String.format("%.0f", energyErr[j])
              + " dE/dT=" + String.format("%.0f", dEdT)
      ...
      double mbErr = computeMassBalanceError();
      System.out.println("E-iter " + iter + ": maxDT="
          + String.format("%.3f", maxDeltaT) + " maxE="
          + String.format("%.0f", maxEErr) + " mb="
          + String.format("%.4f%%", mbErr * 100)
          + " T[0]=" + String.format("%.1f", T[0] - 273.15)
          + " T[N-1]=" + String.format("%.1f", T[N - 1] - 273.15));
      ...
      if (mbErr > 1.0) {
        System.out.println("Newton-E diverging (mb="
            + String.format("%.1f%%", mbErr * 100) + "), reducing damping");
        restoreTrayState(bestLiq, bestT, bestV);
        evaluateThermo();
        dampE *= 0.5;
    ...
    System.out.println("Phase 3 done: T[0]=" + String.format("%.2f", T[0] - 273.15)
        + " T[N-1]=" + String.format("%.2f", T[N - 1] - 273.15)
        + " mb=" + String.format("%.4f%%", bestMbErr * 100));
  }
   ...
   *
   * <p>
   * After the BP method converges on material balance, tray temperatures may be
   * off because BP determines T from bubble-point (sum Kx = 1), which is
   * inappropriate for wide-boiling / absorber-type columns. The SR method instead
   * determines T from the energy balance:
   * </p>
   * <ol>
   ...
   *
   * <p>
   * Reference: Seader, Henley &amp; Roper, "Separation Process Principles",
   * Chapter 10.4 — Sum-Rates method for absorber/stripper columns.
   * </p>
   */
  private void solveSumRatesPhase() {
    System.out.println("[SR] Starting Sum-Rates energy correction. T[0]="
        + String.format("%.1f", T[0] - 273.15) + "C  T[N-1]="
        + String.format("%.1f", T[N - 1] - 273.15) + "C");
    ...
    for (int j = 0; j < N; j++) {
      ...
      if (feedMolesJ > 1e-10) {
        System.out.println("[SR-FEED] tray " + j
            + ": feedLTotal=" + String.format("%.1f", feedLTotal[j])
            + " feedHL=" + String.format("%.1f", feedHL[j])
            + " feedVTotal=" + String.format("%.1f", feedVTotal[j])
            + " feedHV=" + String.format("%.1f", feedHV[j])
            + " feedEnthalpy=" + String.format("%.0f", feedLTotal[j] * feedHL[j]
                + feedVTotal[j] * feedHV[j]));
      }
    }
    ...
    logger.info("SR: T bounds: min={}C max={}C",
        String.format("%.1f", minTrayTemp - 273.15),
    ...
    for (int iter = 0; iter < maxIter; iter++) {
      ...
      newV[N - 1] = Math.max(
          newV[N - 2] + feedLTotal[N - 1] + feedVTotal[N - 1] - newL[N - 1], 1e-10);
      ...
      for (int j = 0; j < N; j++) {
        ...
        if (iter == 0) {
          ...
          System.out.println("[SR-DIAG] tray " + j
              + ": T=" + String.format("%.1fC", T[j] - 273.15)
              + " hV=" + String.format("%.1f", hV[j])
              + " hL=" + String.format("%.1f", hL[j])
              + " V=" + String.format("%.1f", V[j])
              + " L=" + String.format("%.1f", L[j])
              + " Hout=" + String.format("%.0f", hOutJ)
              + " Hin(noFeed)=" + String.format("%.0f", hInJ)
              + " feedH=" + String.format("%.0f", feedH)
              + " Hin(total)=" + String.format("%.0f", hInTotal)
              + " Ej=" + String.format("%.0f", energyErr[j])
              + " dEdT=" + String.format("%.1f", dEdT)
              + " rawDT=" + String.format("%.2f", rawDeltaT));
        }
      ...
      if (iter % 10 == 0 || iter < 5) {
        System.out.println("[SR] iter " + iter + ": dT=" + String.format("%.3f", maxDeltaT)
            + " mb=" + String.format("%.4f%%", mbErr * 100)
            + " energy=" + String.format("%.1f%%", eErr * 100)
            + " T[0]=" + String.format("%.1f", T[0] - 273.15)
            + " T[top]=" + String.format("%.1f", T[N - 1] - 273.15)
            + " damp=" + String.format("%.3f", dampT));
        logger.info(
            "SR iter {}: maxDT={} mbErr={}% energy={}% T[0]={} T[N-1]={} damp={}",
            iter, String.format("%.3f", maxDeltaT),
            String.format("%.4f", mbErr * 100),
            String.format("%.2f", eErr * 100),
            String.format("%.1f", T[0] - 273.15),
            String.format("%.1f", T[N - 1] - 273.15),
      ...
      if (eErr < bestEnergyErr
          || (mbErr < 0.01 && eErr < bestEnergyErr * 1.05)) {
        bestEnergyErr = eErr;
        bestMbErr = mbErr;
        saveTrayState(bestLiq, bestT, bestV);
      }
      ...
      if (maxDeltaT < 0.05 && mbErr < 0.01) {
        logger.info("SR converged at iter {}: mbErr={}% energy={}%",
            iter,
            String.format("%.4f", mbErr * 100),
            String.format("%.2f", eErr * 100));
        break;
      ...
      if (mbErr > 5.0) {
        System.out.println("[SR] mass balance degraded to "
            + String.format("%.1f%%", mbErr * 100) + " — reducing damping");
    ...
    System.out.println("[SR] Done: mb=" + String.format("%.4f%%", finalMb * 100)
        + " energy=" + String.format("%.1f%%", finalE * 100)
        + " T[0]=" + String.format("%.1f", T[0] - 273.15)
        + " T[top]=" + String.format("%.1f", T[N - 1] - 273.15));
    logger.info("SR Phase done: mbErr={}% energy={}% T[0]={} T[N-1]={}",
        String.format("%.4f", finalMb * 100),
        String.format("%.2f", finalE * 100),
        String.format("%.1f", T[0] - 273.15),
        String.format("%.1f", T[N - 1] - 273.15));
   ...
   *
   * <p>
   * The relative error for each tray is |E_j| / max(|H_out_j|, |H_in_j|). This
   * gives a dimensionless metric comparable to the mass balance error fraction.
   * </p>
   *
   ...
   *
   * <p>
   * E_j = H_out_j - H_in_j where H_out_j = V[j]*hV[j] + L[j]*hL[j] and
   * H_in_j = V[j-1]*hV[j-1] + L[j+1]*hL[j+1] + feedEnthalpy[j].
   * </p>
   *
   ...
   *
   * <p>
   * For component i, the material balance on tray j gives:
   * a_j * l_{i,j-1} + b_j * l_{i,j} + c_j * l_{i,j+1} = d_j
   ...
   *
   * <p>
   * Finds T such that sum(K_i(T, P, x) * x_i) = 1.
   * Uses Wilson K-values for speed in the BP method.
   * </p>
   *
   * @param j tray index
   ...
   *
   * <p>
   * Material balance for component i on tray j:
   * M_{i,j} = l_{i,j}(1 + s_{i,j}) - l_{i,j-1} - v_{i,j+1} - f_{i,j} = 0 where
   * s_{i,j} =
   * K_{i,j} * V_j / L_j is the stripping factor, and v_{i,j} = K_{i,j} * l_{i,j}
   * * V_j / L_j
   *
   * <p>
   * Energy balance for tray j: H_j = L_j*h_L_j + V_j*h_V_j - L_{j-1}*h_L_{j-1} -
   * V_{j+1}*h_V_{j+1} - F_L_j*h_FL_j - F_V_j*h_FV_j - Q_j = 0
   *
   * <p>
   ...
   *
   * <p>
   * The Jacobian has block-tridiagonal structure. Only entries in the
   * tri-diagonal bands are
   * computed. For variable k on tray jj, we compute d(residual_j)/d(var_{jj,k})
   * for
   * j in {jj-1, jj, jj+1}. After perturbing a variable on tray jj, only tray jj's
   * thermo
   * is re-evaluated (K-values, enthalpies depend only on local T and x).
   * </p>
   *
  ...
  /**
   * Evaluate thermodynamic properties for a single tray. Used during Jacobian
   * perturbation to avoid
   ...
   *
   * <p>
   * The exact Murphree definition mixes the actual outlet vapor with the
   * vapor entering from below: y_actual = y_in + eta * (y_eq - y_in). Embedding
   * that into the simultaneous MESH solve requires the residual on tray j to
   * couple to vap[j-1], which would expand the Jacobian bandwidth.
   * </p>
   *
   * <p>
   ...
   * <li>eta -&gt; 0 -&gt; K_eff -&gt; 1.0, so y = K_eff*x -&gt; x and the
   * stage becomes passive (vapor leaves with the same composition as the
   * liquid).</li>
   * </ul>
   * This is the same correction used by many shortcut and equation-tearing
   * column codes (e.g. ChemSep documentation, Edmister 1957) and is mass- and
   * energy-balance consistent because the residual equations always use the
   * scaled K[j][i].
   * </p>
   *
  ...
  /**
   * Compute molar enthalpy for a single phase (liquid or vapor) at given
   * composition, temperature
   * and pressure using the EOS.
   *
   * <p>
   * This forces the EOS to evaluate at the specified phase root (liquid or vapor)
   * rather than doing
   * a flash. This is essential for energy balance in the BP method: hL must be
   * evaluated at the
   * actual liquid composition x[j] using the liquid root, and hV at the actual
   * vapor composition
   * y[j] using the vapor root. Using the enthalpies from a TPflash of x[j] gives
   * wrong hV because
   ...
   * @param isVapor     true for vapor-phase enthalpy (vapor root), false for
   *                    liquid (liquid root)
   ...
   *
   * <p>
   * Activates when at least one tray has a fixed temperature spec — the case
   * where the smooth Wilson BP cascade under-resolves a sharp T-pinch. The
   * algorithm follows Boston &amp; Sullivan (1974):
   * </p>
   * <ol>
   * <li>Outer loop: compute base K-value {@code Kb[j]} as the x-weighted
   * geometric mean of {@code K[j][i]}, relative volatilities
   * {@code alpha[j][i] = K[j][i] / Kb[j]}, and an Antoine-style fit
   * {@code ln(Kb) = A - B/T} via a Wilson-K perturbation at {@code T + 5K}.</li>
   * <li>Inner loop (fixed {@code alpha}, {@code A}, {@code B}): solve one
   * tridiagonal MB per component for {@code x[j][i]}; apply the bubble-point
   * criterion {@code sum(alpha * x) * Kb = 1} to get {@code Kb_new}; invert
   * the Antoine fit for {@code T_new}; clamp T-pinned trays to
   * {@code fixedTemperature[j]}; update {@code liq[j][i]} and
   * {@code vap[j][i]} preserving the current {@code L[j]} and {@code V[j]}
   * totals.</li>
   * <li>Repeat until the relative change in {@code alpha} between outer
   * iterations falls below tolerance.</li>
   * </ol>
   *
   * <p>
   * Mass balance is preserved by the tridiagonal solve. V[j] and L[j] are
   * held at their BP-Wilson values; the downstream SR / Newton phase polishes
   * the energy balance.
   * </p>
   */
  private void runBostonSullivanRefinement() {
    ...
    for (int outer = 0; outer < maxOuter; outer++) {
      ...
      for (int j = 0; j < N; j++) {
        ...
        double shift = 0;
        for (int i = 0; i < C; i++) {
          double KiPert = (Pc[i] / Pbar)
              * Math.exp(5.37 * (1.0 + omega[i]) * (1.0 - Tc[i] / Tpert));
          double KiBase = (Pc[i] / Pbar)
              * Math.exp(5.37 * (1.0 + omega[i]) * (1.0 - Tc[i] / Tj));
          double xi = liq[j][i] / Lsum;
          shift += xi
              * Math.log(Math.max(KiPert, 1e-30) / Math.max(KiBase, 1e-30));
        }
      ...
      logger.info(
          ...
          String.format("%.3e", maxAlphaChange),
          String.format("%.2f", T[0] - 273.15),
          String.format("%.2f", T[N / 2] - 273.15),
          String.format("%.2f", T[N - 1] - 273.15),
          String.format("%.1f", V[0]),
          String.format("%.1f", V[N - 1]));
    }

    logger.info(
        "BS done: T[0]={}C->{}C  T[N-1]={}C->{}C",
        String.format("%.2f", initialT0 - 273.15),
        String.format("%.2f", T[0] - 273.15),
        String.format("%.2f", initialTtop - 273.15),
        String.format("%.2f", T[N - 1] - 273.15));
  }
  ...
  /**
   * Wilson-K bubble-point temperature for a composition at a given pressure.
   * Solves sum(z_i * K_i(T,P)) = 1 by Newton iteration. Used only as an
   * initial-guess generator for the BP method T-profile.
   *
  ...
  /**
   * Wilson-K dew-point temperature for a composition at a given pressure.
   * Solves sum(z_i / K_i(T,P)) = 1 by Newton iteration. Used only as an
   * initial-guess generator for the BP method T-profile.
   *
   * @param z      molar composition
   ...
   * @param k    intra-tray variable index: 0..C-1 are liquid flows, C is
   *             temperature, C+1 is vapor
   *             flow
   ...
   *
   * <p>
   * The overall mass balance is: V[N-1] + L[0] = total_feed. This method
   * returns the absolute relative error |V[N-1] + L[0] - feed| / feed.
   * </p>
   *
   ...
   *
   * <p>
   * Limits each variable's per-iteration change to a physically reasonable
   * magnitude to prevent Newton from leaving the basin of attraction. The
   * whole step vector is scaled by the smallest acceptable ratio so the
   * descent direction is preserved.
   * </p>
   *
   * <p>
   * Bounds: |dT| &le; 10 K, |dV| &le; 0.5*V + 0.05*flowScale,
   * |d(liq_ij)| &le; 0.5*liq_ij + 1e-3*flowScale.
   * </p>
   *
   ...
   *
   * <p>
   * Includes bounds enforcement: temperatures must remain positive, component
   * flows must remain
   * non-negative.
   * </p>
   *
   ...
   *
   * <p>
   * The Jacobian J is block-tridiagonal with block size (C+2). Blocks: A_j
   * (sub-diagonal, coupling
   * to tray j-1), B_j (diagonal, tray j), C_j (super-diagonal, coupling to tray
   * j+1).
   * </p>
   *
   * <p>
   * Uses the Thomas algorithm (block LU factorization) adapted for block
   * matrices.
   * </p>
   *
   ...
   *
   * <p>
   * For each tray, creates a two-phase thermo system at the converged T, P, and
   * overall
   * composition (from liquid + vapor flows), performs a TPflash, then extracts
   * the gas phase
   * via {@code phaseToSystem(0)} and the liquid phase via
   * {@code phaseToSystem(1)}.
   * The phase flow rates are set to V[j] and L[j] from the solver variables.
   * </p>
   *
  ...
  private void applyResultsToColumn(UUID id, int iterations, double finalNorm, long startTime) {
    for (int j = 0; j < N; j++) {
      ...
      gasSystem.init(0);
      gasSystem.init(2);
      tray.setCachedGasOutStream(
          new neqsim.process.equipment.stream.Stream("gas_" + j, gasSystem));
