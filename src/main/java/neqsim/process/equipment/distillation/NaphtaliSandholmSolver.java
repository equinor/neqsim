 ...
 *
 * <p>
 * Solves the full MESH (Material balance, Equilibrium, Summation, Heat balance) system of equations simultaneously
 * using Newton-Raphson iteration with block-tridiagonal Jacobian structure.
 * </p>
 *
 * <p>
 * This solver handles wide-boiling-range systems where sequential substitution (tray-by-tray) methods diverge. The
 * variable set per tray j is: liquid component flows l_{i,j} for each component i, temperature T_j, and vapor flow rate
 * V_j. This gives (C+2) variables per tray for C components, and N*(C+2) total variables for N trays.
 * </p>
 *
 * <p>
 * Reference: Naphtali, L.M. and Sandholm, D.P. (1971). "Multicomponent Separation Calculations by Linearization", AIChE
 * Journal, 17(1), 148-153.
 * </p>
 *
...
public class NaphtaliSandholmSolver {
  ...
  /**
   * Per-stage Murphree efficiency cached at solver init time. trayEta[j] is forced to 1.0 for the reboiler (j=0) and
   * the condenser (j=N-1 when present) so those stages remain rigorous equilibrium. Other stages use the value
   * registered on the parent column via setMurphreeEfficiency(stage, value), defaulting to the global value when no
   * per-stage override is set.
   *
   * The efficiency is embedded into the simultaneous MESH solution through the Edmister proxy K_eff[j][i] =
   * K[j][i]^trayEta[j]. At trayEta = 1.0 the stage is rigorous equilibrium; at trayEta -&gt; 0 the K-values approach
   * 1.0 and the tray becomes passive (vapor passes through ~ unchanged), matching the behaviour of a heavily de-rated
   * tray
   */
  ...
  /**
   * Fixed temperature specification for each tray in K. NaN means the temperature is a free variable (solved by the
   * energy balance).
   */
  private double[] fixedTemperature;

  /**
   * Per-tray seed temperature [K] used by the initializer when the tray is NOT fixed-T (i.e. T[j] is a Newton
   * variable). NaN means "no seed, use Wilson estimate". This lets a user pass a reboiler T value as a hint when they
  ...
  /**
   * Enable Boston-Sullivan inside-out refinement of the seed when at least one tray temperature is pinned (e.g. T-spec
   * reboiler). The smooth Wilson BP seed misses sharp T-pinches; B-S inner loop (per-component tridiag MB + bubble-T)
   * captures them and gives SR / Newton a sharper starting point. Auto-detected: only fires when
   * {@code fixedTemperature[j]} is set on at least one tray. Default true.
   */
  ...
  /**
   * When true, the column uses overall mass balance closure L[0] = totalFeed - V[N-1] instead of V[0] =
   * boilupRatio*L[0]. Enabled automatically when the user pins reboiler temperature but does not explicitly set a
   * boilup ratio (i.e. boilupRatio is still the NeqSim default of 0.1). With pinned T, V[0] must be free to be
   * determined by physics, not constrained to an arbitrary default ratio.
   */
  ...
  /**
   * Original feed thermo systems cloned before column init() corrupts them. Map of tray index to list of
   * SystemInterface clones. Null if not provided.
   */
  private Map<Integer, List<SystemInterface>> originalFeedSystems;

  /**
   * Original feed molar flow rates in mol/hr, saved before column init(). Map of tray index to list of flow rates. Null
   * if not provided.
   */
   ...
   *
   * <p>
   * The column's init() method may modify feed streams in-place (shared object references with trays). This constructor
   * accepts deep clones of the original feed thermo systems and their molar flow rates, taken before init() runs, so
  ...
  public boolean solve(UUID id) {
    ...
    try {
      ...
      logger.debug("NS after BP: ||F|| = {} converged={}", String.format("%.6e", norm), bpConverged);
      ...
      if (logger.isDebugEnabled()) {
        for (int j = 0; j < N; j++) {
          ...
          logger.debug("  Tray {}: T={}C L={} V={} matRes={} hRes={} sumRes={}", j,
              String.format("%.1f", T[j] - 273.15), String.format("%.4f", L[j]), String.format("%.4f", V[j]),
              String.format("%.4e", matNorm), String.format("%.4e", Math.abs(residual[base + C])),
  ...
  private void initialize() {
    ...
    for (Map.Entry<Integer, List<StreamInterface>> entry : feedMap.entrySet()) {
      ...
      for (int fi = 0; fi < feeds.size(); fi++) {
        ...
        double beta = feedSys.getBeta(); // vapor fraction (only meaningful when 2 phases
                                         // present)
    ...
    useOverallMBClosure = hasReboiler && !Double.isNaN(fixedTemperature[0]) && Math.abs(boilupRatio - 0.1) < 1e-9;
    if (useOverallMBClosure) {
    ...
    if (logger.isDebugEnabled()) {
      for (int j = 0; j < N; j++) {
        logger.debug("  Tray {}: T={}K P={}bara L={} V={}{}", j, String.format("%.2f", T[j]),
            String.format("%.2f", P[j] / 1e5), String.format("%.4f", L[j]), String.format("%.4f", V[j]),
  ...
  /**
   * Initialize tray temperatures, flows, and compositions using Wilson K-values and constant molar overflow estimates.
   */
   ...
   *
   * <p>
   * V[j] is NOT recomputed here — it is a free variable of the solver. The vapor component flows v_{i,j} = K_{i,j} *
   * x_{i,j} * V_j are computed from the current V[j] and K-values.
   * </p>
   */
   ...
   * <li>Computes L[j] from per-tray material balance: L[j] = L[j+1] + V[j-1] - V[j] + F_j (top down).</li>
   * <li>Updates liq[j][i] from x[i] = z_i preserved-ratio normalization against L[j] (preserves the Rachford-Rice flash
   * compositions set by initializeTrayState).</li>
   * </ol>
   *
   * <p>
   * Mass conservation is satisfied at the initial guess to within numerical tolerance; Newton then refines energy
  ...
  private void seedSolutionForDirectNewton() {
    ...
    logger.info(
        ...
        String.format("%.4f", totalFeed), String.format("%.4f", overheadTotal), String.format("%.4f", bottomsTotal),
        String.format("%.4f", boilup), String.format("%.4f", V[0]), String.format("%.4f", V[N - 1]),
        String.format("%.4f", L[0]), String.format("%.4f", L[N - 1]), String.format("%.4e", maxCompMBerr));
  }
   ...
   *
   * <p>
   * Decomposes the MESH equations into sequential subproblems: 1. Material balance: tridiagonal Thomas algorithm for
   * each component 2. Summation: bubble point calculation for temperature on each tray 3. Energy: energy balance for
   * vapor flow on each tray
   * </p>
   *
   * @return true if converged
   */
  private boolean solveBubblePointMethod() {
    ...
    for (int iter = 0; iter < 80; iter++) {
      ...
      if (iter % 10 == 0) {
        logger.debug("BP-Wilson iter {}: maxDeltaT={} maxDeltaV={}", iter, String.format("%.4f", maxDeltaT),
            String.format("%.4f", maxDeltaV));
      }
      if (maxDeltaT < 0.1 && (maxDeltaV < 0.1 || iter <= 10)) {
        logger.info("BP-Wilson converged at iter {}: maxDeltaT={}", iter, String.format("%.4f", maxDeltaT));
        break;
      }
    }
    ...
    for (int eosIter = 0; eosIter < maxEosIter; eosIter++) {
      ...
      if (eosIter % 5 == 0 || eosIter < 5) {
        double eErr = computeMaxRelativeEnergyError();
        logger.info("BP-EOS iter {}: dT={} dV={} mbErr={}% energy={}% sumKxErr={} T[0]={} T[N-1]={}", eosIter,
            String.format("%.2f", maxDeltaT), String.format("%.2f", maxDeltaV), String.format("%.4f", mbErr * 100),
            String.format("%.2f", eErr * 100), String.format("%.6f", maxSumKxErr), String.format("%.1f", T[0] - 273.15),
      ...
      if (mbErr > 0.5) {
        logger.warn("BP-EOS diverging (massBalErr={}%), reverting to best", String.format("%.2f", mbErr * 100));
    ...
    logger.info("BP-EOS complete: bestMassBalErr={}%", String.format("%.4f", bestEosMassBalErr * 100));
   ...
   *
   * <p>
   * After the BP method converges on material balance/equilibrium, tray temperatures may be off because the BP method
   * only solves MESH equations without the E (energy balance). This method computes the enthalpy of all streams
   * entering each tray, mixes them, then PH-flashes to find the energy-consistent temperature. The resulting K-values
   * and flows are fed back to the tridiagonal solver, and the process iterates until the temperature profile
   * stabilizes.
   * </p>
   */
   ...
   *
   * <p>
   * After the BP method converges on material balance, tray temperatures may be off because BP determines T from
   * bubble-point (sum Kx = 1), which is inappropriate for wide-boiling / absorber-type columns. The SR method instead
   ...
   *
   * <p>
   * Reference: Seader, Henley &amp; Roper, "Separation Process Principles", Chapter 10.4 — Sum-Rates method for
  ...
  private void solveSumRatesPhase() {
    ...
    for (int iter = 0; iter < maxIter; iter++) {
      ...
      if (maxDeltaT < 0.05 && mbErr < 0.01) {
        logger.info("SR converged at iter {}: mbErr={}% energy={}%", iter, String.format("%.4f", mbErr * 100),
            String.format("%.2f", eErr * 100));
        break;
      }
   ...
   *
   * <p>
   * The relative error for each tray is |E_j| / max(|H_out_j|, |H_in_j|). This gives a dimensionless metric comparable
   * to the mass balance error fraction.
   * </p>
   *
   ...
   *
   * <p>
   * Material balance for component i on tray j: M_{i,j} = l_{i,j}(1 + s_{i,j}) - l_{i,j-1} - v_{i,j+1} - f_{i,j} = 0
   * where s_{i,j} = K_{i,j} * V_j / L_j is the stripping factor, and v_{i,j} = K_{i,j} * l_{i,j} * V_j / L_j
   *
   * <p>
   * Energy balance for tray j: H_j = L_j*h_L_j + V_j*h_V_j - L_{j-1}*h_L_{j-1} - V_{j+1}*h_V_{j+1} - F_L_j*h_FL_j -
   * F_V_j*h_FV_j - Q_j = 0
   *
   * <p>
  ...
  /**
   * Evaluate thermodynamic properties for a single tray. Used during Jacobian perturbation to avoid full column
   * re-evaluation. V[j] is NOT overwritten — it is a free variable.
   *
   * @param j tray index
   */
  /**
   ...
   *
   * <p>
   * The exact Murphree definition mixes the actual outlet vapor with the vapor entering from below: y_actual = y_in +
   * eta * (y_eq - y_in). Embedding that into the simultaneous MESH solve requires the residual on tray j to couple to
   * vap[j-1], which would expand the Jacobian bandwidth.
   * </p>
   *
   * The Edmister approximation K_eff = K^eta has the correct limits:
   * <ul>
   * <li>eta = 1.0 -&gt; K_eff = K (rigorous equilibrium)</li>
   * <li>eta -&gt; 0 -&gt; K_eff -&gt; 1.0, so y = K_eff*x -&gt; x and the stage becomes passive (vapor leaves with the
   * same composition as the liquid).</li>
   * </ul>
   * This is the same correction used by many shortcut and equation-tearing column codes (e.g. ChemSep documentation,
   * Edmister 1957) and is mass- and energy-balance consistent because the residual equations always use the scaled
   * K[j][i].
   *
  ...
  /**
   * Compute molar enthalpy for a single phase (liquid or vapor) at given composition, temperature and pressure using
   * the EOS.
   *
   * <p>
   * This forces the EOS to evaluate at the specified phase root (liquid or vapor) rather than doing a flash. This is
   * essential for energy balance in the BP method: hL must be evaluated at the actual liquid composition x[j] using the
   * liquid root, and hV at the actual vapor composition y[j] using the vapor root. Using the enthalpies from a TPflash
   * of x[j] gives wrong hV because the flash vapor has a different composition than the actual column vapor.
   * </p>
   *
   ...
   *
   * <p>
   * Activates when at least one tray has a fixed temperature spec — the case where the smooth Wilson BP cascade
   * under-resolves a sharp T-pinch. The algorithm follows Boston &amp; Sullivan (1974):
   * </p>
   * <ol>
   * <li>Outer loop: compute base K-value {@code Kb[j]} as the x-weighted geometric mean of {@code K[j][i]}, relative
   * volatilities {@code alpha[j][i] = K[j][i] / Kb[j]}, and an Antoine-style fit {@code ln(Kb) = A - B/T} via a
   * Wilson-K perturbation at {@code T + 5K}.</li>
   * <li>Inner loop (fixed {@code alpha}, {@code A}, {@code B}): solve one tridiagonal MB per component for
   * {@code x[j][i]}; apply the bubble-point criterion {@code sum(alpha * x) * Kb = 1} to get {@code Kb_new}; invert the
   * Antoine fit for {@code T_new}; clamp T-pinned trays to {@code fixedTemperature[j]}; update {@code liq[j][i]} and
   * {@code vap[j][i]} preserving the current {@code L[j]} and {@code V[j]} totals.</li>
   * <li>Repeat until the relative change in {@code alpha} between outer iterations falls below tolerance.</li>
   * </ol>
   *
   * <p>
   * Mass balance is preserved by the tridiagonal solve. V[j] and L[j] are held at their BP-Wilson values; the
   * downstream SR / Newton phase polishes the energy balance.
   * </p>
   */
  private void runBostonSullivanRefinement() {
    ...
    for (int outer = 0; outer < maxOuter; outer++) {
      ...
      for (int j = 0; j < N; j++) {
        ...
        double shift = 0;
        for (int i = 0; i < C; i++) {
          double KiPert = (Pc[i] / Pbar) * Math.exp(5.37 * (1.0 + omega[i]) * (1.0 - Tc[i] / Tpert));
      ...
      if (outer > 0 && maxAlphaChange < alphaTol) {
        logger.info("BS outer {} converged on alpha (maxRelChange={})", outer, String.format("%.3e", maxAlphaChange));
      ...
      logger.info(
          ...
          outer + 1, innerUsed, String.format("%.3f", maxDTinner), String.format("%.3e", maxAlphaChange),
          String.format("%.2f", T[0] - 273.15), String.format("%.2f", T[N / 2] - 273.15),
          String.format("%.2f", T[N - 1] - 273.15), String.format("%.1f", V[0]), String.format("%.1f", V[N - 1]));
    }

    logger.info("BS done: T[0]={}C->{}C  T[N-1]={}C->{}C", String.format("%.2f", initialT0 - 273.15),
        String.format("%.2f", T[0] - 273.15), String.format("%.2f", initialTtop - 273.15),
        String.format("%.2f", T[N - 1] - 273.15));
  }

  /**
   * Thomas algorithm for tridiagonal systems {@code a[j] * x[j-1] + b[j] * x[j] + c[j] * x[j+1] = d[j]}.
   *
  ...
  /**
   * Compute fugacity coefficients for a single phase (liquid or vapor) at given composition, temperature and pressure
   * using a forced single-phase EOS root.
   *
   * <p>
   * Mirrors {@link #computeSinglePhaseEnthalpy} — sets one phase, forces the requested phase type (GAS or LIQUID), and
   * reads the fugacity coefficients directly. This is the correct way to evaluate K-values inside a column MESH
   * residual, because K_i = phi_L(x) / phi_V(y) must be evaluated at the tray's actual compositions x and y, NOT at a
   * flash composition.
   * </p>
   *
  ...
  private boolean computeSinglePhaseFugacityCoefficients(double[] composition, double tempK, double pressBar,
    ...
    try {
      ...
      logger.debug("Single-phase fugacity failed for {} phase at T={}K P={}bar", isVapor ? "vapor" : "liquid", tempK,
          pressBar);
      return false;
    }
  }
  ...
  private double computeSinglePhaseEnthalpy(double[] composition, double tempK, double pressBar, boolean isVapor) {
    try {
      SystemInterface sys = referenceSystem.clone();
      sys.setTemperature(tempK);
      sys.setPressure(pressBar);
      ...
      sys.setForcePhaseTypes(true);
      sys.init(2);
      return sys.getPhase(0).getEnthalpy() / Math.max(sys.getPhase(0).getNumberOfMolesInPhase(), 1e-20);
    } catch (Exception e) {
      logger.debug("Single-phase enthalpy failed for {} phase at T={}K P={}bar", isVapor ? "vapor" : "liquid", tempK,
          pressBar);
      return 0.0;
    }
  }
   ...
   *
   * <p>
   * Limits each variable's per-iteration change to a physically reasonable magnitude to prevent Newton from leaving the
   * basin of attraction. The whole step vector is scaled by the smallest acceptable ratio so the descent direction is
   * preserved.
   * </p>
   *
   * <p>
   * Bounds: |dT| &le; 10 K, |dV| &le; 0.5*V + 0.05*flowScale, |d(liq_ij)| &le; 0.5*liq_ij + 1e-3*flowScale.
   * </p>
   *
   ...
   *
   * <p>
   * The Jacobian J is block-tridiagonal with block size (C+2). Blocks: A_j (sub-diagonal, coupling to tray j-1), B_j
   * (diagonal, tray j), C_j (super-diagonal, coupling to tray j+1).
   * </p>
   *
   * <p>
  ...
  private void applyResultsToColumn(UUID id, int iterations, double finalNorm, long startTime) {
    ...
    logger.info(
        ...
        iterations, String.format("%.6e", finalNorm), String.format("%.6e", massBalErr), String.format("%.4f", topFlow),
        String.format("%.4f", botFlow), String.format("%.4f", totalFeedFlow), String.format("%.2f", solveTime));
  }
}
