 ...
 *
 * <p>
 * The screen first delegates convergence, solver, conservation, and material-product qualification to
 * {@link SarirAtmosphericFractionationResult}. It then calculates ASTM D86 curves from the two live liquid side-draw
 * fluids and delegates each strict 95 liquid-volume-percent comparison to {@link SarirD86ProductComparison}. Published
 * laboratory, HYSYS, and specification values remain read-only evidence and are never solver controls or tuning
 * targets.
 * </p>
 *
 * <p>
 * Total naphtha is excluded because the atmospheric case does not resolve the published light/heavy naphtha split.
 * Residual is excluded because its published specification is the nonnumeric, open-ended {@code 550+} boundary.
 * </p>
 */
public final class SarirAtmosphericProductQualityScreen {
   ...
   * @throws IllegalStateException if the case is not qualified, either side draw is non-material, or a finite strict
   * T95 cannot be calculated
   */
  public static Result evaluate(SarirAtmosphericFractionationCase model) {
    Objects.requireNonNull(model, "model");
    SarirAtmosphericFractionationResult fractionationResult = SarirAtmosphericFractionationResult.evaluate(model);
    ...
    for (int i = 0; i < PRODUCT_LABELS.length; i++) {
      SarirAtmosphericFractionationResult.ProductResult productResult = fractionationResult
          .getProduct(PRODUCT_LABELS[i]);
      ...
      if (productFluid == null) {
        throw new IllegalStateException("Sarir product-quality screening requires a thermodynamic product fluid");
      }
  ...
  public static final class Result {
    ...
    private Result(SarirAtmosphericFractionationResult fractionationResult,
    ...
    public SarirD86ProductComparison.Result getComparison(String productLabel) {
      ...
      throw new IllegalArgumentException("Unsupported Sarir product-quality label: " + productLabel);
    }
  }
}
