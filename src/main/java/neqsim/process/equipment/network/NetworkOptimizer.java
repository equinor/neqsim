...
public class NetworkOptimizer {
  ...
  /**
   * Register a bounded whole-network decision variable.
   *
  ...
  /**
   * Register a typed hard or soft constraint.
   *
  ...
  /**
   * Add a composable objective term.
   *
  ...
  /**
   * Clear generalized variables, constraints, objectives, and trajectory.
   */
  public void clearGeneralizedProblem() {
  ...
  /** @return immutable registered decision variables */
  public List<NetworkDecisionVariable> getDecisionVariables() {
    return Collections.unmodifiableList(decisionVariables);
  }

  /** @return immutable registered constraints */
  public List<NetworkConstraint> getConstraints() {
    return Collections.unmodifiableList(constraints);
  }

  /** @return immutable registered objective terms */
  public List<NetworkObjective> getObjectives() {
    return Collections.unmodifiableList(objectives);
  }

  /** @return immutable latest candidate trajectory */
  public List<NetworkCandidateEvaluation> getTrajectory() {
    return Collections.unmodifiableList(trajectory);
  }

  /**
   * Set deterministic optimizer seed.
   *
   * @param seed seed
   */
   ...
   * @param maximumRateKgHr maximum source/sink rate
  ...
  public OptimizationResult optimize() {
    if (!decisionVariables.isEmpty()) {
      return optimizeRegisteredProblem();
    }
  ...
  public NetworkCandidateEvaluation evaluateCandidate(double[] values) {
    ...
    try {
      ...
      network.run();
      boolean solverConverged = network.isConverged();
      evaluation.setSolverConverged(solverConverged);
      ...
      evaluation.setObjectiveValue(objectiveValue);
  ...
  /**
   * Evaluate a named candidate map.
   *
  ...
  public NetworkCandidateEvaluation evaluateCandidate(Map<String, Double> candidate) {
    ...
    return evaluateCandidate(values);
  }

  /**
   * Optimize the registered generalized problem in normalized coordinates.
   *
  ...
  private double evaluateLegacyObjectiveWithoutPenalty() {
    double production = network.getTotalSinkFlow() * 3600.0;
  ...
  private void restoreDecisionValues(double[] values) {
    try {
  ...
  public static class OptimizationResult {
    ...
    /** Generalized candidate contract, or null for legacy choke-only runs. */
    public NetworkCandidateEvaluation candidateEvaluation;

    /** Named generalized decisions, including units in candidateEvaluation. */
    public Map<String, Double> decisionValues;
    ...
    /**
     * Serialize the optimization result and candidate diagnostics.
     *
     * @return stable JSON
     */
