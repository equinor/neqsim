...
public class TransientPipe extends TwoPortEquipment implements PipeLineInterface {
  ...
  private void initializeFromStream() {
    ...
    // Store initial mass flow - initially outlet equals inlet (steady state
    // assumption)
  ...
  public void run(UUID id) {
    ...
    while (simulationTime < maxSimulationTime) {
      ...
      // This ensures slugs continue moving even when outlet sections have low
      // velocity
  ...
  private void updateOutletFromStream() {
    ...
    // Update outlet pressure if using pressure BC, but only if not explicitly set
    // by user
  ...
  private void advanceTimeStep(double dt) {
    ...
    // Update pressure profile only for boundary-adjacent sections to maintain
    // consistency
  ...
  private void updateBoundaryPressures() {
    // For CONSTANT_PRESSURE boundaries, pressure is already set by
    // applyBoundaryConditions
    // For CONSTANT_FLOW boundaries, update pressure in boundary section based on
    // neighbor
  ...
  private void addSourceTerms(double[] U, PipeSection section, double dt) {
    ...
    // Friction source term: use only friction component (not gravity which is
    // already added above)
    // Use effective properties for friction if in slug to be consistent with
    // pressure profile
    ...
    // Get friction gradient only (calculatePressureGradient returns total, so
    // subtract gravity)
  ...
  private void updatePrimitiveVariables(PipeSection section, PipeSection oldSection, double[] U,
    ...
    // Normalize and enforce strict bounds [0.001, 0.999] to prevent single-phase
    // collapse
    ...
    if (alpha_L < 0.001) {
      ...
      // Superficial velocities: U_SG ≈ alpha_G * u_m, U_SL ≈ alpha_L * u_m (as
      // initial estimate)
      section.setGasVelocity(u_m);
      section.setLiquidVelocity(u_m);
      ...
      // Solve for velocities satisfying both momentum conservation and drift flux
      // relation
      ...
      // v_G = (v_d + C0 * rho_m * u_m / rho_L) / (1 - C0 * alpha_G + C0 * rho_G *
      // alpha_G / rho_L)
    ...
    if (!isBoundaryCell) {
      ...
      // Boundary cells: use old pressure, will be overridden by
      // updateBoundaryPressures()
  ...
  private void applyBoundaryConditions() {
    // Inlet temperature boundary condition (always maintain inlet stream
    // temperature)
    if (inStream != null && inStream.getFluid() != null) {
  ...
  private void checkConvergence() {
    ...
    // These are instantaneous values; proper residuals need previous time step
    // comparison
