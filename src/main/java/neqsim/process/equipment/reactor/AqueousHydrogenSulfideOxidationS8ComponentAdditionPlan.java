 ...
 * {@link AqueousHydrogenSulfideOxidationS8ComponentAmountProjection}. This class does not inspect or mutate a
 * thermodynamic system, reserve an idempotency key, run a flash, or predict sulfur deposition.
 * </p>
 *
 * @author esol
 * @version $Id: $
...
public final class AqueousHydrogenSulfideOxidationS8ComponentAdditionPlan {
   ...
   * @throws IllegalArgumentException if transition evidence, identifiers, or numeric values are invalid or if the
   * addition is not representable
   */
  public static Result create(AqueousHydrogenSulfideOxidationS8TransferLedger.Result prior,
      ...
      AqueousHydrogenSulfideOxidationS8TransferLedgerTransition.Result transition, String targetStateIdentifier,
      String applicationIdempotencyKey, double priorS8AmountMol) {
    String targetIdentifier = requireIdentifier(targetStateIdentifier, "Target-state identifier");
    String idempotencyKey = requireIdentifier(applicationIdempotencyKey, "Application idempotency key");
    ...
    double closureToleranceMol = 8.0 * Math.max(Math.ulp(Math.abs(candidateAmountMol)),
    ...
    if (Math.abs(amountClosureResidualMol) > closureToleranceMol) {
      throw new IllegalArgumentException("S8 component-addition plan does not close on its transferred amount");
    }

    return new Result(projection.getLedgerIdentifier(), projection.getProductIdentityBasisIdentifier(),
        targetIdentifier, idempotencyKey, projection.getPriorCheckpointHex(), projection.getCandidateCheckpointHex(),
        projection.getTransitionDigestHex(), projection.isUnchanged(), projection.isStrictAppend(), priorS8AmountMol,
        incrementMol, projection.getTransferredS8AmountKmol(), candidateAmountMol, reconstructedIncrementMol,
  ...
  public static final class Result implements Serializable {
    ...
    private Result(String ledgerIdentifier, String productIdentityBasisIdentifier, String targetStateIdentifier,
        String applicationIdempotencyKey, String priorCheckpointHex, String candidateCheckpointHex,
        String transitionDigestHex, boolean unchanged, boolean strictAppend, double priorS8AmountMol,
        double transferredS8AmountMol, double transferredS8AmountKmol, double candidateS8AmountMol,
    ...
    public String getMolecularWeightBasisIdentifier() {
      return AqueousHydrogenSulfideOxidationS8ComponentAmountProjection.MOLECULAR_WEIGHT_BASIS_IDENTIFIER;
    }
