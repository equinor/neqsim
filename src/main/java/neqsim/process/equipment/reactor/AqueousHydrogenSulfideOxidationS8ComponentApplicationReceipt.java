 ...
 *
 * <p>
 * The caller supplies the target identity, application idempotency key, and thermodynamic
 * systems immediately before and after its separately owned application step. The verifier
 * compares deterministic component inventories, including total-mole closure, and returns
 * immutable evidence. It does not reserve or consume the key, call {@code addComponent}, run a
 * flash, or authenticate either system as the real target.
 * </p>
 *
 * @author esol
 * @version $Id: $
...
public final class AqueousHydrogenSulfideOxidationS8ComponentApplicationReceipt {
   ...
   * @throws IllegalArgumentException if identities, inventories, or numerical closure do not
   *         match the plan
   */
  public static Result verify(AqueousHydrogenSulfideOxidationS8ComponentAdditionPlan.Result plan,
      ...
      SystemInterface priorTarget, SystemInterface candidateTarget) {
    if (plan == null) {
      throw new IllegalArgumentException("S8 component-addition plan cannot be null");
    }
    requireExactIdentity(plan.getTargetStateIdentifier(), observedTargetStateIdentifier,
        "Target-state identifier");
    requireExactIdentity(plan.getApplicationIdempotencyKey(), observedApplicationIdempotencyKey,
        "Application idempotency key");
    if (!AqueousHydrogenSulfideOxidationS8Transfer.S8_COMPONENT_NAME
        .equals(plan.getComponentName())) {
    ...
    if (plan.requiresMutation() && priorTarget == candidateTarget) {
      throw new IllegalArgumentException(
          "Positive S8 application requires distinct before and after system objects");
    }
    ...
    requireClose(plan.getPriorS8AmountMol(), priorS8AmountMol,
        "Observed prior S8 amount does not match the plan");
    ...
    TreeMap<String, Double> candidateNonS8 = withoutComponent(candidate.componentAmountsMol,
        componentName);
    if (!priorNonS8.keySet().equals(candidateNonS8.keySet())) {
      throw new IllegalArgumentException(
          "Non-S8 component identities changed during the observed application");
    }
    ...
    for (Map.Entry<String, Double> entry : priorNonS8.entrySet()) {
      ...
      requireClose(entry.getValue(), observed,
          "Non-S8 component amount changed during the observed application");
    ...
    requireIncrementClose(plan.getTransferredS8AmountMol(), observedIncrementMol,
        priorS8AmountMol, candidateS8AmountMol,
        "Observed S8 amount increment does not match the plan");
    ...
    requireIncrementClose(observedIncrementMol, observedTotalIncrementMol,
        prior.totalAmountMol, candidate.totalAmountMol,
        "Total-mole change does not close on the observed S8 increment");

    return new Result(plan.getLedgerIdentifier(), plan.getProductIdentityBasisIdentifier(),
        plan.getMolecularWeightBasisIdentifier(), plan.getTargetStateIdentifier(),
        plan.getApplicationIdempotencyKey(), plan.getPriorCheckpointHex(),
        plan.getCandidateCheckpointHex(), plan.getTransitionDigestHex(), plan.isUnchanged(),
        plan.isStrictAppend(), componentName, priorS8AmountMol, plan.getTransferredS8AmountMol(),
        candidateS8AmountMol, observedIncrementMol, planApplicationResidualMol,
        prior.totalAmountMol, candidate.totalAmountMol, observedTotalIncrementMol,
        totalAmountClosureResidualMol, priorNonS8.size(), maximumNonS8InventoryResidualMol);
  }
  ...
  private static TreeMap<String, Double> withoutComponent(Map<String, Double> amounts,
  ...
  private static void requireClose(double expected, double observed, String message) {
    ...
    double tolerance = COMPARISON_ULPS
        * Math.max(Math.ulp(Math.abs(expected)), Math.ulp(Math.abs(observed)));
  ...
  private static void requireIncrementClose(double expected, double observed, double priorAmount,
    ...
    double tolerance = COMPARISON_ULPS * Math.max(Math.ulp(Math.abs(expected)),
        Math.max(Math.ulp(Math.abs(observed)), Math.max(Math.ulp(Math.abs(priorAmount)),
            Math.ulp(Math.abs(candidateAmount)))));
  ...
  public static final class Result implements Serializable {
    ...
    private Result(String ledgerIdentifier, String productIdentityBasisIdentifier,
        String molecularWeightBasisIdentifier, String targetStateIdentifier,
        String applicationIdempotencyKey, String priorCheckpointHex, String candidateCheckpointHex,
        String transitionDigestHex, boolean unchanged, boolean strictAppend, String componentName,
        double priorS8AmountMol, double plannedS8IncrementMol, double candidateS8AmountMol,
        double observedS8IncrementMol, double planApplicationResidualMol,
        double priorTotalAmountMol, double candidateTotalAmountMol,
        double observedTotalIncrementMol, double totalAmountClosureResidualMol,
        int preservedNonS8ComponentCount, double maximumNonS8InventoryResidualMol) {
