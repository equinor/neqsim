...
/**
 * Calculates terminal settling velocity for droplets/bubbles using the Schiller-Naumann drag correlation, which covers
 * the full range from Stokes (creeping) flow through the intermediate regime to the Newton (turbulent) regime.
 *
 * <p>
 ...
 *
 * <p>
 * References: Schiller, L. and Naumann, Z. (1935), "A drag coefficient correlation", <i>Zeitschrift des Vereines
 * Deutscher Ingenieure</i>, 77, 318-320. Clift, R., Grace, J.R., and Weber, M.E. (1978), <i>Bubbles, Drops and
 * Particles</i>, Academic Press, New York.
 * </p>
 *
 * @author NeqSim team
 * @version 1.0
...
public class DropletSettlingCalculator implements Serializable {
  ...
  /**
   * Calculates the terminal settling velocity of a single droplet or bubble using the Schiller-Naumann drag
   * correlation.
   *
   * <p>
   * Positive velocity means downward (settling), applicable when dispersed phase is heavier. Negative velocity means
   ...
   */
  public static double calcTerminalVelocity(double dropletDiameter, double continuousDensity, double dispersedDensity,
    ...
    double vStokes = dropletDiameter * dropletDiameter * deltaRho * G / (18.0 * continuousViscosity);
   ...
   *
   * <p>
   * In a real separator, turbulence generated at the inlet redistributes small droplets, increasing the effective cut
   * diameter beyond the quiescent gravity value. This correction applies the Csanady (1963) passive-particle turbulent
   * diffusion model, simplified to an engineering formula (Koenders et al., 2015):
   * </p>
   *
   * <p>
   ...
   *
   * <p>
   * Effective cut diameter correction: $d_{cut,turb} = d_{cut,grav} \cdot \left(1 + \frac{D_t}{v_t \cdot
   * H}\right)^{0.25}$
   * </p>
   *
   * <p>
   * Turbulence intensity increases with gas load: at 50% of design K-factor the correction is negligible; above 80% it
   * becomes significant and can shift the effective cut diameter by 20-60%.
   * </p>
   *
   * <p>
   * References: Csanady, G.T. (1963). Turbulent diffusion of heavy particles in the atmosphere. <i>J. Atmos. Sci.</i>,
   * 20, 201-208. Koenders, M.A., Slot, J.J., Hoeijmakers, H.W.M. (2015). Numerical simulation of gas-liquid separator
   * performance. <i>SPE Production and Operations</i>, 30(3), 215-225.
   * </p>
   *
  ...
  public static double calcTurbulenceCorrectedCutDiameter(double gravityCutDiameter, double gasVelocity,
      double settlingHeight, double kFactor, double designKFactor, double gasDensity, double liquidDensity,
      double gasViscosity) {
    ...
    double vt = Math.abs(calcTerminalVelocity(gravityCutDiameter, gasDensity, liquidDensity, gasViscosity));
    if (vt <= 0) {
      return gravityCutDiameter;
    }
   ...
   *
   * <p>
   * API 12J (2014), <i>Specification for Oil and Gas Separators</i>, specifies minimum performance criteria for gravity
   * separators in upstream oil and gas service.
   * </p>
   */
   ...
   * </p>
   * <ul>
   * <li><b>Gas section — K-factor</b>: operating K must not exceed 0.107 m/s (vertical, no mist eliminator) or 0.120
   * m/s (horizontal, no mist eliminator). With a mist eliminator the limit depends on the device type — 0.12 to 0.30
   * m/s.</li>
   * <li><b>Gas section — droplet removal</b>: gravity section alone must remove droplets &ge; 100 &mu;m from gas (API
   * 12J Table 2 for clean service).</li>
   * <li><b>Liquid section — residence time</b>: minimum 3 minutes (180 s) for two-phase separators in clean service; 5
   * minutes (300 s) for three-phase.</li>
   * </ul>
   *
   * <p>
   * Reference: API Specification 12J, 8th Edition (2014). <i>Specification for Oil and Gas Separators</i>. American
   ...
   *
   * <p>
   * This is the smallest droplet that can settle across the available height within the available residence time. In
   * the Stokes regime:
   * </p>
   *
   * <p>
   ...
   */
  public static double calcCriticalDiameter(double availableHeight, double residenceTime, double continuousDensity,
