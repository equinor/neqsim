...
/**
 * Catalog of reusable {@link Calculator} presets. These presets provide declarative building blocks that can be paired
 * with {@link Calculator#setCalculationMethod(BiConsumer)} to encourage consistent AI- or rule-generated logic across
 * simulations.
 */
public final class CalculatorLibrary {
  ...
  public enum Preset {
    /**
     * Performs an enthalpy-based energy balance using the input streams and flashes the output stream at its current
     * pressure to match the summed enthalpy.
     */
    ENERGY_BALANCE,

    /**
     * Sets the output stream temperature to the hydrocarbon dew point of the first input stream at the output stream's
     * pressure.
     */
    DEW_POINT_TARGETING,

    /**
     * Anti-surge recycle calculation. Adjusts a splitter's recycle flow so that the compressor inlet actual volumetric
     * flow stays at or above the surge flow. Works in actual flow (m3/hr) space to match the compressor chart's surge
     * curve coordinates.
     */
    ANTI_SURGE
  }
  ...
  /**
   * Resolve a preset calculation by name (case-insensitive). This is useful when wiring calculators declaratively from
   * metadata or AI-generated instructions.
   *
   ...
   */
  public static BiConsumer<ArrayList<ProcessEquipmentInterface>, ProcessEquipmentInterface> byName(String presetName) {
    if (presetName == null) {
      throw new IllegalArgumentException("Preset name cannot be null");
    }

    String normalized = presetName.trim().toUpperCase(Locale.ROOT).replace(" ", "_").replace("-", "_");
   ...
   * @return a {@link BiConsumer} ready for {@link Calculator#setCalculationMethod(BiConsumer)}
   */
  public static BiConsumer<ArrayList<ProcessEquipmentInterface>, ProcessEquipmentInterface> preset(Preset preset) {
    switch (preset) {
    case ENERGY_BALANCE:
  ...
  /**
   * Create an energy-balance calculator. The output stream is flashed at its current pressure so that its enthalpy
   * equals the sum of the input stream enthalpies.
   *
  ...
  /**
   * Create a dew-point targeting calculator. It sets the output stream temperature to the hydrocarbon dew point of the
   * first input stream at the output stream's current pressure. A small positive margin can be applied to stay above
   * dew point.
   *
  ...
  /**
   * Create an anti-surge recycle calculator with no surge margin. The recycle flow through the splitter's second outlet
   * is adjusted so the compressor inlet actual volumetric flow equals the surge flow from the compressor chart.
   *
   * <p>
   * Input variable 0 must be a {@link Compressor} with a loaded compressor chart. The output variable must be a
   * {@link Splitter} whose split stream index 1 is the recycle line.
   * </p>
   *
  ...
  /**
   * Create an anti-surge recycle calculator with a configurable surge margin factor. The target flow is
   * {@code surgeFlow * surgeMarginFactor}. A factor of 1.1 targets 10 % above surge.
   *
   * <p>
   * All flow comparisons use actual volumetric flow (m3/hr) to match the coordinate system of the compressor chart's
   * surge curve.
   * </p>
   *
