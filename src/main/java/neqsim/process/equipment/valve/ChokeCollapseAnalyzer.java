...
/**
 * Diagnoses <em>choke collapse</em> on a {@link ThrottlingValve} — i.e. transitions out of the critical (choked /
 * sonic) flow regime, plus flashing and cavitation flags for liquid service.
 *
 * <p>
 * Theory:
 * </p>
 * <ul>
 * <li>Gas: critical pressure ratio \( r_c = \left(\tfrac{2}{\gamma+1}\right)^{\gamma/(\gamma-1)} \). When the actual \(
 * r = p_2/p_1 \) drops below \( r_c \) the throat is sonic. If \( r \) rises above \( r_c \) the choke has
 * <em>collapsed</em> back to subcritical and downstream pressure now propagates upstream.</li>
 * <li>Liquid: collapse manifests as <em>flashing</em> (\( p_2 &lt; p_v \)) or <em>cavitation</em> (sigma \( =
 * (p_2-p_v)/(p_1-p_2) \) below threshold).</li>
 * </ul>
 *
 * <p>
 * Heat capacity ratio \(\gamma\) is taken from the inlet gas phase ({@link PhaseInterface#getGamma()}). Inlet/outlet
 * pressures are read in bara.
 * </p>
 *
 * @author esol
 * @version 1.0
...
public class ChokeCollapseAnalyzer implements Serializable {
  ...
  /**
   * Build an analyzer for the given valve. The valve must have been run at least once so that inlet and outlet streams
   * carry valid thermodynamic state.
   *
  ...
  /**
   * Override the downstream pressure used for the analysis (otherwise the valve's current outlet pressure is used).
   *
  ...
  public ChokeCollapseResult analyze() {
    ...
    if (inlet == null || inlet.getThermoSystem() == null || outlet == null || outlet.getThermoSystem() == null) {
      result.getRecommendations().add("Valve has no thermo state — call valve.run() before analysing.");
      return result;
    }
    ...
    if (p2 >= p1) {
      ...
      result.getRecommendations().add("Downstream pressure >= upstream pressure: no driving force across the choke.");
      return result;
    }
  ...
  /**
   * Find the downstream/upstream pressure ratio at which the choke transitions between critical and subcritical flow.
   * For gas service this equals \( r_c \) computed from \(\gamma\); for liquid service NaN is returned.
   *
   ...
   */
  private void analyseGas(SystemInterface sys, double p1, double p2, double r, ChokeCollapseResult result) {
  ...
  /**
   * Liquid-service analysis: flashing and cavitation flags via a bubble-point flash at inlet temperature on a cloned
   * inlet system.
   *
   * @param sys inlet thermodynamic system
   * @param p1 inlet pressure in bara
   ...
   * @param result result object to populate
   */
  private void analyseLiquid(SystemInterface sys, double p1, double p2, ChokeCollapseResult result) {
    double pv = estimateVapourPressureBara(sys);
    if (Double.isNaN(pv) || pv <= 0.0) {
      result.getRecommendations().add("Could not estimate vapour pressure; flashing/cavitation check skipped.");
      result.setFlowRegime(ChokeCollapseResult.FlowRegime.SUBCRITICAL);
      return;
    }
  ...
  /**
   * Compute the analytical critical pressure ratio for an ideal gas with the given heat capacity ratio.
   *
  ...
  /**
   * Estimate vapour pressure of the liquid at inlet temperature using a bubble-point flash on a clone of the system.
