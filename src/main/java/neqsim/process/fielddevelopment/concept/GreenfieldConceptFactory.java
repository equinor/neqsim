 ...
 *
 * <p>
 * The templates give comparable starting assumptions for subsea tiebacks, standalone greenfield facilities,
 * subsea-to-shore developments, onshore terminals, and phased brownfield expansion. They are intended for screening and
 * teaching workflows rather than project sanction estimates.
 * </p>
 *
 * @author ESOL
 * @version 1.0
...
public final class GreenfieldConceptFactory {
  ...
  public static DevelopmentCaseTemplate subseaTieback(String name) {
	...
	.reservoir(ReservoirInput.richGas().resourceEstimate(12.0, "GSm3").recoveryFactor(0.70).build())
	.wells(WellsInput.builder().producerCount(2).ratePerWell(1.2e6, "Sm3/d").tubeheadPressure(120.0).build())
  ...
  public static DevelopmentCaseTemplate standaloneFpso(String name) {
	...
	.reservoir(
	    ReservoirInput.blackOil().resourceEstimate(120.0, "MMbbl").recoveryFactor(0.45).waterCut(0.20).build())
	.wells(WellsInput.builder().producerCount(6).ratePerWell(12000.0, "bbl/d").tubeheadPressure(90.0).build())
	.infrastructure(InfrastructureInput.builder().processingLocation(InfrastructureInput.ProcessingLocation.FPSO)
	    ...
	    .exportType(InfrastructureInput.ExportType.STABILIZED_OIL).waterDepth(700.0).tiebackLength(5.0)
	    .exportPipeline(5.0, 16.0).build())
	.build();
  ...
  public static DevelopmentCaseTemplate fixedPlatform(String name) {
    FieldConcept concept = FieldConcept.builder(name).description("Fixed platform with platform wells and gas export")
	.reservoir(
	    ReservoirInput.blackOil().resourceEstimate(150.0, "MMbbl").recoveryFactor(0.50).waterCut(0.15).build())
	.wells(WellsInput.builder().producerCount(8).completionType(WellsInput.CompletionType.PLATFORM)
	...
	.infrastructure(
		...
		.powerSupply(InfrastructureInput.PowerSupply.GAS_TURBINE)
		.exportType(InfrastructureInput.ExportType.STABILIZED_OIL).waterDepth(120.0).tiebackLength(0.0)
		.exportPipeline(80.0, 20.0).build())
	.build();
  ...
  public static DevelopmentCaseTemplate subseaToShore(String name) {
	...
	.reservoir(ReservoirInput.richGas().resourceEstimate(35.0, "GSm3").recoveryFactor(0.72).co2Percent(2.0).build())
	.wells(WellsInput.builder().producerCount(4).ratePerWell(1.5e6, "Sm3/d").tubeheadPressure(150.0).build())
	.infrastructure(InfrastructureInput.builder().processingLocation(InfrastructureInput.ProcessingLocation.ONSHORE)
	    .powerSupply(InfrastructureInput.PowerSupply.POWER_FROM_SHORE)
	    .exportType(InfrastructureInput.ExportType.WET_GAS).waterDepth(300.0).tiebackLength(120.0)
	    .exportPipeline(120.0, 18.0).insulatedFlowline(true).electricHeating(true).build())
	.build();
  ...
  public static DevelopmentCaseTemplate onshoreTerminal(String name) {
    FieldConcept concept = FieldConcept.builder(name).description("Onshore gas processing terminal with dry gas export")
	.reservoir(ReservoirInput.leanGas().resourceEstimate(45.0, "GSm3").recoveryFactor(0.80).co2Percent(0.8).build())
	.wells(WellsInput.builder().producerCount(5).completionType(WellsInput.CompletionType.ONSHORE)
  ...
  public static DevelopmentCaseTemplate phasedBrownfieldExpansion(String name) {
	...
	.reservoir(
	    ReservoirInput.gasCondensate().resourceEstimate(18.0, "GSm3").recoveryFactor(0.68).co2Percent(1.5).build())
	.wells(WellsInput.builder().producerCount(2).ratePerWell(1.0e6, "Sm3/d").tubeheadPressure(125.0).build())
   ...
   */
  private static DevelopmentCaseTemplate buildTemplate(FieldConcept concept, String caseType, int firstProductionYear,
    ...
    Map<Integer, Double> productionProfile = createProductionProfile(concept, gasConcept, firstProductionYear);
	...
	"%s template, first production %d, development duration %d months, %d process blocks", caseType,
	firstProductionYear, developmentDurationMonths, facilityConfig.getBlocks().size());
    ...
    LifecycleEmissionsProfile lifecycleEmissions = emissionsTracker.estimateLifecycle(concept, facilityConfig,
	productionProfile);
	...
	emissionsReport.getTotalPowerMW(), emissionsReport.getTotalEmissionsTonnesPerYear(), firstProductionYear,
	developmentDurationMonths, economics, assumptions, uncertainty, lifecycleEmissions);
  }
   ...
   */
  private static DevelopmentCaseUncertainty createDefaultUncertainty(FieldConcept concept, boolean gasConcept,
      double totalCapexMusd, int developmentDurationMonths) {
	...
	? new UncertaintyRange("Resource", resourceUnit, reservoir.getResourceP10(), reservoir.getResourceP50(),
	    reservoir.getResourceP90())
	: new UncertaintyRange("Resource", resourceUnit, resourceP50 * 0.75, resourceP50, resourceP50 * 1.25);
    UncertaintyRange capex = new UncertaintyRange("CAPEX", "MUSD", totalCapexMusd * 0.85, totalCapexMusd,
	totalCapexMusd * 1.30);
    UncertaintyRange schedule = new UncertaintyRange("Schedule", "months", developmentDurationMonths * 0.85,
	developmentDurationMonths, developmentDurationMonths * 1.25);
    UncertaintyRange price = gasConcept ? new UncertaintyRange("Gas price", "USD/Sm3", 0.15, 0.25, 0.40)
    ...
    return DevelopmentCaseUncertainty.builder().resource(resource).capex(capex).scheduleMonths(schedule).price(price)
	.productionFactor(production).build();
  }
   ...
   */
  private static Map<Integer, Double> createProductionProfile(FieldConcept concept, boolean gasConcept,
      int firstProductionYear) {
    double peakRatePerDay = gasConcept ? concept.getWells().getRatePerWellSm3d() * concept.getWells().getProducerCount()
	: getOilRateBopd(concept);
    ProductionProfileGenerator generator = new ProductionProfileGenerator();
    return generator.generateFromReservoirInput(concept.getReservoir(), peakRatePerDay, gasConcept, firstProductionYear,
	30);
  }
