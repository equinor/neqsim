 ...
 *
 * <p>
 * A branch connects an upstream node to a downstream node and defines the volumetric flow that
 * passes from the upstream node to the downstream node as a function of the two node pressures. The
 * sign convention is positive when flow goes from the upstream node to the downstream node. The
 * branch hides whatever internal physics it uses (well IPR + VLP, flowline hydraulics, choke,
 * pump/compressor boost) behind a single fast {@link #flow(double, double)} evaluation so that
 * {@link NetworkNewtonSolver} can build a Jacobian cheaply.
 * </p>
 *
 * @author NeqSim
 * @version 1.0
