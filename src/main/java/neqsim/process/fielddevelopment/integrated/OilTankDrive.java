 ...
 *
 * <p>
 * Above the bubble point an oil reservoir behaves as a slightly compressible tank. The pressure
 * decline with cumulative produced volume follows from the total-compressibility material balance:
 * </p>
 *
 * <p>
 * p = p_i - N_p / (N c_t)
 ...
 *
 * <p>
 * where {@code N} is the stock-tank oil in place (STOIIP, Sm3), {@code N_p} is the cumulative oil
 * produced and {@code c_t} is the effective total compressibility expressed per Sm3 of pore-volume
 * basis. To keep inputs intuitive this class is parameterised by a depletion gradient (bar of
 * pressure drop per fraction of STOIIP recovered), which is equivalent to {@code 1/(N c_t)} scaled
 * by {@code N}. The pressure is clamped at a configurable abandonment floor.
 * </p>
 *
 * @author NeqSim
 * @version 1.0
...
public class OilTankDrive implements ReservoirDrive {
   ...
   * @param depletionGradientBarPerFraction pressure drop in bar per unit fraction of STOIIP
   *        produced
   * @param abandonmentPressureBara minimum (abandonment) pressure in bara
   */
  public OilTankDrive(double initialPressureBara, double stoiipSm3,
      double depletionGradientBarPerFraction, double abandonmentPressureBara) {
