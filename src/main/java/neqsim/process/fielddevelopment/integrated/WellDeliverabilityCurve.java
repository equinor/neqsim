 ...
 *
 * <p>
 * Commercial integrated-production tools (Petex GAP, Schlumberger Pipesim) decouple the expensive
 * inflow/lift physics from the network pressure-flow solve by precomputing a deliverability curve
 * (a VFP table reduced to wellhead pressure vs. surface rate). This class plays the same role
 * inside {@link NetworkNewtonSolver}: it stores rate as a strictly decreasing function of wellhead
 * (or sandface) back-pressure so the network solver can evaluate flows and derivatives cheaply and
 * robustly, without re-running a full {@code WellSystem} flash inside every Jacobian perturbation.
 * </p>
 *
 * <h2>Conventions</h2>
 * <ul>
 ...
 * <li>Below the lowest tabulated pressure the curve is extrapolated towards the absolute open-flow
 * potential (AOFP).</li>
 * </ul>
 *
 * <h2>Construction</h2>
 * <ul>
 * <li>{@link #fromArrays(double[], double[])} - from sampled or measured (pressure, rate)
 * pairs.</li>
 * <li>{@link #fromVogel(double, double)} - from an absolute open-flow potential and shut-in
 * pressure using a Vogel-like quadratic shape.</li>
 * <li>{@link #fromWellSystem(neqsim.process.equipment.reservoir.WellSystem, double, double, int)} -
 * by sampling a fully configured integrated well model.</li>
 * </ul>
 *
 * @author NeqSim
 * @version 1.0
...
public class WellDeliverabilityCurve implements Serializable {
   ...
   *
   * <p>
   * Uses the Vogel inflow shape q/qmax = 1 - 0.2 (p/ps) - 0.8 (p/ps)^2 mapped to wellhead
   * back-pressure, producing a smooth strictly decreasing curve that is convenient for screening
   * studies and tests.
   * </p>
   *
  ...
  /**
   * Samples a configured {@link neqsim.process.equipment.reservoir.WellSystem} to build a
   * deliverability curve.
   *
   * <p>
   * The well is run at a range of wellhead pressures and the resulting operating rate recorded. The
   * full IPR + VLP physics is therefore captured once, then reused cheaply by the network solver.
   * Non-converged or non-physical samples are clamped to zero.
   * </p>
   *
   ...
   */
  public static WellDeliverabilityCurve fromWellSystem(
      neqsim.process.equipment.reservoir.WellSystem well, double minWellheadPressureBara,
      double maxWellheadPressureBara, int nPoints) {
    if (nPoints < 2) {
    ...
    for (int i = 0; i < nPoints; i++) {
      ...
      double whp =
          minWellheadPressureBara + frac * (maxWellheadPressureBara - minWellheadPressureBara);
      p[i] = whp;
