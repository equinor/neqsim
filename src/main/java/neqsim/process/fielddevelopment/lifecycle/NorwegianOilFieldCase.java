 ...
 *
 * <p>
 * The case is synthetic and must not be interpreted as data for a named field. It represents a medium-sized subsea
 * oil development tied to an FPSO in approximately 300 m water depth. The model includes a compositional PR-EOS PVT
...
public final class NorwegianOilFieldCase {
   ...
   */
  public static FieldLifecycleConcept createCase(String name, double recycleFraction,
      double maximumInjectionRateSm3d) {
    ...
    FieldLifecycleConfiguration configuration = FieldLifecycleConfiguration.builder().startYear(firstOilYear)
        ...
        .minimumBottomHolePressure(250.0).plateauOilRate(23000.0)
        .facilityCapacities(42000.0, 5.5e6, 28000.0).economicLimitOilRate(600.0)
        .waterCut(0.04, 0.78, 3.0, 15.0).standardDensities(835.0, 1025.0)
        .gasInjection(0.0, recycleFraction, gasInjectionCapacity).gridEmissionFactor(0.018)
        .prices(75.0, 0.28).discountRate(0.08).opex(135.0, 8.5).tariffs(2.0, 0.015)
        .capex(firstOilYear - 3, totalCapexMusd * 0.20).capex(firstOilYear - 2, totalCapexMusd * 0.50)
        .capex(firstOilYear - 1, totalCapexMusd * 0.30).build();
  ...
  private static FieldConcept createScreeningConcept(String name) {
    ReservoirInput reservoir = ReservoirInput.blackOil().gor(180.0, "Sm3/Sm3").waterCut(0.04)
        ...
        .resourceUncertainty(550.0, 700.0, 850.0, "MMbbl").recoveryFactor(0.48).build();
    WellsInput wells = WellsInput.builder().producerCount(PRODUCERS).injectorCount(GAS_INJECTORS)
        .tubeheadPressure(85.0).ratePerWell(23000.0 / PRODUCERS, "Sm3/day").productivityIndex(55.0).build();
    InfrastructureInput infrastructure = InfrastructureInput.builder().processingLocation(
        InfrastructureInput.ProcessingLocation.FPSO).waterDepth(WATER_DEPTH_M).tiebackLength(12.0)
        .exportType(InfrastructureInput.ExportType.STABILIZED_OIL).powerSupply(
            InfrastructureInput.PowerSupply.POWER_FROM_SHORE).exportPressure(180.0).build();
  ...
  private static FieldLifecycleModel createModel(String name) {
    ...
    Splitter gasAllocation = new Splitter("Gas export and injection allocation",
        recoveredGasMixer.getOutletStream(), 2);
    ...
    Compressor injectionStageOne = new Compressor("Gas injection compressor stage 1",
        gasAllocation.getSplitStream(1));
  ...
  private static double estimateGasInjectionCapacity() {
    InjectionWellModel injectionWell = new InjectionWellModel(InjectionType.GAS_INJECTOR)
        ...
        .setFormationPermeability(180.0, "mD").setFormationThickness(45.0, "m").setSkinFactor(2.0)
        .setWellDepth(3100.0, "m").setTubingID(0.127, "m").setMaxBHP(390.0, "bara")
        .setFracturePressure(410.0, "bara").setSurfaceInjectionPressure(350.0, "bara");
  ...
  private static double estimateDevelopmentCapexMusd() {
    ...
    producerCost.calculateWellCost("OIL_PRODUCER", "SEMI_SUBMERSIBLE", "SMART_COMPLETION", 3600.0,
        WATER_DEPTH_M, 75.0, 35.0, 0.0, true, 5);
    WellCostEstimator injectorCost = new WellCostEstimator(SubseaCostEstimator.Region.NORWAY);
    injectorCost.calculateWellCost("GAS_INJECTOR", "SEMI_SUBMERSIBLE", "STANDARD", 3400.0, WATER_DEPTH_M,
        65.0, 25.0, 0.0, true, 5);
    double wellsMusd = (PRODUCERS * producerCost.getTotalCost() + GAS_INJECTORS * injectorCost.getTotalCost())
        / 1.0e6;
    ...
    return wellsMusd + surfMusd + fpsoAndTopsidesMusd + gasInjectionAndExportMusd
        + projectAndContingencyMusd;
  }
}
