 ...
 *
 * <p>
 * Determines the required instrumentation for a separator vessel following typical oil and gas practice. A separator
 * typically requires:
 * </p>
 * <ul>
 * <li>Pressure: 2x PT (voted/redundant), 1x PSH (high pressure switch, SIL-rated)</li>
 * <li>Temperature: 1x TT (process temperature)</li>
 * <li>Level: 2x LT (voted/redundant), 1x LSH (high level switch), 1x LSLL (low-low level switch, SIL-rated)</li>
...
public class SeparatorInstrumentDesign extends InstrumentDesign {
  ...
  public void calcDesign() {
    ...
    getInstrumentList().add(new InstrumentSpecification("LT", "Liquid Level", 0.0, 100.0, "%", "AI"));
    // LT-x002: Liquid level (redundant)
    getInstrumentList().add(new InstrumentSpecification("LT", "Liquid Level (Redundant)", 0.0, 100.0, "%", "AI"));
    // LSH: High level switch (safety)
    if (isIncludeSafetyInstruments()) {
      getInstrumentList().add(new InstrumentSpecification("LSH", "High Level Switch", "DI", getDefaultSilLevel()));
    }
    // LSLL: Low-low level switch (safety - pump protection)
    if (isIncludeSafetyInstruments()) {
      getInstrumentList().add(new InstrumentSpecification("LSLL", "Low-Low Level Switch", "DI", getDefaultSilLevel()));
    }

    // === Three-phase separator additions ===
    if (threePhase) {
      // LT: Water/oil interface level
      getInstrumentList().add(new InstrumentSpecification("LT", "Water/Oil Interface Level", 0.0, 100.0, "%", "AI"));
      // LCV: Water dump valve feedback
      getInstrumentList().add(new InstrumentSpecification("ZT", "Water Dump Valve Position", 0.0, 100.0, "%", "AI"));
    }
