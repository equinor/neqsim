...
public class LNGProcessBuilder {
   ...
   *
   * <p>
   * The stream is retained by reference rather than cloned, so its composition, flow, temperature, and pressure remain
   * connected to the surrounding simulation. When the producing equipment belongs to another {@link ProcessSystem}, run
   * that upstream system before this model. Use {@link #setUpstreamProcess(ProcessSystem, StreamInterface)} when the
   * upstream equipment and LNG train should execute and participate in capacity analysis as one process system.
   * </p>
   *
   ...
   *
   * <p>
   * The supplied process system is extended in place with the selected LNG route, and the supplied stream becomes the
   * live LNG feed. This allows inlet pipelines, pretreatment, fractionation, controls, measurement devices, and other
   * NeqSim functionality to be evaluated in the same flowsheet and capacity analysis. The feed stream should be an
   * outlet of equipment already registered in {@code upstreamProcessSystem}.
   * </p>
   *
   * <p>
   * When this integration is used, the feed flow, temperature, and pressure are governed by the upstream process rather
   * than the corresponding builder defaults.
   * </p>
   *
   ...
   * @return this builder
   */
  public LNGProcessBuilder setUpstreamProcess(ProcessSystem upstreamProcessSystem, StreamInterface upstreamFeedStream) {
  ...
  private BuildContext newContext() {
    ProcessSystem process = upstreamProcessSystem == null ? new ProcessSystem(name) : upstreamProcessSystem;
  ...
  private CompressionTrain addTwoStageCompression(BuildContext context, String unitPrefix, StreamInterface suction,
    ...
    Separator suctionScrubber = new Separator(unitPrefix + " compressor suction scrubber", suction);
    context.process.add(suctionScrubber);

    Compressor first = new Compressor(unitPrefix + " compressor stage 1", suctionScrubber.getGasOutStream());
   ...
   */
  private ProductSection addProductSection(BuildContext context, StreamInterface coldNaturalGas) {
    ...
    return new ProductSection(productFlash.getLiquidOutStream(), productFlash.getGasOutStream());
  }
  ...
  private final class BuildContext {
     ...
     */
    private BuildContext(ProcessSystem process, StreamInterface feed, double feedFlowKgPerHour) {
     ...
     */
    private LNGProcessModel toModel(LNGProcessCycle builtCycle, ProductSection product) {
      return new LNGProcessModel(name, builtCycle, process, feed, product.lng, product.flashGas, compressors, expanders,
          exchangers);
    }
  }
}
