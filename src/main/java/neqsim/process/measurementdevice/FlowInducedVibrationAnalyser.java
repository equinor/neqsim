...
public class FlowInducedVibrationAnalyser extends MeasurementDeviceBaseClass {
   ...
   *
   * <p>
   * The LOF correlation classifies the support arrangement qualitatively (a measure of overall
   * stiffness) rather than by a precise support spacing. The order below runs from the stiffest
   * (lowest predicted vibration) to the most flexible (highest predicted vibration).
   * </p>
   */
  public static final String[] VALID_SUPPORT_ARRANGEMENTS =
      new String[] {"Stiff", "Medium stiff", "Medium", "Flexible"};

  /**
   * Informational support spacing in metres.
   *
   * <p>
   * <b>Not used by the LOF or FRMS correlations.</b> The Energy Institute LOF model selects its
   * coefficients from the qualitative {@link #supportArrangement} category, so this value is kept
   * only for documentation / reporting (e.g. an as-built spacing from a STID line list). To obtain
   * a recommended support spacing use
   ...
   * {@inheritDoc}
   *
   * @throws IllegalStateException if the LOF method is requested but the pipe wall thickness is not
   *         a positive number. The LOF correlation divides by the wall thickness (via the
   *         diameter-over-thickness ratio), so a zero or unset thickness would otherwise yield a
   *         silent {@code NaN}/{@code Infinity}.
   */
  @Override
    ...
    if (method.equals("LOF")) {
      if (!(pipe.getThickness() > 0.0)) {
        throw new IllegalStateException("FlowInducedVibrationAnalyser '" + getName()
            + "': the LOF correlation requires a positive pipe wall thickness, but pipe '"
            + pipe.getName() + "' has thickness " + pipe.getThickness()
            + " m. Set it with pipe.setThickness(wallThickness_m) "
      ...
      return C * Math.pow(pipe.getDiameter(), 1.6)
   ...
   *
   * <p>
   * The Energy Institute LOF model selects its coefficients from a qualitative support-arrangement
   * category, not from a precise support spacing. Accepted values (case-insensitive, ignoring
   * surrounding whitespace) are those in {@link #VALID_SUPPORT_ARRANGEMENTS}: {@code "Stiff"},
   * {@code "Medium stiff"}, {@code "Medium"} and {@code "Flexible"}. The value is normalised to its
   * canonical spelling.
   * </p>
   *
   * @param arrangement a {@link java.lang.String} object
   * @throws IllegalArgumentException if {@code arrangement} is null or not one of the accepted
   *         categories
   */
  public void setSupportArrangement(String arrangement) {
    ...
    throw new IllegalArgumentException("supportArrangement '" + arrangement
        + "' is not recognised. Valid values: " + validArrangementsString());
  }
   ...
   *
   * <p>
   * <b>Informational only.</b> This value is not used by the LOF or FRMS correlations; the LOF
   * model uses the qualitative {@link #setSupportArrangement(String) support arrangement} instead.
   * Use this field to record an as-built spacing (e.g. from a STID line list) for reporting.
   * </p>
   *
