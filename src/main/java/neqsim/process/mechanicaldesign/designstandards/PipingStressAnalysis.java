 ...
 *
 * <p>
 * Provides calculations for thermal expansion stress, pipe support spacing (span tables), sustained stress checks, and
 * flexibility analysis. These are screening-level calculations suitable for preliminary design; detailed analysis
 * should use dedicated piping stress software.
 * </p>
 *
 * <p>
 * References:
...
public class PipingStressAnalysis implements Serializable {
   ...
   *
   * <p>
   * Based on simple-beam deflection formula with a maximum allowable midspan deflection of 3mm and bending stress
  ...
  public static double maxSupportSpan(double pipeODMm, double wallThicknessMm, double contentDensityKgM3,
      double insulationThicknessMm, double insulationDensityKgM3, double maxDeflectionMm, double allowableStressMPa) {
    ...
    double insWeightPerM = insulationDensityKgM3 * Math.PI * (insOdM * insOdM - odM * odM) / 4.0 * G;
    ...
    // L = sqrt(8 * sigma * Z / w)
    double stressLimitedSpan = Math.sqrt(8.0 * allowableStressMPa * 1.0e6 * (zMm3 * 1.0e-9) / totalWeightPerM);
   ...
   */
  public static boolean codeStressCheck(double sustainedStressMPa, double expansionStressMPa, double allowableHotMPa,
      double allowableExpansionMPa) {
