 ...
 *
 * <p>
 * Covers acoustic-induced vibration (AIV) screening for piping downstream of pressure-let-down devices, flow-induced
 * vibration (FIV) risk in heat exchangers, and pulsation screening for reciprocating compressors per API 618.
 * </p>
 *
 * <p>
 * References:
 * </p>
 * <ul>
 * <li>Energy Institute: Guidelines for the avoidance of vibration induced fatigue failure (2008)</li>
...
public class VibrationAssessment implements Serializable {
   ...
   *
   * <p>
   * Calculates the acoustic power level downstream of a pressure-let-down device and compares it to the pipe's
   ...
   *
   * <p>
   * Based on Energy Institute method: PWL = 126.1 + 10*log10(W * eta) where W is the mechanical stream power and eta is
  ...
  public static double acousticPowerLevel(double massFlowKgS, double upstreamPressureBar, double downstreamPressureBar,
      double temperatureK, double molecularWeight) {
   ...
   *
   * <p>
   * Energy Institute correlation: L_screen = 173.6 + 10*log10(D * t^2.5) where D is outer diameter in inches and t is
   ...
   *
   * <p>
   * Checks critical velocity for fluid-elastic instability per TEMA RCB-4.6. Risk is HIGH if cross-flow velocity
  ...
  public static VibrationRisk fivHeatExchangerScreening(double crossFlowVelocityMS, double tubePitchMm, double tubeODMm,
      double tubeNaturalFreqHz, double shellFluidDensityKgM3, double tubeMassPerLengthKgM) {
  ...
  public static VibrationRisk reciprocatingPulsationScreening(double dischargePressureBar, double pistonSpeedMps,
      double cylinderVolumeLiters, double pipeDiameterMm) {
