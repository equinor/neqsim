 ...
 *
 * <p>
 * The Bell-Delaware method is an industry-standard approach that corrects the ideal tube-bank heat transfer coefficient
 * using geometry-specific correction factors. It accounts for baffle window flow, baffle leakage, bundle bypass,
 * unequal baffle spacing, and adverse temperature gradients.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * Reference: Bell, K.J. (1981), "Delaware Method for Shell-Side Design", in Heat Exchangers: Thermal-Hydraulic
 * Fundamentals and Design, Hemisphere Publishing.
 * </p>
 *
...
public final class BellDelawareMethod {
   ...
   */
  public static double calcIdealCrossflowHTC(double massFlux, double tubeOD, double mu, double cp, double k,
      double muWall, double triangularPitch) {
  ...
  public static double calcKernShellSidePressureDrop(double massFlux, double shellEquivDiameter, double shellDiameter,
      int baffleCount, double rho, double mu, double muWall) {
    ...
    return f * shellDiameter * (baffleCount + 1) * rho * velocity * velocity / (2.0 * shellEquivDiameter * viscCorr);
  }
  ...
  public static double calcShellEquivDiameter(double tubeOD, double tubePitch, boolean triangularPitch) {
    ...
    if (triangularPitch) {
      ...
      double flowArea = Math.sqrt(3.0) / 4.0 * tubePitch * tubePitch - Math.PI / 8.0 * tubeOD * tubeOD;
   ...
   *
   * <p>
   * J_c accounts for the heat transfer in the baffle window zone compared to the crossflow zone. Typical range: 0.65 to
   * 1.15.
   * </p>
   *
   ...
   *
   * <p>
   * J_l accounts for fluid bypassing through the tube-to-baffle and shell-to-baffle clearances. Typical range: 0.6 to
   * 0.9.
   * </p>
   *
  ...
  public static double calcJl(double tubeToBaffleClearance, double shellToBaffleClearance, double crossflowArea,
      int tubeCount, double tubeOD, double baffleSpacing) {
    if (crossflowArea <= 0) {
      return 0.7;
    }
    ...
    double shellLeakageFraction = shellBaffleArea / Math.max(tubeBaffleArea + shellBaffleArea, 1e-10);
   ...
   */
  public static double calcJb(double bypassArea, double crossflowArea, boolean hasSealing, int sealingPairs,
      int tubeRowsCrossflow) {
    if (crossflowArea <= 0) {
      return 0.8;
    }
   ...
   *
   * <p>
   * J_r accounts for the reduction in heat transfer in laminar flow due to adverse temperature gradients across the
   ...
   */
  public static double calcRb(double bypassArea, double crossflowArea, boolean hasSealing, int sealingPairs,
      int tubeRowsCrossflow) {
    if (crossflowArea <= 0) {
      return 0.7;
    }
