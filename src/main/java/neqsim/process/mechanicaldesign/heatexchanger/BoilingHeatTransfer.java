 ...
 * </p>
 * <ul>
 * <li><b>Chen (1966):</b> Superposition model combining forced-convective and nucleate boiling with a suppression
 * factor S and enhancement factor F.</li>
 * <li><b>Gungor-Winterton (1986, 1987):</b> Enhanced Chen-type correlation with improved enhancement and suppression
 * factor fitting against a larger database (3,600+ data points).</li>
 * </ul>
 *
 * <p>
 ...
 *
 * <p>
 * where h_mac is the forced-convective (macroscopic) component from Dittus-Boelter at liquid-only conditions, and h_mic
 * is the nucleate boiling (microscopic) component from Forster-Zuber. F enhances the convective part due to two-phase
 * flow acceleration, and S suppresses nucleate boiling at high mixture velocities.
 * </p>
 *
 * <p>
 ...
 * </p>
 * <ul>
 * <li>Chen, J.C. (1966). "Correlation for boiling heat transfer to saturated fluids in convective flow." I&amp;EC
 * Process Design and Development, 5(3), 322-329.</li>
 * <li>Gungor, K.E. and Winterton, R.H.S. (1986). "A general correlation for flow boiling in tubes and annuli." Int. J.
 * Heat Mass Transfer, 29(3), 351-358.</li>
 * <li>Gungor, K.E. and Winterton, R.H.S. (1987). "Simplified general correlation for saturated flow boiling and
 * comparisons of correlations with data." Chem. Eng. Res. Des., 65, 148-156.</li>
 * </ul>
 *
...
public final class BoilingHeatTransfer {
   ...
   * <li>h_mic = 0.00122 * (k_l^0.79 * Cp_l^0.45 * rho_l^0.49) / (sigma^0.5 * mu_l^0.29 * h_fg^0.24 * rho_v^0.24) *
   * dT_sat^0.24 * dP_sat^0.75 (Forster-Zuber)</li>
  ...
  public static double calcChenHTC(double massFlux, double vaporQuality, double tubeID, double liquidDensity,
      double vaporDensity, double liquidViscosity, double vaporViscosity, double liquidCp, double liquidConductivity,
      double surfaceTension, double heatOfVaporization, double wallSuperheat, double satPressureDiff) {
    ...
    double h_l = 0.023 * Math.pow(Math.max(Re_l, 1.0), 0.8) * Math.pow(Pr_l, 0.4) * liquidConductivity / tubeID;

    // Martinelli parameter X_tt
    double X_tt = calcMartinelliParameter(x, liquidDensity, vaporDensity, liquidViscosity, vaporViscosity);
   ...
   * @return local two-phase boiling HTC (W/(m2*K))
   */
  public static double calcGungorWintertonHTC(double massFlux, double vaporQuality, double tubeID, double liquidDensity,
      double vaporDensity, double liquidViscosity, double liquidCp, double liquidConductivity, double heatFlux,
    ...
    // Boiling number
    double Bo = (heatOfVaporization > 0 && massFlux > 0) ? heatFlux / (massFlux * heatOfVaporization) : 0.0;

    // Convective enhancement
    double convectiveEnhancement = 0.0;
    if (x < 0.999 && vaporDensity > 0) {
      convectiveEnhancement = 1.12 * Math.pow(x / (1.0 - x), 0.75) * Math.pow(liquidDensity / vaporDensity, 0.41);
    }
   ...
   *
   * <p>
   * For horizontal tubes where Froude number Fr_l &lt; 0.05, applies a stratified flow correction. For subcooled
  ...
  public static double calcGungorWintertonCorrectedHTC(double massFlux, double vaporQuality, double tubeID,
      double liquidDensity, double vaporDensity, double liquidViscosity, double liquidCp, double liquidConductivity,
  ...
  public static double calcAverageHTC(double massFlux, double tubeID, double liquidDensity, double vaporDensity,
      double liquidViscosity, double liquidCp, double liquidConductivity, double heatFlux, double heatOfVaporization,
      double qualityIn, double qualityOut, int intervals) {
