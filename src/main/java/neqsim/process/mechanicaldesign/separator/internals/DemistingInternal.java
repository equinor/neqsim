 ...
 * </p>
 * 
 * Represents a deisting internal (without drainage) for separators. The DemistingInternal reduces
 * liquid carry-over by providing a surface area where droplets can coalesce and settle.
 *
 * @author User
 * @version 1.0
 */
public class DemistingInternal implements Serializable {
  ...
  /**
   * Internal surface area in m². If there are several cyclones, this is cyclone area * number
   */
   ...
   * 
   * Velocity is calculated as: v = Q / A where Q is the volumetric gas flow rate and A is the
   * internal area.
   *
   ...
   * 
   * Pressure drop is calculated using the Euler number: Δp = Eu * ρ * v² / 2 where Eu is the Euler
   ...
   * 
   * Liquid carry-over is the mass of liquid droplets that pass through the internal per unit time.
   * This is a simplified model based on the Souders-Brown equation modified for deisting internals.
   * 
   * Carry-over is reduced by the internal efficiency factor which increases with area.
   *
   * @return liquid carry-over reduction factor (0 to 1, where 1 means no reduction)
   */
  public double calcLiquidCarryOver() {
    ...
    // This is a simplified model: carry-over factor decreases exponentially with area
