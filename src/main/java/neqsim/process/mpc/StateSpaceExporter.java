 ...
 *
 * <p>
 * The StateSpaceExporter converts linearization results into standard state-space representation (A, B, C, D matrices)
 * and exports them in various formats for integration with industrial MPC packages and control libraries.
 * </p>
 *
 * <p>
...
public class StateSpaceExporter implements Serializable {
   ...
   *
   * <p>
   * For a first-order system with gain K and time constant τ, the discrete-time state-space representation with sample
   * time Ts is:
   * </p>
   *
   * <pre>
  ...
  public void exportMATLAB(String filename) throws IOException {
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(filename))) {
      ...
      writer.write("%\n");
      writer.write(String.format("%% Sample Time: %.4f seconds\n", stateSpaceModel.getSampleTime()));
      writer.write("%\n\n");
