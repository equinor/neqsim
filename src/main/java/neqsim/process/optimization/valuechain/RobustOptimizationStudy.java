 ...
 *
 * <p>
 * A setpoint that is optimal for the nominal case can be fragile: small shifts in feed composition,
 * reservoir pressure or ambient temperature may erode its value or push the plant infeasible. This
 * study couples a Monte-Carlo scenario set with an objective evaluator to report the value
 * distribution (P10/P50/P90, mean) and the fraction of scenarios in which a decision stays
 * feasible. It can also pick, from a set of candidate decisions, the one that maximises the median
 * value subject to a feasibility-confidence requirement — i.e. a chance-constrained selection such
 * as "maximise P50 value while remaining feasible in at least 90% of scenarios".
 * </p>
 *
 * <p>
 * Scenarios may be supplied explicitly (deterministic, reproducible) or drawn from a
 * {@link ScenarioSampler}. Outcomes are produced by a {@link ScenarioEvaluator} that, for a given
 * decision and scenario, returns the objective and whether the plant is feasible (typically by
 * configuring and running a NeqSim flowsheet and pricing it with {@link ValueChainObjective}).
 * </p>
 *
...
public class RobustOptimizationStudy implements Serializable {
  ...
  public static class RobustResult implements Serializable {
     ...
     */
    public RobustResult(double[] decision, double p10, double p50, double p90, double mean,
  ...
  public RobustResult evaluateDecision(double[] decision, ScenarioEvaluator evaluator) {
    ...
    if (objectives.isEmpty()) {
      return new RobustResult(decision, Double.NaN, Double.NaN, Double.NaN, Double.NaN,
          feasibleFraction);
    }
    ...
    return new RobustResult(decision, percentile(sorted, 10.0), percentile(sorted, 50.0),
        percentile(sorted, 90.0), mean, feasibleFraction);
  }

  /**
   * Selects, from a set of candidate decisions, the one maximising the median objective while
   * meeting the feasibility-confidence requirement.
   *
   * <p>
   * If no candidate meets the confidence requirement, the candidate with the highest feasible
   * fraction is returned as a fallback.
   * </p>
   *
