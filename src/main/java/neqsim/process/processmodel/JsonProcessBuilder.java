 ...
 *
 * <p>
 * Supports declarative definition of fluids, streams, and equipment with automatic stream wiring by name reference.
 * Designed for web API integration where clients submit JSON process definitions and receive simulation results.
 * </p>
 *
 * <h2>JSON Format:</h2>
 *
...
public class JsonProcessBuilder {
   ...
   *
   * <p>
   * When {@code preBuiltFluid} is non-null, it is used as the default fluid for all streams instead of parsing the
  ...
  public SimulationResult buildFromJsonObject(JsonObject root, SystemInterface preBuiltFluid) {
    ...
    for (String removeName : toRemove) {
      warnings.add("Removed Mixer '" + removeName + "' — outlet stream was null (inlet stream not initialised)");
  ...
  private JsonObject applyRootDesignData(ProcessSystem process, JsonObject root) {
    ...
    if (root.has("designCapacities") && root.get("designCapacities").isJsonObject()) {
      report.add("designCapacities", applyDesignCapacities(process, root.getAsJsonObject("designCapacities")));
    }
    if (root.has("equipmentDesign") && root.get("equipmentDesign").isJsonObject()) {
      report.add("equipmentDesign", applyEquipmentDesignSection(process, root.getAsJsonObject("equipmentDesign")));
    }
    return report;
  }
   ...
   */
  private JsonObject applyEquipmentDesignSection(ProcessSystem process, JsonObject equipmentDesign) {
    JsonObject report = new JsonObject();
    JsonObject generatedDesignCapacities = new JsonObject();

    if (equipmentDesign.has("valveSizingGroups") && equipmentDesign.get("valveSizingGroups").isJsonArray()) {
      report.add("valveSizingGroups", applyValveSizingGroups(equipmentDesign.getAsJsonArray("valveSizingGroups")));
    }
  ...
  private JsonObject createCompressorDriverProperties(JsonObject group) {
    ...
    if (group.has("maxPowerCurveCoefficients") && group.get("maxPowerCurveCoefficients").isJsonArray()) {
      driverProps.add("maxPowerCurveCoefficients", group.get("maxPowerCurveCoefficients").deepCopy());
    }
  ...
  private void addCapacityDataForUnitName(JsonObject designCapacities, JsonObject group, JsonObject capacityProps) {
    ...
    designCapacities.add(group.get("unitName").getAsString(), JsonParser.parseString(capacityProps.toString()));
  }
  ...
  /**
   * Attempts to wire all inlet references for a unit. Returns true only if ALL references resolve successfully,
   ...
   */
  @Deprecated
  private void buildUnit(ProcessSystem process, JsonObject unitDef, SystemInterface defaultFluid, int index) {
   ...
   */
  private ProcessEquipmentInterface createAndWireUnit(ProcessSystem process, String type, String name,
      JsonObject unitDef, SystemInterface defaultFluid) {
   ...
   */
  private ProcessEquipmentInterface createStream(String name, JsonObject unitDef, SystemInterface defaultFluid) {
  ...
  private void applyCompressorDriver(Compressor compressor, JsonObject driverJson) {
    try {
      ...
      double ratedPower = readPowerValue(driverJson, "ratedPower", new CompressorDriver(driverType).getRatedPower());
      ...
      warnings.add("Error applying driver on " + compressor.getName() + ": " + exception.getMessage());
    }
  }
  ...
  private void applyDriverScalarProperties(CompressorDriver driver, JsonObject driverJson) {
    ...
    if (driverJson.has("overloadProtectionEnabled")) {
      driver.setOverloadProtectionEnabled(driverJson.get("overloadProtectionEnabled").getAsBoolean());
    }
  ...
  private void applyAntiSurge(Compressor compressor, JsonObject antiSurgeJson) {
    ...
    if (antiSurgeJson.has("surgeControlLineOffset")) {
      antiSurge.setSurgeControlLineOffset(antiSurgeJson.get("surgeControlLineOffset").getAsDouble());
    }
   ...
   *
   * <p>
   * The Adjuster modifies one equipment property (adjusted variable) to achieve a target value on another equipment
   * property (target variable). JSON format:
   * </p>
   *
   * <pre>
   * "properties": {
   ...
   *
   * <p>
   * Each array element is an object with keys: value, specType, specifiedStream, phaseFrom, phaseTo. These map directly
  ...
  private void applyPropertyObject(Object target, String targetName, String propName, JsonElement value) {
   ...
   * {@link neqsim.thermo.util.readwrite.EclipseFluidReadWrite}) and should be used instead of the fluid definition in
