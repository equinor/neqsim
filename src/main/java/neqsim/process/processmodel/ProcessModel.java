...
public class ProcessModel implements Runnable, Serializable {
  ...
  private static final class AreaExecutionPlan {
    ...
    /** Identity-distinct streams entering the complete model from outside its process areas. */
    private final List<StreamInterface> externalFeedStreams;
     ...
     * @param externalFeedStreams streams entering the complete model from outside its areas
    ...
    private AreaExecutionPlan(List<List<ProcessSystem>> levels,
        ...
        List<StreamInterface> externalFeedStreams, Map<ProcessSystem, Long> structureVersions) {
      this.levels = levels;
      this.successors = successors;
      this.boundaryStreams = boundaryStreams;
      this.boundaryConsumers = boundaryConsumers;
      this.streamProducers = streamProducers;
      this.externalFeedStreams = externalFeedStreams;
  ...
  private AreaExecutionPlan buildAreaExecutionPlan() {
    ...
    if (n == 0) {
      return new AreaExecutionPlan(new ArrayList<>(), successorMap, boundaryStreams, boundaryConsumers, streamProducers,
          externalFeedStreams, structureVersions);
    }
  ...
  public double getTotalFeedFlowRate() {
    double total = 0.0;
    for (StreamInterface stream : getAreaExecutionPlan().externalFeedStreams) {
      try {
        double flow = stream.getFlowRate("kg/hr");
