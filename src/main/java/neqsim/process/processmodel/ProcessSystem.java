...
public class ProcessSystem extends SimulationBaseClass {
  ...
  /**
   * Returns true if the given graph node represents multi-input equipment.
   * Used by {@link #groupNodesBySharedInputStreams(List)} to decide whether
   * shared-stream consumers need to be serialised within a group.
   *
   * @param node the graph node
   * @return {@code true} if the underlying equipment has 2+ inlet streams or
   *         is one of the class-based multi-input types
   */
   ...
   *
   * <p>
   * Useful for agents or developers who expect full parallel execution and want
   * to know which specific unit(s) forced sequential or hybrid mode. Returns a
   * multi-line string with the selected strategy and the equipment blocking
   * higher parallelism (if any).
   * </p>
   *
   ...
   *
   * <p>
   * Unlike {@link #runParallel(UUID)} which uses level barriers, this method
   * fires each unit (or shared-input group) as soon as its direct predecessors
   * have completed. Eliminates straggler penalty on wide asymmetric flowsheets
   * where a slow unit on one branch currently delays unrelated units on other
   * branches.
   * </p>
   *
   * <p>
   ...
   * </p>
   * <ul>
   * <li>Units sharing an input stream are collapsed into a single sequential
   * task via {@link #groupNodesBySharedInputStreams(List)}.</li>
   * <li>Each task starts only after its external predecessor tasks complete.</li>
   * <li>Setter units run sequentially first (as in
   * {@link #runParallel(UUID)}).</li>
   * </ul>
   *
   * @param id calculation identifier for tracking
   * @throws InterruptedException if the thread is interrupted while waiting for
  ...
  public synchronized void runDataflow(UUID id) throws InterruptedException {
    ...
    for (int i = 0; i < tasks.size(); i++) {
      ...
      if (preds.isEmpty()) {
        ...
        future = java.util.concurrent.CompletableFuture.allOf(predArr)
            .thenRunAsync(body, executor);
      }
   ...
   * {@link #invalidateGraph()} but named to reflect the general contract: any
   * mutation of {@code unitOperations} (add/remove/replace/clear) MUST call
   * this method. Keeping the invalidation in one place prevents the common
   * bug of adding a new cache field and forgetting to clear it at one of the
   * mutation sites.
   */
  ...
  /**
   * Apply an acceleration method to every {@link Recycle} unit in this
   * {@code ProcessSystem}. Convenience wrapper that avoids iterating and
   * configuring each {@code Recycle} individually.
   *
   * <p>
   * Wegstein acceleration (see
   * {@link neqsim.process.equipment.util.AccelerationMethod#WEGSTEIN}) typically
   * reduces outer-loop iteration count by 2-3x on single-variable recycle
   * loops and is a safe default for most flowsheets. Broyden's method is more
   * effective for tightly coupled multi-recycle systems.
   * </p>
   *
  ...
  /**
   * Reorder the iterative section so that within each non-trivial SCC the
   * {@link Recycle} units appear <em>after</em> their SCC peers. Relative
   * order of non-Recycle units is preserved exactly (stable), and the
   * position of each SCC block relative to units outside the SCC is
   * preserved (anchored at the first member in insertion order).
   *
   * <p>
   * This is a safe, conservative refinement of the insertion-order iterative
   * section used by {@link #runHybrid(UUID)}. It only moves {@code Recycle}
   * units within their own SCC block, never across SCC boundaries or across
   * non-SCC units, so it cannot change the fixed point of successive
   * substitution. For flowsheets that already follow the common convention
   * of adding {@code Recycle} units last, this method is a no-op.
   * </p>
   *
