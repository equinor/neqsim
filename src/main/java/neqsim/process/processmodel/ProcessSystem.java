...
public class ProcessSystem extends SimulationBaseClass {
   ...
   * does not leak into other code on the same thread. Default is false to preserve historical
   * behaviour. Recycle loops where the same fluid is re-flashed at near-identical conditions
   * benefit most (~10–20% wall-time reduction in benchmark tests). Enable via
   * {@link #setUseFlashWarmStart(boolean)}.
   */
