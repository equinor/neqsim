...
/**
 * Factory that converts {@link DexpiProcessUnit} placeholders into runnable NeqSim process equipment by matching the
 * {@link neqsim.process.equipment.EquipmentEnum} type and applying sizing attributes from DEXPI GenericAttributes.
 *
 * <p>
 * This factory is used by {@link DexpiSimulationBuilder} during the topology walk to instantiate real equipment objects
 * (Separator, Compressor, Valve, etc.) from the DEXPI import placeholders.
 * </p>
 *
 * @author NeqSim
 * @version 1.0
...
public final class DexpiEquipmentFactory {
   ...
   *
   * <p>
   * The method selects the correct NeqSim equipment class based on {@link DexpiProcessUnit#getMappedEquipment()}, wires
  ...
  public static ProcessEquipmentInterface create(DexpiProcessUnit unit, StreamInterface inletStream) {
    ...
    case Ejector:
    default:
      logger.info("No factory mapping for {}; creating pass-through stream '{}'", unit.getMappedEquipment(), name);
