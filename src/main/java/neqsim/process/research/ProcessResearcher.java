 ...
 *
 * <p>
 * This class is the public entry point for early-stage process synthesis. It generates candidate NeqSim process
 * definitions from a {@link ProcessResearchSpec}, optionally evaluates them with the rigorous process simulator,
 * applies bounded decision-variable screening, and returns ranked candidates with warnings, assumptions, and objective
 * values.
 * </p>
 *
...
public class ProcessResearcher {
  ...
  public ProcessResearcher() {
    this(new ProcessCandidateGenerator(), new ProcessCandidateEvaluator(), new ProcessSynthesisFeasibilityPruner());
  }
  ...
  /**
   * Marks feasible candidates dominated by another candidate with at least as good score and lower complexity or power
   * demand.
   *
   ...
   */
  private double utilityValue(ProcessCandidate candidate, String primaryMetric, String fallbackMetric) {
    return candidate.getMetrics().get(primaryMetric, candidate.getMetrics().get(fallbackMetric, 0.0));
  }
}
