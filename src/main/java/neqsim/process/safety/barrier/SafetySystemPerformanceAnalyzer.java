 ...
 *
 * <p>
 * The analyzer bridges the existing barrier-register model with NeqSim instrument and SIS implementations. It can
 * consume measurement devices such as {@link FireDetector} and {@link GasDetector}, event/voting SIFs from
 * {@link SafetyInstrumentedFunction}, and scenario demand cases from consequence or document extraction work. The
 * output is a traceable {@link SafetySystemPerformanceReport} suitable for STID-derived safety critical system reviews.
 * </p>
 *
 * @author ESOL
 * @version 1.0
...
public class SafetySystemPerformanceAnalyzer {
  ...
  private final List<MeasurementDeviceInterface> measurementDevices = new ArrayList<MeasurementDeviceInterface>();
  private final List<SafetyInstrumentedFunction> safetyInstrumentedFunctions = new ArrayList<SafetyInstrumentedFunction>();
  private final List<neqsim.process.safety.risk.sis.SafetyInstrumentedFunction> quantitativeSafetyInstrumentedFunctions = new ArrayList<neqsim.process.safety.risk.sis.SafetyInstrumentedFunction>();
   ...
   * @return this analyzer
   */
  public SafetySystemPerformanceAnalyzer addMeasurementDevice(MeasurementDeviceInterface measurementDevice) {
  ...
  private void checkEffectivenessRequirement(SafetyBarrier barrier, SafetySystemDemand demandCase,
    ...
    double actualEffectiveness = selectFirstFinite(demandCase.getActualEffectiveness(), barrier.getEffectiveness());
  ...
  private void evaluateDemandCapacity(SafetySystemCategory category, SafetySystemDemand demandCase,
    ...
    if (!Double.isNaN(demandCase.getDemandValue()) && !Double.isNaN(demandCase.getCapacityValue())) {
  ...
  private void evaluateInstrumentation(SafetySystemCategory category, SafetyBarrier barrier,
    ...
    assessment.addMetric("relatedQuantitativeSafetyInstrumentedFunctionCount", quantitativeSifs.size());
  }
   ...
   * @param assessment assessment to update
   */
  private void evaluateDemandCompleteness(SafetySystemCategory category, SafetySystemDemand demandCase,
      BarrierAssessment assessment) {
    if (demandCase != null) {
      return;
    }
   ...
   */
  private SafetySystemCategory determineCategory(SafetyBarrier barrier, SafetySystemDemand demandCase) {
    ...
    if (containsAny(text, "fire detector", "gas detector", "f&g", "fgs", "fire and gas", "detection")) {
   ...
   */
  private boolean isRelevantMeasurementDevice(MeasurementDeviceInterface device, SafetySystemCategory category) {
  ...
  private String instrumentIdentifier(MeasurementDeviceInterface device) {
    return device.getTag() == null || device.getTag().trim().isEmpty() ? device.getName() : device.getTag();
  }
   ...
   */
  private List<SafetyInstrumentedFunction> findRelatedSafetyInstrumentedFunctions(SafetyBarrier barrier) {
   ...
   */
  private boolean matchesSafetyInstrumentedFunction(SafetyInstrumentedFunction sif, SafetyBarrier barrier) {
    if (matchesText(sif.getName(), barrier.getId()) || matchesText(sif.getName(), barrier.getName())) {
      return true;
    }
    for (Detector detector : sif.getDetectors()) {
      for (String tag : barrier.getLinkedEquipmentTags()) {
   ...
   */
  private String quantitativeSifIdentifier(neqsim.process.safety.risk.sis.SafetyInstrumentedFunction sif) {
