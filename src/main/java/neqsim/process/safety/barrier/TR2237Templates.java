 ...
 *
 * <p>
 * The templates provide a starting register for agent-assisted standards reviews. They deliberately contain acceptance
 * criteria and barrier links, but no project evidence; agents that read technical documentation should attach
 * {@link DocumentEvidence} items before crediting the barriers.
 * </p>
 *
...
public final class TR2237Templates {
  ...
  public static BarrierRegister createOnshoreTemplate() {
	...
	"Prevent unacceptable overpressure by relief, HIPPS, or depressurization functions.", DemandMode.LOW_DEMAND,
	0.01, Double.NaN, 8760.0, 30.0, "Scenario relief loads and SIL/LOPA targets are documented and verified.",
	BarrierType.PREVENTION);
    addStandardWithBarrier(register, "PS-03", "Fire and gas detection",
	"Detect fire or gas release early enough to initiate required executive actions.", DemandMode.CONTINUOUS,
	Double.NaN, 0.99, Double.NaN, 10.0,
	"Detector coverage and voting logic meet the project fire and gas philosophy.", BarrierType.BOTH);
    addStandardWithBarrier(register, "PS-04", "Emergency shutdown",
	"Isolate hazardous inventories and place equipment in a safe state on demand.", DemandMode.LOW_DEMAND, 0.01,
	Double.NaN, 8760.0, 30.0, "Shutdown valves, logic solver, and final elements meet assigned SIL targets.",
	BarrierType.MITIGATION);
    addStandardWithBarrier(register, "PS-05", "Emergency depressurization",
	"Reduce pressure and inventory before escalation or rupture in fire scenarios.", DemandMode.LOW_DEMAND, 0.01,
	Double.NaN, 8760.0, 900.0,
	...
	"Control or suppress fires to prevent unacceptable escalation.", DemandMode.LOW_DEMAND, 0.10, Double.NaN,
	4380.0, 120.0, "Firewater, deluge, monitors, and foam systems meet demand, coverage, and duration needs.",
	BarrierType.MITIGATION);
    addStandardWithBarrier(register, "PS-08", "Passive fire protection",
	"Maintain structural and equipment integrity for the required fire endurance period.", DemandMode.CONTINUOUS,
	Double.NaN, 0.995, Double.NaN, Double.NaN,
	...
	"Supply power to safety-critical loads for the required endurance time.", DemandMode.LOW_DEMAND, 0.05,
	Double.NaN, 4380.0, 15.0, "Emergency power capacity, start reliability, autonomy, and load list are verified.",
	BarrierType.MITIGATION);
    addStandardWithBarrier(register, "PS-10", "Escape and evacuation",
	"Provide safe escape, evacuation, and rescue functions during major accident events.", DemandMode.CONTINUOUS,
	Double.NaN, 0.99, Double.NaN, Double.NaN,
	"Escape routes, muster, evacuation means, and drills are documented and maintained.", BarrierType.MITIGATION);
  ...
  private static void addStandardWithBarrier(BarrierRegister register, String id, String title, String safetyFunction,
      ...
      double responseTimeSeconds, String acceptanceCriterion, BarrierType barrierType) {
    PerformanceStandard standard = new PerformanceStandard(id).setTitle(title).setSafetyFunction(safetyFunction)
	.setDemandMode(demandMode).setTargetPfd(targetPfd).setRequiredAvailability(availability)
	.setProofTestIntervalHours(proofTestIntervalHours).setResponseTimeSeconds(responseTimeSeconds)
	.addAcceptanceCriterion(acceptanceCriterion);
    SafetyBarrier barrier = new SafetyBarrier(id + "-BARRIER").setName(title).setDescription(safetyFunction)
	.setType(barrierType).setStatus(BarrierStatus.AVAILABLE).setPerformanceStandard(standard)
	.addEquipmentTag("TBD");
