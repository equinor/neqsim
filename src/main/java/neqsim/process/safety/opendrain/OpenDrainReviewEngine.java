...
public class OpenDrainReviewEngine {
   ...
   */
  public OpenDrainReviewResult evaluateItem(OpenDrainReviewItem item, double defaultLiquidLeakRateKgPerS) {
  ...
  private void checkDesignStandardEvidence(OpenDrainReviewItem item, OpenDrainReviewResult result) {
    List<String> standards = item.getStringList("designStandards", "standards", "drainageSystemStandards");
  ...
  private void checkSpillCollection(OpenDrainReviewItem item, OpenDrainReviewResult result) {
    if (!requiresDrainageReview(item)) {
      result.addAssessment(OpenDrainAssessment.notApplicable("OD-S001-9.4.1-COLLECTION", CLAUSE_9_4_1,
	  "The item is not identified as a flammable or hazardous liquid source."));
      return;
    }
    addRequiredBoolean(result, item, "OD-S001-9.4.1-COLLECTION", CLAUSE_9_4_1, "hasOpenDrainMeasures",
  ...
  private void checkCapacity(OpenDrainReviewItem item, OpenDrainReviewResult result,
    ...
    double fireWater = item.getDouble(0.0, "fireWaterCapacityKgPerS", "fireWaterRateKgPerS", "delugeLoadKgPerS");
    double leakRate = item.getDouble(defaultLiquidLeakRateKgPerS, "liquidLeakRateKgPerS", "worstCredibleLeakKgPerS",
	"processFireLeakKgPerS");
    double required = Math.max(0.0, fireWater) + Math.max(0.0, leakRate);
    boolean hasCapacity = item.hasAny("drainageCapacityKgPerS", "openDrainCapacityKgPerS", "capacityKgPerS");
    double capacity = item.getDouble(Double.NaN, "drainageCapacityKgPerS", "openDrainCapacityKgPerS", "capacityKgPerS");
    OpenDrainAssessment assessment;
    if (!hasCapacity) {
    ...
    if (!item.hasAny("fireWaterCapacityKgPerS", "fireWaterRateKgPerS", "delugeLoadKgPerS")) {
      assessment.addDetail("warning", "No fire-water load was supplied; capacity check used process leak load only.");
    }
    result.addAssessment(assessment);
  }
  ...
  private void checkBunding(OpenDrainReviewItem item, OpenDrainReviewResult result) {
    boolean tankArea = item.getBoolean(false, "hasFlammableLiquidTanks", "tankArea", "flammableLiquidVesselsPresent");
    if (!tankArea) {
    ...
    addRequiredBoolean(result, item, "OD-S001-9.4.1-BUNDING", CLAUSE_9_4_1, "bundingAndDrainageAdequate",
  ...
  private void checkFloaterSpreading(OpenDrainReviewItem item, OpenDrainReviewResult result) {
    ...
    addRequiredBoolean(result, item, "OD-S001-9.4.1-FLOATER", CLAUSE_9_4_1, "preventsCrossAreaSpreading",
  ...
  private void checkHazardousDrainTank(OpenDrainReviewItem item, OpenDrainReviewResult result) {
	...
	|| (item.getBoolean(false, "hazardousArea") && item.getBoolean(false, "collectionTankPresent"));
    if (!applicable) {
  ...
  private void checkDrainSeparation(OpenDrainReviewItem item, OpenDrainReviewResult result) {
    addRequiredBoolean(result, item, "OD-S001-9.4.2-CLOSED-OPEN", CLAUSE_9_4_2, "closedOpenDrainInteractionPrevented",
	"Closed and open drain systems cannot interact.",
    ...
    Boolean combined = item.getBooleanObject("hazardousNonHazardousCombined", "hazardousNonHazardousShareSump");
    OpenDrainAssessment assessment;
    ...
    if (item.getBoolean(false, "commonCaissonOrSump", "sharedCaisson", "sharedSump")) {
      addRequiredBoolean(result, item, "OD-S001-9.4.2-COMMON-SUMP", CLAUSE_9_4_2, "nonHazardousBackflowPrevented",
      ...
      result.addAssessment(OpenDrainAssessment.notApplicable("OD-S001-9.4.2-COMMON-SUMP", CLAUSE_9_4_2,
	  "No common sump or caisson is identified."));
    }
  }
  ...
  private void checkSealsAndVentRouting(OpenDrainReviewItem item, OpenDrainReviewResult result) {
    addRequiredBoolean(result, item, "OD-S001-9.4.2-SEAL-BACKPRESSURE", CLAUSE_9_4_2, "sealDesignedForMaxBackpressure",
	"Liquid seal is designed for maximum backpressure.",
    ...
    if (item.getBoolean(false, "multipleFireAreas", "crossesFireAreas", "fireAreaInterface")) {
      addRequiredBoolean(result, item, "OD-S001-9.4.2-FIRE-SEALS", CLAUSE_9_4_2, "liquidFireSealsBetweenFireAreas",
      ...
      result.addAssessment(OpenDrainAssessment.notApplicable("OD-S001-9.4.2-FIRE-SEALS", CLAUSE_9_4_2,
	  "No fire-area interface is identified."));
    }
  }
  ...
  private void checkHelideckDrainage(OpenDrainReviewItem item, OpenDrainReviewResult result) {
    ...
    addRequiredBoolean(result, item, "OD-S001-9.4.2-HELIDECK", CLAUSE_9_4_2, "dedicatedPipeDrainage",
	"Helideck drain has dedicated pipe drainage.",
  ...
  private void checkDrillingProductionSeparation(OpenDrainReviewItem item, OpenDrainReviewResult result) {
	...
	&& !containsIgnoreCase(item.getAreaType(), "drilling")) {
      result.addAssessment(OpenDrainAssessment.notApplicable("OD-S001-9.4.2-DRILLING-PROD", CLAUSE_9_4_2,
	  "The item is not identified as a drilling drain."));
      return;
    }
    Boolean connected = item.getBooleanObject("drillingProductionOpenDrainConnected", "connectedToProductionOpenDrain");
    OpenDrainAssessment assessment;
    if (connected == null) {
	  ...
	  "Drilling and production open drains are separated.", "Keep the separation evidence in the review record.");
    }
  ...
  private void checkTemporaryStorageDrainage(OpenDrainReviewItem item, OpenDrainReviewResult result) {
    ...
    addRequiredBoolean(result, item, "OD-S001-9.4.2-TOTE", CLAUSE_9_4_2, "adequateCollectionDrainForTote",
  ...
  private void checkFireDivisionIntegrity(OpenDrainReviewItem item, OpenDrainReviewResult result) {
	...
	"fireDivisionPenetration")) {
      result.addAssessment(OpenDrainAssessment.notApplicable("OD-S001-9.4.3-FIRE-DIVISION", CLAUSE_9_4_3,
	  "No drain-box or pipe fire-division penetration is identified."));
      return;
    }
    addRequiredBoolean(result, item, "OD-S001-9.4.3-FIRE-DIVISION", CLAUSE_9_4_3, "fireDivisionIntegrityMaintained",
	"Drain boxes and piping do not impair fire divisions.",
  ...
  private void addRequiredBoolean(OpenDrainReviewResult result, OpenDrainReviewItem item, String requirementId,
      String clause, String key, String passMessage, String recommendation) {
  ...
  private double calculateConfidence(OpenDrainReviewItem item) {
    ...
    if (item.hasAny("hasOpenDrainMeasures", "backflowPrevented", "closedOpenDrainInteractionPrevented")) {
      score += 0.20;
    }
  ...
  private boolean requiresDrainageReview(OpenDrainReviewItem item) {
    Boolean source = item.getBooleanObject("sourceHasFlammableOrHazardousLiquid", "flammableOrHazardousLiquidSource");
    if (source != null) {
      return source.booleanValue();
    }
    String searchable = (item.getAreaType() + " " + item.getDrainSystemType()).toLowerCase();
    return searchable.contains("process") || searchable.contains("hazard") || searchable.contains("hydrocarbon")
	|| searchable.contains("tank") || searchable.contains("well") || searchable.contains("drilling")
	|| searchable.contains("helideck") || searchable.contains("storage") || searchable.trim().isEmpty();
  }
  ...
  private boolean isHelideck(OpenDrainReviewItem item) {
    return item.getBoolean(false, "helideckDrain", "helideckArea") || containsIgnoreCase(item.getAreaType(), "helideck")
