...
public class ProcessSafetySystemReviewEngine {
  ...
  public ProcessSafetySystemReviewReport evaluate(ProcessSafetySystemReviewInput input) {
    ProcessSafetySystemReviewInput safeInput = input == null ? new ProcessSafetySystemReviewInput() : input;
    ProcessSafetySystemReviewReport report = new ProcessSafetySystemReviewReport(safeInput.getProjectName());
  ...
  public ProcessSafetySystemReviewResult evaluateItem(ProcessSafetySystemReviewItem item) {
    ProcessSafetySystemReviewItem safeItem = item == null ? new ProcessSafetySystemReviewItem() : item;
  ...
  private ProcessSafetySystemReviewResult evaluateCoverage(ProcessSafetySystemReviewInput input) {
    ProcessSafetySystemReviewItem coverageItem = new ProcessSafetySystemReviewItem().setFunctionId("CLAUSE10-COVERAGE")
	.setFunctionType("COVERAGE").setEquipmentTag("process-safety-system");
    ...
    addCoverageAssessment(result, input, "PSS-COVERAGE-SECONDARY", CLAUSE_10_4_7, "SECONDARY_PRESSURE_PROTECTION",
  ...
  private void addCoverageAssessment(ProcessSafetySystemReviewResult result, ProcessSafetySystemReviewInput input,
      String requirementId, String clause, String typeToken, String passMessage, String recommendation) {
    if (hasType(input, typeToken)) {
	  ...
	  "No " + typeToken + " evidence item was found in the normalized Clause 10 input.", recommendation));
    }
  }
  ...
  private void checkPsdValves(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
    if (!isPsdItem(item)) {
      result.addAssessment(ProcessSafetySystemAssessment.notApplicable("PSS-10.4.2-PSD", CLAUSE_10_4_2,
	  "The item is not identified as a PSD or shutdown valve function."));
      return;
    }
  ...
  private void checkPsvProtection(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
    if (!isPsvItem(item)) {
      result.addAssessment(ProcessSafetySystemAssessment.notApplicable("PSS-10.4.3-PSV", CLAUSE_10_4_3,
	  "The item is not identified as a PSV or relief protection function."));
      return;
    }
	...
	"requiredCapacityKgPerS");
    double capacity = item.getDouble(Double.NaN, "psvCapacityKgPerS", "ratedCapacityKgPerS", "reliefCapacityKgPerS");
	...
	assessment = ProcessSafetySystemAssessment.fail("PSS-10.4.3-CAPACITY", CLAUSE_10_4_3, "CRITICAL",
	    "PSV rated capacity is below the documented relief load.",
      ...
      result.addAssessment(ProcessSafetySystemAssessment.warning("PSS-10.4.3-CAPACITY", CLAUSE_10_4_3, "HIGH",
	  "PSV relief load and rated capacity were not both supplied.",
  ...
  private void checkAlarmActions(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
    if (!isAlarmItem(item)) {
      result.addAssessment(ProcessSafetySystemAssessment.notApplicable("PSS-10.4.4-ALARM", CLAUSE_10_4_4,
	  "The item is not identified as an alarm or operator action."));
      return;
    }
	...
	"availableOperatorResponseTimeSeconds", "operator response time", "available operator response time");
  }
  ...
  private void checkResponseTime(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
    if (!isActionOrInstrumentedItem(item)) {
      result.addAssessment(ProcessSafetySystemAssessment.notApplicable("PSS-10.4.5-RESPONSE", CLAUSE_10_4_5,
	  "The item is not an action or instrumented safety function."));
      return;
    }
    compareTimes(result, item, "PSS-10.4.5-RESPONSE", CLAUSE_10_4_5, "actualResponseTimeSeconds",
	"requiredResponseTimeSeconds", "actual response time", "required response time");
  }
  ...
  private void checkLogicSolver(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
    if (!isLogicItem(item)) {
      result.addAssessment(ProcessSafetySystemAssessment.notApplicable("PSS-10.4.6-LOGIC", CLAUSE_10_4_6,
	  "The item is not identified as a logic solver, SIF, or PSD function."));
      return;
    }
  ...
  private void checkSecondaryPressureProtection(ProcessSafetySystemReviewItem item,
    ...
    if (!isSecondaryPressureItem(item)) {
      result.addAssessment(ProcessSafetySystemAssessment.notApplicable("PSS-10.4.7-SECONDARY", CLAUSE_10_4_7,
	  "The item is not identified as instrumented secondary pressure protection."));
      return;
    }
    S001SecondaryPressureProtectionCriteria criteria = S001SecondaryPressureProtectionCriteria.fromItem(item);
    if (criteria.isEmpty()) {
      result.addAssessment(ProcessSafetySystemAssessment.warning("PSS-10.4.7-SECONDARY", CLAUSE_10_4_7, "HIGH",
    ...
    if (check.isAcceptable()) {
      ...
      assessment = ProcessSafetySystemAssessment.warning("PSS-10.4.7-SECONDARY", CLAUSE_10_4_7, "HIGH",
	  "Secondary pressure protection evidence is incomplete.",
	  "Provide complete pressure basis and annual frequency target before crediting this protection concept.");
    } else {
      assessment = ProcessSafetySystemAssessment.fail("PSS-10.4.7-SECONDARY", CLAUSE_10_4_7, "CRITICAL",
  ...
  private void checkUtilityDependencies(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
    ...
    if (Boolean.TRUE.equals(utilityDependent)) {
      ...
      result.addAssessment(ProcessSafetySystemAssessment.info("PSS-10.4.8-UTILITY-LOSS", CLAUSE_10_4_8,
	  "No positive utility dependency was supplied for this item.",
  ...
  private void checkPsdPrinciples(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
    if (!isPsdItem(item)) {
      result.addAssessment(ProcessSafetySystemAssessment.notApplicable("PSS-10.4.9-PSD", CLAUSE_10_4_9,
	  "The item is not identified as a PSD function."));
      return;
    }
  ...
  private void checkSurvivability(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
	...
	"survivabilityTimeMin", "required survivability time", "documented survivability time");
  }
  ...
  private void checkOperationalEvidence(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
    boolean hasOperationalEvidence = item.hasAny("tagreaderSource", "instrumentDataSource", "bypassActive",
	"overrideActive", "proofTestOverdue", "tripDemandFailures", "observedResponseTimeSeconds");
    if (!hasOperationalEvidence) {
      result.addAssessment(ProcessSafetySystemAssessment.info("PSS-OPS-INSTRUMENT-DATA", "Operational evidence",
    ...
    boolean overrideActive = item.getBoolean(false, "overrideActive", "sifOverridden", "psdOverridden");
    ...
    if (bypassActive || overrideActive) {
      assessment = ProcessSafetySystemAssessment.fail("PSS-OPS-INSTRUMENT-DATA", "Operational evidence", "HIGH",
      ...
      assessment = ProcessSafetySystemAssessment.warning("PSS-OPS-INSTRUMENT-DATA", "Operational evidence", "MEDIUM",
      ...
      assessment = ProcessSafetySystemAssessment.pass("PSS-OPS-INSTRUMENT-DATA", "Operational evidence",
  ...
  private void checkEvidenceQuality(ProcessSafetySystemReviewItem item, ProcessSafetySystemReviewResult result) {
    ...
    if (item.getSourceReferences().isEmpty()) {
      sourceAssessment = ProcessSafetySystemAssessment.warning("PSS-EVIDENCE-SOURCES", "Evidence quality", "MEDIUM",
	  "No technical-document source references are attached.",
	  "Attach STID, C&E, SRS, PSV list, instrument data, or tagreader source references.");
    } else {
      sourceAssessment = ProcessSafetySystemAssessment.pass("PSS-EVIDENCE-SOURCES", "Evidence quality",
	  "Technical-document source references are attached.",
  ...
  private void addRequiredBoolean(ProcessSafetySystemReviewResult result, ProcessSafetySystemReviewItem item,
      String requirementId, String clause, String[] keys, String passMessage, String recommendation) {
    Boolean documented = item.getBooleanObject(keys);
    ProcessSafetySystemAssessment assessment;
    if (documented == null) {
      assessment = ProcessSafetySystemAssessment.warning(requirementId, clause, "HIGH",
	  "Required process safety evidence is missing: " + joinKeys(keys) + ".", recommendation);
    } else if (!documented.booleanValue()) {
      assessment = ProcessSafetySystemAssessment.fail(requirementId, clause, "CRITICAL",
	  "Process safety requirement is not met: " + joinKeys(keys) + " is false.", recommendation);
    } else {
  ...
  private void compareMinimum(ProcessSafetySystemReviewResult result, ProcessSafetySystemReviewItem item,
      String requirementId, String clause, String minimumKey, String providedKey, String minimumLabel,
    ...
    if (!Double.isFinite(minimum) || !Double.isFinite(provided)) {
      assessment = ProcessSafetySystemAssessment.warning(requirementId, clause, "MEDIUM",
	  "Minimum-value evidence is incomplete for " + providedLabel + " versus " + minimumLabel + ".",
  ...
  private double calculateConfidence(ProcessSafetySystemReviewItem item) {
    ...
    if (item.hasAny("requiredResponseTimeSeconds", "actualResponseTimeSeconds", "operatorResponseTimeSeconds")) {
      score += 0.15;
    }
    if (item.hasAny("logicSolverCertified", "psvCapacityKgPerS", "maximumEventPressureBara")) {
      score += 0.15;
    }
  ...
  private boolean isActionOrInstrumentedItem(ProcessSafetySystemReviewItem item) {
    return isPsdItem(item) || isAlarmItem(item) || isLogicItem(item) || isSecondaryPressureItem(item)
	|| item.hasAny("requiredResponseTimeSeconds", "actualResponseTimeSeconds");
  }
  ...
  private boolean isSecondaryPressureItem(ProcessSafetySystemReviewItem item) {
    return containsAny(item.getFunctionType(), "secondary_pressure", "secondary pressure", "10.4.7", "overpressure")
	|| item.hasAny("maximumEventPressureBara", "eventPressureBara", "demandFrequencyPerYear");
  }
