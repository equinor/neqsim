 ...
 *
 * <p>
 * Build a tree of {@link FaultTreeNode} objects (basic events with probabilities and gates AND / OR / VOTING(k-of-n))
 * and call {@link #topEventProbability(FaultTreeNode)} to recursively compute the top-event probability.
 *
 * <p>
 * Common-cause failures (CCF) can be applied at OR / VOTING gates using the β-factor model (IEC 61508 Annex F): P_total
 * = (1 − β) · P_independent + β · P_basic.
 *
 * <p>
 * <b>References:</b>
 * <ul>
...
public class FaultTreeAnalyzer implements Serializable {
  ...
  /**
   * Compute top-event probability using gate-specific combinatorics, with rare-event approximation for OR gates (P ≈ Σ
   * p_i bounded ≤ 1).
   *
  ...
  private boolean evaluateLogical(FaultTreeNode node, Set<String> failedSet) {
    ...
    switch (node.gate) {
    case AND:
      return trueCount == node.children.size();
    case OR:
      return trueCount >= 1;
    case VOTING:
      return trueCount >= node.kOfN;
    default:
      return false;
    }
  }
