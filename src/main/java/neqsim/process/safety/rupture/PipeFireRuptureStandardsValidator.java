 ...
 * S-001 / TR2000-style review is present before a result is treated as design grade.
 * </p>
 *
 * @author ESOL
 * @version 1.0
...
public final class PipeFireRuptureStandardsValidator implements Serializable {
  ...
  public PipeFireRuptureStandardsValidator() {
	...
	new ArrayList<String>(Arrays.asList("API 521 / ISO 23251 depressurization and fire exposure basis",
	    ...
	    "TR2000 piping class, pipe-size, and material basis",
  ...
  public SafetyStudyReadiness validate(PipeFireRuptureDataSource dataSource, PipeFireRuptureResult result) {
    ...
    if (!dataSource.isTr2000PipeRowsFetched()) {
      readiness.addWarning("TR2000", "TR2000 pipe-size and pipe-element rows are not fetched and joined.",
	  "Use latest Issue revision and verify the PCS row for the nominal pipe size.");
    }
    ...
    if (!dataSource.isPidTopologyVerified()) {
	  ...
	  "Trace valves, nozzles, relief/blowdown connections, and battery limits from STID/P&ID evidence.");
    }
    if (result == null) {
