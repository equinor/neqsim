 ...
 *
 * <p>
 * The validator does not certify compliance. It records whether the evidence needed for an API 521
 * / ISO 23251 / NORSOK S-001 and piping-specification review is present before a result is treated
 * as design grade.
 * </p>
 *
 * @author ESOL
...
public final class PipeFireRuptureStandardsValidator implements Serializable {
  ...
  public PipeFireRuptureStandardsValidator() {
    this.standardsApplied = Collections.unmodifiableList(new ArrayList<String>(
        Arrays.asList("API 521 / ISO 23251 depressurization and fire exposure basis",
   ...
   */
  public SafetyStudyReadiness validate(PipeFireRuptureDataSource dataSource,
    ...
    if (!dataSource.isBlowdownProfileVerified()) {
      readiness.addWarning("API521",
          "Pressure profile is not marked verified against depressurization basis.",
          "Run governed blowdown/depressurization and attach the pressure profile evidence.");
    }
    if (!dataSource.isPidTopologyVerified()) {
      readiness.addWarning("NORSOK_S001",
          "Isolation boundary and blowdown path topology are not verified.",
    ...
    if (result == null) {
      ...
      readiness.addInfo("source_term",
          "Rupture is predicted and a source-term handoff should be reviewed.",
          "Feed rupture pressure and release estimate into consequence/dispersion assessment.");
    } else {
      readiness.addInfo("source_term",
          "No rupture was predicted within the simulated exposure duration.",
