...
/**
 * Read-through adapter that presents a multi-area {@link ProcessModel} as a single {@link ProcessSystem} so that the
 * existing {@link ProductionOptimizer} engine can optimize a whole plant without any change to its proven
 * single-{@code ProcessSystem} search and evaluation logic.
 *
 * <p>
 * The adapter overrides only the three operations the optimizer engine performs on a {@code ProcessSystem}:
 * </p>
 * <ul>
 * <li>{@link #run(UUID)} &mdash; delegates to {@link ProcessModel#runUntilConverged(int, double)} so cross-area
 * recycles and shared streams converge between iterations (instead of a single {@code ProcessSystem.run()} pass).</li>
 * <li>{@link #getUnitOperations()} &mdash; returns the units of every area concatenated in area insertion order, so the
 * optimizer's capacity / bottleneck scan covers the entire plant.</li>
 * <li>{@link #getConstrainedEquipment()} &mdash; aggregates capacity-constrained equipment across all areas.</li>
 * </ul>
 *
 * <p>
 * Because the inherited {@link ProcessSystem#getUnit(String)} is implemented in terms of {@link #getUnitOperations()},
 * manipulated-variable setters and objective / constraint functions written against the {@code ProcessSystem} API (for
 * example {@code proc.getUnit("Compressor")}) resolve units across all areas with no API change. Area-qualified
 * addresses of the form {@code "Area::Unit"} are also supported.
 * </p>
 *
 * <p>
 * This class is purely additive: it does not modify {@link ProcessModel} or {@link ProcessSystem}. The adapter is a
 * read-through view &mdash; it never adds units to its own internal list and never calls {@code super.run()}.
 * </p>
 *
...
public class ProcessModelOptimizationView extends ProcessSystem {
  ...
  /**
   * Creates a view over the given model using robust default convergence settings (50 iterations, tolerance 5e-3, which
  ...
  /**
   * Resolves a unit by name across all areas. Area-qualified addresses of the form {@code "Area::Unit"} are resolved
   * against the named area first; plain names fall back to a plant-wide search via the inherited
   * {@link ProcessSystem#getUnit(String)} (which iterates {@link #getUnitOperations()}).
   *
