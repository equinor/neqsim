...
public class Compressor extends TwoPortEquipment implements CompressorInterface {
    ...
    public void run() {
        ...
        if (useOutTemperature) {
            if (useRigorousPolytropicMethod) {
                ...
                this.polytropicHeadMeter = this.polytropicFluidHead * 1000.0 / ThermodynamicConstantsInterface.gravity;
                return;
            } else {
                ...
                this.polytropicEfficiency = polytropicPower / getThermoSystem().getFlowRate("kg/sec")
                        / (dH / getThermoSystem().getFlowRate("kg/sec"));
                ...
                this.polytropicFluidHead = polytropicPower / getThermoSystem().getFlowRate("kg/sec") / 1000.0;
    ...
    public void displayResult() {
        ...
        for (int i = 0; i < getThermoSystem().getNumberOfPhases(); i++) {
            for (int j = 0; j < getThermoSystem().getPhases()[0].getNumberOfComponents(); j++) {
                ...
                table[j + 1][i + 1] = nf.format(getThermoSystem().getPhases()[i].getComponents()[j].getx(), buf,
                        test).toString();
                table[j + 1][4] = "[-]";
            }
            buf = new StringBuffer();
            ...
            table[getThermoSystem().getPhases()[0].getNumberOfComponents() + 2][i + 1] = nf
                    .format(getThermoSystem().getPhases()[i].getPhysicalProperties().getDensity(),
                            buf, test)
                    .toString();
            ...
            table[getThermoSystem().getPhases()[0].getNumberOfComponents() + 4][i + 1] = nf
                    .format(getThermoSystem().getPhases()[i].getMolarMass() * 1000, buf, test)
                    .toString();
     ...
     *                        {@link neqsim.processSimulation.processEquipment.compressor.CompressorChart}
     *                        object
     */
     ...
     *
     * @param antiSurge a
     *                  {@link neqsim.processSimulation.processEquipment.compressor.AntiSurge}
     *                  object
     */
     ...
     * </p>
     * 
     * @param useGERG2008 a boolean. Set true to use GERG2008 and false to not use
     *                    it.
     */
