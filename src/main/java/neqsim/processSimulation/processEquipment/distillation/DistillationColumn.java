...
public class DistillationColumn extends ProcessEquipmentBaseClass implements DistillationInterface {
  ...
  public void init() {
    ...
    for (int i = 1; i < numberOfTrays - 1; i++) {
      ...
      getTray(i).getFluid().prettyPrint();
    }
    ...
    if (numberOfTrays == 1) {
      ...
      if (isDoInitializion()) {
        ...
        for (int i = 0; i < numberOfTrays; i++) {
          ...
          ((SimpleTray) trays.get(i)).getFluid().prettyPrint();
        }
      }
      err = 1.0e10;
      double errOld;
      ...
      do {
        ...
        massBalanceCheck();
      } while (err > 1e-6 && err < errOld && iter < 10); // && !massBalanceCheck());
      ...
      for (int i = 0; i < numberOfTrays; i++) {
        trays.get(i).getFluid().prettyPrint();
