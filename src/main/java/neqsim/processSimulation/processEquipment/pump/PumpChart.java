...
public class PumpChart implements PumpChartInterface, java.io.Serializable {
	...
	public void setCurves(double[] chartConditions, double[] speed, double[][] flow, double[][] head,
		...
		for (int i = 0; i < speed.length; i++) {
			...
			for (int j = 0; j < flow[i].length; j++) {
				...
				double flowFanLaw = flow[i][j] * speed[i] / speed[0]; // MLLU: not correct. speed[0] should be the
																		// requested speed
		...
		reducedEfficiencyFunc = new PolynomialFunction(
				fitter.fit(reducedEfficiencyFitter.toList()));
	...
	public void plot() {
		...
		neqsim.dataPresentation.JFreeChart.graph2b graph2 = new neqsim.dataPresentation.JFreeChart.graph2b(flow,
				efficiency, Arrays.stream(speed).mapToObj(String::valueOf).toArray(String[]::new),
				"eff vs flow", "flow", "eff");
		graph2.setVisible(true);
		neqsim.dataPresentation.JFreeChart.graph2b graph3 = new neqsim.dataPresentation.JFreeChart.graph2b(redflow,
				redhead, Arrays.stream(speed).mapToObj(String::valueOf).toArray(String[]::new), "red head vs red flow",
				"red flow", "red head");
