...
public class RecycleController implements Serializable{
	...
	public void init() {
		   for (Recycle recyc : recycleArray) { 
			   if(recyc.getPriority()<minimumPriorityLevel) minimumPriorityLevel=recyc.getPriority();
			   if(recyc.getPriority()>maximumPriorityLevel) maximumPriorityLevel=recyc.getPriority();
	...
	public boolean doSolveRecycle(Recycle recycle) {
		if(recycle.getPriority()==getCurrentPriorityLevel()) return true;
		else return false;
	}
	
	public boolean isHighestPriority(Recycle recycle) {
		if(recycle.getPriority()==maximumPriorityLevel) return true;
		else return false;
	}
