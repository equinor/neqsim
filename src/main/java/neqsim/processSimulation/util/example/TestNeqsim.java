...
public class TestNeqsim {
    ...
    public static void main(String[] args) {
        ...
        SystemInterface testFluid = new SystemSrkEos(298.15, 10.0);//SystemSrkCPAstatoil(298.1, 10.9); //298.15 K
   ...
   //     testFluid.addTBPfraction("C7", 0.0137641, 86.18 / 1000.0, 0.664);
    ...
    //    testFluid.addComponent("MEG", 0.173083);
        ...
        double Cpone = stream1.getThermoSystem().getPhase(0).getBeta() * CpVapour + stream1.getThermoSystem().getPhase(1).getBeta() * CpOil + stream1.getThermoSystem().getPhase(2).getBeta() * Cpliquid;
        ...
        double Density1 = stream1.getThermoSystem().getWtFraction(0) * DensityVapour + stream1.getThermoSystem().getWtFraction(1) * DensityOil + stream1.getThermoSystem().getWtFraction(2) * Densityliquid;
        // operations.run();
        System.out.println("work " + compressor1.getTotalWork() + " density " + Density1 + " Cp " + Cpone + " SPEED OF SOUND " + stream1.getThermoSystem().getPhase(0).getSoundSpeed());

        compressor1.solveEfficiency(390.15);
        compressor1.getOutStream().displayResult();
        System.out.println("Hvap " + stream1.getThermoSystem().getHeatOfVaporization() + " POLI " + compressor1.getPolytropicEfficiency() + " dentity " + stream1.getThermoSystem().getDensity() + " cp " + stream1.getThermoSystem().getCp());
        stream1.getThermoSystem().display();

        double massFlowGas = stream1.getThermoSystem().getPhase(0).getBeta() * stream1.getThermoSystem().getPhase(0).getMolarMass();
        double massFlowLiq = stream1.getThermoSystem().getPhase(2).getBeta() * stream1.getThermoSystem().getPhase(2).getMolarMass();
        double massFlowOil = stream1.getThermoSystem().getPhase(1).getBeta() * stream1.getThermoSystem().getPhase(1).getMolarMass();
        ...
        System.out.println("inlet stream -  GMF " + GMF + "  GVF " + GVF + " Z IN " + stream1.getThermoSystem().getZ() + " Z OUT " + compressor1.getOutStream().getThermoSystem().getZ());
        /* temperature[i] = compressor1.getOutStream().getTemperature();
         work [i] = compressor1.getTotalWork();
       
         Cp_Vapour [i] = compressor1.getOutStream().getThermoSystem().getPhase(0).getCp();
         Cp_liquid [i] = compressor1.getOutStream().getThermoSystem().getPhase(1).getCp();
         Cp [i] = compressor1.getOutStream().getThermoSystem().getPhase(0).getBeta() * Cp_Vapour [i] + compressor1.getOutStream().getThermoSystem().getPhase(1).getBeta()* Cp_liquid [i];
         ...
         Density [i] = compressor1.getOutStream().getThermoSystem().getWtFraction(0) * Density_Vapour [i] + compressor1.getOutStream().getThermoSystem().getWtFraction(1)* Density_liquid [i];
         */
        // }
