...
public class Standard_ISO6578 extends neqsim.standards.Standard {
    ...
    double[][] KMKMcorrectionFactor1Matrix = {
        {-0.007, -0.008, -0.009, -0.01, -0.013, -0.015, -0.017},
        {0.165, 0.19, 0.22, 0.25, 0.295, 0.345, 0.4},
        {0.34, 0.375, 0.44, 0.5, 0.59, 0.7, 0.825},
        {0.475, 0.535, 0.61, 0.695, 0.795, 0.92, 1.06},
        {0.635, 0.725, 0.81, 0.92, 1.035, 1.2, 1.39},
        {0.735, 0.835, 0.945, 1.055, 1.21, 1.37, 1.59},
        {0.84, 0.95, 1.065, 1.205, 1.385, 1.555, 1.8},
        {0.92, 1.055, 1.18, 1.33, 1.525, 1.715, 1.95},
        {1.045, 1.155, 1.28, 1.45, 1.64, 1.86, 2.105},
        {1.12, 1.245, 1.38, 1.55, 1.75, 1.99, 2.272}};
    double[][] KMKMcorrectionFactor2Matrix = {
        {-0.01, -0.015, -0.024, -0.032, -0.043, -0.058, -0.075},
        {0.24, 0.32, 0.41, 0.6, 0.71, 0.95, 1.3},
        {0.42, 0.59, 0.72, 0.91, 1.13, 1.46, 2},
        {0.61, 0.77, 0.95, 1.23, 1.48, 1.92, 2.4},
        {0.75, 0.92, 1.15, 1.43, 1.73, 2.2, 2.6},
        {0.91, 1.07, 1.22, 1.63, 1.98, 2.42, 3},
        {1.05, 1.22, 1.3, 1.85, 2.23, 2.68, 3.4},
        {1.19, 1.37, 1.45, 2.08, 2.48, 3, 3.77},
        {1.33, 1.52, 1.65, 2.3, 2.75, 3.32, 3.99},
        {1.45, 1.71, 2, 2.45, 2.9, 3.52, 4.23}};
    double[] ISO6578temperatures = {93.15, 98.15, 103.15, 108.15, 113.15, 118.15, 123.15, 128.15, 133.15};
    double[] ISO6578molarMasses = {16.0, 17.0, 18.0, 19.0, 20.0, 21.0, 22.0, 23.0, 24.0, 25.0, 26.0, 27.0, 28.0, 29.0, 30.0};
    ...
    double[] Vitemperatures = ISO6578temperatures;//{-180.0, -175.0, -170.0, -165.0, -160.0, -155.0, -150.0, -145.0, -140.0};
    ...
    public void calculate() {
        ...
        for (int i = 0; i < thermoSystem.getPhase(0).getNumberOfComponents(); i++) {
            ...
            } else if (thermoSystem.getPhase(0).getComponent(i).getName().equals("i-pentane") ||thermoSystem.getPhase(0).getComponent(i).getName().equals("iC5")) {
                a = 9.524e-8;
                b = 8.727e-5;
                c = 8.063e-2;
            ...
            //  Vi[i] = c + a * Math.pow(thermoSystem.getTemperature(), 2.0) + b * thermoSystem.getTemperature();
            try {
                Vi[i] = function.value(thermoSystem.getTemperature());
            } catch (Exception e) {
           //     e.printStackTrace();
              //  System.out.println("volume "+ (thermoSystem.getPhase(0).getMolarVolume())/10e4);
               Vi[i] = thermoSystem.getPhase(1).getMolarVolume()/100.0;
               // Vi[i] = 0.036315;
            }
           //     System.out.println("volume "+ (thermoSystem.getPhase(1).getMolarVolume())/100.0);
