...
public class UKspecifications_ICF_SI extends neqsim.standards.Standard {
    ...
    public double getValue(String returnParameter, java.lang.String returnUnit) {
        ...
        if (returnParameter.equals("SootIndex")) {
            return 0.896 * Math.atan(0.0255 * thermoSystem.getPhase(0).getComponent("propane").getz() - 0.0233 * thermoSystem.getPhase(0).getComponent("nitrogen").getz()+ 0.617);
        } else {
    ...
    public double calcPropaneNumber() {
        ...
        double[][] Amatrix = {{1.0, 1.0}, {1.0, 3.0}};//{thermoSystem.getNumberOfMoles(), thermoSystem.getNumberOfMoles()*iso6976.getAverageCarbonNumber()}};
        double[] bmatrix = {(thermoSystem.getTotalNumberOfMoles() - iso6976.getTotalMolesOfInerts()), avgCarbon * (thermoSystem.getTotalNumberOfMoles() - iso6976.getTotalMolesOfInerts())};
        ...
        try {
            System.out.println("propane number " + (nitrogenCalc + ans2.getEntry(1, 0)) / thermoSystem.getTotalNumberOfMoles() * 100.0);
    ...
    public double calcWithNitrogenAsInert() {
        ...
        if (!localIso6976.getThermoSystem().getPhase(0).hasComponent("nitrogen")) {
            localIso6976.getThermoSystem().addComponent("nitrogen", localIso6976.getThermoSystem().getNumberOfMoles()*1e-50);
