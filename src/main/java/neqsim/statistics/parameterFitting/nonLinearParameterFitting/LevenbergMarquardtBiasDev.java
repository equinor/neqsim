...
public class LevenbergMarquardtBiasDev extends LevenbergMarquardt {
    ...
    public double calcChiSquare(){
        ...
        for(int i=0;i<sampleSet.getLength();i++){
            ...
            chiSquare += (sampleSet.getSample(i).getSampleValue()- this.calcValue(sampleSet.getSample(i)))/ sampleSet.getSample(i).getStandardDeviation();
        }
        return chiSquare;
    }
    ...
    public double[] calcBetaMatrix(){
        ...
        for(int i=0;i<beta.length;i++){
            ...
            for(int j=0;j<sampleSet.getLength();j++){
                beta[i] += (sampleSet.getSample(j).getSampleValue()- calcValue(sampleSet.getSample(j)))/sampleSet.getSample(j).getStandardDeviation()*dyda[j][i];
            }
        }
        return beta;
    }
