...
public class NewtonSolveABCD extends Object implements java.io.Serializable {
    ...
    public void setfvec(){
        ...
        for (int i=0;i<characterizeClass.getLength();i++) {
            fvec.set(i,0,
            Math.log(characterizeClass.getTBPfractions(i))-characterizeClass.getCoef(0)-characterizeClass.getCoef(1)*(i+characterizeClass.getFirstPlusFractionNumber()));
        }
        
        for (int i=characterizeClass.getLength();i<2*characterizeClass.getLength();i++) {
            fvec.set(i,0,
            characterizeClass.getTBPdens(i-characterizeClass.getLength())-characterizeClass.getCoef(2)-characterizeClass.getCoef(3)*Math.log((i+characterizeClass.getFirstPlusFractionNumber()-characterizeClass.getLength())));
        }
        
         for (int i=0;i<characterizeClass.getLength();i++) {
             calcTPBfraction[i] = Math.exp(characterizeClass.getCoef(0)+characterizeClass.getCoef(1)*(i+characterizeClass.getFirstPlusFractionNumber()));
         }
