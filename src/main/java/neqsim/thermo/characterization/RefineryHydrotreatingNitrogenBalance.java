 ...
 *
 * <p>
 * The calculation is limited to stoichiometric accounting with an explicit hydrogen-use
 * assumption. It does not identify nitrogen species or predict reaction pathways, kinetics,
 * catalyst performance, operating conditions, heat duty, hydrocarbon yield, or compliance.
 *
 * @author esolbr1
 * @version 1.0
 */
public final class RefineryHydrotreatingNitrogenBalance implements Serializable {
  ...
  private RefineryHydrotreatingNitrogenBalance(double feedMassKg, double feedNitrogenMassFraction,
      ...
      double initialNitrogenMassKg, double nitrogenRemovedMassKg, double remainingNitrogenMassKg,
      double hydrogenConsumedMassKg, double ammoniaProducedMassKg,
      double hydrogenRetainedInLiquidMassKg, double productMassKg,
      double achievedProductNitrogenMassFraction, double totalMassBalanceResidualKg,
   ...
   * @param targetProductNitrogenMassFraction requested total-nitrogen mass fraction on the
   *        calculated liquid product mass
   * @param hydrogenMolesPerNitrogenMole explicit hydrogen consumption assumption in moles of H2
   *        per mole of nitrogen removed; must be at least 1.5
   * @return immutable balance receipt
   */
  public static RefineryHydrotreatingNitrogenBalance calculate(double feedMassKg,
      double feedNitrogenMassFraction, double targetProductNitrogenMassFraction,
    ...
    if (targetProductNitrogenMassFraction > feedNitrogenMassFraction) {
      throw new IllegalArgumentException(
          "targetProductNitrogenMassFraction must not exceed feedNitrogenMassFraction");
    }
    if (!Double.isFinite(hydrogenMolesPerNitrogenMole)
        || hydrogenMolesPerNitrogenMole < 1.5) {
      throw new IllegalArgumentException(
          "hydrogenMolesPerNitrogenMole must be finite and at least 1.5");
    }

    double initialNitrogenMassKg = feedMassKg * feedNitrogenMassFraction;
    double liquidMassChangePerNitrogenMass =
        (hydrogenMolesPerNitrogenMole * HYDROGEN_MOLAR_MASS_KG_PER_MOL
            - AMMONIA_MOLAR_MASS_KG_PER_MOL)
            / NITROGEN_MOLAR_MASS_KG_PER_MOL;
    double denominator =
        1.0 + targetProductNitrogenMassFraction * liquidMassChangePerNitrogenMass;
    ...
    double nitrogenRemovedMassKg =
        (initialNitrogenMassKg - targetProductNitrogenMassFraction * feedMassKg)
            / denominator;
    double nitrogenMolesRemoved =
        nitrogenRemovedMassKg / NITROGEN_MOLAR_MASS_KG_PER_MOL;
    ...
    double hydrogenRetainedInLiquidMassKg = nitrogenMolesRemoved
        * (hydrogenMolesPerNitrogenMole - 1.5) * HYDROGEN_MOLAR_MASS_KG_PER_MOL;
    ...
    double totalMassBalanceResidualKg =
        feedMassKg + hydrogenConsumedMassKg - productMassKg - ammoniaProducedMassKg;
    double nitrogenBalanceResidualKg =
        initialNitrogenMassKg - remainingNitrogenMassKg - nitrogenRemovedMassKg;

    double toleranceKg = 1.0e-12 * Math.max(1.0, feedMassKg);
    if (!Double.isFinite(productMassKg) || productMassKg <= 0.0
    ...
    return new RefineryHydrotreatingNitrogenBalance(feedMassKg, feedNitrogenMassFraction,
        targetProductNitrogenMassFraction, hydrogenMolesPerNitrogenMole, initialNitrogenMassKg,
        nitrogenRemovedMassKg, remainingNitrogenMassKg, hydrogenConsumedMassKg,
        ammoniaProducedMassKg, hydrogenRetainedInLiquidMassKg, productMassKg,
        achievedProductNitrogenMassFraction, totalMassBalanceResidualKg,
   ...
   * @param targetProductNitrogenMassFraction requested nitrogen mass fraction on the calculated
   *        liquid product mass
   * @param hydrogenMolesPerNitrogenMole explicit hydrogen consumption assumption in moles of H2
  ...
  public static RefineryHydrotreatingNitrogenBalance calculateForAssay(double feedMassKg,
      OilAssayCharacterisation assay, double targetProductNitrogenMassFraction,
      double hydrogenMolesPerNitrogenMole) {
    Objects.requireNonNull(assay, "assay");
    return calculate(feedMassKg, assay.getBulkNitrogenMassFraction(),
        targetProductNitrogenMassFraction, hydrogenMolesPerNitrogenMole);
  }
