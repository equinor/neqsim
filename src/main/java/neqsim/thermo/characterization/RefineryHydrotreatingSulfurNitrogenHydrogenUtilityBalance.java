...
/**
 * Immutable hydrogen utility receipt derived from a qualified coupled sulfur/nitrogen throughput balance.
 *
 * <p>
 ...
 * @version 1.0
 */
public final class RefineryHydrotreatingSulfurNitrogenHydrogenUtilityBalance implements Serializable {
  ...
  private RefineryHydrotreatingSulfurNitrogenHydrogenUtilityBalance(
      RefineryHydrotreatingSulfurNitrogenHydrogenRecycleThroughputBalance throughputBalance,
      double hydrogenLowerHeatingValueMegaJoulePerKg, double hydrogenCostPerKg, double freshHydrogenMassFlowKgPerHour,
      double consumedHydrogenMassFlowKgPerHour, double exportHydrogenMassFlowKgPerHour,
      double freshHydrogenChemicalPowerMegaWatt, double consumedHydrogenChemicalPowerMegaWatt,
      double exportHydrogenChemicalPowerMegaWatt, double hydrogenEnergyBalanceResidualMegaWatt,
      double freshHydrogenCostPerHour, double freshHydrogenCostPerTonneFeed, double freshHydrogenUtilizationFraction) {
   ...
   *
   * @param throughputBalance qualified coupled sulfur/nitrogen throughput receipt
   ...
   */
  public static RefineryHydrotreatingSulfurNitrogenHydrogenUtilityBalance calculate(
      RefineryHydrotreatingSulfurNitrogenHydrogenRecycleThroughputBalance throughputBalance,
    ...
    if (!Double.isFinite(hydrogenLowerHeatingValueMegaJoulePerKg) || hydrogenLowerHeatingValueMegaJoulePerKg <= 0.0) {
      throw new IllegalArgumentException("hydrogenLowerHeatingValueMegaJoulePerKg must be finite and positive");
    }
    ...
    double hydrogenMolarMassKgPerKmol = RefineryHydrotreatingSulfurBalance.HYDROGEN_MOLAR_MASS_KG_PER_MOL
        * MOLES_PER_KILOMOLE;
    ...
    double exportHydrogenMassFlow = throughputBalance.getExportHydrogenMolarFlowKmolPerHour()
        * hydrogenMolarMassKgPerKmol;
    double massResidual = freshHydrogenMassFlow - consumedHydrogenMassFlow - exportHydrogenMassFlow;
    double massTolerance = 1.0e-12 * Math.max(1.0, freshHydrogenMassFlow);
    if (!allFiniteNonNegative(freshHydrogenMassFlow, consumedHydrogenMassFlow, exportHydrogenMassFlow)
        || Math.abs(massResidual) > massTolerance) {
      throw new IllegalArgumentException("throughput balance does not define a closed external hydrogen balance");
    }

    double freshPower = freshHydrogenMassFlow * hydrogenLowerHeatingValueMegaJoulePerKg / SECONDS_PER_HOUR;
    double consumedPower = consumedHydrogenMassFlow * hydrogenLowerHeatingValueMegaJoulePerKg / SECONDS_PER_HOUR;
    double exportPower = exportHydrogenMassFlow * hydrogenLowerHeatingValueMegaJoulePerKg / SECONDS_PER_HOUR;
    ...
    double freshCostPerTonneFeed = freshCostPerHour * KILOGRAMS_PER_TONNE
        / throughputBalance.getFeedMassFlowKgPerHour();
    double utilizationFraction = freshHydrogenMassFlow == 0.0 ? 0.0 : consumedHydrogenMassFlow / freshHydrogenMassFlow;

    return new RefineryHydrotreatingSulfurNitrogenHydrogenUtilityBalance(throughputBalance,
        hydrogenLowerHeatingValueMegaJoulePerKg, hydrogenCostPerKg, freshHydrogenMassFlow, consumedHydrogenMassFlow,
        exportHydrogenMassFlow, freshPower, consumedPower, exportPower, energyResidual, freshCostPerHour,
        freshCostPerTonneFeed, utilizationFraction);
  }
  ...
  /** @return upstream coupled throughput receipt */
