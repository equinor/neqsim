 ...
 *
 * <p>
 * Excess-Gibbs-energy (activity-coefficient) component for the water-nitric-acid-sulfuric-acid system, using the Van
 * Laar model of Taleb, Ponche and Mirabel (1996). The activity coefficient and the pure-component saturation vapour
 * pressure are obtained from {@link neqsim.thermo.util.empiric.NitricSulfuricAcidVaporPressure}, so the component
 * reproduces the gamma-phi identity implemented by that utility. The activity model is from the cited source; the
 * utility documents an engineering adjustment to the pure HNO3 vapour pressure correlation.
 * </p>
 *
 * <p>
 * The fugacity coefficient is forced onto the symmetric (Raoult) reference state {@code phi_i = gamma_i * P0_i / P},
 * regardless of the {@code referenceStateType} stored in the component database. This guarantees the gamma-phi
 * equilibrium identity {@code fugacity_i = x_i * phi_i * P = gamma_i * x_i * P0_i} for water, nitric acid and sulfuric
 * acid even though the two acids are tagged as "solute" (Henry's-law) species in the NeqSim component database.
 * </p>
 *
 * @author NeqSim
 * @version $Id: $Id
...
public class ComponentGEVanLaarAcid extends ComponentGE {
  ...
  /**
   * Acid identity: 1 = water, 2 = nitric acid, 3 = sulfuric acid, 0 = other/inert species.
   */
  ...
  private double pureVaporPressureBar(double temperature) {
    if (acidIndex == 1) {
      return NitricSulfuricAcidVaporPressure.pureVaporPressureWater(temperature) / PASCALS_PER_BAR;
    } else if (acidIndex == 2) {
      return NitricSulfuricAcidVaporPressure.pureVaporPressureNitricAcid(temperature) / PASCALS_PER_BAR;
    } else if (acidIndex == 3) {
      return NitricSulfuricAcidVaporPressure.pureVaporPressureSulfuricAcid(temperature) / PASCALS_PER_BAR;
    }
   ...
   *
   * <p>
   * Overridden to enforce the symmetric (Raoult) reference state for every component, giving
   * {@code phi_i = gamma_i * P0_i / P}. This bypasses the Henry's-law branch that
   * {@link ComponentGE#fugcoef(PhaseInterface)} would otherwise apply to the acids and yields the gamma-phi identity
   * {@code fugacity_i = gamma_i * x_i * P0_i}.
   * </p>
   */
  @Override
