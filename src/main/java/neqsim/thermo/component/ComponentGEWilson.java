...
public class ComponentGEWilson extends ComponentGE{
    ...
    public double getCharEnergyParamter(PhaseInterface phase1, int comp1, int comp2) {
        ...
        double param1 = 0.0; //((ComponentWaxWilson) phase1.getComponent(comp1)).getWilsonInteractionEnergy(phase1);
        double param2 = 0.0;// ((ComponentWaxWilson) phase1.getComponent(comp2)).getWilsonInteractionEnergy(phase1);
        if(comp1==comp2) {
            return 1.0;
        }
        // this need to be corrected accordint to how to select energy of shortest carbon molecule .....
        if (phase1.getComponent(comp1).getMolarMass()> phase1.getComponent(comp2).getMolarMass()) {
            ...
            //param1 = ((ComponentWaxWilson) phase1.getComponent(comp1)).getWilsonInteractionEnergy(phase1);
            //param2 = ((ComponentWaxWilson) phase1.getComponent(comp2)).getWilsonInteractionEnergy(phase1);
        } else {
            param1 = ((ComponentGEWilson) phase1.getComponent(comp1)).getWilsonInteractionEnergy(phase1);
            param2 = ((ComponentGEWilson) phase1.getComponent(comp2)).getWilsonInteractionEnergy(phase1);
        }
        double energyParameter = Math.exp(-(param2 - param1) / (ThermodynamicConstantsInterface.R * phase1.getTemperature()));
    ...
    public double getWilsonInteractionEnergy(PhaseInterface phase1) {
        ...
        double deltaHvap = R * getTC() * (deltaHvap0 + omega * deltaHvap1 + omega * omega * deltaHvap2) * 4.1868;
        ...
        double Ttrans = 420.42 - 134.784 * Math.exp(-4.344 * Math.pow(carbonnumber + 6.592, 0.14627));
