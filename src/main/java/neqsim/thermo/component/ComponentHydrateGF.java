    ...
    public double fugcoef2(PhaseInterface phase, int numberOfComps, double temp, double pres) {
        ...
        if (componentName.equals("water")) {
            ...
            if (hydrateStructure == -1) {
                fugasityCoeffisient = getEmptyHydrateStructureVapourPressure(hydrateStructure, temp) * Math.exp(solvol / (R * temp) * (pres - getEmptyHydrateStructureVapourPressure(hydrateStructure, temp)) * 1e5) / pres;
            } else {
                double val = 0.0;
                double tempy = 1.0;
...
/*
                System.out.println("fugacity " + (getEmptyHydrateStructureVapourPressure(hydrateStructure, temp) * Math.exp(solvol / (R * temp) * (pres - getEmptyHydrateStructureVapourPressure(hydrateStructure, temp)) * 1e5) / pres));
                ...
                System.out.println("fugacity folas " + (getEmptyHydrateStructureVapourPressure(hydrateStructure, temp) * Math.exp(solvol / (R * temp) * (pres - getEmptyHydrateStructureVapourPressure(hydrateStructure, temp)) * 1e5)));
*/

                //System.out.println("pointing " +(Math.exp(solvol/(R*temp)*((pres-getEmptyHydrateStructureVapourPressure(hydrateStruct,temp))*1e5))));
    ...
    public double fugcoef(PhaseInterface phase, int numberOfComps, double temp, double pres) {
        ...
        if (hydrateStructure == -1) {
            stableStructure = -1;
            //fugasityCoeffisient = 1.02 * getEmptyHydrateStructureVapourPressure(hydrateStructure, temp) * Math.exp(getMolarVolumeHydrate(hydrateStructure, temp) / (R * temp) * (pres - getEmptyHydrateStructureVapourPressure(hydrateStructure, temp)) * 1e5) / pres;
            ...
            double LNFUG_ICEREF = refWaterFugacityCoef - (dhf / (R * tmi)) * (tmi / temp - 1.0) + (dcp / R) * ((tmi / temp) - 1.0 - Math.log(tmi / temp));
            double VM = 0.0;
            double K1 = 1.6070E-4;
            double K2 = 3.4619E-7;
...
*/
    ...
    public double calcCKI(int stucture, int cavityType, PhaseInterface phase) {
        ...
        return Ak[stucture][cavityType] / (phase.getTemperature()) * Math.exp(Bk[stucture][cavityType] / (phase.getTemperature()));
    }
}
