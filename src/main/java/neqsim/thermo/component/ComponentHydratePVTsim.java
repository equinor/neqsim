        ...
        if (hydrateStructure == -1) {
            ...
            for (int structure = 0; structure < 2; structure++) {
                ...
                if (componentName.equals("water")) {
                    ...
                    fugasityCoeffisient = alphaWater * Math.exp(
                            val + calcDeltaChemPot(phase, numberOfComps, temp, pres, hydrateStructure) + wateralphaRef)
                            / pres;
...
}logFugasityCoeffisient=Math.log(fugasityCoeffisient);hydrateStructure=stableStructure;
