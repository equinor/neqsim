...
public class ComponentModifiedFurstElectrolyteEosMod2004 extends ComponentSrk {
    ...
    public ComponentModifiedFurstElectrolyteEosMod2004(String component_name, double moles, double molesInPhase,
            int compnumber) {
    ...
    public ComponentModifiedFurstElectrolyteEosMod2004 clone() {
        ...
        public void Finit(PhaseInterface phase, double temp, double pres, double totMoles,
                ...
                Wi = ((PhaseModifiedFurstElectrolyteEosMod2004) phase).calcWi(componentNumber,
                                phase, temp, pres, numberOfComponents);
                WiT = ((PhaseModifiedFurstElectrolyteEosMod2004) phase).calcWiT(componentNumber,
                                phase, temp, pres, numberOfComponents);
                                ...
                                * R * temp) * diElectricdndT
                                + electronCharge * electronCharge * avagadroNumber
                                                / (vacumPermittivity * Math.pow(
                                                                ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
        ...
        public double dFLRdNdV(PhaseInterface phase, int numberOfComponents, double temperature,
                        double pressure) {
                return 1e-5 * (((PhaseModifiedFurstElectrolyteEosMod2004) phase).dFdAlphaLRdX()
                                * XLRi
                                ...
                                * ((ComponentModifiedFurstElectrolyteEosMod2004) ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                ...
                                + ((PhaseModifiedFurstElectrolyteEosMod2004) phase).dFdAlphaLR()
                                                * dAlphaLRdndn(j, phase, numberOfComponents,
                                                                temperature, pressure)
                                + ((PhaseModifiedFurstElectrolyteEosMod2004) phase).dFdAlphaLRdX()
                                                * alphai
                                                * ((ComponentModifiedFurstElectrolyteEosMod2004) ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
        ...
        public double calcXLRdN(PhaseInterface phase, int numberOfComponents, double temperature,
                ...
                return Math.pow(getIonicCharge(), 2.0)
                                * ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                ...
                                / (1.0 + ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                                .getShieldingParameter()
        ...
        public double calcSolventdiElectricdndn(int j, PhaseInterface phase, int numberOfComponents,
                        double temperature, double pressure) {
                if (getIonicCharge() != 0 || ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                .getComponents()[j].getIonicCharge() != 0) {
                        return 0.0;
                }
                double ans2 = 0.0;
                for (int i = 0; i < numberOfComponents; i++) {
        ...
        public double calcdiElectricdn(PhaseInterface phase, int numberOfComponents,
                ...
                double X = (1.0 - ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic())
                                / (1.0 + ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                                .getEpsIonic() / 2.0);
                double Y = ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                .getSolventDiElectricConstant() - 1.0;
                double dXdf = getEpsIonici() * -3.0 / 2.0 / Math
                                .pow(((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic()
                                                / 2.0 + 1.0, 2.0);
        ...
        public double calcdiElectricdndV(PhaseInterface phase, int numberOfComponents,
                        double temperature, double pressure) {
                double dXdf = ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonicdV()
                                * -3.0 / 2.0
                                / Math.pow(((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                                .getEpsIonic() / 2.0 + 1.0, 2.0);
                double dYdf = getSolventDiElectricConstantdn();
                double d1 = ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                .getSolventDiElectricConstant();
                double d2 = epsIoniciV * -3.0 / 2.0 / Math
                                .pow(((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic()
                                                / 2.0 + 1.0, 2.0);
                double d3 = ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonicdV()
                                * epsIonici * 3.0 / 2.0
                                / Math.pow(((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                                .getEpsIonic() / 2.0 + 1.0, 3.0);
        ...
        public double dFSR2dNdT(PhaseInterface phase, int numberOfComponents, double temperature,
                ...
                return ((PhaseModifiedFurstElectrolyteEosMod2004) phase).FSR2W() * WiT
                                + ((PhaseModifiedFurstElectrolyteEosMod2004) phase).FSR2epsW()
                                                * epsi
                                                * ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                                                .getWT();
        }
        ...
        public double dFSR2dNdV(PhaseInterface phase, int numberOfComponents, double temperature,
                        double pressure) {
                return 1.0e-5 * (+((PhaseModifiedFurstElectrolyteEosMod2004) phase).FSR2epseps()
                                * epsi
                                * ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsdV()
                                + ((PhaseModifiedFurstElectrolyteEosMod2004) phase).FSR2eps()
                                                * epsiV
                                + ((PhaseModifiedFurstElectrolyteEosMod2004) phase).FSR2epsW() * Wi
                                                * ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                                                .getEpsdV()
                                + ((PhaseModifiedFurstElectrolyteEosMod2004) phase).FSR2epsV()
                                                * epsi
