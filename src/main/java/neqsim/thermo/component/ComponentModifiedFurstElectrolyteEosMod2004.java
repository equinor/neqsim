...
public class ComponentModifiedFurstElectrolyteEosMod2004 extends ComponentSrk {
    ...
    double Wi = 0, WiT = 0.0, epsi = 0, epsiV = 0, epsIonici = 0, epsIoniciV = 0, dEpsdNi = 0,
            ...
            diElectricdn = 0, diElectricdndV = 0, diElectricdndT = 0, bornterm = 0, alphai = 0.0,
            alphaiT = 0.0, alphaiV = 0.0, XLRi = 0.0, XBorni = 0.0;
    ...
    public void Finit(PhaseInterface phase, double temp, double pres, double totMoles, double beta,
        ...
        WiT = ((PhaseModifiedFurstElectrolyteEosMod2004) phase).calcWiT(componentNumber, phase,
        ...
        alphai = -(electronCharge * electronCharge * avagadroNumber) / (vacumPermittivity * Math
                .pow(((PhaseModifiedFurstElectrolyteEosMod2004) phase).getDiElectricConstant(), 2.0)
                * R * temp) * diElectricdn;
        alphaiT = -electronCharge * electronCharge * avagadroNumber
                / (vacumPermittivity * Math.pow(
                        ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getDiElectricConstant(),
                        2.0) * R * temp)
                * diElectricdndT
                + electronCharge * electronCharge * avagadroNumber / (vacumPermittivity * Math.pow(
                        ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getDiElectricConstant(),
                        2.0) * R * temp * temp) * diElectricdn
                + 2.0 * electronCharge * electronCharge * avagadroNumber
                        / (vacumPermittivity
                                * Math.pow(((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                                        .getDiElectricConstant(), 3.0)
                                * R * temp)
                        * diElectricdn * ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
    ...
    public double calcSolventdiElectricdn(PhaseInterface phase, int numberOfComponents,
        ...
        public ComponentModifiedFurstElectrolyteEosMod2004(int number, double TC, double PC,
                        double M, double a, double moles) {
                super(number, TC, PC, M, a, moles);
        }
    ...
    public double calcSolventdiElectricdndn(int j, PhaseInterface phase, int numberOfComponents,
        ...
        public Object clone() {
                ...
                try {
                        clonedComponent =
                                        (ComponentModifiedFurstElectrolyteEosMod2004) super.clone();
                } catch (Exception e) {
                        logger.error("Cloning failed.", e);
                }
    ...
    public double calcdiElectricdn(PhaseInterface phase, int numberOfComponents, double temperature,
        ...
        double Y = ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getSolventDiElectricConstant()
                - 1.0;
        double dXdf = getEpsIonici() * -3.0 / 2.0 / Math.pow(
                ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic() / 2.0 + 1.0, 2.0);
    ...
    public double calcdiElectricdndV(PhaseInterface phase, int numberOfComponents,
        ...
        double dXdf = ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonicdV() * -3.0 / 2.0
                / Math.pow(
                        ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic() / 2.0 + 1.0,
                        2.0);
        double dYdf = getSolventDiElectricConstantdn();
        double d1 =
                ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getSolventDiElectricConstant();
        double d2 = epsIoniciV * -3.0 / 2.0 / Math.pow(
                ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic() / 2.0 + 1.0, 2.0);
        double d3 = ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonicdV() * epsIonici
                * 3.0 / 2.0
                / Math.pow(
                        ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic() / 2.0 + 1.0,
                        3.0);
    ...
    public double calcdiElectricdndn(int j, PhaseInterface phase, int numberOfComponents,
        ...
        double dXdfdfj = getEpsIonici() * -3.0 / 2.0 / Math.pow(
                ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic() / 2.0 + 1.0, 2.0);

        double dXdf =
                ((ComponentModifiedFurstElectrolyteEosMod2004) ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                        .getComponents()[j]).getEpsIonici() * getEpsIonici() * 3.0 / 2.0
                        / Math.pow(((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic()
                                / 2.0 + 1.0, 3.0);
        double d1 =
                ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getSolventDiElectricConstant();
    ...
    public double calcdiElectricdndT(PhaseInterface phase, int numberOfComponents,
        ...
        double X = (1.0 - ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic())
                / (1.0 + ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic() / 2.0);
        double Y =
                ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getSolventDiElectricConstantdT();
        double dXdf = getEpsIonici() * -3.0 / 2.0 / Math.pow(
                ((PhaseModifiedFurstElectrolyteEosMod2004) phase).getEpsIonic() / 2.0 + 1.0, 2.0);
        ...
        return ((PhaseModifiedFurstElectrolyteEosMod2004) phase).FSR2epseps() * epsi
                * ((ComponentModifiedFurstElectrolyteEosMod2004) ((PhaseModifiedFurstElectrolyteEosMod2004) phase)
                ...
                + ((PhaseModifiedFurstElectrolyteEosMod2004) phase).FSR2W()
                        * ((PhaseModifiedFurstElectrolyteEosMod2004) phase).calcWij(componentNumber,
                                j, phase, temperature, pressure, numberOfComponents)
                + ((PhaseModifiedFurstElectrolyteEosMod2004) phase).FSR2epsW() * epsi
                        * ((PhaseModifiedFurstElectrolyteEosMod2004) phase).calcWi(j, phase,
    ...
    public double dEpsdNidV(PhaseInterface phase, int numberOfComponents, double temperature,
        ...
        return (-avagadroNumber * pi / 6.0 * Math.pow(lennardJonesMolecularDiameter * 1.0e-10, 3.0)
                * (1.0 / (Math.pow(
                        phase.getMolarVolume() * 1.0e-5 * phase.getNumberOfMolesInPhase(), 2.0))));
    }
