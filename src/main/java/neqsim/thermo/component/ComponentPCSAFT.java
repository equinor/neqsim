...
public class ComponentPCSAFT extends ComponentSrk{
    ...
    private double F1dispVolTermdn = 1.0, F1dispSumTermdn = 1.0, F1dispI1dn = 1.0, F2dispI2dn=1.0, F2dispZHCdn=1.0, F2dispVolTermdn=1.0,F2dispSumTermdn=1;
    ...
    public void Finit(PhaseInterface phase, double temp, double pres, double totMoles, double beta, int numberOfComponents,  int type){
        ...
        F1dispVolTermdn = 1.0*ThermodynamicConstantsInterface.avagadroNumber/((PhasePCSAFT)phase).getVolumeSAFT();
        ...
        F1dispI1dn = ((PhasePCSAFT)phase).calcF1dispI1dN()*getDnSAFTdi() +  ((PhasePCSAFT)phase).calcF1dispI1dm()*getDmSAFTdi();
        F2dispI2dn = ((PhasePCSAFT)phase).calcF2dispI2dN()*getDnSAFTdi() +  ((PhasePCSAFT)phase).calcF2dispI2dm()*getDmSAFTdi();
        
        F2dispZHCdn = ((PhasePCSAFT)phase).getF2dispZHCdN()*getDnSAFTdi() +  ((PhasePCSAFT)phase).getF2dispZHCdm()*getDmSAFTdi();
...
//        System.out.println("term getF23 " + dF_DISP2_SAFTdN(phase,numberOfComponents,temperature,pressure));
//        
//        System.out.println("term furgacity coef " + getFugasityCoefficient());
        return useHS*dF_HC_SAFTdN(phase,numberOfComponents,temperature,pressure)+useDISP1*dF_DISP1_SAFTdN(phase,numberOfComponents,temperature,pressure)+useDISP2*dF_DISP2_SAFTdN(phase,numberOfComponents,temperature,pressure);
    }
    
    public double dF_HC_SAFTdN(PhaseInterface phase, int numberOfComponents, double temperature, double pressure){
        return ((PhasePCSAFT)phase).F_HC_SAFT()/phase.getNumberOfMolesInPhase() + phase.getNumberOfMolesInPhase()*(getDmSAFTdi()*((PhasePCSAFT)phase).getAHSSAFT()+((PhasePCSAFT)phase).getmSAFT()*getdahsSAFTdi()
        -       (mSAFTi-1.0)/phase.getNumberOfMolesInPhase()*Math.log(((PhasePCSAFT)phase).getGhsSAFT())
                + ((PhasePCSAFT)phase).getMmin1SAFT()/phase.getNumberOfMolesInPhase()*Math.log(((PhasePCSAFT)phase).getGhsSAFT())
                -
    ...
    public double dF_DISP1_SAFTdN(PhaseInterface phase, int numberOfComponents, double temperature, double pressure){
        return ((PhasePCSAFT)phase).F_DISP1_SAFT()/phase.getNumberOfMolesInPhase() + phase.getNumberOfMolesInPhase()*(
                (-2.0*ThermodynamicConstantsInterface.pi*F1dispVolTermdn*((PhasePCSAFT)phase).getF1dispSumTerm()*((PhasePCSAFT)phase).getF1dispI1()
                -2.0*ThermodynamicConstantsInterface.pi*((PhasePCSAFT)phase).getF1dispVolTerm()*F1dispSumTermdn*((PhasePCSAFT)phase).getF1dispI1()
                -2.0*ThermodynamicConstantsInterface.pi*((PhasePCSAFT)phase).getF1dispVolTerm()*((PhasePCSAFT)phase).getF1dispSumTerm()*F1dispI1dn
    ...
    public double calcdmSAFTdi(PhaseInterface phase, int nuberOfComponents, double temp, double pres){
        return mSAFTi/phase.getNumberOfMolesInPhase() - ((PhasePCSAFT)phase).getmSAFT()/phase.getNumberOfMolesInPhase();
    }
    
    public double dF_DISP2_SAFTdN(PhaseInterface phase, int numberOfComponents, double temperature, double pressure){
        
        return ((PhasePCSAFT)phase).F_DISP2_SAFT()/phase.getNumberOfMolesInPhase() + phase.getNumberOfMolesInPhase()*(
                (
                -ThermodynamicConstantsInterface.pi*getDmSAFTdi()*((PhasePCSAFT)phase).getF1dispVolTerm()*((PhasePCSAFT)phase).getF2dispSumTerm()*((PhasePCSAFT)phase).getF2dispI2()*((PhasePCSAFT)phase).getF2dispZHC()
                -ThermodynamicConstantsInterface.pi*((PhasePCSAFT)phase).getmSAFT()*F2dispVolTermdn*((PhasePCSAFT)phase).getF2dispSumTerm()*((PhasePCSAFT)phase).getF2dispI2()*((PhasePCSAFT)phase).getF2dispZHC()
                -ThermodynamicConstantsInterface.pi*((PhasePCSAFT)phase).getmSAFT()*((PhasePCSAFT)phase).getF1dispVolTerm()*F2dispSumTermdn*((PhasePCSAFT)phase).getF2dispI2()*((PhasePCSAFT)phase).getF2dispZHC()
                -ThermodynamicConstantsInterface.pi*((PhasePCSAFT)phase).getmSAFT()*((PhasePCSAFT)phase).getF1dispVolTerm()*((PhasePCSAFT)phase).getF2dispSumTerm()*F2dispI2dn*((PhasePCSAFT)phase).getF2dispZHC()
                -ThermodynamicConstantsInterface.pi*((PhasePCSAFT)phase).getmSAFT()*((PhasePCSAFT)phase).getF1dispVolTerm()*((PhasePCSAFT)phase).getF2dispSumTerm()*((PhasePCSAFT)phase).getF2dispI2()*F2dispZHCdn
                ));///(ThermodynamicConstantsInterface.R*temperature);
    }
    ...
    public double calcF1dispSumTermdn(PhaseInterface phase, int numberOfComponents, double temperature, double pressure){
        ...
        for(int i=0;i<phase.getNumberOfComponents();i++){
            temp1 += phase.getComponent(i).getNumberOfMolesInPhase()*phase.getComponent(i).getmSAFTi()*getmSAFTi() *
                    Math.sqrt(getEpsikSAFT()/temperature*phase.getComponent(i).getEpsikSAFT()/temperature)*(1.0-((PhaseEosInterface)phase).getMixingRule().getBinaryInteractionParameter(componentNumber, i)) *
    ...
    public double calcF2dispSumTermdn(PhaseInterface phase, int numberOfComponents, double temperature, double pressure){
        ...
        for(int i=0;i<phase.getNumberOfComponents();i++){
            temp1 += phase.getComponent(i).getNumberOfMolesInPhase()*phase.getComponent(i).getmSAFTi()*getmSAFTi() *
                    getEpsikSAFT()/temperature*phase.getComponent(i).getEpsikSAFT()/temperature*Math.pow((1.0-((PhaseEosInterface)phase).getMixingRule().getBinaryInteractionParameter(componentNumber, i)),2.0) *
                    Math.pow(0.5*(phase.getComponent(i).getSigmaSAFTi()+getSigmaSAFTi()),3.0);
       // System.out.println("kij "+ ((PhaseEosInterface)phase).getMixingRule().getBinaryInteractionParameter(componentNumber, i));
    ...
    public double calcdnSAFTdi(PhaseInterface phase, int nuberOfComponents, double temp, double pres){
        double temp1 = phase.getNumberOfMolesInPhase()/((PhasePCSAFT)phase).getVolumeSAFT()*getmSAFTi()*Math.pow(getdSAFTi(),3.0)*1.0/phase.getNumberOfMolesInPhase()
        + 1.0/((PhasePCSAFT)phase).getVolumeSAFT()*((PhasePCSAFT)phase).getDSAFT()
        - 1.0/Math.pow(phase.getNumberOfMolesInPhase(),1.0)*phase.getNumberOfMolesInPhase()/((PhasePCSAFT)phase).getVolumeSAFT()*((PhasePCSAFT)phase).getDSAFT();
