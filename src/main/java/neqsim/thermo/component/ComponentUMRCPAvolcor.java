 ...
 *
 * <p>
 * Volume-translated UMR-CPA component. It combines the UMR-CPA physical and association term
 * (inherited from {@link ComponentUMRCPA}) with the consistent Peneloux volume translation of
 * {@link ComponentPRvolcor}. The translation parameter <i>c</i> and its mole-number derivatives
 * (C<sub>i</sub>, C<sub>ij</sub>, C<sub>iT</sub>) are injected into the reduced residual Helmholtz
 * energy mole-derivatives, so the translation propagates consistently into the fugacity
 * coefficient, partial molar volumes and all caloric derivatives.
 * </p>
 *
 * <p>
 * The translation acts only on the cubic (physical) part of the Helmholtz energy (Option A): the
 * per-component <i>c</i> equals the inherited UMR-CPA Peneloux shift, which is the PR shift for
 * non-associating compounds and zero for associating compounds until a dedicated UMR-CPA Rackett Z
 * is regressed. The Wertheim association term is left unchanged.
 * </p>
 *
 * @author Even Solbraa
 * @version $Id: $Id
...
public class ComponentUMRCPAvolcor extends ComponentUMRCPA {
  ...
  public void Finit(PhaseInterface phase, double temp, double pres, double totMoles, double beta,
    ...
    if (initType >= 2) {
      CiT = ((PhaseUMRCPAvolcor) phase).calcCiT(componentNumber, phase, temp, pres,
          numberOfComponents);
    }
    if (initType >= 3) {
      for (int j = 0; j < numberOfComponents; j++) {
        Cij[j] = ((PhaseUMRCPAvolcor) phase).calcCij(componentNumber, j, phase, temp, pres,
            numberOfComponents);
      }
    }
  }
  ...
  public double dFdN(PhaseInterface phase, int numberOfComponents, double temperature,
    ...
    return super.dFdN(phase, numberOfComponents, temperature, pressure)
        + ((PhaseUMRCPAvolcor) phase).FC() * getCi();
  }
  ...
  public double dFdNdV(PhaseInterface phase, int numberOfComponents, double temperature,
    ...
    return super.dFdNdV(phase, numberOfComponents, temperature, pressure)
        + ((PhaseUMRCPAvolcor) phase).FCV() * getCi();
  }
  ...
  public double dFdNdN(int j, PhaseInterface phase, int numberOfComponents, double temperature,
    ...
    double deltaC = locFnC * locCj + locFBC * locCj * getBi()
        + (locFnC + locFCC * locCj + locFBC * compArray[j].getBi() + locFCD * compArray[j].getAi())
            * getCi()
