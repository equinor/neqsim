...
public class ComponentWonWax extends ComponentSolid {
     ...
     */
    public ComponentWonWax(String component_name, double moles, double molesInPhase,
            int compnumber) {
    ...
    public double getWonParam(PhaseInterface phase1) {
        ...
        double Hvap = 1.9858775 * getTC()
                * (deltaHvap0 + omega * deltaHvap1 + omega * omega * deltaHvap2);
        return Math.sqrt((Hvap - Hf - 1.9858775 * phase1.getTemperature()) / (getMolarMass()
                / (0.8155 + 0.6273e-4 * getMolarMass() - 13.06 / getMolarMass())));
    }
}
