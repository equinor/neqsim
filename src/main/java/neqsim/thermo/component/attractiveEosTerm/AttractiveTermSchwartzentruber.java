...
public class AttractiveTermSchwartzentruber extends AttractiveTermBaseClass {
        ...
        return Math.pow(
                1.0 + m * (1.0 - Math.sqrt(temperature / getComponent().getTC())) - parameters[0]
                        ...
                        * (1.0 + parameters[1] * temperature / getComponent().getTC()
                                + parameters[2]
                                        * Math.pow(temperature / getComponent().getTC(), 2.0)),
                2.0);
    }
    ...
    public double diffalphaT(double temperature) {
        return 2.0
                ...
                * (-m / Math.sqrt(temperature / getComponent().getTC()) / getComponent().getTC()
                        / 2.0
                        ...
                        - parameters[0] * (1.0 - temperature / getComponent().getTC())
                                * (parameters[1] / getComponent().getTC() + 2.0 * parameters[2]
                                        * temperature
                                        / (getComponent().getTC() * getComponent().getTC())));
    }
                        ...
                        - parameters[0] * (1.0 - temperature / getComponent().getTC())
                                * (parameters[1] / getComponent().getTC() + 2.0 * parameters[2]
                                        * temperature
                                        / (getComponent().getTC() * getComponent().getTC())),
                2.0)
                + 2.0 * (1.0 + m * (1.0 - Math.sqrt(temperature / getComponent().getTC()))
                        - parameters[0] * (1.0 - temperature / getComponent().getTC()) * (1.0
                                ...
                                + 2.0 * parameters[0] / getComponent().getTC()
                                        * (parameters[1] / getComponent().getTC() + 2.0
                                                * parameters[2] * temperature
                                                / (getComponent().getTC() * getComponent().getTC()))
                                - 2.0 * parameters[0] * (1.0 - temperature / getComponent().getTC())
                                        * parameters[2]
                                        / (getComponent().getTC() * getComponent().getTC()));
    }
