...
public class PhaseCSPsrkEos extends PhaseSrkEos{
    ...
    public PhaseCSPsrkEos() {
        ...
        brefBWRSPhase = (Math.pow(2.0,1.0/3.0)-1.0)/3.0 * R * refBWRSPhase.getComponent(0).getTC() / refBWRSPhase.getComponent(0).getPC();
        mrefBWRSPhase = (0.48 + 1.574 * refBWRSPhase.getComponent(0).getAcentricFactor() - 0.175 * refBWRSPhase.getComponent(0).getAcentricFactor() *
        refBWRSPhase.getComponent(0).getAcentricFactor());
        acrefBWRSPhase = 1.0/(9.0*(Math.pow(2.0,1.0/3.0)-1.0)) * R * R * refBWRSPhase.getComponent(0).getTC() * refBWRSPhase.getComponent(0).getTC() / refBWRSPhase.getComponent(0).getPC();
    }
    ...
    public void addcomponent(String componentName, double moles,double molesInPhase, int compNumber){
        ...
        componentArray[compNumber] = new ComponentCSPsrk(componentName, moles, molesInPhase, compNumber);
    ...
    public void init(double totalNumberOfMoles, int numberOfComponents, int type, int phase, double beta){ // type = 0 start init type =1 gi nye betingelser
        ...
        if(type==0){ refBWRSPhase.init(1.0,1,0,phase,1.0); refBWRSPhase.init(1.0,1,3,phase,1.0);}
        else {
    ...
    public double getF(){
        return f_scale_mix*refBWRSPhase.getF()/refBWRSPhase.getNumberOfMolesInPhase()*refBWRSPhase.getTemperature()/getTemperature();
    }
    
    
    public double dFdV(){
        return (f_scale_mix*refBWRSPhase.dFdV()/refBWRSPhase.getNumberOfMolesInPhase()/h_scale_mix)*refBWRSPhase.getTemperature()/getTemperature();
    }
    
    public double dFdVdV(){
        return (f_scale_mix*refBWRSPhase.dFdVdV()/refBWRSPhase.getNumberOfMolesInPhase()/h_scale_mix/h_scale_mix)*refBWRSPhase.getTemperature()/getTemperature();
    }
    
    
    public double dFdVdVdV(){
        return (f_scale_mix*refBWRSPhase.dFdVdVdV()/refBWRSPhase.getNumberOfMolesInPhase()/h_scale_mix/h_scale_mix/h_scale_mix)*refBWRSPhase.getTemperature()/getTemperature();
    }
