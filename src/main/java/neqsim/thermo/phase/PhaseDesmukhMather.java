...
public class PhaseDesmukhMather extends PhaseGE {
    ...
    public void setMixingRule(int type){
        ...
        for(int k=0; k<getNumberOfComponents(); k++){
            ...
            for(int l=k; l<getNumberOfComponents(); l++){
                try{
                    if(k==l){
                        if(getComponents()[l].getComponentName().equals("MDEA") && getComponents()[k].getComponentName().equals("MDEA")){
                            aij[k][l] = -0.0828487;
    ...
    public double getExessGibbsEnergy(PhaseInterface phase, int numberOfComponents, double temperature, double pressure, int phasetype){
        ...
        for (int i=0; i < numberOfComponents; i++){
            GE += phase.getComponents()[i].getx()*Math.log(((ComponentDesmukhMather) componentArray[i]).getGamma(phase, numberOfComponents, temperature,  pressure, phasetype));
        }
