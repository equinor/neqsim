                ...
                if (type > 1) {
                    ...
                    if (type > 2) {
                        for (int p = 0; p < numberOfComponents; p++) {
                            ...
                            Klkni[p][i][j] = KlkMatrix.get(i, j) * (t1 + t2 + ((ComponentElectrolyteCPA) getComponent(p)).calc_lngi(this));
...
// second order method not working correctly and not used t the moment b ecause of numerical stability
    ...
    public boolean solveX2(int maxIter) {
        ...
        do {
            ...
            err = 0.0;
            for (int i = 0; i < getTotalNumberOfAccociationSites(); i++) {
                old = ((ComponentCPAInterface) getComponent(moleculeNumber[i])).getXsite()[assSiteNumber[i]];
                neeval = 0;
                for (int j = 0; j < getTotalNumberOfAccociationSites(); j++) {
                    neeval += getComponent(moleculeNumber[j]).getNumberOfMolesInPhase() * delta[i][j] * ((ComponentCPAInterface) getComponent(moleculeNumber[j])).getXsite()[assSiteNumber[j]];
  ...
  /**
    ...
    public double molarVolume2(double pressure, double temperature, double A, double B, int phasetype) throws neqsim.util.exception.IsNaNException, neqsim.util.exception.TooManyIterationsException {
...
// System.out.println("BonV: " + BonV + " "+"  itert: " +   iterations +" " +h + " " +dh + " B " + Btemp  + " gv" + gV() + " fv " + fv() + " fvv" + fVV());
    ...
    public double calc_lngVV() {
        ...
        double val = -x / (getTotalVolume() * getTotalVolume() * u) + xV / (getTotalVolume() * u) - x / (getTotalVolume() * u * u) * (-1.0) * xV;
        return -val;
//        
...
//        return gvv;
    ...
    public double calc_lngVVV() {
        ...
        double gvv = -0.21434375 / (temp1 * temp1 * temp1) * getB() * getB() * getB() / (totVol5 * totVol) - 0.135375E1 / (temp1 * temp1) * getB() * getB() / (totVol5) - 0.285E1 / (temp1) * getB() / (totVol4);
        return gvv;
    }
}
