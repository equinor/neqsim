...
public class PhaseGENRTLmodifiedWS extends PhaseGENRTLmodifiedHV {
    ...
    public double getExessGibbsEnergy(PhaseInterface phase, int numberOfComponents, double temperature, double pressure, int phasetype){
        ...
        for (int i=0; i < numberOfComponents; i++){
            ...
            if(type==1) {
                GE += phase.getComponents()[i].getx()*Math.log(((ComponentGENRTLmodifiedWS) componentArray[i]).getGamma(phase, numberOfComponents, temperature,  pressure, phasetype, alpha, Dij, DijT, intparam, mixRule));
            }
        }
