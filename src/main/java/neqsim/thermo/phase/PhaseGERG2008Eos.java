...
public class PhaseGERG2008Eos extends PhaseEos {
  ...
  protected transient double referenceMolarDensityGuess = Double.NaN;
  ...
  private void calculateLogFugacityCoefficients() {
    ...
    if (!Double.isFinite(molarDensity) || molarDensity <= 0.0 || !Double.isFinite(compressibilityFactor)
      ...
      throw new IllegalStateException("GERG fugacity calculation requires positive density and compressibility");
    }
    ...
    for (int component = 0; component < componentCount; component++) {
      ...
      boolean centralDifference = moleFractions[component] > 2.0 * perturbation;
      ...
      if (centralDifference) {
        double minusMoles = 1.0 - perturbation;
        double[] minusComposition = perturbComposition(moleFractions, component, -perturbation, minusMoles);
        double minusAlphaR = residualHelmholtzEnergy(molarDensity * minusMoles, minusComposition);
        residualChemicalPotential = (plusMoles * plusAlphaR - minusMoles * minusAlphaR) / (2.0 * perturbation);
      } else {
  ...
  public double getDensity_GERG2008() {
    ...
    double molarDensity = gerg.getMolarDensity(referenceMolarDensityGuess);
