 ...
 *
 * <p>
 * Excess-Gibbs-energy (activity-coefficient) liquid phase for the water-nitric-acid-sulfuric-acid system, using the Van
 * Laar model of Taleb, Ponche and Mirabel (1996). The phase holds
 * {@link neqsim.thermo.component.ComponentGEVanLaarAcid} components, each of which evaluates its activity coefficient
 * and pure-component vapour pressure from {@link neqsim.thermo.util.empiric.NitricSulfuricAcidVaporPressure}.
 * </p>
 *
 * @author NeqSim
 * @version $Id: $Id
...
public class PhaseGEVanLaarAcid extends PhaseGE {
  ...
  /**
   * Molar excess Gibbs energy contribution G^E/(R T n), updated on each evaluation.
   */
  ...
  public void setAlpha(double[][] alpha) {
    throw new UnsupportedOperationException(
        "The Van Laar acid phase uses fixed correlation parameters and does not accept an alpha matrix.");
  }
  ...
  public void setDij(double[][] Dij) {
    throw new UnsupportedOperationException(
        "The Van Laar acid phase uses fixed correlation parameters and does not accept a Dij matrix.");
  }
  ...
  public void setDijT(double[][] DijT) {
    throw new UnsupportedOperationException(
        "The Van Laar acid phase uses fixed correlation parameters and does not accept a temperature Dij matrix.");
  }
  ...
  public double getExcessGibbsEnergy(PhaseInterface phase, int numberOfComponents, double temperature, double pressure,
    ...
    for (int i = 0; i < numberOfComponents; i++) {
      double gammaI = ((ComponentGEInterface) componentArray[i]).getGamma(phase, numberOfComponents, temperature,
          pressure, pt, null, null, null, null);
