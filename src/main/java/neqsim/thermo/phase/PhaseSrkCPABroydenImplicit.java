 ...
 *
 * <p>
 * Improves on the fully implicit coupled Newton-Raphson approach of Igben et al. (2026) by using Broyden's rank-1
 * update of the inverse Jacobian after the first full Newton step. This reduces the per-iteration cost from O(n_s^3)
 * (Gaussian elimination) to O(n_s^2) (rank-1 update via the Sherman-Morrison formula), while maintaining superlinear
 * convergence.
 * </p>
 *
 * <p>
 ...
 * </p>
 * <ol>
 * <li>Iteration 1: Compute full analytic Jacobian J, invert to get H = J^{-1}, solve delta_x = -H * R</li>
 * <li>Iterations 2+: Update H via Broyden's "good" formula: H_{k+1} = H_k + (dx - H_k*df) * (dx^T * H_k) / (dx^T * H_k
 * * df)</li>
 * <li>Periodic refresh: Recompute full Jacobian when convergence stalls (||R_new||/||R_old|| &gt; 0.5 for 2 consecutive
 * steps)</li>
 * </ol>
 *
 * <p>
 ...
 * </p>
 * <ul>
 * <li>C.G. Broyden, A class of methods for solving nonlinear simultaneous equations, Math. Comput. 19 (1965)
 * 577-593.</li>
 * <li>O.N. Igben et al., Fully implicit algorithm for CPA EOS, Fluid Phase Equilib. 608 (2026) 114734.</li>
 * </ul>
 *
 * @author Even Solbraa
 * @version 1.0
...
public class PhaseSrkCPABroydenImplicit extends PhaseSrkCPAs {
  ...
  public static String getProfileSummary() {
	...
	: 0;
    return String.format("Calls=%d  AvgIters=%.1f  JacobianEvals=%d  BroydenUpdates=%d (%.0f%%)  " + "Fallbacks=%d",
   ...
   *
   * <p>
   * Broyden quasi-Newton implicit molar volume calculation. Uses the coupled (n_s+1)-dimensional system but replaces
   * full Jacobian evaluation with Broyden rank-1 updates after the first Newton step. The inverse Jacobian H is
   * maintained directly and updated via the Sherman-Morrison formula.
   * </p>
   */
  @Override
