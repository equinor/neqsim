 ...
 *
 * <p>
 * Implements the algorithm from: "Fully implicit algorithm for Cubic Plus Association equation of state", Fluid Phase
 * Equilibria 608 (2026) 114734.
 * </p>
 *
 * <p>
 ...
 * </p>
 * <ul>
 * <li>Solves molar volume and association site fractions simultaneously using coupled Newton-Raphson</li>
 * <li>Eliminates inner iterations for XA solving during volume calculation</li>
 * <li>Includes restart criterion to suppress unnecessary root searches in supercritical regions</li>
...
public class PhaseSrkCPAfullyImplicit extends PhaseSrkCPAs {
  ...
  public static String getProfileSummary() {
    ...
    return String.format("Calls=%d  AvgIters=%.1f  Fallbacks=%d", implicitCallCount, avgIters, fallbackCount);
  }
   ...
   *
   * <p>
   * Fully implicit molar volume calculation. Solves the normalized density zeta = b/v and association site fractions
   * X_k simultaneously using a coupled Newton-Raphson method.
   * </p>
   *
   * <p>
   * Per Igben et al. (2026), the key speedup is eliminating inner XA iterations during the volume loop. The CPA
   * pressure derivatives (dFCPAdV, dFCPAdVdV) are computed directly from the current X_k values using O(ns^2) sums,
   * avoiding costly matrix inversions.
   * </p>
   */
  @Override
  public double molarVolume(double pressure, double temperature, double A, double B, PhaseType pt)
    ...
    do {
      ...
      for (int k = 0; k < ns; k++) {
        ...
        for (int j = 0; j < ns; j++) {
          ...
          sumDeriv += siteMoles[j] * xSite[j] * (dDeltadV * totalVol - deltaKJ) / (totalVol * totalVol);
        }
      ...
      for (int k = 0; k < ns; k++) {
        ...
        jacobian[ns][k] = Btemp / numberOfMolesInPhase * cpaon * (siteMoles[k] / totalVol) * gdv1 * sumJ;
      }
    ...
    if (!converged) {
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("Implicit non-convergence: ns=" + ns + " pt=" + pt + " iters=" + iterations + " zeta=" + zeta
            + " bestRes=" + bestMaxResidual + " P=" + pressure + " T=" + temperature);
      }
      return super.molarVolume(pressure, temperature, A, B, pt);
    }
   ...
   *
   * <p>
   * For type == 1 (volume derivatives), computes FCPA, dFCPAdV, dFCPAdVdV, dFCPAdVdVdV from scratch using Gaussian
   * elimination to solve H*XV = KlkV*ksi directly. This avoids the expensive EJML hessianMatrix.invert() in the
   * parent's solveX().
   * </p>
   *
   * <p>
   * Key insight: KlkV[i][j] = fV * Klk[i][j] where fV = gcpav - 1/V. Similarly for KlkVV and KlkVVV. This means only
   * Klk needs to be stored; volume derivatives are scalar multiples.
   * </p>
   *
   * <p>
