 ...
 * </p>
 * <ul>
 * <li><b>Fully implicit coupled Newton-Raphson</b> from Igben et al. (2026): simultaneous solution of molar volume and
 * association site fractions, eliminating inner iterations.</li>
 * <li><b>Site type reduction</b>: groups equivalent association sites (same deltaNog row on same component) into types
 * with multiplicities, reducing the system dimension from (n_s + 1) to (p + 1).</li>
 * </ul>
 *
 * <p>
 * The Newton Jacobian is built analytically on the reduced (p+1)-dimensional system at every iteration (no Broyden
 * approximation), solved via Gaussian elimination O(p^3). This gives both the per-iteration cost reduction of dimension
 * reduction AND the quadratic convergence of full Newton.
 * </p>
 *
 * <p>
...
public class PhaseSrkCPAfullyImplicitReduced extends PhaseSrkCPAs {
   ...
   *
   * <p>
   * Fully implicit + reduced molar volume solver. Operates on the reduced (p+1)-dimensional system where p is the
   * number of unique association site types. Uses full Newton Jacobian at every iteration (no Broyden approximation),
  ...
  public double molarVolume(double pressure, double temperature, double A, double B, PhaseType pt)
	...
	logger.debug("Implicit-reduced non-convergence: ns=" + ns + " p=" + numTypes + " pt=" + pt + " iters="
	    + iterations + " zeta=" + zeta + " P=" + pressure + " T=" + temperature);
      }
      return super.molarVolume(pressure, temperature, A, B, pt);
    }
  ...
  /**
   * Solve a linear system A*x = b in-place (b overwritten with solution) using Gaussian elimination with partial
   * pivoting.
   *
