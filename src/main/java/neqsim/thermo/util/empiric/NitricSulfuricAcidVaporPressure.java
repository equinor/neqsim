 ...
 *
 * <p>
 * Implements the Van Laar activity-coefficient model of Taleb, Ponche and Mirabel for the equilibrium partial vapour
 * pressures of water, nitric acid and sulfuric acid over their binary and ternary liquid mixtures. The model is valid
 * at low (stratospheric) temperatures, roughly 190-298 K. The activity expressions reproduce the H2O-HNO3-H2SO4 model
 * used in polar-stratospheric-cloud and aerosol modelling. The pure HNO3 vapour-pressure coefficients in this
 * implementation include the engineering adjustment documented below.
 * </p>
 *
 * <p>
 * Reference: D. Taleb, J. L. Ponche and P. Mirabel, "Vapor pressures in the ternary system water-nitric acid-sulfuric
 * acid at low temperature", Journal of Geophysical Research, 101(D20), 25967-25977, 1996.
 * </p>
 *
 * <p>
 * Component indexing follows the paper: component 1 = water (H2O), component 2 = nitric acid (HNO3), component 3 =
 * sulfuric acid (H2SO4). The three constituent binary systems are I = H2O-HNO3, II = H2O-H2SO4 and III = HNO3-H2SO4.
 * All temperatures are in kelvin and all logarithms in the model are base-10. Pure-component and partial pressures
 * returned by this class are expressed in pascal (Pa).
 * </p>
 *
 * @author NeqSim
 * @version $Id: $Id
...
public class NitricSulfuricAcidVaporPressure {
  /** Lower recommended temperature for the Taleb activity model, in kelvin. */
  public static final double MIN_RECOMMENDED_TEMPERATURE_K = 190.0;

  /** Upper recommended temperature for the Taleb activity model, in kelvin. */
  public static final double MAX_RECOMMENDED_TEMPERATURE_K = 298.0;
  ...
  /**
   * System II (H2O-H2SO4) Van Laar exponent B for sulfuric acid (B_II,3 = 1/B_II,1).
   */
  ...
  /**
   * System III (HNO3-H2SO4) Van Laar exponent B for sulfuric acid (B_III,3 = 1/B_III,2).
   */
  private static final double B_III_3 = 1.0 / 0.4;

  /**
   * System III (HNO3-H2SO4) Van Laar parameter A for nitric acid (A_III,2), fitted at 273 K.
   */
  private static final double A_III_2 = -250.52;

  /**
   * System III (HNO3-H2SO4) Van Laar parameter A for sulfuric acid (A_III,3), fitted at 273 K.
   */
  ...
  /**
   * Check whether a temperature is inside the recommended Taleb-model range.
   *
   * <p>
   * Calculations outside the range remain available for explicit engineering extrapolation; this method lets callers
   * distinguish extrapolation from source-validated use.
   * </p>
   *
   * @param temperature temperature in kelvin
   * @return {@code true} from 190 K through 298 K, inclusive
   */
  public static boolean isWithinRecommendedTemperatureRange(double temperature) {
    return Double.isFinite(temperature) && temperature >= MIN_RECOMMENDED_TEMPERATURE_K
        && temperature <= MAX_RECOMMENDED_TEMPERATURE_K;
  }
  ...
  public static double pureVaporPressureWater(double temperature) {
    ...
    double log10Pmbar = 8.42926609 - 1827.17843 / temperature - 71208.271 / (temperature * temperature);
   ...
   *
   * <p>
   * Values A = 7.57628, B = 1470.385, C = 43 are fitted simultaneously to two physical constraints: (1) the
   * experimental normal boiling point of pure HNO3 at 1 atm (83 degC = 356.15 K, P = 760 torr); and (2) a 6.9 %
   * increase in P0 at 273.15 K relative to the original Pennington (1951) / Taleb et al. (1996) values (A = 7.61628, B
   * = 1486.238) which systematically under-predicted the Vandoni (1944) ternary salting-out peak pressures at 273 K by
   * approximately 7 %. The updated C = 43 K is unchanged from the original Pennington value.
   * </p>
   */
  ...
  /**
   * Antoine coefficient C (temperature offset, K) for the pure HNO3 vapour pressure correlation.
   *
   * @see #HNO3_ANTOINE_A
   */
   ...
   * fitted to the experimental normal boiling point (83 degC at 1 atm) and to the Vandoni (1944) ternary P_HNO3 peak
   * data at 273 K as compiled by Taleb, Ponche and Mirabel (1996). Valid range: approximately 190-400 K.
   * </p>
   *
   * @param temperature temperature in kelvin (valid roughly 190-400 K)
   * @return pure nitric acid vapour pressure in pascal (Pa)
   * @throws IllegalArgumentException if temperature is not finite or does not exceed the Antoine offset
   */
  public static double pureVaporPressureNitricAcid(double temperature) {
    if (!Double.isFinite(temperature) || temperature <= HNO3_ANTOINE_C) {
      throw new IllegalArgumentException("temperature must be finite and greater than the HNO3 Antoine offset "
          + HNO3_ANTOINE_C + " K, was " + temperature);
    }
   ...
   * @param temperature temperature in kelvin
   * @return the product of temperature and the base-10 logarithm of the water activity coefficient
   */
   ...
   * @param x3 mole fraction of sulfuric acid (H2SO4)
   * @param temperature temperature in kelvin
   * @return the product of temperature and the base-10 logarithm of the nitric acid activity coefficient
   */
   ...
   * @param x3 mole fraction of sulfuric acid (H2SO4)
   * @param temperature temperature in kelvin
   * @return the product of temperature and the base-10 logarithm of the sulfuric acid activity coefficient
   */
  ...
  public static double partialPressureWater(double x1, double x2, double x3, double temperature) {
    return activityCoefficientWater(x1, x2, x3, temperature) * x1 * pureVaporPressureWater(temperature);
  }
   ...
   */
  public static double partialPressureNitricAcid(double x1, double x2, double x3, double temperature) {
    return activityCoefficientNitricAcid(x1, x2, x3, temperature) * x2 * pureVaporPressureNitricAcid(temperature);
  }
   ...
   */
  public static double partialPressureSulfuricAcid(double x1, double x2, double x3, double temperature) {
    return activityCoefficientSulfuricAcid(x1, x2, x3, temperature) * x3 * pureVaporPressureSulfuricAcid(temperature);
  }
   ...
   * @return a three-element array of mole fractions ordered as {x_H2O, x_HNO3, x_H2SO4}
   */
  public static double[] moleFractionsFromMassFractions(double massWater, double massNitric, double massSulfuric) {
    if (!Double.isFinite(massWater) || !Double.isFinite(massNitric) || !Double.isFinite(massSulfuric) || massWater < 0.0
        || massNitric < 0.0 || massSulfuric < 0.0) {
      throw new IllegalArgumentException("Mass fractions must be finite and non-negative");
    }
