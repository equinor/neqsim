...
public class HydrateFlash {
 ...
 //SystemInterface testSystem = new SystemUMRPRUEos(273.15 - 10.5, 5.0);
       ...
       // testSystem.addComponent("propane", 1.0);
        /*
         testSystem.addComponent("propane", 1.88);
         testSystem.addComponent("i-butane", 0.08);
         testSystem.addComponent("n-butane", 0.12);
         testSystem.addComponent("n-pentane", 1.64);
...
//            testOps.hydrateFormationTemperature(0);
        ...
        //System.out.println("activity coef water " + testSystem.getPhase(1).getActivityCoefficientSymetric(testSystem.getPhase(1).getComponent("water").getComponentNumber()));

        //    System.out.println("wt% TEG " + 100 * testSystem.getPhase(1).getComponent("TEG").getx() * testSystem.getPhase(1).getComponent("TEG").getMolarMass() / (testSystem.getPhase(1).getComponent("TEG").getx() * testSystem.getPhase(1).getComponent("TEG").getMolarMass() + testSystem.getPhase(1).getComponent("water").getx() * testSystem.getPhase(1).getComponent("water").getMolarMass()));
        //testSystem.display();
        //     System.out.println("kg vann/MSm^3 gas " + (testSystem.getPhase(0).getComponent("water").getx() * testSystem.getPhase(0).getComponent("water").getMolarMass() * 101325.0 / 8.314 / 288.15) * 1.0e6);
        // System.out.println("activity coef water " + testSystem.getPhase(1).getActivityCoefficientSymetric(1));
        //        int n = testSystem.getNumberOfPhases()-1;
//        double megwtfrac = testSystem.getPhase(n).getComponent("MEG").getMolarMass()*testSystem.getPhase(n).getComponent("MEG").getx()/testSystem.getPhase(n).getMolarMass();
//        System.out.println("wt % MEG " + megwtfrac*100);
/*
         * SystemInterface testSystem2 = new SystemSrkCPAstatoil(273.0 - 12,
         * 60.0); testSystem2.addComponent("methane", 1.0 -
         * testSystem.getPhase(0).getComponent("water").getx());
         * testSystem2.addComponent("water",
         * testSystem.getPhase(0).getComponent("water").getx());
         * testSystem2.createDatabase(true);
         *
         * testSystem2.setMixingRule(7); testSystem2.init(0);
         * testSystem2.init(1); ThermodynamicOperations testOps2 = new
         * ThermodynamicOperations(testSystem2); try {
         * testOps2.waterDewPointTemperatureFlash();
         *
         * } catch (Exception e) { e.printStackTrace();
         * System.out.println(e.toString()); } testSystem2.display();
         *
         */
    }
}
