...
public class TestUNIFAC_1 {
    ...
    public static void main(String args[]) {
        ...
        logger.info("wt% TEG "
                + (testSystem.getPhase(1).getComponent(0).getx() * testSystem.getPhase(1).getComponent(0).getMolarMass()
                        / testSystem.getPhase(1).getMolarMass() * 100.0));
        logger.info("wt% MDEA "
                + (testSystem.getPhase(1).getComponent(1).getx() * testSystem.getPhase(1).getComponent(1).getMolarMass()
                        / testSystem.getPhase(1).getMolarMass() * 100.0));
        logger.info("wt% Piperazine "
                + (testSystem.getPhase(1).getComponent(2).getx() * testSystem.getPhase(1).getComponent(2).getMolarMass()
                        / testSystem.getPhase(1).getMolarMass() * 100.0));
        logger.info("wt% water "
                + (testSystem.getPhase(1).getComponent(3).getx() * testSystem.getPhase(1).getComponent(3).getMolarMass()
                        / testSystem.getPhase(1).getMolarMass() * 100.0));
        ...
        for (int i = 0; i < 2000; i++) {
            try {
                ...
                // + " percent MDEA succefully reclaimed "+ percentBack+ " percent Piperazine
                // succefully reclaimed "+ percentBackPip+ " percent water succefully reclaimed
                // "+ percentBackWater);
