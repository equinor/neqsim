    ...
    public static void main(String[] args) {
        ...
        try {
            for (int i = 0; i < 30; i++) {
                ...
                neqsim.thermodynamicOperations.ThermodynamicOperations opsAntione = new neqsim.thermodynamicOperations.ThermodynamicOperations(
                        testSystemAntoine);

                double sample1[] = { i * 5 + 273.15 }; // temperature
                double standardDeviation1[] = { 0.1, 0.1, 0.1 }; // std.dev temperature // presure std.dev pressure
                double val = Math
                        .log(testSystemAntoine.getPhase(0).getComponent(0).getAntoineVaporPressure(273.15 + i * 5));
