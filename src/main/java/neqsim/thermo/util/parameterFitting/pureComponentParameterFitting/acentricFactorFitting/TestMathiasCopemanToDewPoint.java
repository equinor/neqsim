...
//                        1.0874, -1.7465, 1.5765} ; //n-heptane
        ...
        double guess[] = { 0.7131945439, 0.4140076386, -2.5691833434, 4.3710359391, -1.1086000763, 7.3866869416,
                1.1183339865, -2.1831081128, 0.9948380388, 1.5665073967, -5.5636308059, -1.4091055663, 0.8232405436,
                -0.3922156128, -1.4347079752, 1.3771397711, -1.2103436623, 0.0028643344, 1.3423413878, -0.3330681068,
                -2.1604332713, 1.1554915030, -1.7359747766, 1.1084438340, 1.1800358360, -1.4810259893, 1.4887592454 };//

        String nameList[] = { "methane", "ethane", "propane", "n-butane", "i-butane", "n-pentane", "c-hexane",
                "benzene", "n-heptane" };

        for (int compNumb = 0; compNumb < nameList.length; compNumb++) {
            dataSet = database.getResultSet("SELECT * FROM PureComponentVapourPressures WHERE ComponentName='"
            ...
            try {
                ...
                while (dataSet.next()) {
                    ...
                    if (i % numerOfPoint == 0) {
                        ...
                        double standardDeviation1[] = { 0.1, 0.1, 0.1 }; // std.dev temperature // presure std.dev
                                                                         // pressure
        ...
        dataSet = database.getResultSet("SELECT * FROM dewPointDataSynthHCStatoil WHERE Pressure<80.0");// "0 AND
