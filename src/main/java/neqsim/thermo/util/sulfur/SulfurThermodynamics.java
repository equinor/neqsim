 ...
 *
 * <p>
 * Gas-phase sulfur is treated as an ideal reacting mixture of {@code S2} through {@code S8}. The equilibrium
 * calculation minimizes the ideal-gas Gibbs energy subject to a single sulfur-atom balance. Reference enthalpies and
 * entropies are from the NIST-JANAF Thermochemical Tables, fourth edition (Chase, 1998). Heat capacities for
 * {@code S2-S7} are constant reference values; {@code S8} uses the NIST Shomate correlation. This is a considerably
 * better representation than treating all sulfur vapour as {@code S8}, while keeping the calculation independent of
 * cubic-EOS pseudo-component parameters.
 * </p>
 *
 * <p>
 * The vapour-pressure correlation interpolates the reviewed sulfur vapour-pressure table reported by Meyer (1976),
 * based on Jensen, Baker, and Rau. Interpolation is linear in {@code ln(P)} versus {@code 1/T}, which is locally
 * consistent with the Clausius-Clapeyron equation. No extrapolation is performed beyond 312.15-1308.15 K.
 * </p>
 *
 * @author esol
 * @version 1.0
...
public final class SulfurThermodynamics {
   ...
   *
   * <p>
   * The supplied pressure is the total partial pressure of elemental sulfur vapour, not the total process pressure. The
   * returned fractions therefore sum to one over {@code S2-S8}.
   * </p>
   *
  ...
  public static Map<String, Double> calculateAllotropeMoleFractions(double temperatureK,
    ...
    for (SpeciesData species : SPECIES) {
      double exponent = (species.sulfurAtoms * chemicalPotential - species.gibbsEnergyJPerMol(temperatureK))
          / (R * temperatureK);
   ...
   */
  public static double calculateMeanSulfurAtomsPerMolecule(double temperatureK, double sulfurPartialPressureBar) {
    Map<String, Double> fractions = calculateAllotropeMoleFractions(temperatureK, sulfurPartialPressureBar);
  ...
  public static double calculateVapourPressureBar(double temperatureK) {
    if (!Double.isFinite(temperatureK) || temperatureK < MIN_VAPOUR_PRESSURE_TEMPERATURE_K
        || temperatureK > MAX_VAPOUR_PRESSURE_TEMPERATURE_K) {
      throw new IllegalArgumentException("temperatureK must be within " + MIN_VAPOUR_PRESSURE_TEMPERATURE_K + "-"
    ...
    for (int i = 0; i < VAPOUR_PRESSURE_TEMPERATURE_K.length - 1; i++) {
      ...
      if (temperatureK <= upperTemperature) {
        ...
        double interpolationFraction = (inverseTemperature - inverseLower) / (inverseUpper - inverseLower);
        double logPressure = Math.log(VAPOUR_PRESSURE_BAR[i])
            + interpolationFraction * (Math.log(VAPOUR_PRESSURE_BAR[i + 1]) - Math.log(VAPOUR_PRESSURE_BAR[i]));
  ...
  public static double calculateDewPointTemperatureK(double sulfurPartialPressureBar) {
    ...
    if (sulfurPartialPressureBar < minimumPressure || sulfurPartialPressureBar > maximumPressure) {
      throw new IllegalArgumentException(
          "sulfurPartialPressureBar must be within " + minimumPressure + "-" + maximumPressure + " bar");
    }
  ...
  public static double calculateS8IdealGasHeatCapacityJPerMolK(double temperatureK) {
    ...
    return 180.6697 + 1.918988 * reducedTemperature - 0.520130 * reducedTemperature * reducedTemperature
   ...
   */
  private static double logSumOfSpeciesActivities(double temperatureK, double atomicChemicalPotential) {
    double maximumExponent = -Double.MAX_VALUE;
    double[] exponents = new double[SPECIES.length];
    for (int i = 0; i < SPECIES.length; i++) {
      SpeciesData species = SPECIES[i];
      exponents[i] = (species.sulfurAtoms * atomicChemicalPotential - species.gibbsEnergyJPerMol(temperatureK))
          / (R * temperatureK);
  ...
  public static final class SpeciesData implements Serializable {
    ...
    private SpeciesData(int sulfurAtoms, double enthalpyJPerMolAt298K, double entropyJPerMolKAt298K,
        double heatCapacityJPerMolK) {
    ...
    private double gibbsEnergyJPerMol(double temperatureK) {
      ...
      if (sulfurAtoms == 8) {
        ...
        enthalpy = enthalpyJPerMolAt298K + calculateS8EnthalpyIncrementJPerMol(boundedTemperature);
        entropy = entropyJPerMolKAt298K + calculateS8EntropyIncrementJPerMolK(boundedTemperature);
      } else {
        enthalpy = enthalpyJPerMolAt298K + heatCapacityJPerMolK * (temperatureK - REFERENCE_TEMPERATURE_K);
        entropy = entropyJPerMolKAt298K + heatCapacityJPerMolK * Math.log(temperatureK / REFERENCE_TEMPERATURE_K);
      }
    ...
    private double calculateS8EnthalpyIncrementJPerMol(double temperatureK) {
      ...
      return 1000.0
          * (180.6697 * (temperature - reference) + 1.918988 / 2.0 * (temperature * temperature - reference * reference)
              - 0.520130 / 3.0 * (Math.pow(temperature, 3.0) - Math.pow(reference, 3.0))
              + 0.044394 / 4.0 * (Math.pow(temperature, 4.0) - Math.pow(reference, 4.0))
    ...
    private double calculateS8EntropyIncrementJPerMolK(double temperatureK) {
      double temperature = temperatureK / 1000.0;
      double reference = REFERENCE_TEMPERATURE_K / 1000.0;
      return 180.6697 * Math.log(temperature / reference) + 1.918988 * (temperature - reference)
          - 0.520130 / 2.0 * (temperature * temperature - reference * reference)
          + 0.044394 / 3.0 * (Math.pow(temperature, 3.0) - Math.pow(reference, 3.0))
          + 2.270155 / 2.0 * (1.0 / (temperature * temperature) - 1.0 / (reference * reference));
    }
  }
}
