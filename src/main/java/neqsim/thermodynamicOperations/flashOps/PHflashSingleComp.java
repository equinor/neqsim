...
public class PHflashSingleComp extends Flash implements java.io.Serializable {
	...
	public void run() {
		...
		if (system.doSolidPhaseCheck()) {
			system.init(3, 3);
			solidEnthalpy = system.getPhases()[3].getEnthalpy() / system.getPhases()[3].getNumberOfMolesInPhase()
					* system.getTotalNumberOfMoles();
			
			if (Hspec < liqEnthalpy && Hspec > solidEnthalpy) {
				double solidbeta = (Hspec - liqEnthalpy) / (gasEnthalpy - liqEnthalpy);
			}
		}*/
