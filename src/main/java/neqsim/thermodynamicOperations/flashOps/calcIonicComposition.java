...
public class calcIonicComposition extends Flash {
        ...
        public void run() {
                ...
                for (int i = 0; i < system.getPhase(phaseNumber).getNumberOfComponents(); i++) {
                        ...
                        resultTable[ionNumber + 1][0] = system.getPhase(phaseNumber).getComponent(i)
                                        .getComponentName();
                        resultTable[ionNumber + 1][1] = Double.valueOf(nf.format(system
                                        .getPhase(phaseNumber).getComponent(i)
                                        .getNumberOfMolesInPhase()
                                        / (system.getPhase(phaseNumber).getComponent("water")
                                                        .getNumberOfMolesInPhase()
                                                        * system.getPhase(phaseNumber)
                                                                        .getComponent("water")
                                                                        .getMolarMass())
                                        * 1000)).toString();
                        resultTable[ionNumber + 1][2] = Double.valueOf(nf.format(system
                                        .getPhase(phaseNumber).getComponent(i)
                                        .getNumberOfMolesInPhase()
                                        * system.getPhase(phaseNumber).getComponent(i)
                                                        .getMolarMass()
                                        / (system.getPhase(phaseNumber).getComponent("water")
                                                        .getNumberOfMolesInPhase()
                                                        * system.getPhase(phaseNumber)
                        ...
                        resultTable[ionNumber + 1][3] = Double
                                        .valueOf(nf.format(system.getPhase(phaseNumber)
                                                        .getActivityCoefficient(i, system
