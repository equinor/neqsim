            ...
            for(int i=0;i<system.getPhase(0).getNumberOfComponents();i++){
                ...
                for(int j=0;j<flashComp.length;j++){
                    if(flashComp[j].equals(system.getPhase(0).getComponent(i).getName())){
                        diff += Math.abs((system.getPhase(1).getComponent(i).getx()*system.getPhase(1).getComponent(i).getFugasityCoefficient()*system.getPhase(1).getPressure())-(system.getPhase(0).getComponent(i).getx()*system.getPhase(0).getComponent(i).getFugasityCoefficient()*system.getPhase(0).getPressure()));
