...
public class bubblePointTemperatureNoDer extends constantDutyTemperatureFlash {
	...
	public void run() {
		...
		if (system.getPhase(0).getNumberOfComponents() == 1) {
			...
			if(system.getTemperature()>system.getPhase(0).getComponent(0).getTC() || system.getTemperature()<system.getPhase(0).getComponent(0).getTriplePointTemperature()) {
		...
		if (system.getPhase(0).getNumberOfComponents() == 1
				&& Math.abs(system.getPhases()[1].getComponents()[0].getFugasityCoeffisient()
						/ system.getPhases()[0].getComponents()[0].getFugasityCoeffisient() - 1.0) < 1e-20) {
			setSuperCritical(true);
			return;
		}
	}
