...
public class freezingPointTemperatureFlashOld extends constantDutyTemperatureFlash{
    ...
    public void run(){
        ...
        for(int k=0;k<system.getPhases()[0].getNumberOfComponents();k++){
            if(system.getPhase(3).getComponent(k).fugcoef(system.getPhase(3))<9e4 && system.getPhase(3).getComponent(k).doSolidCheck()){ // checks if solid can be formed from component k
                ...
                do{
                    ...
                    for(int i=0;i<system.getNumberOfPhases();i++){
                        ...
                        deriv -= 0.01*system.getPhases()[i].getBeta()*(system.getPhases()[3].getComponents()[k].getFugasityCoeffisient()*Math.exp(system.getPhases()[i].getComponents()[k].getdfugdt())*-1.0/Math.pow(system.getPhases()[i].getComponents()[k].getFugasityCoeffisient(),2.0) + Math.exp(system.getPhases()[3].getComponents()[k].getdfugdt())/system.getPhases()[i].getComponents()[k].getFugasityCoeffisient());
                    }
                    if(iterations>=2) {
