            ...
            //if(system.getPhase(0).getComponent(k).fugcoef(system.getPhase(0))<9e4){//&& system.getPhase(3).getComponent(k).doSolidCheck()){ // solidCheck variablen er satt naar man kaller setSolidCheck Funksjonen som maa kjores faer du kjorer scriptet.
                ...
                do{
                    ...
                    if(CCequation){
                        ...
                        dfugdt = Math.log((system.getPhase(0).getComponent(k).getCCsolidVaporPressuredT(temp)*Math.exp(solvol/(R*temp)*(pres-Pvapsolid)))/pres);
                    }
                    else{
                    ...
                    for(int i=0;i<system.getNumberOfPhases();i++){
                        funk -= system.getPhases()[i].getBeta()*SolidFug*SolVapFugCoeff/system.getPhases()[i].getComponents()[k].getFugasityCoeffisient();
                        deriv -= 0.01*system.getPhases()[i].getBeta()*(SolidFug*SolVapFugCoeff*Math.exp(system.getPhases()[i].getComponents()[k].getdfugdt())*-1.0/Math.pow(system.getPhases()[i].getComponents()[k].getFugasityCoeffisient(),2.0)
