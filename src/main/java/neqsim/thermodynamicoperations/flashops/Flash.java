...
public abstract class Flash extends BaseOperation {
  ...
  /**
   * Retry stability analysis with amplified Wilson K-values using an independent system clone. Called when the standard
   * stability analysis declares the system stable, to catch near-critical instability (e.g. near the cricondenbar where
   ...
   *
   * <p>
   * The standard {@link #stabilityAnalysis()} uses Wilson K-value based initial guesses, which assume gas-liquid
   * equilibrium (GLE). At temperatures well below the critical temperature of the lightest component, Wilson K-values
   * converge to values less than 1 for all components, making them ineffective for detecting LLE. This method
   * complements the standard analysis by initializing trial phases as nearly-pure heaviest and lightest components,
   * which is the same approach used by {@link TPmultiflash}.
   * </p>
   *
   ...
   *
   * <p>
   * Automatic checks are enabled for systems where LLE/VLLE is common even if users have not explicitly turned on
   * {@code setMultiPhaseCheck(true)}.
   * </p>
   *
  ...
  protected boolean shouldRunAutomaticLLECheck() {
    ...
    if (modelName.contains("CPA") || lowerModelName.contains("electrolyte") || lowerModelName.contains("pitzer")) {
      return true;
    }
    ...
    if (system.hasComponent("water")) {
      return system.doEnhancedMultiPhaseCheck();
    }
   ...
   *
   * <p>
   * Enhanced checks are intended for systems where ordinary Wilson K-value stability analysis is known to miss polar,
   * associating, electrolyte, or sour-fluid liquid splits. Hydrocarbon-only PR/SRK mixtures are handled by the ordinary
   * multiphase flash path; applying the enhanced numerical filters there can create isolated phase-map artifacts near
   * phase boundaries.
   * </p>
   *
