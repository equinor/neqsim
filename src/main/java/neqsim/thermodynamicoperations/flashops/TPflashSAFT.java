 ...
 *
 * <p>
 * Uses successive substitution with separate single-phase systems for liquid and gas volume solving. This avoids the
 * issue where the SAFT volume solver cannot find both gas and liquid roots at the feed composition (unlike cubic EOS
 * where the cubic always has mathematical roots).
 * </p>
 *
 * @author Even Solbraa
 * @version 1.0
...
public class TPflashSAFT extends TPflash {
   ...
   *
   * <p>
   * After a converged VLE flash, checks if the liquid phase is unstable with respect to liquid-liquid splitting. Uses
   ...
   *
   * <p>
   * The 3-phase R-R equations are: F1 = sum_i z_i*(K_V_i-1) / [1 + betaV*(K_V_i-1) + betaL1*(K_LL_i-1)] = 0 F2 = sum_i
   ...
   */
  private double[] solve3PhaseRachfordRice(double[] z, double[] KV, double[] KLL, double betaV0, double betaL10) {
    double betaV = betaV0;
