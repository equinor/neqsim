 ...
 *
 * <p>
 * Given temperature T and volume V, solves for pressure P using Newton iteration. The calculation iteratively adjusts
 * pressure until the system volume matches the specified volume.
 * </p>
 *
 * @author even solbraa
 * @version $Id: $Id
...
public class TVflash extends Flash {
   ...
   *
   * <p>
   * Uses a clone of the system set to TV mode (constant volume, single-phase). The EOS then computes pressure
   * analytically from the Helmholtz free energy derivative ({@code P = -dF/dV + nRT/V}) without iterating on volume.
   * This is the same approach used by gradient theory for interfacial tension calculations.
   * </p>
   *
   * <p>
   * This only works reliably for single-phase systems. For two-phase states the EOS will compute a pressure
   * corresponding to the metastable single-phase root, but this still provides a much better initial guess than the
   * arbitrary starting pressure.
   * </p>
   *
   ...
   *
   * <p>
   * The algorithm first attempts a fast single-phase pressure estimate using the TV specification approach (same as
   * gradient theory): given T and V, the EOS computes P analytically via {@code calcPressure()} on a clone. If the
   * system turns out to be single-phase, this converges in 1-2 iterations. For two-phase systems, it provides a good
   * initial guess for the Newton iteration.
   * </p>
   *
   * <p>
   * The Newton loop uses 1/10 damped analytical dV/dP derivatives with 2x max step safety bounds. Early termination
   * when error starts growing ({@code error &lt; errorOld}) prevents oscillation near phase boundaries.
   * </p>
   *
  ...
  public double solveQ() {
    ...
    } while ((error > TOLERANCE && iterations < MAX_ITERATIONS && error < errorOld) || iterations < 3);
