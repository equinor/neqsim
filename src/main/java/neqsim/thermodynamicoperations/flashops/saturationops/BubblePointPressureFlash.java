  ...
  public void run() {
    ...
    if (singleComponent) {
      ...
      if (Double.isNaN(pGuess) || pGuess < minGuess || pGuess > maxGuess) {
        // Fall back to a Clausius-Clapeyron style extrapolation using the component
        // heat of
        // vaporisation.
      ...
      do {
        ...
        if (!usedNewton) {
          if (!Double.isFinite(residual)) {
            ...
            newPressure = Math.max(1e-6, system.getPressure() * (1.0 - Math.signum(residual) * 0.1));
          }
        }
        ...
        if (singleComponent) {
          ...
          double upperBound = Double.isFinite(maxPurePressure) ? maxPurePressure : Double.POSITIVE_INFINITY;
    ...
    boolean nearCritical = system.getTemperature() >= 0.999 * pseudoTc
        && system.getPressure() >= 0.999 * pseudoPc;
