...
public class CheckScalePotential extends ConstantDutyTemperatureFlash {
    ...
    try (neqsim.util.database.NeqSimDataBase database = new neqsim.util.database.NeqSimDataBase();
      ...
      while (dataSet.next()) {
        ...
        if (ksp <= 0.0 || Double.isNaN(ksp) || Double.isInfinite(ksp)) {
          logger.warn("Invalid Ksp for " + saltName + ": " + ksp + " at T=" + temperatureK + " K, skipping");
          continue;
        }
  ...
  private double computeHydromagnesiteSR(double m1, double m2, int compNumb1, int compNumb2, int waterCompNumb,
      double waterDenom, double stoc1, double stoc2, double ksp) {
    if (m1 < MIN_MOLALITY || m2 < MIN_MOLALITY) {
      return 0.0;
    }
    ...
    double gamma1 = system.getPhase(phaseNumber).getActivityCoefficient(compNumb1, waterCompNumb);
    double gamma2 = system.getPhase(phaseNumber).getActivityCoefficient(compNumb2, waterCompNumb);
    double gamma3 = system.getPhase(phaseNumber).getActivityCoefficient(waterCompNumb, waterCompNumb);
    ...
    double lnIAP = stoc1 * Math.log(gamma1 * m1) + stoc2 * Math.log(gamma2 * m2) + 3.0 * Math.log(gamma3 * m3)
        + 2.0 * Math.log(gamma4 * m4);
    double lnKsp = Math.log(ksp);
    double lnSR = lnIAP - lnKsp;
