      ...
      for (int i = 0; i < pressures.length; i++) {
        ...
        if (thermoSystem.hasPhaseType("gas") && acceptedFlash) {
          ...
          do {
             ...
             * props[k][i][j - 2]) / (temperatures[j - 1] - temperatures[j - 2]) * (temperatures[j]
             * - temperatures[j - 1])) + (props[k][i - 1][j] + (props[k][i - 1][j] - props[k][i -
             * 2][j]) / (pressures[i - 1] - pressures[i - 2]) * (pressures[i] - pressures[i - 1])));
             * if (names[k].equals("GAS MASS FRACTION") && props[k][i][j] < 0) { props[k][i][j] = 0;
             * } }
             */
        ...
        if (thermoSystem.hasPhaseType("oil") && acceptedFlash) {
          ...
          do {
             ...
             * props[k][i][j - 2]) / (temperatures[j - 1] - temperatures[j - 2]) * (temperatures[j]
             * - temperatures[j - 1])) +(props[k][i][j - 1] + (props[k][i][j - 1] - props[k][i][j -
             * 3]) / (temperatures[j - 1] - temperatures[j - 3]) * (temperatures[j] - temperatures[j
             * - 1]))); }
             */
            if (j < 2 && i < 2) {
          ...
          props[k][i][j] =
              oilSystem.getPhase(0).getEnthalpy() / oilSystem.getPhase(0).getNumberOfMolesInPhase()
                  / oilSystem.getPhase(0).getMolarMass();
          names[k] = "LIQUID ENTHALPY";
          units[k] = "J/KG";
          k++;
        ...
        if (thermoSystem.hasPhaseType("aqueous") && acceptedFlash) {
          ...
          do {
             ...
             * - temperatures[j - 1])) +(props[k][i][j - 1] + (props[k][i][j - 1] - props[k][i][j -
             * 3]) / (temperatures[j - 1] - temperatures[j - 3]) * (temperatures[j] - temperatures[j
             * - 1]))); if (names[k].equals("WATER VAPOR MASS FRACTION") && props[k][i][j] > 1) {
             * props[k][i][j] = 1; } }
             */
            if (j > 1) {
              props[k][i][j] = props[k][i][j - 1] + (props[k][i][j - 1] - props[k][i][j - 2])
             ...
             * props[k][i][j-2]) / (temperatures[j - 1] - temperatures[j - 2]) * (temperatures[j] -
             * temperatures[j - 1]); if (j>1) { props[k][i][j] = props[k][i][j-1] +
             ...
             * props[k][i][j - 2]) / (temperatures[j - 1] - temperatures[j - 2]) * (temperatures[j]
             * - temperatures[j - 1]); } else if (j < 2) { props[k][i][j] = props[k][i - 1][j] +
             * (props[k][i - 1][j] - props[k][i - 2][j]) / (pressures[i - 1] - pressures[i - 2]) *
             * (pressures[i] - pressures[i - 1]); } else { props[k][i][j] = props[k][i - 1][j - 1] +
             * (props[k][i][j - 1] - props[k][i][j - 2]) / (temperatures[j - 1] - temperatures[j -
             * 2]) * (temperatures[j] - temperatures[j - 1]) + (props[k][i - 1][j] - props[k][i -
             * 2][j]) / (pressures[i - 1] - pressures[i - 2]) * (pressures[i] - pressures[i - 1]); }
             * props[k][i][j] = 0.0;
             */
            k++;
        ...
        if (thermoSystem.hasPhaseType("oil") && thermoSystem.hasPhaseType("aqueous")
          ...
          if (continuousDerivativesExtrapolation && (i >= 2 || j >= 2)) {
            ...
            if (j > 1 && LWS == 0) {
               ...
               * if (j > 2) { props[k][i][j] = 0.5*((props[k][i][j - 1] + (props[k][i][j - 1] -
               * props[k][i][j - 2]) / (temperatures[j - 1] - temperatures[j - 2]) *
               * (temperatures[j] - temperatures[j - 1])) +(props[k][i][j - 1] + (props[k][i][j - 1]
               * - props[k][i][j - 3]) / (temperatures[j - 1] - temperatures[j - 3]) *
               * (temperatures[j] - temperatures[j - 1]))); if
