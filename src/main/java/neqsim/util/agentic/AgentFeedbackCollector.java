 ...
 *
 * <p>
 * Maintains a rolling log of agent session outcomes, categorized by task type, agent name, and failure mode. Provides
 * summary statistics that can be used to identify recurring failure patterns and guide improvements to both the AI
 * agent prompts and the underlying NeqSim Java codebase.
 * </p>
 *
...
public class AgentFeedbackCollector implements Serializable {
   ...
   *
   * <p>
   * Analyzes failure frequency by category and agent, and returns prioritized, actionable recommendations. This is the
   * core of the cross-session learning loop described in the framework: failures drive recommendations, which in turn
   ...
   *
   * <p>
   * Divides sessions into windows of 10 and computes the success rate in each window. An increasing trend indicates
   * that the framework is improving over time through accumulated knowledge (skill updates, API gap fixes, new
   ...
   *
   * <p>
   * Cross-references discovered API gaps with failure frequency to determine which missing capability would fix the
   * most failures if implemented. This drives the development flywheel described in the framework: Gap discovery leads
