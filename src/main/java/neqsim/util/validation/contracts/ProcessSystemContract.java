 ...
 *
 * <p>
 * Defines requirements and guarantees for {@link ProcessSystem} implementations. AI agents can use this contract to
 * validate entire process flowsheets before running simulations.
 * </p>
 *
  ...
  public ValidationResult checkPreconditions(ProcessSystem processSystem) {
    ...
    for (int i = 0; i < units.size(); i++) {
      ...
      if (name == null || name.isEmpty()) {
        result.addWarning("process.unit[" + i + "].name", "Equipment at index " + i + " has no name",
      ...
      for (ValidationResult.ValidationIssue issue : equipResult.getIssues()) {
        if (issue.getSeverity() == ValidationResult.Severity.CRITICAL) {
          result.addError("process." + name + "." + issue.getCategory(), issue.getMessage(), issue.getRemediation());
        } else if (issue.getSeverity() == ValidationResult.Severity.MAJOR) {
          result.addWarning("process." + name + "." + issue.getCategory(), issue.getMessage(), issue.getRemediation());
        }
      }
    }
  ...
  public ValidationResult checkPostconditions(ProcessSystem processSystem) {
    ...
    for (int i = 0; i < units.size(); i++) {
      ...
      try {
        ...
        result.addWarning("process.unit[" + i + "]", "Could not verify equipment state: " + e.getMessage(), "");
      }
    }
