...
/**
 * Comprehensive benchmark of the cDFT surface tension model against experimental data and other NeqSim IFT models
 * (Parachor, Gradient Theory) across multiple pure components and EOS types.
 */
public class CDFTSurfaceTensionBenchmarkTest {
   ...
   */
  private void printRow(String component, String eos, double tempK, double sigmaExp, double sigmaCDFT, double sigmaGT,
      double sigmaParachor) {
  ...
  void benchmarkPureComponents() {
    ...
    assertTrue(cdftWithin50pct >= totalTests / 2, "At least half should be within 50% of experiment");
  }
  ...
  void testHeavierAlkanes_PR() {
	...
	continue;
      ...
      double cdft = computeIFT(sys, "cDFT");
      double par = computeIFT(sys, "Parachor");
      logger.printf(org.apache.logging.log4j.Level.INFO, "| %-12s | %8.3f | %8.3f |%n", comp, cdft, par);
