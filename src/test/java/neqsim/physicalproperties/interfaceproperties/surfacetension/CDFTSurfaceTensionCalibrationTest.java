 ...
 *
 * <p>
 * Sweeps the kernel half-width parameter lambda over a range of values and evaluates the average absolute deviation
 * (AAD) against experimental surface tension data for pure alkanes (C1-C6), N2, and CO2. The optimal universal lambda
 * is the value that minimises AAD across all species simultaneously.
 * </p>
 *
 * @author Agent
 * @version 1.0
...
class CDFTSurfaceTensionCalibrationTest {
   ...
   *
   * <p>
   * Run this test to identify the optimal universal kernel range parameter. The output is a table of lambda vs AAD that
  ...
  void fineSweepAroundOptimal() {
    ...
    logger.printf(org.apache.logging.log4j.Level.INFO, "%-8s | %-12s | %-12s%n", "lambda", "AAD-PR (%)", "AAD-SRK (%)");
    logger.info("---------|--------------|-------------");
    ...
    for (double lambda = 0.50; lambda <= 1.60; lambda += 0.025) {
      ...
      logger.printf(org.apache.logging.log4j.Level.INFO, "%-8.3f | %12.2f | %12.2f%n", lambda, aadPR, aadSRK);
    }
  }
}
