...
class ChemistryCoupledModelsTest {
  ...
  /**
   * Builds a minimal pipe and exercises the closed-loop deposition solver. Verifies that the iterative diameter
   * sequence is monotonically non-increasing and the solver either converges or terminates with a documented
   * blocked-pipe state.
   */
  @Test
  void closedLoopDepositionSolverConverges() {
    ...
    ClosedLoopDepositionSolver solver = new ClosedLoopDepositionSolver(pipe, acc).setMaxIterations(5)
        .setToleranceM(1e-3);
    solver.solve();
    ...
    for (int i = 1; i < solver.getDiameterHistoryM().size(); i++) {
      assertTrue(solver.getDiameterHistoryM().get(i) <= solver.getDiameterHistoryM().get(i - 1) + 1e-9,
  ...
  void packedBedScavengerProducesBreakthroughCurve() {
    PackedBedScavengerReactor bed = new PackedBedScavengerReactor().setGeometry(0.5, 2.0, 0.4)
        .setMedia(5.0, 1100.0, 1.0).setRateConstant(8.0).setFeed(2.0, 0.005).setDiscretisation(30, 100)
        .setSimulationTime(3600.0 * 24.0 * 60.0, 0.05).evaluate();
