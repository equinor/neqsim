...
/**
 * Tests for {@link AntiSurgeRecycleCalculator}.
 *
 * @author NeqSim
 * @version 1.0
 */
public class AntiSurgeRecycleCalculatorTest {
  /** Chart conditions for the reference compressor map. */
  private static final double[] CHART_CONDITIONS = new double[] { 0.3, 1.0, 1.0, 1.0 };

  /** Speed lines for the reference compressor map. */
  private static final double[] SPEED = new double[] { 12913, 12298, 11683, 11098, 10453, 9224, 8609, 8200 };

  /**
   * Build a charted compressor on a natural-gas suction stream at the given flow rate.
   *
   * @param flowMSm3PerDay the suction flow rate in MSm3/day
   * @return a configured compressor ready to run
   */
  private Compressor buildCompressor(double flowMSm3PerDay) {
    ...
    testFluid.setTotalFlowRate(flowMSm3PerDay, "MSm3/day");

    Stream stream = new Stream("suction", testFluid);
    stream.run();
  ...
  @Test
  public void testRecycleOpensAndHoldsControlLine() {
    // Low suction flow: the natural operating point sits below the surge control line.
    ...
    Assertions.assertTrue(result.isRecycleActive(), "recycle should open at low suction flow");
    Assertions.assertTrue(result.isConverged(), "recycle iteration should converge");
    Assertions.assertTrue(result.getRecycleMassFlow() > 0.0, "recycle mass flow should be positive");
    // Inlet held on the control line => distance to surge approx the configured margin.
    Assertions.assertEquals(0.05, result.getDistanceToSurge(), 0.02,
	"compressor inlet should sit on the anti-surge control line");
    Assertions.assertEquals(result.getControlFlow(), result.getInletVolumeFlow(), 0.01 * result.getControlFlow(),
	"inlet volume flow should match the control flow");
  }

  @Test
  public void testNoRecycleWhenAboveControlLine() {
    // High suction flow: the natural operating point already sits above the control line.
    Compressor comp = buildCompressor(4.00);
    Stream suction = (Stream) comp.getInletStream();
    ...
    Assertions.assertFalse(result.isRecycleActive(), "no recycle should be required at high flow");
    Assertions.assertEquals(0.0, result.getRecycleMassFlow(), 1.0e-9, "recycle mass flow should be zero");
    Assertions.assertNull(result.getRecycleStream(), "no recycle stream when inactive");
