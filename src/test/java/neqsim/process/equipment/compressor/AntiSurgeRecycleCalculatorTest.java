...
public class AntiSurgeRecycleCalculatorTest {
  ...
  private static final double[] SPEED =
      new double[] {12913, 12298, 11683, 11098, 10453, 9224, 8609, 8200};
  ...
  public void testRecycleOpensAndHoldsControlLine() {
    ...
    Assertions.assertTrue(result.getRecycleMassFlow() > 0.0,
        "recycle mass flow should be positive");
    ...
    Assertions.assertEquals(result.getControlFlow(), result.getInletVolumeFlow(),
        0.01 * result.getControlFlow(), "inlet volume flow should match the control flow");
  }
  ...
  public void testNoRecycleWhenAboveControlLine() {
    ...
    Assertions.assertEquals(0.0, result.getRecycleMassFlow(), 1.0e-9,
        "recycle mass flow should be zero");
