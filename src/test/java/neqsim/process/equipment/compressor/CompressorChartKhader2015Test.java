...
public class CompressorChartKhader2015Test {
  ...
  void testSetCurves() {
    ...
    Assertions.assertEquals(0.00256412315, compChart
        .getCorrectedCurves(chartConditions, speed, flow, head, polyEff, polyEff).get(0).correctedFlowFactor[0],
        0.0001);
  ...
  void testGetNewCurves() {
    ...
    CompressorChartKhader2015 compChart = new CompressorChartKhader2015(actualFluid, referenceFluid, 0.9);
    compChart.setCurves(chartConditions, speed, flow, head, flow, polyEff);
    ...
    for (int i = 0; i < newcomprcurve.size(); i++) {
      ...
      for (int j = 0; j < curve.flow.length; j++) {
        Assertions.assertNotEquals(flow[i][j], curve.flow[j], 1e-6, "Flow mismatch at curve " + i + ", point " + j);
        Assertions.assertNotEquals(head[i][j], curve.head[j], 1e-6, "Head mismatch at curve " + i + ", point " + j);
  ...
  void testSingleSpeedCurves() {
    ...
    Assertions.assertNotNull(compChart.getSurgeCurve(), "Surge curve object should exist (but not active)");
    Assertions.assertFalse(compChart.getSurgeCurve().isActive(),
        "Surge curve should not be active for single speed compressor");
    Assertions.assertNotNull(compChart.getStoneWallCurve(), "Stone wall curve object should exist (but not active)");
    ...
    logger.info("  Surge point (min flow): " + minFlow + " m3/hr at head " + singleCurve.head[0] + " kJ/kg");
    logger.info("  Stone wall point (max flow): " + maxFlow + " m3/hr at head "
        + singleCurve.head[singleCurve.head.length - 1] + " kJ/kg");
    logger.info("  Note: These are SINGLE POINTS, not curves. Multiple speeds are needed for surge/stone wall curves.");
    ...
    Assertions.assertTrue(effValue > 0, "Efficiency should be calculated for single speed compressor");
    ...
    Assertions.assertTrue(minFlow < maxFlow, "Surge point flow should be less than stone wall point flow");
    ...
    Assertions.assertTrue(safeSurgeFlow > actualSurgeFlow, "Safe surge flow should be higher than actual surge flow");
    Assertions.assertTrue(safeSurgeFlow10pct > safeSurgeFlow, "Higher safety factor should give more margin");

    logger.info("\nKey Point for Single Speed Compressors:");
    logger.info("  Since speed cannot be adjusted, the safety margin is ESSENTIAL to prevent surge.");
    logger.info("  The AntiSurge system will recirculate/add flow if operation gets too " + "close to surge point.");
    ...
    Assertions.assertEquals(minFlow, surgeFlowAtSpeed, 0.01, "getSurgeFlowAtSpeed should return minimum flow");
    Assertions.assertEquals(maxFlow, stoneWallFlowAtSpeed, 0.01, "getStoneWallFlowAtSpeed should return maximum flow");
    ...
    logger.info("  Safe flow (with " + ((antiSurge.getSurgeControlFactor() - 1.0) * 100) + "% margin): "
        + safeFlowHead[0] + " m3/hr");
    ...
    Assertions.assertTrue(actualFlow < stoneWallFlowAtSpeed, "Should be operating below stone wall point");
    Assertions.assertTrue(distanceToSurge > 0, "Distance to surge should be positive (operating above surge)");
