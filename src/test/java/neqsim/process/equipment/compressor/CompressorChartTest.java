...
public class CompressorChartTest {
  private static final Logger logger = LogManager.getLogger(CompressorChartTest.class);
  ...
  public void setUp() {
     ...
     * surgehead) double[] stoneWallflow = new double[] { 923.2, 950.0, 980.0, 1000.0 }; double[] stoneWallHead = new
  ...
  public void test_Run() {
    ...
    /*
     * logger.info("power " + comp1.getPower()); logger.info("is surge " + comp1.getAntiSurge().isSurge()); System.out
     * .println("fraction in anti surge line " + comp1.getAntiSurge().getCurrentSurgeFraction());
     * logger.info("Polytropic head from curve:" + comp1.getPolytropicHead()); logger.info("Polytropic eff from curve:"
     * + comp1.getPolytropicEfficiency() * 100.0); logger.info("flow " +
     * stream_1.getThermoSystem().getFlowRate("m3/hr"));
     *
     * logger.info("speed " + comp1.getCompressorChart().getSpeed( stream_1.getThermoSystem().getFlowRate("m3/hr") +
     * 10.0, comp1.getPolytropicHead())); logger.info("pressure out " + comp1.getOutletPressure());
     ...
     * comp1.getPolytropicEfficiency()); logger.info("temperature out " + (comp1.getOutTemperature() - 273.15) + " C");
     * logger.info("calculated speed " + calcSpeed); logger.info("power " + comp1.getPower());
     */
  ...
  public void runCurveTest() {
    ...
    double[][] flow = new double[][] {{4512.7, 5120.8, 5760.9, 6401, 6868.27},
        {4862.47, 5486.57, 6172.39, 6858.21, 7550.89}, {5237.84, 5852.34, 6583.88, 7315.43, 8046.97, 8266.43},
        {5642.94, 6218.11, 6995.38, 7772.64, 8549.9, 9000.72}, {6221.77, 6583.88, 7406.87, 8229.85, 9052.84, 9768.84},
    ...
    double[][] head = new double[][] {{61.885, 59.639, 56.433, 52.481, 49.132},
        ...
        {92.493, 90.312, 86.3, 81.079, 74.658, 70.216}, {103.512, 102.073, 97.83, 92.254, 85.292, 77.638},
    ...
    double[][] polyEff = new double[][] {{78.3, 78.2, 77.2, 75.4, 73.4},
        ...
        {78.3, 78.3, 77.5, 75.8, 73}, {78.2, 78.4, 77.7, 76.1, 73.5, 72.5}, {78.2, 78.4, 77.9, 76.4, 74, 71.9},
        {78.3, 78.4, 78, 76.7, 74.5, 71.2}, {78.3, 78.4, 78.1, 77, 74.9, 71.3, 70.5},
        {78.4, 78.1, 77.1, 75, 71.4, 70.2}, {78.3, 78.2, 77.2, 75.2, 71.7, 69.5}, {78.2, 78.2, 77.3, 75.5, 72.2, 69.6}};

    comp1.getCompressorChart().setCurves(chartConditions, speed, flow, head, polyEff);
    comp1.getCompressorChart().setHeadUnit("kJ/kg");

    double[] surgeflow = new double[] {4512.7, 4862.47, 5237.84, 5642.94, 6221.77, 6888.85, 7109.83, 7598.9};
    double[] surgehead = new double[] {61.885, 71.416, 81.621, 92.493, 103.512, 114.891, 118.595, 126.747};
    comp1.getCompressorChart().getSurgeCurve().setCurve(chartConditions, surgeflow, surgehead);
    // comp1.getAntiSurge().setActive(true);
    comp1.getAntiSurge().setSurgeControlFactor(1.0);
    comp1.run();
  ...
  public void runCurveTest2() {
    ...
    org.apache.logging.log4j.LogManager.getLogger(CompressorChartTest.class).debug("power " + comp1.getPower("MW"));
    org.apache.logging.log4j.LogManager.getLogger(CompressorChartTest.class)
        .debug("polytropic head " + comp1.getPolytropicFluidHead());
    org.apache.logging.log4j.LogManager.getLogger(CompressorChartTest.class)
        .debug("polytropic efficiency " + comp1.getPolytropicEfficiency());
