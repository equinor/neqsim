 ...
 *
 * <p>
 * Verifies that an expander does not report the spurious 150% utilization that the inherited
 * compressor fallback produced, that its simulation is recognised as valid despite negative
 * (recovered) power, that the consumed-power constraints are replaced by a {@code recoveredPower}
 * constraint, and that the recovered-power provenance is surfaced in the utilization snapshot JSON.
 * </p>
 *
 * @author NeqSim
 * @version 1.0
...
public class ExpanderCapacityTest {
  ...
  void testNoSpuriousFullUtilization() {
    ...
    Assertions.assertTrue(expander.getPower("kW") < 0.0,
        "Expander should report negative (recovered) power");
    ...
    Assertions.assertNotEquals(1.5, maxUtil, 1.0e-9,
        "Expander max utilization must not equal the 1.5 fallback value");
  }
