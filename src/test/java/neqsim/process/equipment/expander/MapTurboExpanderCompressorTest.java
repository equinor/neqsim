...
/**
 * Tests for {@link MapTurboExpanderCompressor}, the map-based single-shaft turboexpander-compressor.
 */
public class MapTurboExpanderCompressorTest extends neqsim.NeqSimTest {
  ...
  private void setCompressorChart(MapTurboExpanderCompressor machine, Stream compressorFeed) {
    ...
    double[][] polyEff = new double[][] { { 77.245, 79.415, 80.738, 80.523, 79.221, 75.472, 69.603, 58.732 },
  ...
  public void testBalancedSpeedMode() {
    ...
    MapTurboExpanderCompressor machine = new MapTurboExpanderCompressor("TEC", expanderFeed, compressorFeed);
    ...
    Assertions.assertTrue(machine.getAvailableShaftPower() > 0.0, "Expander should produce positive shaft power");
    ...
    Assertions.assertTrue(machine.getShaftSpeed() >= minSpeed - 1.0 && machine.getShaftSpeed() <= maxSpeed + 1.0,
        "Shaft speed " + machine.getShaftSpeed() + " outside chart range [" + minSpeed + ", " + maxSpeed + "]");

    // When not speed-limited, the power balance must close.
    if (!machine.isSpeedLimited()) {
      double relativeResidual = Math.abs(machine.getPowerBalanceResidual()) / machine.getAvailableShaftPower();
    ...
    Assertions.assertTrue(machine.getCompressorOutStream().getPressure() > compressorFeed.getPressure(),
  ...
  public void testSpecifiedPressuresMode() {
    ...
    MapTurboExpanderCompressor machine = new MapTurboExpanderCompressor("TEC2", expanderFeed, compressorFeed);
    machine.setExpanderOutletPressure(30.0);
    machine.setExpanderIsentropicEfficiency(0.85);
  ...
  public void testInfeasibleTurndownPath() {
    ...
    MapTurboExpanderCompressor machine = new MapTurboExpanderCompressor("TEC3", expanderFeed, compressorFeed);
    machine.setExpanderOutletPressure(30.0);
    machine.setExpanderIsentropicEfficiency(0.85);
    machine.setMechanicalEfficiency(0.98);
    setCompressorChart(machine, compressorFeed);
    ...
    Assertions.assertFalse(machine.isFeasible(), "Machine should be infeasible at the turndown limit");
    Assertions.assertEquals(MapTurboExpanderCompressor.OperatingStatus.UNDER_POWER_SURGE, machine.getOperatingStatus(),
        "Should be under-power / surge limited");
    ...
    double minSpeed = machine.getCompressor().getCompressorChart().getMinSpeedCurve();
    Assertions.assertEquals(minSpeed, machine.getShaftSpeed(), 1.0, "Shaft should be pegged at the minimum map speed");
  }
  ...
  public void testToJsonReportsKeyResults() {
    ...
    MapTurboExpanderCompressor machine = new MapTurboExpanderCompressor("TEC4", expanderFeed, compressorFeed);
    machine.setExpanderOutletPressure(30.0);
    machine.setExpanderIsentropicEfficiency(0.85);
    setCompressorChart(machine, compressorFeed);
    machine.setShaftMode(MapTurboExpanderCompressor.ShaftMode.BALANCED_SPEED);
    ...
    Assertions.assertTrue(json.contains("powerBalanceResidual_MW"), "JSON should report power balance residual");
    Assertions.assertTrue(machine.toMap().containsKey("feasible"), "Result map should contain the feasibility flag");
  }
}
