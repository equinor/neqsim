...
public class FlareFrustumRadiationCalculatorTest {
  ...
  void testWindTiltsFlame() {
    ...
    assertTrue(windy.getFlameTiltDegrees() > calm.getFlameTiltDegrees(), "Higher wind should tilt the flame more");
    assertTrue(windy.getCentroidHorizontal() > calm.getCentroidHorizontal(), "Wind should move the centroid downwind");
  }
  ...
  void testAllowableFluxCheck() {
    ...
    assertTrue(near.getRadiantHeatFlux() > far.getRadiantHeatFlux(), "Closer receptor should see higher flux");
    assertTrue(far.isWithinAllowable(), "Far receptor should be within the allowable flux");
  }

  /**
   * The {@code fromFlare} bridge should populate mass flow and lower heating value from a run process flare.
   */
  @Test
  void testFromProcessFlare() {
    ...
    assertTrue(calc.getTotalHeatRelease() > 0.0, "Total heat release should be populated from the flare");
    assertTrue(calc.getRadiantHeatFlux() > 0.0, "Radiant flux should be positive");
    assertTrue(calc.getRadiatedPower() > 0.0, "Radiated power should be positive");
    assertNotNull(calc.toJson());
  }
