...
/**
 * Scenario tests (exercise only) for GibbsReactorCO2 using the tables provided in the attachments. These tests run the
 * reactor and print outlet mole-fractions (ppm) for manual verification.
 */
@Tag("slow")
public class GibbsReactorCO2ScenariosTest {
  ...
  private void assertSelectedPpm(SystemInterface outSys, String[] names, double[] expectedPpm, double tol,
      String label) {
  ...
  /**
   * Create a base SystemSrkEos with CO2 = 1e6 moles/sec and other components initialized to 0. Temperature in K and
   * pressure in bara."hydrogen", "N2O3", "N2O", "nitrogen", "N2H4", "COS", "ammonia",
   */
   ...
   *
   * <p>
   * This test verifies that bulk phase reactions are skipped when CO2 density falls below the 300 kg/m³ threshold. At
   * low pressure (e.g., 10 bara) and higher temperature, the CO2 density will be low enough to trigger this condition.
   * The outlet composition should match the inlet composition exactly since no reactions occur.
   * </p>
   */
  @Test
  public void scenarioGasPhaseReactionsStopped() {
    ...
    Assertions.assertTrue(density < 300.0, "Test setup error: density should be below 300 kg/m³ for this scenario");
