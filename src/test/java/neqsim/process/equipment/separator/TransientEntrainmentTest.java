...
/**
 * Tests that entrainment corrections are applied during transient (dynamic) separator simulation via
 * {@code runTransient()}. Verifies that the detailed performance calculator modifies outlet compositions when enabled,
 * and that the vessel inventory remains unaffected.
 *
 * @author copilot
 * @version 1.0
 */
public class TransientEntrainmentTest {
  ...
  void testTransientEntrainmentChangesGasComposition() {
    ...
    double[] gasCompNoEntrain = sepNoEntrain.getGasOutStream().getFluid().getMolarComposition().clone();
  ...
  void testTransientEntrainmentPreservesVesselInventory() {
    ...
    assertTrue(relativeChange < 0.5, "Vessel moles should not change drastically in one timestep: " + relativeChange);
  }
  ...
  void testTransientEntrainmentFractionsArePhysical() {
    ...
    assertTrue(oilInGas >= 0.0 && oilInGas < 1.0, "Oil-in-gas fraction must be in [0,1): " + oilInGas);
    assertTrue(waterInGas >= 0.0 && waterInGas < 1.0, "Water-in-gas fraction must be in [0,1): " + waterInGas);
    assertTrue(gasInOil >= 0.0 && gasInOil < 1.0, "Gas-in-oil fraction must be in [0,1): " + gasInOil);
  }
  ...
  void testSteadyStateRoutePreserved() {
    ...
    assertTrue(sep.getGasOutStream().getFluid().getNumberOfPhases() >= 1, "Gas outlet should have at least one phase");
  }
}
