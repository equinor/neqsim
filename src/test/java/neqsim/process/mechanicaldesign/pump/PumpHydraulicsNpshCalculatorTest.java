...
public class PumpHydraulicsNpshCalculatorTest {
  ...
  void testPowerCalculation() {
    ...
    assertTrue(calc.getBrakePower() > calc.getHydraulicPower(), "Brake power should exceed hydraulic power");
  }

  /**
   * The {@code fromPump} bridge should populate density, flow, head and suction pressure from a run pump.
   */
  @Test
  void testFromProcessPump() {
    ...
    assertTrue(calc.getHydraulicPower() > 0.0, "Hydraulic power should be positive for a pressure-raising pump");
    assertTrue(calc.getBrakePower() > 0.0, "Brake power should be positive for a pressure-raising pump");
    assertNotNull(calc.toJson());
  }
}
