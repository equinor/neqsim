...
public class LargeCombinedModelsTest {
   ...
   * @return a ProcessSystem object representing the well stream and manifold
   *         model
   */
   ...
   * 
   * This test performs the following checks: 1. Runs the well stream and manifold
   * process model. 2.
   * Asserts that the flow rate of the "HP well stream" matches the expected first
   * stage flow rate.
   * 3. Asserts that the flow rates of the split streams from the "HP manifold"
   * match the expected
   * split ratios. 4. Asserts that the total flow rate of the "HP well stream" is
   * conserved across
   * the split streams. 5. Asserts that the flow rate of the "LP well stream"
   * matches the expected
   * second stage flow rate. 6. Asserts that the total flow rate of the "LP well
   * stream" is
   ...
   * 
   * This test performs the following steps: 1. Creates a process system for the
   * well stream and
   * manifold model. 2. Asserts that the flow rate of the "HP well stream" matches
   * the expected
   * input flow rate. 3. Creates two separation train processes for the split
   * streams from the HP
   * and LP manifolds. 4. Runs the main process and both separation train
   * processes. 5. Asserts that
   * the combined gas outflow rate from the dew point scrubbers in both separation
   * train processes
   * matches the expected value. 6. Asserts that the mass balance is maintained by
   * checking that the
   * total flow rate of the well streams equals the sum of the flow rates of the
   * export oil and
