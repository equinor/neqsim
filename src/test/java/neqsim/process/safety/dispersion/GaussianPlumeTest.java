...
class GaussianPlumeTest {
  ...
  void groundConcentrationDecreasesWithDistance() {
    GaussianPlume plume = new GaussianPlume(1.0, 10.0, 5.0, GaussianPlume.Stability.D, GaussianPlume.Terrain.RURAL);
  ...
  void distanceToConcentrationConsistentWithDirectEvaluation() {
    ...
    GaussianPlume plume = new GaussianPlume(10.0, 0.0, 4.0, GaussianPlume.Stability.D, GaussianPlume.Terrain.RURAL);
    double thresh = 1.0e-3;
    double dist = plume.distanceToConcentration(thresh);
    assertTrue(Double.isFinite(dist) && dist > 0.0, "distance must be a finite positive number");
  ...
  void sigmasArePositive() {
    GaussianPlume plume = new GaussianPlume(1.0, 5.0, 3.0, GaussianPlume.Stability.B, GaussianPlume.Terrain.URBAN);
