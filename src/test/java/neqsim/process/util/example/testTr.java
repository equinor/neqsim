...
public class testTr {
  ...
  Assertions.assertTrue(vfpTable.contains("VFPPROD"));Assertions.assertTrue(vfpTable.contains("BHP values"));}
  ...
  ThreePhaseSeparator finalSeparator = new ThreePhaseSeparator(
      "Final Separator");finalSeparator.addStream(train1Outlet.getOutletStream());finalSeparator.addStream(train2Outlet.getOutletStream());finalSeparator.addStream(train3Outlet.getOutletStream());finalSeparator.addStream(train4Outlet.getOutletStream());finalSeparator.setInternalDiameter(3.0);finalSeparator.run();processSystem.add(finalSeparator);

  Splitter splitter2 = new Splitter("Test Splitter2", finalSeparator
      .getGasOutStream());splitter2.setSplitFactors(new double[]{0.95/3.0,1.0/3.0,1.05/3.0});splitter2.run();processSystem.add(splitter2);
  ...
  Manifold manifold = new Manifold(
      "Compressor Outlet Manifold");manifold.addStream(upstreamCompressorTrain1);manifold.addStream(upstreamCompressorTrain2);manifold.addStream(upstreamCompressorTrain3);manifold.setSplitFactors(new double[]{1.0/3.0,1.0/3.0,1.0/3.0});manifold.setHeaderInnerDiameter(1.5);manifold.setBranchInnerDiameter(0.6);manifold.run();processSystem.add(manifold);
  ...
  // Auto-size separators and compressors (not pipes)
  for(
  neqsim.process.equipment.ProcessEquipmentInterface equipment:processSystem.getUnitOperations()){if(equipment instanceof Separator)
  {
    ((Separator) equipment).autoSize();
  }else if(equipment instanceof Compressor)
  {
  ...
  // Configure ups1 compressor with driver curve (speed-dependent max power)
  Compressor ups1Comp = (Compressor) processSystem
      .getUnit("ups1 Compressor");ups1Comp.getMechanicalDesign().setMaxDesignPower(50000.0);
  String jsonFilePath =
      "src/test/resources/compressor_curves/example_compressor_curve.json";ups1Comp.loadCompressorChartFromJson(jsonFilePath);ups1Comp.setSolveSpeed(true);
  // Set up driver with speed-dependent max power curve
  // P_max(N) = maxPower * (a + b*(N/N_rated) + c*(N/N_rated)^2)
  // At N=N_rated (7383 RPM): factor = a + b + c = 1.0 (full power)
  // At N=0.7*N_rated: factor should be ~0.85 (less power at lower speeds)
  CompressorDriver driver1 = new CompressorDriver(DriverType.GAS_TURBINE,
      40500.0);driver1.setRatedSpeed(7383.0);driver1.setMaxPowerCurveCoefficients(0.3,0.5,0.2); // Gives
                                                                                                // ~0.86
                                                                                                // at
                                                                                                // 70%
                                                                                                // speed,
  ...
  // Configure ups2 compressor with driver curve
  Compressor ups2Comp = (Compressor) processSystem
      .getUnit("ups2 Compressor");ups2Comp.getMechanicalDesign().setMaxDesignPower(50000.0);
  String jsonFilePathUps2 =
      "src/test/resources/compressor_curves/compressor_curve_ups2.json";ups2Comp.loadCompressorChartFromJson(jsonFilePathUps2);ups2Comp.setSolveSpeed(true);
  CompressorDriver driver2 = new CompressorDriver(DriverType.GAS_TURBINE,
      40500.0);driver2.setRatedSpeed(7383.0);driver2.setMaxPowerCurveCoefficients(0.3,0.5,0.2);ups2Comp.setDriver(driver2);

  // Configure ups3 compressor with driver curve
  Compressor ups3Comp = (Compressor) processSystem
      .getUnit("ups3 Compressor");ups3Comp.getMechanicalDesign().setMaxDesignPower(50000.0);
  String jsonFilePathUps3 =
      "src/test/resources/compressor_curves/compressor_curve_ups3.json";ups3Comp.loadCompressorChartFromJson(jsonFilePathUps3);ups3Comp.setSolveSpeed(true);
  CompressorDriver driver3 =
      new CompressorDriver(DriverType.GAS_TURBINE, 45000.0);driver3.setRatedSpeed(6726.3); // Different
                                                                                           // rated
                                                                                           // speed
                                                                                           // for
                                                                                           // ups3
  driver3.setMaxPowerCurveCoefficients(0.3,0.5,0.2);ups3Comp.setDriver(driver3);
  ...
  ups1Comp.setMaxDischargeTemperature(168.0,"C");ups2Comp.setMaxDischargeTemperature(168.0,"C");ups3Comp.setMaxDischargeTemperature(168.0,"C");

  // Reinitialize capacity constraints to include discharge temperature
  ups1Comp.reinitializeCapacityConstraints();ups2Comp.reinitializeCapacityConstraints();ups3Comp.reinitializeCapacityConstraints();

  processSystem.run();

  // Get initial values
  double initialFlow =
      ((Compressor) processSystem.getUnit("ups1 Compressor")).getInletStream().getFlowRate("kg/hr");

  // Change flow rate and run once - tests that flow propagates correctly
  // with hasMultiInputEquipment() detecting multi-input Separator
  inletStream.setFlowRate(1957870.58288790,"kg/hr");processSystem.run();
  ...
  // Print flow change info
  System.out.println("\n=== FLOW PROPAGATION TEST ===");System.out.println("Initial flow: "+String.format("%.2f kg/hr",initialFlow));System.out.println("New flow: "+String.format("%.2f kg/hr",newFlow));System.out.println("Flow change: "+String.format("%.2f kg/hr",Math.abs(newFlow-initialFlow)));

  // Print pressure out of each pipe in the process
  System.out.println("\n=== PIPE VELOCITY ANALYSIS ===");
  double maxAllowedVelocity = 30.0; // m/s - max for gas pipes
  System.out.println(String.format("%-25s  %8s  %8s  %10s  %10s  %10s  %8s","Pipe Name","Vin","Vout","Vmax","Diameter","Flow","Status"));System.out.println(String.format("%-25s  %8s  %8s  %10s  %10s  %10s  %8s","","(m/s)","(m/s)","(m/s)","(m)","(kg/hr)",""));for(
  int i = 0;i<100;i++)
  {
    System.out.print("-");
  }System.out.println();for(
  neqsim.process.equipment.ProcessEquipmentInterface equipment:processSystem.getUnitOperations())
  {
    if (equipment instanceof PipeBeggsAndBrills) {
      PipeBeggsAndBrills pipe = (PipeBeggsAndBrills) equipment;
      double vIn = pipe.getInletSuperficialVelocity();
  ...
  System.out.println("\n=== EQUIPMENT CAPACITY UTILIZATION ===");
  java.util.Map<String, Double> utilizationSummary =
      processSystem.getCapacityUtilizationSummary();for(
  java.util.Map.Entry<String, Double> entry:utilizationSummary.entrySet())
  {
    System.out.println(String.format("%-30s: %6.2f%%", entry.getKey(), entry.getValue()));
  }

  // Print compressor power utilization
  Compressor comp = (Compressor) processSystem.getUnit("ups1 Compressor");if(comp!=null)
  {
    double powerKW = comp.getPower("kW");
    double maxPower = comp.getMechanicalDesign().maxDesignPower;
    System.out.println("\n=== COMPRESSOR POWER ===");
    ...
    System.out.println(
        "Distance to Surge: " + String.format("%.2f%%", comp.getDistanceToSurge() * 100.0));
    System.out.println(
        "Distance to Stonewall: " + String.format("%.2f%%", comp.getDistanceToStoneWall() * 100.0));
    System.out.println("Inlet Volume Flow: "
        + String.format("%.0f m3/hr", comp.getInletStream().getFlowRate("m3/hr")));
    System.out.println(
        "Inlet Pressure: " + String.format("%.2f bara", comp.getInletStream().getPressure("bara")));
    System.out.println("Outlet Pressure: "
        + String.format("%.2f bara", comp.getOutletStream().getPressure("bara")));
  ...
  // Print equipment near capacity limit
  System.out.println("\n=== EQUIPMENT NEAR CAPACITY LIMIT (>90%) ===");
  java.util.List<String> nearLimit =
      processSystem.getEquipmentNearCapacityLimit();if(nearLimit.isEmpty())
  {
    System.out.println("No equipment near capacity limit");
  }else
  {
  ...
  // Print bottleneck detection results
  neqsim.process.equipment.capacity.BottleneckResult bottleneck = processSystem
      .findBottleneck();System.out.println("\n=== BOTTLENECK ANALYSIS ===");if(bottleneck!=null&&bottleneck.hasBottleneck())
  {
    System.out.println("Bottleneck Equipment: " + bottleneck.getEquipmentName());
    System.out.println("Constraint: " + bottleneck.getConstraintName());
    System.out
  ...
  System.out.println("\n=== COMPRESSOR DISCHARGE TEMPERATURES ===");System.out.println(String.format("Max allowable: %.0f°C",ups1Comp.getMaxDischargeTemperature("C")));for(
  neqsim.process.equipment.ProcessEquipmentInterface equip:processSystem.getUnitOperations())
  {
    if (equip instanceof Compressor) {
  ...
  // ============================================
  StringBuilder separator = new StringBuilder();for(
  int i = 0;i<60;i++)
  {
    separator.append("=");
  }System.out.println("\n"+separator.toString());System.out.println("=== PRODUCTION OPTIMIZATION ===");System.out.println(separator.toString());

  // Get the inlet stream to use as the decision variable
  Stream feedStream = (Stream) processSystem.getUnit("Inlet Stream");
  double currentFlow = feedStream.getFlowRate(
  ...
  System.out.println("=== SINGLE-VARIABLE OPTIMIZATION RESULTS ===");System.out.println("Optimal flow rate: "+String.format("%.0f kg/hr",singleVarResult.getOptimalRate()));System.out.println("Feasible: "+singleVarResult.isFeasible());System.out.println("Iterations: "+singleVarResult.getIterations());if(singleVarResult.getBottleneck()!=null)
  {
  ...
  double increasePercent = (productionIncrease / originalFlow)
      * 100.0;System.out.println("\n=== PRODUCTION POTENTIAL ===");System.out.println("Current production: "+String.format("%.0f kg/hr",originalFlow));System.out.println("Maximum production: "+String.format("%.0f kg/hr",bestFeasibleFlow));if(productionIncrease>0)
  {
  ...
  // ============================================
  System.out.println("\n"+separator.toString());System.out.println("=== MULTI-VARIABLE OPTIMIZATION ===");System.out.println("Optimizing: Inlet Flow + Splitter Distribution");System.out.println(separator.toString());
  ...
  OptimizationConfig multiVarConfig = new OptimizationConfig(baseFlow * 0.90, baseFlow * 1.15)
      .rateUnit("kg/hr").tolerance(baseFlow * 0.005).maxIterations(30).defaultUtilizationLimit(1.0)
  ...
  System.out.println("=== MULTI-VARIABLE OPTIMIZATION RESULTS ===");System.out.println("Optimal inlet flow: "+String.format("%.0f kg/hr",bestFlow));System.out.println("Optimal balance factor: "+String.format("%.3f",bestBalanceFactor));System.out.println("Optimal split factors:");System.out.println("  Train 1 (ups1): "+String.format("%.1f%%",bestF1*100));System.out.println("  Train 2 (ups2): "+String.format("%.1f%%",bestF2*100));System.out.println("  Train 3 (ups3): "+String.format("%.1f%%",bestF3*100));System.out.println("Feasible: "+multiVarResult.isFeasible());System.out.println("Iterations: "+multiVarResult.getIterations());if(multiVarResult.getBottleneck()!=null)
  {
  ...
  double improvement = (bestFlow - originalFlow) / originalFlow
      * 100;System.out.println("\n=== COMPARISON ===");System.out.println("Initial flow: "+String.format("%.0f kg/hr",originalFlow));System.out.println("Optimized flow: "+String.format("%.0f kg/hr",bestFlow));System.out.println("Improvement: "+String.format("%.1f%%",improvement));
  ...
  // ============================================
  System.out.println("\n"+separator.toString());System.out.println("=== PARETO MULTI-OBJECTIVE OPTIMIZATION ===");System.out.println("Objectives: Maximize Throughput vs Minimize Power");System.out.println(separator.toString());
  ...
  System.out.println("\nPareto Front Size: "+paretoResult.getParetoFrontSize());System.out.println("Total Iterations: "+paretoResult.getTotalIterations());System.out.println("\nPareto Front:");System.out.println(paretoResult.toMarkdownTable());

  // Print utopia and nadir points
  System.out.println("Utopia Point (best individual values):");for(
  java.util.Map.Entry<String, Double> entry:paretoResult.getUtopiaPoint().entrySet())
  {
    System.out.println("  " + entry.getKey() + ": " + String.format("%.2f", entry.getValue()));
  }System.out.println("\nNadir Point (worst values on Pareto front):");for(
  java.util.Map.Entry<String, Double> entry:paretoResult.getNadirPoint().entrySet())
  {
    System.out.println("  " + entry.getKey() + ": " + String.format("%.2f", entry.getValue()));
  }

  // Restore best solution from single-variable optimization and show final state
  feedStream.setFlowRate(singleVarResult.getOptimalRate(),"kg/hr");optimizerSplitter.setSplitFactors(new double[]{bestF1,bestF2,bestF3});processSystem.run();

  // Show final compressor power utilization
  System.out.println("\n=== FINAL COMPRESSOR POWER UTILIZATION ===");for(
  neqsim.process.equipment.ProcessEquipmentInterface equip:processSystem.getUnitOperations())
  {
    if (equip instanceof Compressor) {
      Compressor c = (Compressor) equip;
      if (c.getName().startsWith("ups")) {
        ...
        System.out.println(String.format("  %-15s: Power=%.0f kW, Utilization=%.1f%%", c.getName(),
