...
/**
 * Dynamic validation benchmark test for the anti-surge recycle loop. Verifies that the
 * {@link AntiSurgeController} arrests a flow-reduction surge transient that the open-loop reference
 * case fails to survive.
 *
 * @author NeqSim
 * @version 1.0
 */
public class AntiSurgeDynamicBenchmarkTest {
  ...
  void testControllerArrestsSurgeTransient() {
    ...
    assertTrue(benchmark.isSurgeAvoided(),
        "controller should keep the machine out of surge; min margin = "
            + benchmark.getMinimumSurgeMargin());
    assertTrue(benchmark.getMaximumValveOpening() > 5.0,
        "recycle valve should open during the transient; max opening = "
    ...
    assertTrue(benchmark.getMinimumSurgeMargin() > 0.0,
        "closed-loop margin should remain positive");
  }

  /**
   * Without the controller the same disturbance must drive the machine into surge, confirming the
   * benchmark is a genuine challenge and the controller adds value.
   */
  @Test
