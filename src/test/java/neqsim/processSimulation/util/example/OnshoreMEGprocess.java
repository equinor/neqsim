...
public class OnshoreMEGprocess {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "wt% lean MEG  " + MEGFeed.getFluid().getPhase(PhaseType.AQUEOUS).getWtFrac("MEG") * 100.0);
    ...
    System.out.println("lean MEG wt% "
        + column.getLiquidOutStream().getFluid().getPhase(PhaseType.AQUEOUS).getWtFrac("MEG")
            * 100.0);
    System.out.println("hydrate temperature 1 "
        + (inletGasCooler.getOutletStream().getHydrateEquilibriumTemperature() - 273.15)
        + " wt% MEG "
        + inletGasCooler.getOutletStream().getFluid().getPhase(PhaseType.AQUEOUS).getWtFrac("MEG")
            * 100.0);
