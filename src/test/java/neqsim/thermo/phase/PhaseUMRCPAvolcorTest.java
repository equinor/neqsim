 ...
 *
 * <p>
 * The tests check that (1) the extensive translation {@code C} and its temperature derivative are
 * aggregated consistently from the per-component Peneloux shifts (the new {@code C}-parameter
 * plumbing in the Helmholtz F-function), (2) the translated densities of non-associating components
 * are positive and physically reasonable and the translation actually shifts them relative to the
 * untranslated UMR-CPA model, (3) the density-derived caloric and acoustic properties (which use
 * the temperature and volume derivatives of F) remain finite and physical, confirming the
 * derivatives are consistent, and (4) a water/methane mixture still flashes into the correct
 * phases.
 * </p>
 *
 * <p>
 * Note: unlike a pure cubic EOS, the exact Peneloux identity {@code v_translated = v_base - c} does
 * <em>not</em> hold for the volume-translated CPA model, because the association term is evaluated
 * at the physical (translated) volume; the association contribution therefore perturbs the volume
 * root. The tests below validate the model through robust physical invariants instead of that
 * identity.
 * </p>
 */
public class PhaseUMRCPAvolcorTest {
  ...
  void translationAggregationIsConsistent() {
    ...
    Assertions.assertEquals(phase.getC() / totalMoles, phase.getc(),
        Math.max(1e-12, 1e-9 * Math.abs(phase.getc())), "intensive translation c must equal C / n");
  }
  ...
  void nonAssociatingDensitiesArePhysicalAndTranslationIsActive() {
    ...
    Assertions.assertTrue(translatedLiquidIndex >= 0,
        "Expected translated system to contain a liquid phase");
  ...
  void waterMethaneMixtureFlashesAndDensitiesArePositive() {
    ...
    for (int i = 0; i < system.getNumberOfPhases(); i++) {
      ...
      Assertions.assertTrue(rho > 0.0 && Double.isFinite(rho),
          "all phase densities must be positive and finite");
    }
  }

  /**
   * Builds and flashes a single-/multi-component UMR-CPA system. When {@code translated} is true
   * the volume-translated {@link SystemUMRCPAvolcor} is used; otherwise the untranslated
   * {@link SystemUMRCPAEoS} with the legacy density-only correction switched off is used as the
   * reference.
   */
  private SystemInterface prepareSystem(String componentName, double temperature, double pressure,
      boolean translated) {
