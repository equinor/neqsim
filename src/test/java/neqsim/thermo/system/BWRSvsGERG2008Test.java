...
public class BWRSvsGERG2008Test {
  ...
  private void compareAtPressure(double pressure) {
    ...
    double bwrsPhiCH4 =
        bwrs.getPhase(0).getComponent("methane").fugcoef(bwrs.getPhase(0));
    double bwrsPhiC2H6 =
        bwrs.getPhase(0).getComponent("ethane").fugcoef(bwrs.getPhase(0));
    double bwrsCp =
        bwrs.getPhase(0).getCp() / bwrs.getPhase(0).getNumberOfMolesInPhase();
    ...
    double gergPhiCH4 =
        gerg.getPhase(0).getComponent("methane").fugcoef(gerg.getPhase(0));
    double gergPhiC2H6 =
        gerg.getPhase(0).getComponent("ethane").fugcoef(gerg.getPhase(0));
    double gergCp =
        gerg.getPhase(0).getCp() / gerg.getPhase(0).getNumberOfMolesInPhase();
    double gergJT = -1.0 / gerg.getPhase(0).getCp()
        * (gerg.getPhase(0).getMolarVolume() * gerg.getPhase(0).getNumberOfMolesInPhase()
            + gerg.getTemperature() * gerg.getPhase(0).getdPdTVn()
                / gerg.getPhase(0).getdPdVTn());
