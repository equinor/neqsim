...
class KlabFlashBugTest {
  static SystemInterface fluid1 = null;
  static ThermodynamicOperations thermoOps;

  static String[] components = new String[] {"water", "nitrogen", "CO2", "H2S", "methane", "ethane",
      "propane", "i-butane", "n-butane", "i-pentane", "n-pentane", "n-hexane"};
  static double[] fractions1 =
      new double[] {2.18, 0.48, 1.77, 0, 62.88, 12.36, 9.58, 2.21, 2.47, 0.98, 0.46, 4.63};
  static double[] fractions2 =
      new double[] {0.054, 0.454, 1.514, 0, 89.92, 5.324, 1.535, 0.232, 0.329, 0.094, 0.107, 0.437};
  ...
  public void testPHflash() {
    for (int i = 0; i < P_bar.length; i++) {
      ...
      assertTrue(Math.abs(errH[i]) < 0.5);
    }
  }
  ...
  public void testPSflash() {
    for (int i = 0; i < P_bar.length; i++) {
      ...
      assertTrue(Math.abs(errS[i]) < 0.5);
    }
  }
}
