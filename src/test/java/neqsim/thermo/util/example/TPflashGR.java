...
public class TPflashGR {
  ...
  public static void main(String[] args) {
    ...
    /*
     * for (int i = 0; i < P_bar.length; i++) { fluid1.setPressure(P_bar[i]); thermoOps.PHflash(enthalpy[i]); errH[i] =
     * fluid1.getTemperature() - T_C[i] - 273.15; logger.info("err " + errH[i]); //assertTrue(Math.abs(errH[i]) < 1e-2);
     * }
     */
  }
}
